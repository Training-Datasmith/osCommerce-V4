...
class MultiSafepayAPI
{
    ...
    public function startDirectXMLTransaction()
    {
        $this->checkSettings();
    ...
    public function startDirectBankTransfer()
    {
        $this->checkSettings();
    ...
    public function createIdealIssuersRequest()
    {
    ...
    public function _isXmlSectionEmpty($section)
    {
        return isset($section['VALUE']);
    }

    public function processStatusReply($rootNode)
    {
        $xml = $rootNode;
        $result = [];
    ...
    public function createDirectXMLTransactionRequest()
    {
        $issuer = '';
        if (!empty($this->issuer)) {
            $issuer = ' issuer="'.$this->xmlEscape($this->issuer).'"';
    ...
    public function createDirectBankTransferTransactionRequest()
    {
        $issuer = '';
        if (!empty($this->issuer)) {
            $issuer = ' issuer="'.$this->xmlEscape($this->issuer).'"';
     ...
     */
    public function createGatewaysRequest()
    {
        $request = '<?xml version="1.0" encoding="UTF-8"?>
     ...
     */
    public function createUpdateTransactionRequest()
    {
        $request = '<?xml version="1.0" encoding="UTF-8"?>
    <updatetransaction>
    <merchant>
    ...
    public function setDefaultTaxZones($globalRate = true, $shippingTaxed = true)
    {
     ...
     */
    public function xmlEscape($strin)
    {
        $strout = null;
        $strin = strval($strin);
...
class MspCart
{
    ...
    public $item_arr;
    public $shipping_arr;
    public $default_tax_rules_arr;
    public $alternate_tax_tables_arr;
    public $xml_data;
     ...
     *                         defaults to 'USD'
     */
    public function __construct($id = '', $key = '', $server_type = 'sandbox', $currency = 'EUR')
    {
        $this->merchant_id = $id;
     ...
     * @return void
     */
    public function SetMerchantCalculations(
        $url,
        $tax_option = 'false',
        $coupons = 'false',
        $gift_cert = 'false'
    ...
    public function GetXML()
    {
        ...
        foreach ($this->item_arr as $item) {
            ...
            if ($item->item_weight != '' && $item->numeric_weight !== '') {
                $xml_data->EmptyElement('item-weight', [ 'unit' => $item->item_weight,
            ...
            if ($item->digital_content) {
                ...
                if (!empty($item->digital_url)) {
                    $xml_data->element('description', substr(
                        $item->digital_description,
                        0,
                        MAX_DIGITAL_DESC
                    ));
        ...
        foreach ($this->shipping_arr as $ship) {
            ...
            if ($ship->type == 'flat-rate-shipping' ||
               $ship->type == 'merchant-calculated-shipping'
                ...
                $xml_data->EmptyElement('width', ['unit' => $ship->ShippingPackage->unit,
                                               'value' => $ship->ShippingPackage->width,
                                                ]);
                $xml_data->EmptyElement('length', ['unit' => $ship->ShippingPackage->unit,
                                                'value' => $ship->ShippingPackage->length,
                                                ]);
                $xml_data->EmptyElement('height', ['unit' => $ship->ShippingPackage->unit,
        ...
        if ((count($this->alternate_tax_tables_arr) != 0)
            ...
            if ($this->merchant_calculated_tax != '') {
                $xml_data->Push(
                    'tax-tables',
                    ['merchant-calculated' => $this->merchant_calculated_tax]
                );
            } else {
     ...
     */
    public function CheckoutServer2ServerButton(
        $url,
        $size = 'large',
        $variant = true,
        $loc = 'en_US',
     ...
     * @return string the button's html
     */
    public function CheckoutButtonCode(
        $size = 'large',
        $variant = true,
        $loc = 'en_US',
        $showtext = true,
    ...
    public function CheckoutButtonNowCode(
        $size = 'large',
        $variant = true,
        $loc = 'en_US',
        $showtext = true,
     ...
     * @return string the button's html
     */
    public function CheckoutHTMLButtonCode(
        $size = 'large',
        $variant = true,
        $loc = 'en_US',
        $showtext = true,
...
class MspItem
{
    ...
    public $unit_price;
    public $quantity;
    public $merchant_private_item_data;
    public $merchant_item_id;
    public $tax_table_selector;
    public $email_delivery;
    public $digital_content = false;
    public $digital_description;
    public $digital_key;
    public $digital_url;

    public $item_weight;
    public $numeric_weight;
     ...
     *
     */
    public function __construct($name, $desc, $qty, $price, $item_weight = '', $numeric_weight = '')
    {
        $this->item_name = $name;
        ...
        if ($item_weight != '' && $numeric_weight !== '') {
            ...
            $this->numeric_weight = (float)$numeric_weight;
        }
    }
...
class MspCustomFields
{
    ...
    public function _GetXmlLocalized(&$xml_data, $field, $value)
    {
        if (is_array($value)) {
...
{
    public $standardField = null;

    public $name       = null;
    public $type       = null;
    public $label      = null;
    public $default    = null;
    public $savevalue  = null;
    public $options    = [];
    public $validation = [];
    public $filter     = null;

    public $descriptionTop = [];
    public $descriptionRight = [];
    ...
    public function AddRestrictions($filter)
    {
        $this->filter = $filter;
    }

    public function SetStandardField($name, $optional = false)
    {
...
class MspCustomFieldValidation
{
    ...
    public function __construct($type, $data, $error)
    {
        $this->type  = $type;
        $this->data  = $data;
        $this->error = $error;
...
class MspCustomFieldFilter
{
    ...
    public function AddAllowedPostalArea($country_code)
    {
        $this->allowed_country_codes_arr[] = $country_code;
    }

    public function AddExcludedPostalArea($country_code)
    {
        $this->excluded_country_codes_arr[] = $country_code;
    }
}
