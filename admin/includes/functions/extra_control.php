...
function tep_draw_calendar_jquery($html_field_name, $value = '', $params = '')
{
    ...
    if (sizeof($s) == 1) {
        ...
        $_new_fomat = $_new_fomat_js = [];
        foreach ($ex as $item) {
            switch ($item) {
                case 'F':case 'm':case 'M':case 'n': $_new_fomat[] = 'm';
                    $_new_fomat_js[] = 'mm';
                    break;
                case 'd':case 'D':case 'j': case 'l': case 'N':  case 'w': $_new_fomat[] = 'd';
                    $_new_fomat_js[] = 'dd';
                    break;
                case 'Y':case 'y':case 'o': $_new_fomat[] = 'Y';
                    $_new_fomat_js[] = 'yy';
                    break;
                default: break;
            }
