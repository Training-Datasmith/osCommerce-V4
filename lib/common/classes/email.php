...
class email
{
    ...
    public function __construct($headers = '')
    {
         ...
         */
        $this->image_types = ['gif' => 'image/gif', 'jpg' => 'image/jpeg', 'jpeg' => 'image/jpeg', 'jpe' => 'image/jpeg', 'bmp' => 'image/bmp', 'png' => 'image/png', 'tif' => 'image/tiff', 'tiff' => 'image/tiff', 'swf' => 'application/x-shockwave-flash'];
    ...
    public function add_bcc($bcc_string)
    {
        $this->bcc = array_merge($this->bcc, preg_split('/[,;]/', $bcc_string, -1, PREG_SPLIT_NO_EMPTY));
        $this->bcc = array_unique($this->bcc);
    }
    public function encode_header($str)
    {
        if (preg_match('/.+<[^@]+@[^>]+>/', $str)) {
    ...
    public function encode_header_str($str)
    {
        $x = 0;
    ...
    public function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '')
    {
        ...
        if (strpos($to_addr, ',') !== false) {
            ...
            $to = $to_name != '' ? strpos($to_addr, '<') !== false ? '"' . $to_name . '" ' . $to_addr : '"' . $to_name . '" <' . $to_addr . '>' : $to_addr;
        }
        ...
        if (!empty($this->bcc)) {
            ...
            if (count($grepped) > 0) {
                $existing_bcc_idx = key($grepped);
                $xtra_headers[$existing_bcc_idx] .= ', ' . implode(', ', $this->bcc);
            } else {
