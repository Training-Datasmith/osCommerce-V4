...
class dropshipping
{
    ...
    public function get_included_module($module)
    {
    ...
    {
        if (($object = $this->get_included_module($module)) !== false) {
            if (is_object($object) && method_exists($object, 'process') && $object->enabled) {
                $response = $object->process($params);
                if ($response) {
                    \Yii::$app->session->set_flash('success', DROPSHIPPING_PROCESS_SUCCESSED);
                } else {
                    \Yii::$app->session->set_flash('danger', DROPSHIPPING_PROCESS_DECLINED);
                }
            }
        }
        return;
    }
    public function render_order_process_button()
    {
        $html = [];
        foreach ($this->get_included_modules() as $object) {
            if (is_object($object) && $object->enabled) {
                if (method_exists($object, 'renderOrderProcessButton')) {
                    $html[] = $object->render_order_process_button();
                } else {
