...
class Net_HTTP_Client
{
    ...
    {
        if ($this->debug & DBGTRACE) {
            echo "setProxy( $proxyHost, $proxyPort )\n";
        }
        $this->useProxy = true;
    ...
    {
        if ($this->debug & DBGTRACE) {
            echo "addHeader( $headerName, $headerValue )\n";
        }
     ...
     * @see RFC2518 "HTTP Extensions for Distributed Authoring WEBDAV"
     **/
    public function Move($srcUri, $destUri, $overwrite = true)
    {
        if ($this->debug & DBGTRACE) {
            echo "Move( $srcUri, $destUri, $overwrite )\n";
        }
        if ($overwrite) {
     ...
     * @see RFC2518 "HTTP Extensions for Distributed Authoring WEBDAV"
     **/
    public function Copy($srcUri, $destUri, $overwrite = true)
    {
        if ($this->debug & DBGTRACE) {
            echo "Copy( $srcUri, $destUri, $overwrite )\n";
        }
        if ($overwrite) {
            $this->requestHeaders['Overwrite'] = 'T';
        } else {
     ...
     * @see RFC2518 "HTTP Extensions for Distributed Authoring WEBDAV"
     **/
    public function PropFind($uri, $scope = 0)
    {
        if ($this->debug & DBGTRACE) {
     ...
     **/
    public function Lock($uri, $lockScope, $lockType, $lockOwner)
    {
...
<D:lockscope><D:$lockScope/></D:lockscope>\n<D:locktype><D:$lockType/></D:locktype>
    ...
    public function sendCommand($command)
    {
        ...
        if ($this->socket == false or feof($this->socket)) {
            ...
            if (! $this->socket) {
                if ($this->debug & DBGSOCK) {
                    echo "FAILED : $this->replyString ($this->reply)\n";
                }
                return false;
            }
        }
    ...
    public function processReply()
    {
        ...
        if ($this->debug & DBGINDATA) {
            echo "replyLine: $this->replyString\n";
        }
    ...
    /**
      ...
      **/
    public function processHeader($lastLine = CRLF)
    {
        if ($this->debug & DBGLOW) {
