...
class Rss
{
    ...
    private function item($data)
    {
        ...
        if (isset($data->title)) {
            $this->item[$this->item_count]->addChild('title', htmlspecialchars($data->title));
        }
        if (isset($data->link)) {
            $this->item[$this->item_count]->addChild('link', $data->link);
        }
        if ($this->channel->creator) {
            $this->addCdataChild($this->item[$this->item_count], 'dc:creator', $this->channel->creator, 'http://purl.org/dc/elements/1.1/');
        }
        if (isset($data->pubDate)) {
            $this->item[$this->item_count]->addChild('pubDate', \DateTime::createFromFormat('Y-m-d H:i:s', $data->pubDate)->format(DATE_RSS));
        }
        ...
        if (isset($data->description)) {
            $this->addCdataChild($this->item[$this->item_count], 'description', $data->description);
        }
        ...
        if (isset($data->content)) {
            $this->addCdataChild($this->item[$this->item_count], 'content:encoded', $data->content, 'http://purl.org/rss/1.0/modules/content/');
        }
