...
use common\models\Customers_Basket;
use Yii;
...
class shopping_cart
{
    ...
    public $content_type;
    public $giveaway;
    // separated storage for GA products
    public $platform_id;
    //current customer selected platform
    /// full shit with shared session between platforms
    public $language_id;
    //current customer selected language
    public $currency;
    //current customer selected currency
    public $basket_id;
    //current customer basket id, used for recovery cart
    public $admin_id = 0;
    //used at backend
    ...
    public function set_reference($reference_id)
    {
        $this->reference_id = (int) $reference_id;
    }
    public function empty_reference()
    {
        $this->reference_id = null;
    }
    ...
    public function is_empty_products()
    {
    ...
    public function restore_order()
    {
        ...
        while ($d = tep_db_fetch_array($res)) {
            if (strlen($d['template_uprid']) > strlen($d['uprid'])) {
                $d['uprid'] = $d['template_uprid'];
            }
            if ($d['is_giveaway']) {
                $this->giveaway = [];
                $this->giveaway[$d['uprid']] = ['qty' => $d['products_quantity'], 'reserved_qty' => $d['products_quantity'], 'promo_id' => $d['promo_id']];
            } else {
                $contents[$d['uprid']] = ['qty' => $d['products_quantity'], 'parent' => $d['parent_product'], 'reserved_qty' => $d['products_quantity']];
                if ($d['gift_wrapped']) {
                    $contents[$d['uprid']]['gift_wrap'] = 1;
                }
                $this->set_overwrite($d['uprid'], 'name', $d['products_name']);
                $this->set_overwrite($d['uprid'], 'model', $d['products_model']);
                $this->set_overwrite($d['uprid'], 'price', $d['products_price']);
                $this->set_overwrite($d['uprid'], 'final_price', $d['final_price']);
                if ($ext = \common\helpers\Acl::check_extension_allowed('PackUnits', 'allowed')) {
                    $_units = $ext::query_order_admin($this->order_id, $d, $d['orders_products_id']);
                    if (is_array($_units) && count($_units)) {
                        foreach ($_units as $k => $v) {
                            $this->set_overwrite($d['uprid'], $k, $v);
                        }
                    }
                }
            }
            ...
            if (tep_not_null($d['overwritten'])) {
                ...
                if (is_array($_temp)) {
                    foreach ($_temp as $_k => $_v) {
                        $this->set_overwrite($d['uprid'], $_k, $_v);
                    }
                }
            }
        }
        ...
        $this->set_order_status($res['orders_status']);
        $this->contents = $contents;
        $this->get_content_type();
        $this->restore_totals();
    }
    public $cc_array = [];
    public function clear_cc_items()
    {
        $this->cc_array = [];
    }
    public function add_cc_item($coupon_code)
    {
        if (!empty($coupon_code)) {
            if (!is_array($this->cc_array)) {
                $this->cc_array = [];
            }
            $coupon_result = \common\models\Coupons::get_coupon_by_code($coupon_code, true);
            if ($coupon_result->coupon_type != 'G') {
                if ($coupon_result->single_per_order) {
                    $this->cc_array = [];
                } else if (count($this->cc_array) > 0) {
                    $any_single_used = \common\models\Coupons::find()->where(['IN', 'coupon_id', array_keys($this->cc_array)])->and_where(['single_per_order' => 1])->count();
                    if ($any_single_used) {
                        $this->cc_array = [];
                    }
                }
    ...
    public function remove_cc_item($code)
    {
        if (is_array($this->cc_array)) {
    ...
    public function restore_totals()
    {
        if ($this->order_id > 0) {
            $this->clear_cc_items();
            ...
            while ($totals = tep_db_fetch_array($totals_query)) {
                ...
                $this->totals[$totals['class']] = [
                    'title' => $totals['title'],
                    'value' => ['in' => $totals['value_inc_tax'], 'ex' => $totals['value_exc_vat']],
                    //$totals['value'],
                    'class' => $totals['class'],
                    'text' => $totals['text'],
                ...
                if ($totals['class'] == 'ot_tax' && $this->totals[$totals['class']]['value_exc_vat'] != $this->totals[$totals['class']]['value_inc_tax']) {
                    ...
                    $this->totals[$totals['class']]['value'] = ['in' => abs($this->totals[$totals['class']]['value_inc_tax'] - $this->totals[$totals['class']]['value_exc_vat']), 'ex' => abs($this->totals[$totals['class']]['value_inc_tax'] - $this->totals[$totals['class']]['value_exc_vat'])];
                }
            }
            ...
            while ($removed = tep_db_fetch_array($removed_query)) {
                $this->add_hidden_module($removed['class']);
            }
            ...
            $this->set_adjusted($adj['adjusted']);
        }
    }
    public function set_total_key($code, $value)
    {
    ...
    public function set_total_tax($code, $value, $prefix)
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return;
        }
    ...
    public function set_total_paid($value, $prefix = '+', $comment = '')
    {
        $currencies = \Yii::$container->get('currencies');
        if (!\frontend\design\Info::is_totally_admin()) {
            return;
        }
        if (!is_array($this->totals['ot_paid'] ?? null)) {
            $this->totals['ot_paid'] = [];
        }
        if (!is_array($this->totals['ot_paid']['value'] ?? null)) {
            $this->totals['ot_paid']['value'] = [];
        }
        if (is_array($this->totals['ot_paid']['value'] ?? null)) {
        ...
        $this->totals['ot_paid']['info'][] = ['comment' => $comment];
    }
    public function get_paid_info()
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return false;
        }
        if (isset($this->totals['ot_paid']['info'])) {
            $info = ['info' => $this->totals['ot_paid']['info'], 'status' => $this->get_status_after_paid()];
            return $info;
        }
        return false;
    }
    public function get_status_after_paid()
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return false;
        }
    ...
    public function get_total_tax_prefix()
    {
        return $this->totals['ot_tax']['prefix'];
    }
    public function get_total_key($code)
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return false;
        }
    ...
    public function get_total_key_ex($code)
    {
        return $this->totals[$code] ?? null;
    }
    public function get_total_title($code)
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return;
        }
    ...
    public function set_total_title($code, $value)
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return;
        }
    ...
    public function clear_total_key($code)
    {
        if (isset($this->totals[$code])) {
            unset($this->totals[$code]);
        }
    }
    public function clear_totals($only_values = false)
    {
        if (!$only_values) {
    ...
    public function exist_hidden_module($code)
    {
        return in_array($code, $this->hidden);
    }
    public function clear_hidden_module($code)
    {
    ...
    public function clear_hidden_modules()
    {
        $this->hidden = [];
    }
    public function get_hidden_modules()
    {
        return $this->hidden;
    }
    public function before_restore()
    {
        ...
        if (is_bool($this->config_temporary_stock_enable) && $this->config_temporary_stock_enable == false) {
            return;
        }
    ...
    public function restore_contents($uid = false)
    {
        global $languages_id;
        ...
        $def_id = false;
        if ($multi_cart = \common\helpers\Acl::check_extension('MultiCart', 'allowed')) {
            if ($multi_cart::allowed()) {
                $multi_cart::save_baskets_info($customer_id);
                $def_id = $multi_cart::get_current_cart_key();
            }
        }
        if (!$def_id) {
            if (($def_id = Customers_Basket::get_basket_id($customer_id)) !== false) {
                $this->basket_id = $def_id;
            } else {
                $this->basket_id = $this->generate_cart_id();
            }
        }
        ...
        if (sizeof($this->giveaway) > 0) {
            Customers_Basket::clear_giveaway($customer_id);
            $this->save_give_aways();
        }
        ...
        // }} WA - keep applied coupons after login
        $this->restore_content_products($uid);
        // {{ Virtual Gift Card
        if ($customer_id > 0 && Session::get_session()->has('gift_handler')) {
            tep_db_query('update ' . TABLE_VIRTUAL_GIFT_CARD_BASKET . " set customers_id = '" . (int) $customer_id . "', session_id = '' where (length(virtual_gift_card_code) = 0 || !activated) and customers_id = '0' and session_id = '" . Session::get_session()->get('gift_handler') . "'");
            Session::get_session()->remove('gift_handler');
        }
        // }}
        ...
        $this->cleanup();
        $this->check_giveaway();
        // GAW depends on customers group
        /** @var \common\extensions\MultiCart\MultiCart $MultiCart */
        if ($multi_cart = \common\helpers\Acl::check_extension('MultiCart', 'allowed')) {
            if ($multi_cart::allowed()) {
                $multi_cart::restore_carts($this, $customer_id);
            }
        }
        Yii::$app->get('PropsHelper')::after_cart_restore($this);
    }
    public function save_contents()
    {
        if (is_array($this->contents) && !Yii::$app->user->is_guest) {
            $customer_id = Yii::$app->user->get_id();
            if ($multi_cart = \common\helpers\Acl::check_extension('MultiCart', 'allowed')) {
                if ($multi_cart::allowed()) {
                    $multi_cart::save_basket_info($this, $customer_id);
                }
            }
            foreach ($this->contents as $products_id => $val) {
                Customers_Basket::save_product([$products_id => $val], $customer_id, $this);
            }
        }
        foreach (\common\helpers\Hooks::get_list('shopping-cart/save-contents') as $filename) {
            include $filename;
        }
    }
    public function save_give_aways()
    {
        $customer_id = Yii::$app->user->get_id();
        // insert new GA into DB
        if (is_array($this->giveaway) && $customer_id) {
            foreach ($this->giveaway as $products_id => $giveaway) {
                Customers_Basket::save_product([$products_id => $giveaway], $customer_id, $this);
            }
        }
    }
    public function restore_content_products($uid = false)
    {
        // reset per-session cart contents, but not the database contents
        $customer_id = Yii::$app->get('storage')->get('customer_id');
        if ($uid !== false) {
            $product_basket_query = Customers_Basket::find()->where(['basket_id' => $uid]);
            $products_basket = $product_basket_query->with('productAttributes')->as_array()->all();
        } else {
            $products_basket = Customers_Basket::get_products($customer_id, $this->basket_id);
        }
        if ($products_basket) {
            foreach (\common\helpers\Hooks::get_list('shopping-cart/restore-content-products/products-basket') as $filename) {
                include $filename;
            }
            foreach ($products_basket as $products) {
                ...
                if ($products['is_pack'] == 1) {
                    $this->contents[$products['products_id']] = ['qty' => $products['customers_basket_quantity'], 'parent' => $products['parent_product'], 'unit' => $products['unit'], 'pack_unit' => $products['pack_unit'], 'packaging' => $products['packaging'], 'is_pack' => 1];
                } else {
                    $this->contents[$products['products_id']] = ['qty' => $products['customers_basket_quantity'], 'parent' => $products['parent_product']];
                }
        ...
        while ($d = tep_db_fetch_array($ga_query)) {
            if ($d['gaw_id'] == 0) {
                ...
                if ($gaw_id = \common\helpers\Gifts::allowed_gaw($d['products_id'], $d['customers_basket_quantity'])) {
    ...
    public function reset($reset_database = false)
    {
        ...
        $this->products_array = [];
        $this->clear_cc_items();
        if (!Yii::$app->user->is_guest && $reset_database == true) {
            Customers_Basket::clear_basket(Yii::$app->user->get_id());
        ...
        if (!$this->ignore_hooks && get_class($this) == 'common\classes\shopping_cart') {
            foreach (\common\helpers\Hooks::get_list('shopping-cart/reset') as $filename) {
                include $filename;
            }
        }
    }
    public function reset_db($basket_id = null)
    {
        if (!Yii::$app->user->is_guest) {
            Customers_Basket::clear_basket(Yii::$app->user->get_id(), 0, $basket_id);
            //$this->basketID = $this->generate_cart_id();
        }
    }
    public function __sleep()
    {
        ...
        if (get_class() != get_called_class()) {
            $reflection = new \Reflection_Object($this);
            foreach ($prop_names as $key => $prop_name) {
                if (!$reflection->has_property($prop_name)) {
    ...
    public function add_cart($products_id, $qty = '1', $attributes = '', $notify = true, $gaw_id = 0, $gift_wrap = null, $props = '')
    {
        ...
        // {{
        if ((int) $gaw_id > 0) {
            // product added as GA, check it
            $products_id = Inventory_Helper::get_uprid($products_id, $attributes);
            $products_id = Inventory_Helper::normalize_id($products_id);
            if (\common\helpers\Product::is_giveaway($products_id)) {
                // only GA can be added as GA
                ...
                if ($gaw_id == \common\helpers\Gifts::allowed_gaw($products_id, $qty)) {
                    $gaw_data = ['qty' => $qty, 'gaw_id' => $gaw_id];
                } else {
                    $gaw_data = \common\helpers\Gifts::get_max_quantity($products_id);
                }
                $this->giveaway[$products_id] = ['qty' => $gaw_data['qty'], 'gaw_id' => $gaw_data['gaw_id']];
                ...
                if ($customer_id > 0) {
                    // update database
                    Customers_Basket::clear_giveaway($customer_id);
                    Customers_Basket::save_product([$products_id => $this->giveaway[$products_id]], $customer_id, $this);
                }
            }
        } else {
            // ordinary product
            ...
            if ($ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                $bundle_products = $ext::get_bundle_products($products_id, \common\classes\platform::current_id());
            ...
            if ($ext = \common\helpers\Extensions::is_allowed('LinkedProducts')) {
                $bundle_products = $ext::get_children_products($products_id);
                if (count($bundle_products) > 0) {
            ...
            if ($ext = \common\helpers\Extensions::is_allowed('SupplierPurchase')) {
                $products_id = $ext::make_uprid($ext::get_supplier_from_uprid($original_products_id) ? $original_products_id : $products_id, $attributes);
            ...
            // update cart
            if (!empty($props)) {
                $products_id = Yii::$app->get('PropsHelper')::cart_uprid($products_id, $props);
            }
            if ($qty == 0) {
            ...
            \common\components\google\widgets\Google_Tagmanger::set_event('addToCart');
            if ($this->in_cart($products_id) && (int) $gaw_id == 0) {
                if (is_null($gift_wrap)) {
                    $gift_wrap = $this->is_gift_wrapped($products_id);
                }
                $this->update_quantity($products_id, $pack_qty, $attributes, $gift_wrap);
                if (\frontend\design\Info::is_totally_admin()) {
                ...
                if (is_array($pack_qty)) {
                    $this->contents[$products_id] = ['qty' => $qty, 'parent' => '', 'unit' => $pack_qty['unit'], 'pack_unit' => $pack_qty['pack_unit'], 'packaging' => $pack_qty['packaging'], 'is_pack' => 1];
                } else {
                ...
                if (!empty($props)) {
                    $props = Yii::$app->get('PropsHelper')::on_cart_add($props);
                    $this->contents[$products_id]['props'] = $props;
                }
                $this->contents[$products_id]['platform_id'] = \common\classes\platform::current_id();
                // insert into db
                if (!Yii::$app->user->is_guest) {
                    Customers_Basket::save_product([$products_id => $this->contents[$products_id]], $customer_id, $this);
                }
            }
            // {{
        }
        ...
        $this->cleanup();
        if (!empty($props)) {
            Yii::$app->get('PropsHelper')::cart_changed($this);
        }
        ...
        $this->check_giveaway();
        foreach (\common\helpers\Hooks::get_list('shopping-cart/add-cart') as $filename) {
            include $filename;
        }
        if ((int) $gaw_id > 0) {
    ...
    public function update_props($products_id, $props)
    {
        ...
        $this->contents[$products_id]['props'] = $props;
        if (!Yii::$app->user->is_guest) {
            Customers_Basket::save_product([$products_id => $this->contents[$products_id]], Yii::$app->user->get_id(), $this);
        }
        Yii::$app->get('PropsHelper')::cart_changed($this);
        return true;
    }
    ...
    public function update_basket_info()
    {
        global $languages_id;
        ...
        $customer_id = Yii::$app->user->get_id();
        if ($this->basket_id && $customer_id) {
            tep_db_query('update ' . TABLE_CUSTOMERS_BASKET . " set basket_id = '" . (int) $this->basket_id . "', language_id = '" . (int) $this->language_id . "', currency = '" . tep_db_input($this->currency) . "' where customers_id = '" . (int) $customer_id . "'");
    ...
    public function update_quantity($products_id, $quantity = '', $attributes = '', $gift_wrap = false)
    {
        $customer_id = Yii::$app->user->get_id();
        if (empty($quantity)) {
            return true;
        ...
        $platform_id = isset($this->contents[$products_id]['platform_id']) ? $this->contents[$products_id]['platform_id'] : \common\classes\platform::current_id();
        if (is_array($pack_qty)) {
            $this->contents[$products_id] = ['qty' => $quantity, 'parent' => $parent, 'reserved_qty' => $reserved_qty, 'relation_type' => $relation_type, 'unit' => $pack_qty['unit'], 'pack_unit' => $pack_qty['pack_unit'], 'packaging' => $pack_qty['packaging'], 'is_pack' => 1];
        } else {
            $this->contents[$products_id] = ['qty' => $quantity, 'parent' => $parent, 'reserved_qty' => $reserved_qty, 'relation_type' => $relation_type];
        }
        if ($props !== false) {
            $this->contents[$products_id]['props'] = $props;
        }
        ...
        foreach (\common\helpers\Hooks::get_list('shopping-cart/update-quantity') as $filename) {
            include $filename;
        }
    ...
    public function cleanup()
    {
        ...
        $this->max_length = 0;
        $this->max_height = 0;
        foreach (\common\helpers\Hooks::get_list('shopping-cart/cleanup/before') as $filename) {
            include $filename;
        }
        if (is_array($this->contents)) {
            ...
            if (!$this->ignore_hooks) {
                foreach (\common\helpers\Hooks::get_list('shopping-cart/cleanup') as $filename) {
                    include $filename;
                }
            }
            foreach (array_reverse($this->contents, true) as $key => $value) {
                ...
                if (!defined('SHOPPING_CART_SHARE') || SHOPPING_CART_SHARE != 'True') {
                    if ($this->platform_id && \common\classes\platform::is_multi()) {
                        $products2c_join .= ' inner join ' . TABLE_PLATFORMS_PRODUCTS . " plp on p.products_id = plp.products_id  and plp.platform_id = '" . $this->platform_id . "' " . ' inner join ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c ON p2c.products_id=p.products_id ' . ' inner join ' . TABLE_PLATFORMS_CATEGORIES . " plc ON plc.categories_id=p2c.categories_id AND plc.platform_id = '" . $this->platform_id . "' ";
                    }
                }
                if (USE_MARKET_PRICES == 'True' || \common\helpers\Extensions::is_customer_groups_allowed()) {
                    ...
                    $product_check_query = tep_db_query('select count(*) as total from ' . TABLE_PRODUCTS . " p {$products2c_join} " . ' where p.products_status = 1 ' . \common\helpers\Product::get_sql_product_restrictions(['p', 'pd', 's', 'sp', 'pp'], false) . ' ' . " and p.products_id = '" . (int) $key . "'");
                }
                ...
                if (\common\helpers\Extensions::is_allowed('Inventory')) {
                    if (!\common\helpers\Inventory::is_valid_variant($key)) {
                        $this->contents[$key]['qty'] = 0;
                        // bad uprid - clear
                    }
                }
                ...
                if (defined('MAX_PRODUCT_PRICE') && MAX_PRODUCT_PRICE > 0) {
                    $currencies = \Yii::$container->get('currencies');
                    $new_item_price = \common\helpers\Product::get_products_price($key, $value['qty']);
                    $products_tax_class_id = \common\helpers\Product::get_products_info($key, 'products_tax_class_id');
                    $new_full_price = $currencies->calculate_price($new_item_price, \common\helpers\Tax::get_tax_rate($products_tax_class_id), $value['qty']);
                    if ($new_full_price > (int) MAX_PRODUCT_PRICE) {
                        $this->contents[$key]['qty'] = 0;
                    }
                }
                if (defined('TEMPORARY_STOCK_ENABLE') && TEMPORARY_STOCK_ENABLE == 'true') {
                ...
                if ($this->contents[$key]['qty'] < 1) {
                    ...
                    if (!Yii::$app->user->is_guest) {
                        Customers_Basket::delete_product(Yii::$app->user->get_id(), $key);
                    }
                    foreach (\common\helpers\Hooks::get_list('shopping-cart/cleanup/delete-product') as $filename) {
                        include $filename;
                    }
                }
            }
        }
        foreach (\common\helpers\Hooks::get_list('shopping-cart/cleanup/after') as $filename) {
            include $filename;
        }
    }
    public function count_contents()
    {
        ...
        if (is_array($this->contents)) {
            foreach ($this->contents as $products_id => $value) {
                ...
                if (tep_not_null(Array_Helper::get_value($this->contents, [$products_id, 'parent']))) {
                    continue;
                }
    ...
    public function get_reserved_quantity($products_id, $ga = 0)
    {
        if ($ga == 1) {
            ...
            if (isset($this->giveaway[$products_id])) {
                ...
                return $this->giveaway[Inventory_Helper::get_prid($products_id)]['reserved_qty'];
            } else {
                foreach ($this->giveaway as $_ga_uprid => $ga_info) {
                    if (Inventory_Helper::get_prid($products_id) == Inventory_Helper::get_prid($_ga_uprid)) {
                        //InventoryHelper::get_uprid
    ...
    public function in_cart($keys)
    {
        if (is_array($keys)) {
            ...
            return $keys;
        } elseif (isset($this->contents[$keys])) {
    ...
    public function in_giveaway($products_id, $qty = 0, $gaw_id = false)
    {
        if (!is_array($this->giveaway)) {
            return false;
        }
        if ($gaw_id === false) {
            ...
            foreach ($this->giveaway as $uprid => $data) {
                $ga_pids[Inventory_Helper::get_prid($uprid)] = $data['qty'];
            }
            if (isset($ga_pids[Inventory_Helper::get_prid($products_id)])) {
                if ($qty == 0 || $ga_pids[Inventory_Helper::get_prid($products_id)] == $qty) {
                    return true;
                } else {
                    return false;
                }
    ...
    public function remove($products_id)
    {
        ...
        if (is_array($this->contents)) {
            ...
            foreach ($this->contents as $key => $val) {
                if ($this->contents[$key]['parent'] == $products_id) {
                    $this->contents[$key]['qty'] = 0;
                    if ($this->exist_owerwritten($key)) {
                        $this->clear_overwriten($key);
                    }
                    $remove_children[] = $key;
                }
            }
        ...
        if (empty($this->order_id)) {
            // don't clean up saved orders because disabled products disapear
            $this->cleanup();
        } else {
            foreach ($remove_children as $key) {
                unset($this->contents[$key]);
            }
        }
        unset($remove_children);
        /* PC configurator addon end */
        if (!tep_session_is_registered('last_removed')) {
            tep_session_register('last_removed');
        }
        $_tmp = ['products_id' => $products_id, 'data' => Array_Helper::get_value($this->contents, $products_id)];
        $last_removed = base64_encode(serialize($_tmp));
        \common\components\google\widgets\Google_Tagmanger::set_event('removeFromCart');
        unset($this->contents[$products_id]);
        if ($this->exist_owerwritten($products_id)) {
            $this->clear_overwriten($products_id);
        }
        // remove from database
        $customer_id = Yii::$app->user->get_id();
        Customers_Basket::delete_product($customer_id, $products_id);
        // {{ Virtual Gift Card
        if (preg_match("/(\\d+)\\{0\\}(\\d+)/", $products_id, $arr)) {
            tep_db_query('delete from ' . TABLE_VIRTUAL_GIFT_CARD_BASKET . " where (length(virtual_gift_card_code) = 0 || !activated) and virtual_gift_card_basket_id = '" . (int) $arr[2] . "' and products_id = '" . (int) $arr[1] . "' and " . ($customer_id > 0 ? " customers_id = '" . (int) $customer_id . "'" : " session_id = '" . Session::get_session()->get('gift_handler') . "'"));
        }
        // }}
        // assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
        $this->cart_id = $this->generate_cart_id();
        ...
        Yii::$app->get('PropsHelper')::cart_changed($this);
        foreach (\common\helpers\Hooks::get_list('shopping-cart/remove') as $filename) {
            include $filename;
        }
    }
    public function remove_giveaway($products_id)
    {
        $products_id = Inventory_Helper::normalize_id($products_id);
        $_remove_uprid = '';
        if (isset($this->giveaway[$products_id])) {
            ...
            unset($this->giveaway[Inventory_Helper::get_prid($products_id)]);
            $_remove_uprid = Inventory_Helper::get_prid($products_id);
        } else {
            foreach (array_keys($this->giveaway) as $_ga_uprid) {
                if (Inventory_Helper::get_prid($_ga_uprid) == Inventory_Helper::get_prid($products_id)) {
        ...
        if (!empty($_remove_uprid) && !Yii::$app->user->is_guest) {
            Customers_Basket::delete_product(Yii::$app->user->get_id(), $_remove_uprid, 1);
        }
        // assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
        $this->cart_id = $this->generate_cart_id();
        //$this->update_basket_info();
    ...
    public function calculate()
    {
        ...
        */
        ...
        foreach ($this->contents as $products_id => $val) {
            ...
            if ($product = tep_db_fetch_array($product_query)) {
                ...
                if ($this->contents[$products_id]['parent'] != '') {
                    ...
                    $check_parent = \common\models\Products::find()->select('products_pctemplates_id')->where('products_id=:products_id', [':products_id' => \common\helpers\Inventory::get_prid($this->contents[$products_id]['parent'])])->as_array()->one();
                    if ($check_parent['products_pctemplates_id']) {
                        ...
                        $_qty = $this->get_qty($products_id, false, true);
                        $products_price = $price_instance->get_configurator_price(['qty' => $_qty]);
                        $special_price = $price_instance->get_configurator_special_price(['qty' => $_qty]);
                        ...
                        $_qty = $this->get_qty($products_id, false, true);
                        if ($ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                            $bundle_products = $ext::get_bundle_products(\common\helpers\Inventory::get_prid($this->contents[$products_id]['parent']), \common\classes\platform::current_id());
                            if (count($bundle_products) > 0 && $bundle_products[\common\helpers\Inventory::get_prid($products_id)] > 0) {
                                // if parent is bundle
                                $price_instance = \common\models\Product\Bundle_Price::get_instance($products_id);
                                $products_price = $price_instance->get_bundle_price(['qty' => $_qty, 'parent' => $this->contents[$products_id]['parent']]);
                                $special_price = $price_instance->get_bundle_special_price(['qty' => $_qty, 'parent' => $this->contents[$products_id]['parent']]);
                                if ($special_price !== false) {
                                    $_spd = $price_instance->get_special_price_details(['qty' => $_qty, 'parent' => $this->contents[$products_id]['parent']]);
                                    if (\common\helpers\Specials::qty_in_range($_spd, $qty) !== false) {
                                        //could be null
                        ...
                        if ($products_price === false) {
                            ...
                            $products_price = $price_instance->get_inventory_price(['qty' => $_qty]);
                            $special_price = $price_instance->get_inventory_special_price(['qty' => $_qty]);
                            if ($special_price !== false) {
                                $_spd = $price_instance->get_special_price_details(['qty' => $_qty]);
                                if (\common\helpers\Specials::qty_in_range($_spd, $qty) !== false) {
                                    //could be null
                                    $specials_id = $_spd['specials_id'] ?? 0;
                                } else {
                    ...
                    $price_instance = \common\models\Product\Price::get_instance($products_id)->update_uprid($products_id);
                    $_qty = $this->get_qty($products_id, false, true);
                    $products_price = $price_instance->get_inventory_price(['qty' => $_qty]);
                    $special_price = $price_instance->get_inventory_special_price(['qty' => $_qty]);
                    if ($special_price !== false) {
                ...
                if ($product['products_file'] == '') {
                    ...
                    if ($ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                        $products_weight = $ext::get_weight($product);
                        $products_volume = $ext::get_volume($product);
                    } else {
                ...
                if (\common\helpers\Extensions::is_allowed('Inventory') && !Inventory_Helper::disabled_on_product($products_id)) {
                    $simple_uprid = Inventory_Helper::normalize_id($products_id);
                    if (($inventory_weight = Inventory_Helper::get_inventory_weight_by_uprid($simple_uprid)) > 0) {
                        $products_weight += $inventory_weight;
                    }
                } else if (isset($this->contents[$products_id]['attributes'])) {
                ...
                if ($ext = \common\helpers\Acl::check_extension_allowed('PackUnits', 'allowed')) {
                    if ($pack_units_data = $ext::get_products_cart_frontend($products_id, $this->contents)) {
            ...
            if (is_array($this->collections) && $this->contents[$products_id]['parent'] == '') {
                foreach ($this->collections as $collection) {
                    foreach ($collection['products'] as $prid) {
                        if ($prid == Inventory_Helper::get_prid($products_id)) {
        ...
        // {{ Virtual Gift Card
        if ($customer_id > 0 || Session::get_session()->get('gift_handler')) {
            $virtual_gift_card_query = tep_db_query('select vgcb.virtual_gift_card_basket_id, vgcb.products_id, if(length(pd1.products_name), pd1.products_name, pd.products_name) as products_name, p.products_model, p.products_image, p.products_weight, p.products_tax_class_id, vgcb.products_price, vgcb.virtual_gift_card_recipients_name, vgcb.virtual_gift_card_recipients_email, vgcb.virtual_gift_card_message, vgcb.virtual_gift_card_senders_name, c.code as currency_code from ' . TABLE_VIRTUAL_GIFT_CARD_BASKET . ' vgcb, ' . TABLE_CURRENCIES . ' c, ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_PRODUCTS . ' p left join ' . TABLE_PRODUCTS_DESCRIPTION . " pd1 on pd1.products_id = p.products_id and pd1.language_id = '" . (int) $languages_id . "' and pd1.platform_id = '" . intval(Yii::$app->get('platform')->config()->get_platform_to_description()) . "' where (length(vgcb.virtual_gift_card_code) = 0 || !vgcb.activated) and p.products_id = vgcb.products_id and pd.platform_id = '" . intval(\common\classes\platform::default_id()) . "' and pd.products_id = p.products_id and pd.language_id = '" . (int) $languages_id . "' and vgcb.currencies_id = c.currencies_id and c.status = 1 and " . ($customer_id > 0 ? " vgcb.customers_id = '" . (int) $customer_id . "'" : " vgcb.customers_id = '0' and vgcb.session_id = '" . Session::get_session()->get('gift_handler') . "'"));
            while ($virtual_gift_card = tep_db_fetch_array($virtual_gift_card_query)) {
    ...
    public function have_gift_wrap_products()
    {
        if (!is_array($this->contents)) {
            return false;
        }
    ...
    public function get_gift_wrap_amount()
    {
        if (!is_array($this->contents)) {
            return 0;
        }
    ...
    public function is_gift_wrapped($products_id)
    {
    ...
    public function get_products($selected_products_id = null)
    {
        $currencies = \Yii::$container->get('currencies');
        $currency = \Yii::$app->settings->get('currency');
        ...
        if (empty($selected_products_id) && !empty($this->products_array)) {
            if ($last_products_array_cache_key == $current_products_array_cache_key && !\frontend\design\Info::is_totally_admin()) {
                if (!(Yii::$app->params['reset_static_product_prices_cache'] ?? false)) {
        ...
        foreach ($this->contents as $products_id => $val) {
            if (!is_null($selected_products_id) && $selected_products_id != $products_id) {
                continue;
            }
            if ($this->is_amount($products_id)) {
                $amount_data = $this->product_amount_data($products_id);
                if ($amount_data !== false) {
                    $products_array[$products_id] = $amount_data;
                }
                continue;
            }
            $products_query = tep_db_query('select p.products_id, p.is_virtual, p.stock_indication_id, if(length(pd1.products_name), pd1.products_name, pd.products_name) as products_name, IF(p.is_listing_product AND p.products_status, 1, 0) AS _status, p.products_model, p.products_image, p.products_price, p.products_price_full, p.products_weight, p.products_tax_class_id, p.products_file, p.subscription, p.subscription_code, p.bonus_points_price, p.bonus_points_cost, p.length_cm, p.width_cm, p.height_cm, p.bundle_volume_calc, p.is_bundle from ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_PRODUCTS . ' p left join ' . TABLE_PRODUCTS_DESCRIPTION . " pd1 on pd1.products_id = p.products_id and pd1.language_id='" . (int) $languages_id . "' and pd1.platform_id = '" . intval(Yii::$app->get('platform')->config()->get_platform_to_description()) . "' " . " where p.products_id = '" . (int) $products_id . "' and pd.platform_id = '" . intval(\common\classes\platform::default_id()) . "' and pd.products_id = p.products_id " . " and pd.language_id = '" . (int) $languages_id . "'");
            if ($products = tep_db_fetch_array($products_query)) {
                $c_product = $container->get_product($products_id);
                if (!$c_product) {
                    $data = $products;
                    $data['products_id'] = $products_id;
                    $data['listing_type'] = 'cart';
                    $c_product = $container->load_products($data)->get_product($products_id);
                }
                $prid = $products['products_id'];
                $uprid = Inventory_Helper::normalize_id($products_id);
                $products['products_weight'] = $c_product->get_product_weight($uprid);
                $stock_products_id = Inventory_Helper::normalize_inventory_id($products_id);
                $configurator_koeff = 1;
                $qty = $this->get_qty($products_id, false, true);
                $sw_product = false;
                $products_price = false;
                $standard_price = false;
                $linked_product = !empty($this->contents[$products_id]['relation_type']) && $this->contents[$products_id]['relation_type'] == 'linked';
                if (Array_Helper::get_value($this->contents, [$products_id, 'parent']) != '') {
                    $check_parent = \common\models\Products::find()->select('products_pctemplates_id')->where('products_id=:products_id', [':products_id' => \common\helpers\Inventory::get_prid($this->contents[$products_id]['parent'])])->as_array()->one();
                    if ($check_parent['products_pctemplates_id']) {
                        // if parent is configurator
                        $price_instance = \common\models\Product\Configurator_Price::get_instance($products_id);
                        $products_price = $price_instance->get_configurator_price(['qty' => $qty]);
                        ...
                        if ($special_price !== false) {
                            ...
                            $products_price = $special_price;
                        } else if (\common\helpers\Customer::check_customer_groups($customer_groups_id, 'groups_price_as_special') && $products['products_price'] > $products_price) {
                        ...
                        if ($ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                            ...
                            if (count($bundle_products) > 0 && $bundle_products[\common\helpers\Inventory::get_prid($products_id)] > 0) {
                                // if parent is bundle
                                $price_instance = \common\models\Product\Bundle_Price::get_instance($products_id);
                                $products_price = $price_instance->get_bundle_price(['qty' => $qty, 'parent' => $this->contents[$products_id]['parent']]);
                                $special_price = $price_instance->get_bundle_special_price(['qty' => $qty, 'parent' => $this->contents[$products_id]['parent']]);
                                if ($special_price !== false) {
                                    $_spd = $price_instance->get_special_price_details(['qty' => $qty]);
                                    if (\common\helpers\Specials::qty_in_range($_spd, $qty) !== false) {
                                        //could be null
                        ...
                        if ($products_price === false) {
                            $price_instance = \common\models\Product\Price::get_instance($products_id);
                            $products_price = $price_instance->get_inventory_price(['qty' => $qty]);
                    ...
                    if ($ext = \common\helpers\Extensions::is_allowed('SupplierPurchase')) {
                        $sw_product = $ext::get_ws_product($products_id, \common\classes\platform::current_id());
                        if ($sw_product) {
                            $products_price = $sw_product->get_product_price();
                            $special_price = false;
                            $products['products_name'] = $sw_product->get_product_name();
                        }
                    }
                    if ($products_price === false) {
                        $price_instance = \common\models\Product\Price::get_instance($products_id);
                ...
                if ($ext = \common\helpers\Extensions::is_allowed('Inventory')) {
                    $products = array_replace($products, $ext::get_inventory_settings((int) $stock_products_id, $stock_products_id));
                }
                /*              // bundle products are included to shopping cart, so their weight is calculated separately
                                ...
                                */
                if ($sw_product) {
                    $stock_info = $sw_product->get_stock();
                } else {
                    /** @var \common\extensions\ProductBundles\ProductBundles $ext */
                    if ($products['is_bundle'] && $ext = \common\helpers\Extensions::is_allowed('ProductBundles')) {
                        $stock_info = \common\classes\Stock_Indication::product_info($ext::get_stock_indication_data(['products_id' => $products_id, 'stock_indication_id' => $products['stock_indication_id'], 'products_quantity' => \common\helpers\Product::get_products_stock($products_id), 'cart_qty' => $this->contents[$products_id]['qty'], 'cart_class' => true]));
                        $stock_info['quantity_max'] = \common\helpers\Product::filter_product_order_quantity($stock_products_id, $stock_info['max_qty'], true);
                    } elseif (($ext = \common\helpers\Extensions::is_allowed('Inventory')) && !Inventory_Helper::disabled_on_product($stock_products_id)) {
                        $stock_info = $ext::get_inventory_stock($stock_products_id, $products['stock_indication_id'], $this->contents[$products_id]['qty']);
                    } else {
                        $stock_info = \common\classes\Stock_Indication::product_info(['products_id' => $stock_products_id, 'stock_indication_id' => $products['stock_indication_id'], 'cart_qty' => $this->contents[$products_id]['qty'], 'cart_class' => true, 'products_quantity' => \common\helpers\Product::get_products_stock($stock_products_id)]);
                        $stock_info['quantity_max'] = \common\helpers\Product::filter_product_order_quantity($stock_products_id, $stock_info['max_qty'], true);
                    }
                    if (false && \common\helpers\Acl::check_extension_allowed('ProductBundles') && $products['is_bundle'] && empty($val['attributes'])) {
                        /// bundles kostyl' for now
                        $tmp = isset($val['attributes']) && is_array($val['attributes']) ? $val['attributes'] : [];
                        $attributes_details = \common\helpers\Attributes::get_details($products_id, $tmp, []);
                        $details = \common\helpers\Bundles::get_details($products, $attributes_details);
                ...
                // {{ Collections addon
                if (is_array($this->collections) && Array_Helper::get_value($this->contents, [$products_id, 'parent']) == '') {
                    foreach ($this->collections as $collection) {
                        foreach ($collection['products'] as $prid) {
                            if ($prid == Inventory_Helper::get_prid($products_id)) {
                ...
                if (isset($this->contents[$products_id]['props']) && !empty($this->contents[$products_id]['props'])) {
                    $props = Yii::$app->get('PropsHelper')::xml_to_params($this->contents[$products_id]['props']);
                    //props code here
                    $explain_info = Yii::$app->get('PropsHelper')::explain_params($props, Tax::get_tax_rate($products['products_tax_class_id']));
                }
                $products_array[$products_id] = [
                    'id' => $products_id,
                    'platform_id' => isset($this->contents[$products_id]['platform_id']) ? $this->contents[$products_id]['platform_id'] : \common\classes\platform::current_id(),
                    ...
                    /* PC configurator addon begin */
                    'parent' => Array_Helper::get_value($this->contents, [$products_id, 'parent']),
                    'sub_products' => $this->get_subproducts($products_id),
                    'relation_type' => isset($this->contents[$products_id]['relation_type']) ? $this->contents[$products_id]['relation_type'] : '',
                    /* PC configurator addon end */
                    'tax_class_id' => $products['products_tax_class_id'],
                    'attributes' => isset($this->contents[$products_id]['attributes']) ? $this->contents[$products_id]['attributes'] : [],
                    'overwritten' => $this->get_owerwritten($products_id),
                ...
                /** @var \common\extensions\ProductBundles\ProductBundles $ext*/
                if ($products['is_bundle'] && $ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                    $products_array[$products_id]['volume'] = $ext::get_volume($products);
                } else {
                ...
                if ($ext_bonus_action = \common\helpers\Extensions::is_allowed_and('BonusActions', 'isProductPointsEnabled')) {
                    $bonuses = $ext_bonus_action::get_product_bonuses($products_id, $customer_groups_id, null, $products_array[$products_id]['quantity'], $products_array[$products_id]['parent']);
                    if (is_array($bonuses)) {
                        foreach ($bonuses as $key_bonus => $value_bonus) {
                            $products_array[$products_id][$key_bonus] = $value_bonus;
                        }
                    }
                    unset($bonuses);
                }
                // }}
                if ($ext = \common\helpers\Acl::check_extension_allowed('PackUnits', 'allowed')) {
                    $replace = $ext::get_products_cart_frontend($products_id, $this->contents);
                    if ($replace) {
                ...
                if ($this->exist_owerwritten($products_id)) {
                    $this->over_write($products_id, $products_array[$products_id]);
                    $owd = $this->get_owerwritten($products_id);
                    if (is_array($owd) && count($owd)) {
                        if (isset($owd['final_price_formula']) or isset($owd['final_price'])) {
                            $products_array[$products_id]['final_price'] *= \common\helpers\Product::get_virtual_item_quantity_value($products_id);
                        }
                    }
                    unset($owd);
                }
                foreach (\common\helpers\Hooks::get_list('shopping-cart/get-products/product') as $filename) {
                    include $filename;
                }
                $c_product->attach_details($products_array[$products_id]);
            }
        }
        if ($products_array) {
            /* some Promo may change prices dependently priority */
            if (\common\helpers\Acl::check_extension_allowed('Promotions')) {
                foreach ($products_array as $products_id => $_product) {
                    $promo_price = \common\extensions\Promotions\models\Product\Promotion_Price::get_instance($_product['id']);
                    $promo_price->set_calculate_after(true);
                    $price = $promo_price->get_promotion_price();
                    if ($price !== false) {
                    ...
                    $restruct_qty = $promo_price->get_restrict_quantity();
                    if ($restruct_qty !== false) {
                        $products_array[$products_id]['stock_info']['quantity_max'] = $restruct_qty;
                    }
                }
            }
            /* container is used to sync product prices */
            foreach ($products_array as $products_id => $_product) {
                $product = $container->get_product($products_id);
                if ($product) {
        ...
        if ($products_array) {
            foreach ($products_array as &$_product) {
                $_product['quantity_virtual'] = \common\helpers\Product::get_virtual_item_quantity($_product['id'], $_product['quantity']);
                $_product['quantity_virtual_item'] = \common\helpers\Product::get_virtual_item_quantity_value($_product['id']);
            ...
            $d = \common\helpers\Customer::get_additional_superdiscount(Yii::$app->user->get_id(), $_total);
            if ($d > 0) {
                foreach ($products_array as $products_id => $_product) {
                    $d_settings = new \common\components\Disabled_Settings($products_id);
                    if ($d_settings->apply_group_discount()) {
        ...
        if (sizeof($this->giveaway) > 0) {
            // if we also have GA add them too
            foreach ($this->giveaway as $products_id => $product) {
                ...
                if ($ext = \common\helpers\Extensions::is_allowed('Inventory')) {
                    $products = array_replace($products, $ext::get_inventory_settings($products_id, Inventory_Helper::normalize_inventory_id($products_id)));
                }
                // {{ stock info
                $stock_info = \common\classes\Stock_Indication::product_info(['products_id' => Inventory_Helper::normalize_id($products_id), 'stock_indication_id' => $products['stock_indication_id'], 'cart_qty' => $this->contents[$products_id]['qty'], 'cart_class' => true, 'products_quantity' => \common\helpers\Product::get_products_stock(Inventory_Helper::normalize_id($products_id))]);
                // }} stock info
                $products_array[$products_id . '(GA)'] = ['id' => $products_id, 'name' => $products['products_name'], 'model' => $products['products_model'], 'image' => $products['products_image'], 'stock_info' => $stock_info, 'gift_wrap_allowed' => false, 'gift_wrap_price' => 0, 'gift_wrapped' => false, 'price' => 0, 'ga' => 1, 'gaw_id' => $product['gaw_id'], 'is_virtual' => (int) $products['is_virtual'], 'quantity' => $product['qty'], 'weight' => $products['products_weight'], 'final_price' => 0, 'standard_price' => false, 'tax_class_id' => $products['products_tax_class_id'], 'promo_id' => $product['promo_id'], 'attributes' => isset($product['attributes']) ? $product['attributes'] : ''];
                ...
                /** @var \common\extensions\ProductBundles\ProductBundles $ext*/
                if ($products['is_bundle'] && $ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                    $products_array[$products_id . '(GA)']['volume'] = $ext::get_volume($products);
                } else {
        ...
        foreach ($virtual_gift_cards as $virtual_gift_card) {
            ...
            $products_array[$products_id] = [
                ...
                'attributes' => [0 => $virtual_gift_card['virtual_gift_card_basket_id']],
            ];
        }
        // }}
        foreach (\common\helpers\Hooks::get_list('shopping-cart/get-products') as $filename) {
            include $filename;
        }
            ...
            if ($prod['parent'] == '') {
                ...
                if (is_array($prod['sub_products']) && count($prod['sub_products']) > 0) {
                    foreach ($prod['sub_products'] as $subprid) {
                        if (!isset($products_array[$subprid])) {
                            continue;
                        }
    ...
    public function has_blocked_products()
    {
        $container = \Yii::$container->get('products');
        $has = false;
        foreach ($this->get_products() as $product) {
            $c_product = $container->get_product($product['id']);
            if ($c_product) {
                $has = (bool) ($c_product['block_in_cart'] ?? false) || $has;
                //?? VL if ($has) {break;}
            }
        }
        return $has;
    }
    protected function compact_linked_products()
    {
        if ($ext = \common\helpers\Extensions::is_allowed('LinkedProducts')) {
            $this->products_array = $ext::hook_compact_linked_products($this->products_array) ?? $this->products_array;
        }
    }
    ...
    public function update_final_price($products_array)
    {
        if (!Yii::$app->user->is_guest) {
            if (is_array($products_array)) {
                foreach ($products_array as $products_id => $cprod) {
                    tep_db_query('update ' . TABLE_CUSTOMERS_BASKET . " set final_price = '" . (float) $cprod['final_price'] . "' where customers_id = '" . (int) Yii::$app->user->get_id() . "' and basket_id = '" . $this->basket_id . "' and products_id = '" . tep_db_input($products_id) . "'");
                }
            }
        }
        return;
    ...
    public function show_total_ex_tax()
    {
        $currencies = \Yii::$container->get('currencies');
        if (!$this->products_array && $this->count_contents()) {
            $this->calculate();
    ...
    public function show_volume_weight()
    {
        $volume_weight = 0;
    ...
    public function show_dimensions()
    {
        if (!$this->products_array && $this->count_contents()) {
            $this->calculate();
        } else {
        ...
        return ['max_width' => $this->max_width, 'max_length' => $this->max_length, 'max_height' => $this->max_height, 'max_weight' => $this->max_weight];
    }
    ...
    public function generate_cart_id($length = 5)
    {
        ...
        if ($multi_cart = \common\helpers\Acl::check_extension('MultiCart', 'allowed')) {
            if ($multi_cart::allowed()) {
                // check for existing
                $random = $multi_cart::check_basket_id($random, Yii::$app->user->get_id(), $length);
            }
        }
        return $random;
    }
    ...
    public function __get_content_type()
    {
        ...
        } elseif ($this->count_contents() > 0) {
            // GIFT addon by Senia 2008-04-21
            $this->show_weight();
        ...
        if (is_array($this->contents)) {
            foreach ($this->contents as $products_id => $val) {
                ...
                if (NO_COUNT_ZERO_WEIGHT == 1) {
                    $gv_query = tep_db_query('select products_weight from ' . TABLE_PRODUCTS . " where products_id = '" . Inventory_Helper::get_prid($products_id) . "'");
                    $gv_result = tep_db_fetch_array($gv_query);
    ...
    public function check_giveaway()
    {
        if (is_array($this->giveaway)) {
            foreach ($this->giveaway as $products_id => $giveaway) {
                if (!\common\helpers\Gifts::allowed_gaw($products_id, $giveaway['qty']) && !$giveaway['promo_id']) {
                    unset($this->giveaway[$products_id]);
                    if (!Yii::$app->user->is_guest) {
                        Customers_Basket::delete_product(Yii::$app->user->get_id(), $products_id, 1);
                    }
                }
            }
        }
    }
    public function is_valid_product_data($product_id, $attributes)
    {
        if (!is_array($attributes)) {
            $attributes = [];
        }
        ...
        if (\common\helpers\Attributes::has_product_attributes($product_id)) {
            // include bundle
            // {{ Products Bundle Sets
            $bundle_products = [Inventory_Helper::get_prid($product_id)];
            if ($ext = \common\helpers\Acl::check_extension_allowed('ProductBundles', 'allowed')) {
                $bundle_products = $ext::get_bundles($product_id, \common\classes\platform::current_id());
            }
            // }}
            $valid_options = [];
            $attributes_query = tep_db_query('select products_id, options_id, options_values_id ' . 'from ' . TABLE_PRODUCTS_ATTRIBUTES . ' ' . 'where 1 ' . 'AND ' . (count($bundle_products) > 1 ? " products_id in ('" . implode("','", $bundle_products) . "')" : " products_id = '" . (int) $product_id . "'") . ' ');
    ...
    public function auto_giveaway()
    {
        if (USE_AUTO_GIVEAWAY != 'true') {
            return false;
        }
        $response = \common\helpers\Gifts::get_give_aways_sql(false, true, false);
        // all products, only active , no default sort order
        ///as only 1 gaw in cart the sort order is important.
        $giveaway_query = $response['giveaway_query']->add_order_by('shopping_cart_price, buy_qty desc')->as_array()->all();
    ...
    {
        if (!Yii::$app->user->is_guest) {
            $customers_info = Yii::$app->user->get_identity()->get_customers_info();
            if ($customers_info) {
                $customers_info->update_time_long();
                if (!$only_time) {
                    $customers_info->update_token();
                }
                return $customers_info->get_token();
            }
        }
    }
    public function set_platform($platform_id)
    {
    ...
    public function set_admin($admin_id)
    {
    ...
    public function set_basket_id($basket_id)
    {
        if (!$basket_id) {
            ...
            $this->basket_id = (int) $basket_id;
        }
        return $this;
    }
    public function set_overwrite($uprid, $key, $value)
    {
        if (!\frontend\design\Info::is_totally_admin()) {
            return;
        }
    ...
    public function exist_owerwritten($uprid)
    {
        if (isset($this->overwrite[$uprid])) {
            return true;
        }
    ...
    public function get_owerwritten($uprid)
    {
        if (isset($this->overwrite[$uprid])) {
    ...
    public function get_owerwritten_key($uprid, $key)
    {
    ...
    public function clear_overwriten_key($uprid, $key)
    {
        if (isset($this->overwrite[$uprid]) && isset($this->overwrite[$uprid][$key])) {
            unset($this->overwrite[$uprid][$key]);
        }
    }
    public function clear_overwriten($uprid)
    {
        unset($this->overwrite[$uprid]);
    }
    public function over_write($uprid, &$product)
    {
        $details = $this->get_owerwritten($uprid);
        if (is_array($details) && count($details)) {
            foreach ($details as $key => $value) {
                if (is_callable($value)) {
                    $product[$details[$key . '_data'][0]] = call_user_func_array($value, $details[$key . '_data']);
                } elseif (is_scalar($value) && !($key == 'final_price' && isset($details['final_price_formula']))) {
                    // don't overwrite final_price if formula exists
                    $product[$key] = $value;
                }
            }
        }
    }
    public function set_adjusted($value = 0)
    {
    ...
    public function get_qty($prid, $all_variations = true, $inc_gaw = false, $price_only_gaw = true)
    {
        if (!is_array($this->contents)) {
            return false;
        }
        $in_cart_qty = 0;
        if ($all_variations) {
            $prid = Inventory_Helper::get_prid($prid);
        }
        $cart_products = $this->contents;
        foreach ($cart_products as $id => $cart_products_values) {
            if ($all_variations) {
                if ($prid == Inventory_Helper::get_prid($id)) {
                    $in_cart_qty += $cart_products_values['qty'];
                }
            } else if ($prid == $id) {
                $in_cart_qty += $cart_products_values['qty'];
            }
        ...
        if ($inc_gaw) {
            $cart_products = $this->giveaway;
            if (is_array($cart_products)) {
                foreach ($cart_products as $id => $cart_products_values) {
                    if ($all_variations) {
                        if ($prid == Inventory_Helper::get_prid($id)) {
                            if (!$price_only_gaw || \common\helpers\Gifts::in_qty_discount($cart_products_values['gaw_id'])) {
                                $in_cart_qty += $cart_products_values['qty'];
                            }
                        }
                    } else if ($prid == $id) {
    ...
    /* PC configurator addon begin */
    public function add_cart_cfg($products_id, $qty = '1', $attributes = '', $notify = false, $parent_id = '', $type = 'update', $relation_type = '')
    {
        $customer_id = Yii::$app->user->get_id();
        if ($this->in_cart($products_id) && $this->contents[$products_id]['parent'] == $parent_id && $type == 'update') {
            ...
            $this->contents[$products_id] = ['qty' => $qty, 'parent' => $parent_id, 'relation_type' => $relation_type];
            $this->contents[$products_id]['platform_id'] = \common\classes\platform::current_id();
            if (is_array($attributes)) {
                $this->contents[$products_id]['attributes'] = $attributes;
            }
        ...
        \common\components\google\widgets\Google_Tagmanger::set_event('addToCart');
        $this->cart_id = $this->generate_cart_id();
        foreach (\common\helpers\Hooks::get_list('shopping-cart/add-cart-cfg') as $filename) {
            include $filename;
        }
        return $products_id;
    }
    public function add_configuration(array $post_data, $notify = true, $type = 'update')
    {
        global $new_products_id_in_cart;
        $bundle_products = [];
        $bundle_products_attributes = [];
        $post_data['id'] = $post_data['id'] ?? null;
        $sub = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($post_data['products_id'], $post_data['id']));
        ...
        foreach ($post_data['elements'] as $key => $value) {
            ...
            // {{
            if ($ext = \common\helpers\Acl::check_extension('ProductBundles', 'getBundleProducts')) {
                $bundle_products[$key . '-' . $value] = $ext::get_bundle_products($value, \common\classes\platform::current_id());
                if (is_array($bundle_products[$key . '-' . $value]) && count($bundle_products[$key . '-' . $value]) > 0) {
                    $bundle_products_attributes[$key . '-' . $value] = [];
                    ...
                    if (is_array($attributes)) {
                        foreach ($attributes as $pk => $pv) {
                            if (strpos($pk, '-') !== false) {
                                $temp = explode('-', $pk);
                                $bundle_products_attributes[$key . '-' . $value][$temp[1]][$temp[0]] = $pv;
                                //products options in bundle
                            } else {
                                $bundle_products_attributes[$key . '-' . $value][$value][$pk] = $pv;
                                //main product option
                            }
                        }
                    }
                    foreach ($bundle_products[$key . '-' . $value] as $prid => $bundle_qty) {
                        $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($prid, $bundle_products_attributes[$key . '-' . $value][$prid] ?? null)) . '{sub}' . $sub;
                        $products_uprid .= $key + \common\extensions\Product_Configurator\helpers\Configurator::BUNDLE_ELEMENTS_SHIFT . '|' . $sub_product_id . '|';
                    }
                    continue;
                }
            }
            ...
            if (is_array($post_data['elements_attr'][$key][$value])) {
                $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($value, $post_data['elements_attr'][$key][$value])) . '{sub}' . $sub;
            } else {
                $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($value, $post_data['elements_attr'][$value])) . '{sub}' . $sub;
            }
        ...
        if (isset($post_data['previous_id']) && isset($this->contents[$post_data['previous_id']])) {
            ...
            if ($post_data['pconfig_dont_inc_qty'] ?? false) {
                ...
                // have no idea why he increments here
        ...
        foreach ($post_data['elements'] as $key => $value) {
            ...
            // {{
            \common\helpers\Php8::null_arr_props($bundle_products, [$key . '-' . $value]);
            if (is_array($bundle_products[$key . '-' . $value]) && count($bundle_products[$key . '-' . $value]) > 0) {
                foreach ($bundle_products[$key . '-' . $value] as $prid => $bundle_qty) {
                    $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($prid, $bundle_products_attributes[$key . '-' . $value][$prid] ?? null)) . '{sub}' . $sub;
            ...
            // }}
            if (is_array($post_data['elements_attr'][$key][$value])) {
                $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($value, $post_data['elements_attr'][$key][$value] ?? null)) . '{sub}' . $sub;
                $__ids[] = $this->add_cart_cfg($sub_product_id, $qty * $elements_qty, $post_data['elements_attr'][$key][$value] ?? null, false, $products_uprid, $type);
            } else {
                $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($value, $post_data['elements_attr'][$value]['id'] ?? null)) . '{sub}' . $sub;
    ...
    public function configurator_price($parent_id, $products = '', $with_tax = true, $_default_field = 'final_price')
    {
        $currencies = \Yii::$container->get('currencies');
        if ($this->contents[$parent_id]['qty'] == 0) {
            return;
        }
        ...
        for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
            if (Array_Helper::get_value($products, [$i, 'parent']) == $parent_id) {
                if (isset($products[$i]['relation_type']) && $products[$i]['relation_type'] == 'linked') {
                    continue;
                }
                $configurator_price += $currencies->calculate_price($products[$i][$_default_field], $with_tax ? \common\helpers\Tax::get_tax_rate($products[$i]['tax_class_id']) : 0, $this->contents[$products[$i]['id']]['qty']);
    ...
    public function get_subproducts($parent_id)
    {
        $ar = [];
        if (is_array($this->contents)) {
            foreach ($this->contents as $products_id => $value) {
                if (Array_Helper::get_value($this->contents, [$products_id, 'parent']) == $parent_id) {
                    $ar[] = $products_id;
                }
            }
        }
    ...
    /* PC configurator addon end */
    public function add_bundle($products_id, $bundle_products = [], $qty = '1', $attributes = '', $notify = true, $relation_type = '')
    {
        $bundle_products_attributes = [];
        if (is_array($attributes)) {
            foreach ($attributes as $pk => $pv) {
                if (strpos($pk, '-') !== false) {
                    $temp = explode('-', $pk);
                    $bundle_products_attributes[$temp[1]][$temp[0]] = $pv;
                    //products options in bundle
                } else {
                    $bundle_products_attributes[$products_id][$pk] = $pv;
                    //main product option
                }
            }
        }
        $sub = Inventory_Helper::get_uprid($products_id, $bundle_products_attributes[$products_id] ?? null);
        $sub = Inventory_Helper::normalize_id($sub);
        $products_uprid = $sub . '{tpl}';
        $key = 0;
        foreach ($bundle_products as $prid => $bundle_qty) {
            $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($prid, $bundle_products_attributes[$prid] ?? null)) . '{sub}' . $sub;
            $products_uprid .= ++$key . '|' . $sub_product_id . '|';
        }
        ...
        $__ids = [];
        $__ids[] = $this->add_cart_cfg($products_uprid, $qty, $bundle_products_attributes[$products_id] ?? null, $notify, false, 'update', $relation_type);
        foreach ($bundle_products as $prid => $bundle_qty) {
            $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($prid, $bundle_products_attributes[$prid] ?? null)) . '{sub}' . $sub;
            if (count(\common\helpers\Product::get_child_array($sub_product_id)) == 0) {
                $__ids[] = $this->add_cart_cfg($sub_product_id, $qty * $bundle_qty, $bundle_products_attributes[$prid] ?? null, false, $products_uprid, $type, $relation_type);
            }
        }
        $this->cleanup();
        Yii::$app->get('PropsHelper')::cart_changed($this);
    ...
    public function add_custom_bundle($notify = true, $type = 'update')
    {
        global $new_products_id_in_cart;
        $sub = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid(\Yii::$app->request->post('products_id'), \Yii::$app->request->post('id')));
        $products_uprid = $sub . '{tpl}';
        ...
        foreach ($_POST['custom_bundles'] as $key => $value) {
            ...
            if (is_array($_POST['custom_bundles_attr'][$key][$value] ?? null)) {
                $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($value, $_POST['custom_bundles_attr'][$key][$value])) . '{sub}' . $sub;
            } else {
                $sub_product_id = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($value, $_POST['custom_bundles_attr'][$value] ?? null)) . '{sub}' . $sub;
            }
            $products_uprid .= $key . '|' . $sub_product_id . '|';
        }
    ...
    public function is_amount($uprid)
    {
    ...
    public function add_cart_amount($price, $qty = 1, $name = '', $tax = null, $postfix = true)
    {
        global $order;
        if (empty($name)) {
        ...
        $this->contents[$products_id] = ['qty' => $qty, 'parent' => ''];
        if (!Yii::$app->user->is_guest) {
            $a_basket = Customers_Basket::create(Yii::$app->user->get_id(), $products_id, $qty);
            $a_basket->save();
        }
        $this->set_overwrite($products_id, 'final_price', $price);
        $name_post = '';
        ...
        if (!is_null($tax) && strpos($tax, '_') !== false) {
            ...
            if (count($ex) == 2) {
                ...
                $this->set_overwrite($products_id, 'tax_selected', $tax);
                $this->set_overwrite($products_id, 'tax', $tax_value);
                $this->set_overwrite($products_id, 'tax_class_id', $ex[0]);
                $this->set_overwrite($products_id, 'tax_description', $description);
            } else {
                $this->set_overwrite($products_id, 'tax_selected', 0);
                $this->set_overwrite($products_id, 'tax', 0);
                $this->set_overwrite($products_id, 'tax_class_id', 0);
                $this->set_overwrite($products_id, 'tax_description', '');
            }
        } elseif (!is_null($tax)) {
            $uprid = $products_id;
            $tax_value = \common\helpers\Tax::get_tax_rate($tax);
            $this->set_overwrite($uprid, 'tax_selected', $tax);
            $this->set_overwrite($uprid, 'tax', $tax_value);
            $this->set_overwrite($uprid, 'tax_class_id', $tax);
            $this->set_overwrite($uprid, 'tax_description', \common\helpers\Tax::get_tax_description($tax, $order->tax_address['entry_country_id'], $order->tax_address['entry_zone_id']));
        } else {
            $uprid = $products_id;
            $this->set_overwrite($uprid, 'tax_selected', 0);
            $this->set_overwrite($uprid, 'tax', 0);
            $this->set_overwrite($uprid, 'tax_class_id', 0);
            $this->set_overwrite($uprid, 'tax_description', '');
        }
        $this->set_overwrite($products_id, 'name', $name . $name_post);
        $this->cleanup();
        $this->update_basket_info();
        $this->update_customer_info();
        return $products_id;
    }
    public function edit_cart_amount($products_id, $price = false, $qty = false, $name = false, $tax = false)
    {
        global $order;
        ...
        if ($name !== false && mb_strlen($name) > 0) {
            $this->set_overwrite($products_id, 'name', $name);
        }
        if ($tax !== false) {
            $ex = explode('_', $tax);
            if (count($ex) == 2) {
    ...
    public function product_amount_data($products_id, $add = false)
    {
        $currencies = \Yii::$container->get('currencies');
        ...
        /** @var \common\extensions\Inventory\Inventory $ext */
        if (($ext = \common\helpers\Extensions::is_allowed('Inventory')) && !Inventory_Helper::disabled_on_product($products_id)) {
            $stock_info = $ext::get_inventory_stock($products_id, 0, 1);
        } else {
            $stock_info = \common\classes\Stock_Indication::product_info(['products_id' => $products_id, 'stock_indication_id' => 0, 'max_qty' => $this->contents[$products_id]['qty'] + 300, 'cart_qty' => $this->contents[$products_id]['qty'], 'cart_class' => true, 'products_quantity' => $this->contents[$products_id]['qty'] + 300]);
            $stock_info['quantity_max'] = $this->contents[$products_id]['qty'] + 300;
        }
        $product_name = isset($this->overwrite[$products_id]['name']) ? $this->overwrite[$products_id]['name'] : '';
        $product_model = isset($this->overwrite[$products_id]['model']) ? $this->overwrite[$products_id]['model'] : '';
        $product_qty = isset($this->contents[$products_id]['qty']) ? $this->contents[$products_id]['qty'] : 1;
        $product_final_price = isset($this->overwrite[$products_id]['final_price']) ? $this->overwrite[$products_id]['final_price'] : 0;
        $product_tax = isset($this->overwrite[$products_id]['tax']) ? $this->overwrite[$products_id]['tax'] : 0;
        $products_tax_class_id = isset($this->overwrite[$products_id]['tax_class_id']) ? $this->overwrite[$products_id]['tax_class_id'] : 0;
        $products_tax_selected = isset($this->overwrite[$products_id]['tax_selected']) ? $this->overwrite[$products_id]['tax_selected'] : 0;
        $products_tax_description = isset($this->overwrite[$products_id]['tax_description']) ? $this->overwrite[$products_id]['tax_description'] : '';
        $data = ['id' => $products_id, 'products_id' => $products_id, 'product_id' => $products_id, 'uprid' => $products_id, 'product_name' => $product_name, 'name' => $product_name, 'model' => $product_model, 'image' => '', 'free_delivery' => 0, 'not_for_same_day' => 0, 'gift_wrap_allowed' => 0, 'gift_wrap_price' => 0, 'gift_wrapped' => 0, 'price' => $product_final_price, 'products_file' => '', 'ga' => 0, 'is_virtual' => 0, 'qty' => $product_qty, 'quantity' => $product_qty, 'reserved_qty' => null, 'stock_info' => $stock_info, 'weight' => 0.0, 'product_qty' => $product_qty + 300, 'final_price' => $product_final_price, 'parent' => '', 'sub_products' => false, 'tax' => $product_tax, 'tax_class_id' => $products_tax_class_id, 'tax_description' => $products_tax_description, 'tax_selected' => $products_tax_selected, 'attributes' => '', 'overwritten' => $this->get_owerwritten($products_id), 'subscription' => '0', 'bonus_points_price' => null, 'bonus_points_cost' => null, 'product_valid' => '1', 'final_price_tax' => $currencies->display_price($product_final_price, \common\helpers\Tax::get_tax_rate($products_tax_class_id), 1), 'product_price' => $currencies->display_price($product_final_price, \common\helpers\Tax::get_tax_rate($products_tax_class_id), 1), 'product_unit_price' => $product_final_price, 'special_price' => '', 'special_unit_price' => '', 'product_in_cart' => true, 'current_uprid' => $products_id, 'attributes_array' => [], 'bundles_block' => '', 'bundles' => [], 'order_quantity' => ['order_quantity_minimal' => 1, 'order_quantity_max' => -1, 'order_quantity_step' => 1, 'pack_unit' => 0, 'packaging' => 0], 'is_editing' => false, 'product_attributes' => '<div id="product-attributes"></div>', 'subscription_code' => '', 'product' => ['product_id' => $products_id, 'id' => $products_id, 'products_id' => $products_id, 'uprid' => $products_id, 'qty' => $product_qty, 'quantity' => $product_qty, 'name' => $product_name, 'stock_info' => $stock_info, 'final_price' => $product_final_price, 'sub_products' => false, 'tax_selected' => $products_tax_selected, 'product_price' => $currencies->display_price($product_final_price, \common\helpers\Tax::get_tax_rate($products_tax_class_id), 1), 'final_price_tax' => $currencies->display_price($product_final_price, \common\helpers\Tax::get_tax_rate($products_tax_class_id), 1), 'tax' => $product_tax, 'tax_description' => $products_tax_description, 'tax_class_id' => $products_tax_class_id, 'price' => $product_final_price]];
        /* Now not necessary, maybe in future.....
           ...
           /**/
        if ($this->exist_owerwritten($products_id)) {
            $this->over_write($products_id, $data);
        }
        return $data;
    }
}