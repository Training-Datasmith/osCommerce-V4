...
class shopping_cart
{
    ...
    public function restoreTotals()
    {
        if ($this->order_id > 0) {
            ...
            while ($totals = tep_db_fetch_array($totals_query)) {
                ...
                $this->totals[$totals['class']] = ['title' => $totals['title'],
                    'value' => [
    ...
    public function restoreContentProducts($uid = false)
    {
    ...
    public function add_cart($products_id, $qty = '1', $attributes = '', $notify = true, $gaw_id = 0, $gift_wrap = null, $props = '')
    {
        ...
        if ((int)$gaw_id > 0) { // product added as GA, check it
            ...
            if ($this->in_cart($products_id) && ((int)$gaw_id == 0)) {
                ...
                if (is_array($packQty)) {
                    $this->contents[$products_id] = ['qty' => $qty, 'parent' => '', 'unit' => $packQty['unit'], 'pack_unit' => $packQty['pack_unit'], 'packaging' => $packQty['packaging'], 'is_pack' => 1];
                } else {
    ...
    public function update_quantity($products_id, $quantity = '', $attributes = '', $gift_wrap = false)
    {
        ...
        if (is_array($packQty)) {
            $this->contents[$products_id] = ['qty' => $quantity, 'parent' => $parent, 'reserved_qty' => $reserved_qty, 'relation_type' => $relationType, 'unit' => $packQty['unit'], 'pack_unit' => $packQty['pack_unit'], 'packaging' => $packQty['packaging'], 'is_pack' => 1];
        } else {
            $this->contents[$products_id] = ['qty' => $quantity, 'parent' => $parent, 'reserved_qty' => $reserved_qty, 'relation_type' => $relationType];
        }
        if ($props !== false) {
            $this->contents[$products_id]['props'] = $props;
        }
    ...
    {$total_items = 0;
        if (is_array($this->contents)) {
            foreach ($this->contents as $products_id => $value) {
                ...
                if (tep_not_null(ArrayHelper::getValue($this->contents, [$products_id,'parent']))) {
                    continue;
                }
                ...
                */
                if ($swProduct) {
                    ...
                    if ($products['is_bundle'] && ($ext = \common\helpers\Extensions::isAllowed('ProductBundles'))) {
                        $stock_info = \common\classes\StockIndication::product_info($ext::getStockIndicationData([
                            'products_id' => $products_id,
                    ...
                    } elseif /** @var \common\extensions\Inventory\Inventory $ext */
                    (($ext = \common\helpers\Extensions::isAllowed('Inventory')) && !InventoryHelper::disabledOnProduct($stock_products_id)) {
...
// }}
        ...
        if (sizeof($this->giveaway) > 0) { // if we also have GA add them too
            foreach ($this->giveaway as $products_id => $product) {
                ...
                // }} stock info
                $products_array[$products_id . '(GA)'] = ['id' => $products_id,
                    'name' => $products['products_name'],
                    'model' => $products['products_model'],
                    'image' => $products['products_image'],
                    'stock_info' => $stock_info,
        ...
        foreach ($virtual_gift_cards as $virtual_gift_card) {
            ...
            $products_array[$products_id] = ['id' => $products_id,
                'use_shipwire' => 0,
    ...
    public function is_valid_product_data($product_id, $attributes)
    {
        ...
        if (\common\helpers\Attributes::has_product_attributes($product_id)) {
            // include bundle
            // {{ Products Bundle Sets
            $bundle_products = [InventoryHelper::get_prid($product_id)];
            if ($ext = \common\helpers\Acl::checkExtensionAllowed('ProductBundles', 'allowed')) {
    ...
    /* PC configurator addon begin */
    public function add_cart_cfg($products_id, $qty = '1', $attributes = '', $notify = false, $parent_id = '', $type = 'update', $relationType = '')
    {
        $customer_id = Yii::$app->user->getId();
        if ($this->in_cart($products_id) && $this->contents[$products_id]['parent'] == $parent_id && $type == 'update') {
            ...
            $this->contents[$products_id] = ['qty' => $qty, 'parent' => $parent_id, 'relation_type' => $relationType];
            $this->contents[$products_id]['platform_id'] = \common\classes\platform::currentId();
            if (is_array($attributes)) {
                $this->contents[$products_id]['attributes'] = $attributes;
            }
    ...
    public function configurator_price($parent_id, $products = '', $withTax = true, $_default_field = 'final_price')
    {
        $currencies = \Yii::$container->get('currencies');
    ...
    public function add_bundle($products_id, $bundle_products = [], $qty = '1', $attributes = '', $notify = true, $relationType = '')
    {
        $bundle_products_attributes = [];
        if (is_array($attributes)) {
            foreach ($attributes as $pk => $pv) {
    ...
    public function isAmount($uprid)
    {
