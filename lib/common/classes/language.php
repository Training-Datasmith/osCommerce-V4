...
use app\components\Cart_Factory;
use yii\web\Request;
class language
{
    public static $def_languages = ['ar' => 'ar([-_][a-zA-Z]{2})?|arabic', 'bg' => 'bg|bulgarian', 'br' => 'pt[-_]br|brazilian portuguese', 'ca' => 'ca|catalan', 'cs' => 'cs|czech', 'da' => 'da|danish', 'de' => 'de([-_][a-zA-Z]{2})?|german', 'el' => 'el|greek', 'en' => 'en([-_][a-zA-Z]{2})?|english', 'es' => 'es([-_][a-zA-Z]{2})?|spanish', 'et' => 'et|estonian', 'fi' => 'fi|finnish', 'fr' => 'fr([-_][a-zA-Z]{2})?|french', 'gl' => 'gl|galician', 'he' => 'he|hebrew', 'hu' => 'hu|hungarian', 'id' => 'id|indonesian', 'it' => 'it|italian', 'ja' => 'ja|japanese', 'ko' => 'ko|korean', 'ka' => 'ka|georgian', 'lt' => 'lt|lithuanian', 'lv' => 'lv|latvian', 'nl' => 'nl([-_][a-zA-Z]{2})?|dutch', 'no' => 'no|norwegian', 'pl' => 'pl|polish', 'pt' => 'pt([-_][a-zA-Z]{2})?|portuguese', 'ro' => 'ro|romanian', 'ru' => 'ru|russian', 'sk' => 'sk|slovak', 'sr' => 'sr|serbian', 'sv' => 'sv|swedish', 'th' => 'th|thai', 'tr' => 'tr|turkish', 'uk' => 'uk|ukrainian', 'tw' => 'zh[-_]tw|chinese traditional', 'zh' => 'zh|chinese simplified'];
    public $languages;
    ...
    {
        $this->languages = ['ar' => 'ar([-_][a-zA-Z]{2})?|arabic', 'bg' => 'bg|bulgarian', 'br' => 'pt[-_]br|brazilian portuguese', 'ca' => 'ca|catalan', 'cs' => 'cs|czech', 'da' => 'da|danish', 'de' => 'de([-_][a-zA-Z]{2})?|german', 'el' => 'el|greek', 'en' => 'en([-_][a-zA-Z]{2})?|english', 'es' => 'es([-_][a-zA-Z]{2})?|spanish', 'et' => 'et|estonian', 'fi' => 'fi|finnish', 'fr' => 'fr([-_][a-zA-Z]{2})?|french', 'gl' => 'gl|galician', 'he' => 'he|hebrew', 'hu' => 'hu|hungarian', 'id' => 'id|indonesian', 'it' => 'it|italian', 'ja' => 'ja|japanese', 'ko' => 'ko|korean', 'ka' => 'ka|georgian', 'lt' => 'lt|lithuanian', 'lv' => 'lv|latvian', 'nl' => 'nl([-_][a-zA-Z]{2})?|dutch', 'no' => 'no|norwegian', 'pl' => 'pl|polish', 'pt' => 'pt([-_][a-zA-Z]{2})?|portuguese', 'ro' => 'ro|romanian', 'ru' => 'ru|russian', 'sk' => 'sk|slovak', 'sr' => 'sr|serbian', 'sv' => 'sv|swedish', 'th' => 'th|thai', 'tr' => 'tr|turkish', 'uk' => 'uk|ukrainian', 'tw' => 'zh[-_]tw|chinese traditional', 'zh' => 'zh|chinese simplified'];
        $this->catalog_languages = [];
        $this->dp_language = \frontend\design\Info::platform_def_language();
        if (!$this->dp_language) {
            $this->dp_language = strtolower(DEFAULT_LANGUAGE);
        }
        $paltform_languages = \frontend\design\Info::platform_languages();
        if (\frontend\design\Info::is_totally_admin()) {
            $paltform_languages = \yii\helpers\Array_Helper::get_column(\common\helpers\Language::get_languages(), 'code');
        }
        ...
        if (!is_array($languages)) {
            ...
            while ($language = tep_db_fetch_array($languages_query)) {
                ...
                if (!(isset($this) && get_class($this) == __CLASS__)) {
                    $def_lang = self::$def_languages;
                } else {
                    $def_lang = $this->def_languages;
                }
                $languages[strtolower($language['code'])] = ['id' => $language['languages_id'], 'code' => $language['code'], 'name' => $language['name'], 'image' => $language['image'], 'image_svg' => $language['image_svg'], 'directory' => empty($language['directory']) ? isset($def_lang[$language['code']]) ? substr($def_lang[$language['code']], strrpos($def_lang[$language['code']], '|') + 1) : '' : $language['directory'], 'locale' => $language['locale']];
            }
        }
        return $languages;
    }
    ...
    public function set_language($language, $update = false)
    {
        ...
        } else if (in_array($this->dp_language, $this->paltform_languages)) {
    ...
    public static function default_id($current_platform_id = null)
    {
        if (is_null($current_platform_id)) {
            $current_platform_id = (int) \common\classes\platform::current_id();
        }
        static $_cache = [];
        if (!isset($_cache[$current_platform_id])) {
            $_cache[$current_platform_id] = tep_db_fetch_array(tep_db_query('select l.languages_id from ' . TABLE_PLATFORMS . ' p, ' . TABLE_LANGUAGES . " l where p.platform_id = '" . $current_platform_id . "' and l.code = p.default_language"));
        }
        $query = $_cache[$current_platform_id];
    ...
    public static function get_code($for_language = null, $strict = false)
    {
        ...
        if (is_null($code)) {
            if ($strict) {
                ...
                $code = \frontend\design\Info::platform_def_language();
                if (!$code) {
    ...
    public static function redirect_to_locale(int $languages_id, string $country_code, language $lng, Request $request, string $request_type): void
    {
        ...
        if (defined('ALLOW_LOCALE_REDIRECT') && in_array(ALLOW_LOCALE_REDIRECT, ['Ip', 'Browser'], true)) {
            if (ALLOW_LOCALE_REDIRECT === 'Browser') {
                $country_code = $lng->language['code'];
            }
            if ($country_code && (int) $lng->catalog_languages[$country_code]['id'] !== $languages_id && in_array($country_code, $lng->paltform_languages, true)) {
                try {
                    Cart_Factory::init_cart();
                    //flag First Run
                    $get = (array) \Yii::$app->request->get();
                    $new_url_param = array_merge([$request->get_path_info()], $get + ['language' => $country_code]);
                    $new_url = \Yii::$app->url_manager->create_absolute_url($new_url_param, $request_type === 'SSL' ? 'https' : 'http');
