...
class navigation
{
    ...
    {
        global $PHP_SELF, $request_type, $c_path;
        $set = 'true';
            ...
            if ($this->path[$i]['page'] == basename($PHP_SELF)) {
                if (isset($c_path)) {
                    if (!isset($this->path[$i]['get']['cPath'])) {
                        continue;
                    } else if ($this->path[$i]['get']['cPath'] == $c_path) {
                        array_splice($this->path, $i + 1);
                        $set = 'false';
                        break;
                    } else {
                        $old_c_path = explode('_', $this->path[$i]['get']['cPath']);
        ...
        if ($set == 'true') {
            ...
            $this->path[] = ['page' => basename($PHP_SELF), 'mode' => $request_type, 'get' => $_GET, 'post' => []];
        }
    ...
    {
        if (Yii::$app->request->is_ajax && !$ajax) {
            return '';
        }
        global $PHP_SELF, $request_type;
        if (is_array($page)) {
            $this->snapshot = ['page' => $page['page'] ?? '', 'mode' => $page['mode'] ?? '', 'get' => $page['get'] ?? [], 'post' => $page['post'] ?? []];
        } else if (class_exists('Yii')) {
            $this->snapshot = ['page' => Yii::$app->request->get_path_info(), 'mode' => $request_type, 'get' => Yii::$app->request->get_query_params(), 'post' => Yii::$app->request->get_body_params()];
        } else {
            $this->snapshot = ['page' => basename($PHP_SELF), 'mode' => $request_type, 'get' => $_GET, 'post' => []];
        }
    }
