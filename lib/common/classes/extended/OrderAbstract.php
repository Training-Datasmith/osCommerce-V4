...
abstract class OrderAbstract extends OrderShadowAbstract
{
    ...
    public function prepareDetails($order_id)
    {
    ...
    public function overloadAddressDetails($type = 'delivery')
    {
        ...
        $this->$type = ['name' => $this->data[$type . '_name'],
    ...
    public function query($order_id)
    {
        ...
        while ($totals = tep_db_fetch_array($totals_query)) {
            $this->totals[$totals['sort_order']] = ['title' => $totals['title'],
                'value' => $totals['value'],
                'class' => $totals['class'],
                'code' => $totals['class'],
                'text' => $totals['text'],
...
// EOF: Lango Added for print order mod
    ...
    public function save_details($notified = true)
    {
        ...
        $sql_data_array = ['orders_id' => $this->order_id,
    ...
    public function save_products($notify = true)
    {
        ...
        for ($i = 0, $n = sizeof($this->products); $i < $n; $i++) {
            ...
            (isset($this->products[$i]['attributes'])) {
                $attributes_exist = '1';
        ...
        if (!empty($emailTemplate)) {
            ...
            if ($this->customer['email_address']) {
                \common\helpers\Mail::send(
                    ...
                    $email_subject,
                    $email_text,
                    $platform_config->const_value('STORE_OWNER'),
                    $platform_config->const_value('STORE_OWNER_EMAIL_ADDRESS'),
                    [],
                    '',
                    $attachment,
                    ['add_br' => 'no', 'platform_id' => $this->info['platform_id']]
                );
            }
    ...
    public function send_status_notify($notify_comments = '', $extra_template_params = [], $email_headers = [])
    {
        ...
        if (!empty($emailTemplate)) {
            ...
            \common\helpers\Mail::send(
                ...
                $email_headers,
                $attachment,
                ['add_br' => 'no', 'platform_id' => $platform_id]
            );
            $customer_notified = 1;
