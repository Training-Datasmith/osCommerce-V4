...
class ot_coupon extends ModuleTotal
{
    ...
    public function clearValidProducts()
    {
        $this->valid_products = [];
        $this->validProducts = [];
        $this->totalDetails = [];
    ...
    public function setDiscountDetails(array $data)
    {
    ...
    private function _collect_posts($collect_data)
    {
        if (isset($collect_data['gv_redeem_code']) && !empty($collect_data['gv_redeem_code'])) {
            ...
            if (empty($gv_redeem_code)) {
                return ['error' => true, 'message' => ERROR_NO_REDEEM_CODE];
            }
            ...
            if ($newCode) {
                ...
                if ($check !== true) {
                    return ['error' => true, 'message' => $check];
                }
                ...
                if ($coupon_result->coupon_type != 'G') {
                    ...
                    return ['error' => false, 'message' => $coupon_amount, 'description' => $coupon_result->description->coupon_description];
                }
            }
        }
    }
     ...
     */
    public function calculate_credit($order_total, $coupon_id, $coupon_code)
    {
     ...
     * @return type
     */
    public function calculate_tax_deduction($coupon_id, $amount, $od_amount)
    {
        $ret = 0;
     ...
     * @return numeric
     */
    public function get_order_total($coupon_id)
    {
    ...
    public function isRestrictedByGroup($coupon_result)
    {
    ...
    public function isRestrictedByCountry($get_result)
    {
        if (tep_not_null($get_result['restrict_to_countries'])) {
            ...
            } elseif ($this->delivery['country']['id']) {
    ...
    public function calculateRestrictionsByChildProducts()
    {
