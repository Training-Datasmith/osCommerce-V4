...
class ot_gv extends ModuleTotal
{
    ...
    public function collect_posts($collect_data)
    {
        ...
        if (is_array($result)) {
            if ($this->config['ONE_PAGE_CHECKOUT'] == 'True') {
                ...
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, \common\helpers\Output::get_all_get_params(['error_message']) . 'error_message=' . urlencode($result['message']), 'SSL'));
            }
        }
    }

    public function _collect_posts($collect_data)
    {
        $currencies = \Yii::$container->get('currencies');
        ...
        if (isset($collect_data['gv_redeem_code']) && !empty($collect_data['gv_redeem_code'])) {
            ...
            if ($gv_result) {
                if ($gv_result->redeemTrack && ($gv_result->coupon_type == 'G')) {
                    return ['error' => true, 'message' => ERROR_NO_INVALID_REDEEM_GV];
                }
            } else {
            ...
            if ($gv_result && $gv_result->coupon_type == 'G') {
                if (!$this->manager->isCustomerAssigned()) {
                    ...
                    return ['error' => false, 'message' => ERROR_NO_CUSTOMER];
                }
                // Things to set
        ...
        if ($this->manager->has('cot_gv') && isset($collect_data['cot_gv_amount']) && is_numeric($collect_data['cot_gv_amount'])) {
            if ($this->manager->isCustomerAssigned()) {
                ...
                if ($hasAmount) {
                    ...
                    if ($hasAmount < $new_gv_amount) {
                        $new_gv_amount = $hasAmount;
                    }
                    if ($new_gv_amount > 0) {
    ...
    public function get_order_total()
    {
        ...
        if ($this->include_tax == 'false') {
            $order_total = $order->info['total_exc_tax'];
    ...
    public function configure_keys()
    {
        return [
            ...
            'MODULE_ORDER_TOTAL_GV_ORDER_STATUS_ID_COVERS' => [
