...
class ot_paymentfee extends ModuleTotal
{
    ...
    public function process($replacing_value = -1, $visible = false)
    {
        ...
        if ($pass || $replacing_value != -1) {
            ...
            if ($tof_amount || $visible) {
                ...
                $this->title = sprintf(
                    defined('MODULE_ORDER_TOTAL_PAYMENTFEE__FRONTEND_TITLE') && !empty(MODULE_ORDER_TOTAL_PAYMENTFEE__FRONTEND_TITLE) ? MODULE_ORDER_TOTAL_PAYMENTFEE__FRONTEND_TITLE : $this->title,
                    $moduleTitle
                );
  ...
  PRIMARY KEY (`payment_fee_id`)
    ...
    public function extra_params()
    {
        ...
        $updates = \Yii::$app->request->post('percent_prefix', []);
        if (is_array($updates)) {
        ...
        $updates = \Yii::$app->request->post('percent_value', []);
        if (is_array($updates)) {
            foreach ($updates as $key => $value) {
        ...
        $updates = \Yii::$app->request->post('fixed_prefix', []);
        if (is_array($updates)) {
            foreach ($updates as $key => $value) {
        ...
        $updates = \Yii::$app->request->post('fixed_value', []);
        if (is_array($updates)) {
            foreach ($updates as $key => $value) {
        ...
        $updates = \Yii::$app->request->post('minimum_order', []);
        if (is_array($updates)) {
            foreach ($updates as $key => $value) {
        ...
        $updates = \Yii::$app->request->post('maximum_order', []);
        if (is_array($updates)) {
            foreach ($updates as $key => $value) {
