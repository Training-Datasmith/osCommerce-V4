...
class Mandrill implements MailerInterface
{
    ...
    public function call($url, $params)
    {
        ...
        if ($result === null) {
            throw new Mandrill\Error('We were unable to decode the JSON response from the Mandrill API: ' . $response_body);
        }
    ...
    public function castError($result)
    {
        if ($result['status'] !== 'error' || !$result['name']) {
            throw new Mandrill\Error('We received an unexpected error: ' . json_encode($result));
        }
