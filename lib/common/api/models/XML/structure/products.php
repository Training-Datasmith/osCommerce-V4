...
return ['Header' => 'site/products', 'dependsOn' => ['site/languages', 'site/groups', 'site/currencies'], 'Data' => ['common\models\Products' => [
    ...
    'properties' => ['products_image' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_lrg' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_sm_1' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_xl_1' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_sm_2' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_xl_2' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_sm_3' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_xl_3' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_sm_4' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_xl_4' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_sm_5' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_xl_5' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_sm_6' => ['class' => 'IOAttachment', 'location' => '@images'], 'products_image_xl_6' => ['class' => 'IOAttachment', 'location' => '@images']],
    ...
    'withRelated' => [
        ...
        */
        'categoriesList' => ['xmlCollection' => 'AssignedCategories>AssignedCategory'],
        ...
        'productsAttributes' => ['xmlCollection' => 'Attributes>Attribute'],
    ...
    'afterImport' => function ($model, $data) {
        if (isset($data->data['products_id']) && is_object($data->data['products_id'])) {
            ...
            \common\models\Products_Description::update_all(['platform_id' => $platform_id], 'platform_id = 0');
            ...
            if (!is_array($assign_to_all_platform)) {
                ...
                }, \common\classes\platform::get_list());
            }
            ...
            if ($product = tep_db_fetch_array($check_product_query)) {
                ...
                if ($local_product) {
                    $product_import['without_inventory'] = !\common\helpers\Extensions::is_allowed('Inventory');
                    $product_import['weight_cm'] = $product['products_weight'];
                    $product_import['weight_in'] = $product['products_weight'] * 2.20462;
                    $product_import['assigned_platforms'] = $assign_to_all_platform;
                }
                $product_name = \common\helpers\Product::get_products_name($product['products_id']);
                $product['products_name'] = $product_name;
                $product['products_seo_page_name'] = \common\helpers\Seo::make_product_slug($product, $products_id);
                if ($local_product) {
                    $product_import['descriptions'] = ['*' => ['products_seo_page_name' => (string) $product['products_seo_page_name']]];
                }
                $get_data_r = tep_db_query('SELECT p.products_id, p.products_price, count(pa.options_values_price) ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . ' inner join ' . TABLE_PRODUCTS_ATTRIBUTES . ' pa on pa.products_id = p.products_id ' . "where p.products_id = '" . (int) $products_id . "' group by p.products_id, p.products_price " . ' having count(pa.options_values_price) > 0');
                if (tep_db_num_rows($get_data_r) == 0 || !\common\helpers\Extensions::is_allowed('Inventory')) {
                    if ($local_product) {
                        \common\models\Suppliers_Products::delete_all(['products_id' => $products_id]);
                        $product_import['suppliers_product'] = [['suppliers_model' => $product['products_model'], 'suppliers_id' => \common\helpers\Suppliers::get_default_supplier_id()]];
                    }
                }
                ...
                if (!empty($orig_file) && is_file($tmp_name) && !($check['products_images_id'] > 0)) {
                    if ($local_product) {
                        $product_import['images'][] = ['default_image' => 1, 'image_status' => 1, 'sort_order' => 0, 'image_description' => ['00' => ['image_source_url' => $tmp_name, 'orig_file_name' => pathinfo($tmp_name, PATHINFO_BASENAME), 'image_title' => (string) $product_name]]];
                    }
                }
                for ($i = 1; $i < 7; $i++) {
                    ...
                    if (!empty($orig_file) && is_file($tmp_name) && !($check['products_images_id'] > 0)) {
                        if ($local_product) {
                            $product_import['images'][] = ['default_image' => 0, 'image_status' => 1, 'sort_order' => $i, 'image_description' => ['00' => ['image_source_url' => $tmp_name, 'orig_file_name' => pathinfo($tmp_name, PATHINFO_BASENAME), 'image_title' => (string) $product_name]]];
                        }
                    }
                }
                if ($local_product) {
