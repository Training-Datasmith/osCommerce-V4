...
class Products_Container
{
     ...
     * @return $this
     */
    public function load_products(array $params = [])
    {
        if (isset($params['products_id']) && count($params) == 1) {
            if (!$this->has($params['products_id'])) {
                $this->_preload_product($params['products_id']);
                if ($this->has($params['products_id'])) {
                    $price_product_id = \common\helpers\Product::normalize_price_prid($params['products_id']);
                    $this->container[$params['products_id']]->attach_details(['special_price' => Product::get_products_special_price($price_product_id), 'products_price' => Product::get_products_price($price_product_id, 1), 'settings' => Product::get_settings($price_product_id)]);
                }
            }
            ...
            if (isset($this->container[$params['products_id']]) && $this->container[$params['products_id']] instanceof Product_Item) {
                $diff = @array_diff_assoc($params, (array) $this->container[$params['products_id']]);
                if (count($diff)) {
                    $this->container[$params['products_id']]->attach_details($diff);
                }
            } else {
                $this->container[$params['products_id']] = new Product_Item($params, \ArrayObject::ARRAY_AS_PROPS);
            }
        }
        return $this;
    }
    ...
    {
        return isset($this->container[$products_id]) && $this->container[$products_id] instanceof Product_Item;
    }
    protected function _preload_product($products_id)
    {
        if ($products_id) {
            $product = \common\models\Products::find()->alias('p')->where(['p.products_id' => $products_id])->with(['listingDescription' => function (\yii\db\Active_Query $query) {
                $query->add_select(['products_image_alt_tag_mask', 'products_head_desc_tag', 'products_image_title_tag_mask']);
            }])->one();
            if ($product) {
                $data = $product->get_attributes();
                ...
                if (is_array($product->related_records)) {
                    foreach ($product->related_records['listingDescription'] as $relation) {
                        foreach ($relation->get_attributes() as $name => $value) {
                ...
                $data['products_description'] = \common\classes\Tl_Url::replace_url($data['products_description'] ?? null);
                $data['products_description_short'] = \common\classes\Tl_Url::replace_url($data['products_description_short'] ?? null);
                $data['tax_rate'] = Tax::get_tax_rate($product['products_tax_class_id'] ?? null);
                $this->container[$products_id] = new Product_Item($data, \ArrayObject::ARRAY_AS_PROPS);
            }
        }
     ...
     * @return $this
     */
    public function load_list($products)
    {
        if (is_array($products)) {
            foreach ($products as $params) {
                if (!empty($params['products_id'])) {
                    $this->load_products($params);
                }
            }
        }
        return $this;
    ...
    public function check_attached_details($products_id, $block)
    {
     ...
     * @return $this
     */
    public function attach_details($products_id, $array)
    {
        if (!is_array($array)) {
            return $this;
        }
        if ($this->has($products_id)) {
            $this->container[$products_id]->attach_details($array);
        } else {
            $this->_preload_product(['products_id' => $products_id]);
            $this->container[$products_id]->attach_details($array);
        }
        return $this;
    }
     ...
     */
    public function get_all_products($listing_type = '')
    {
        if (!empty($listing_type)) {
            ...
            if ($map && $use_sort) {
                \yii\helpers\Array_Helper::multisort($map, 'sort_order');
            }
            return $map;
        }
    ...
    public function get_product($products_id)
    {
        ...
        $this->_preload_product($products_id);
        if ($this->has($products_id)) {
            return $this->container[$products_id];
        }
        return false;
