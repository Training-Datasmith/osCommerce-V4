...
use common\models\Products_Popularity;
class Popularity
{
    ...
    public function __construct()
    {
        ...
        Products_Popularity::delete_all("viewed_date < now() - interval {$this->days} day");
    }
    public function clear_current_popularity()
    {
        Products::update_all(['products_popularity' => 0]);
    }
    ...
    public function update_product_visit($products_id)
    {
        if ($products_id && isset($_SESSION['viewed_products']) && !isset($_SESSION['viewed_products'][(int) $products_id])) {
            $today_visit = Products_Popularity::find()->where('products_id=:prid and viewed_date = :date', [':prid' => (int) $products_id, ':date' => date('Y-m-d')])->one();
            if ($today_visit) {
                $today_visit->update_counters(['products_viewed' => 1]);
            } else {
                //recalculate total on 1st visit
                self::calculate_popularity($products_id);
                $today_visit = new Products_Popularity();
                $today_visit->set_attributes(['products_viewed' => 1, 'products_id' => (int) $products_id, 'viewed_date' => date('Y-m-d')], false);
                $today_visit->save(false);
            }
            $_SESSION['viewed_products'][(int) $products_id] = (int) $products_id;
            self::add_popularity($products_id, 'view');
        }
    }
    /**
     *
     ...
     */
    public static function add_popularity($products_id, $what)
    {
        $add = 0;
        switch ($what) {
            case 'view':
    ...
    public static function calculate_popularity($products_id = [])
    {
        $grouped = false;
        $q = new \yii\db\Query();
        $p_expression = ' 0 ';
        if (defined('POPULARITY_WEIGHT_VIEW_WEEK') && (float) POPULARITY_WEIGHT_VIEW_WEEK > 0 || defined('POPULARITY_WEIGHT_VIEW_MONTH') && (float) POPULARITY_WEIGHT_VIEW_MONTH > 0) {
        ...
        //    }
        if (defined('POPULARITY_WEIGHT_PUCHASE_WEEK') && (float) POPULARITY_WEIGHT_PUCHASE_WEEK > 0 || defined('POPULARITY_WEIGHT_PUCHASE_MONTH') && (float) POPULARITY_WEIGHT_PUCHASE_MONTH > 0) {
            $q->left_join('orders_products op', 'op.products_id = p.products_id')->left_join('orders o', 'o.orders_id = op.orders_id and o.date_purchased >= now() - interval :days day', [':days' => defined('POPULARITY_WEIGHT_PUCHASE_MONTH') && (float) POPULARITY_WEIGHT_PUCHASE_MONTH > 0 ? 30 : 7])->group_by('p.products_id');
            $grouped = true;
            if (defined('POPULARITY_WEIGHT_PUCHASE_WEEK') && POPULARITY_WEIGHT_PUCHASE_WEEK > 0) {
        ...
        if (!empty($products_id)) {
            ...
            $q->and_where(['p.products_id' => $products_id]);
        } else {
        ...
        if ($grouped) {
            ...
            $q->add_select(['popularity' => new \yii\db\Expression($p_expression)]);
            $sql = $q->create_command()->raw_sql;
