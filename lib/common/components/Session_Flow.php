...
use app\components\Init_Factory;
use frontend\design\Info;
use yii\base\Application;
use yii\base\Bootstrap_Interface;
class Session_Flow implements Bootstrap_Interface
{
    public function preload()
    {
        \Yii::$container->set_singleton('products', '\common\components\ProductsContainer');
        \Yii::$container->set_singleton('currencies', '\common\classes\Currencies');
        \Yii::$container->set_singleton('message_stack', '\common\classes\MessageStack');
    }
    public function bootstrap($app)
    {
        ...
        $app->on(Application::EVENT_BEFORE_REQUEST, function () use ($app) {
            if (defined('IS_IMAGE_CDN_SERVER') && IS_IMAGE_CDN_SERVER || defined('IS_NESTED_IMAGE_HANDLER')) {
                $app->default_route = 'image/cached';
                if ($app instanceof \yii\web\Application) {
                    $app->catch_all = ['image/cached'];
                }
            }
        });
        ...
        if (defined('DIR_WS_CATALOG_IMAGES')) {
            \Yii::set_alias('@webCatalogImages', DIR_WS_CATALOG_IMAGES);
        } else {
            \Yii::set_alias('@webCatalogImages', DIR_WS_CATALOG . DIR_WS_IMAGES);
        }
        ...
        if ($frontends_session = \common\helpers\Acl::check_extension('FrontendsSession', 'allowed')) {
            if ($frontends_session::allowed()) {
        ...
        if (\Yii::$app->id == 'app-pos') {
        ...
        } elseif (\frontend\design\Info::is_totally_admin()) {
            tep_session_start();
            $session_started = true;
        } else if (SESSION_FORCE_COOKIE_USE == 'True') {
            ...
            $spider_flag = false;
            if (tep_not_null($user_agent) && !\frontend\design\Info::is_totally_admin()) {
        ...
        global $PHP_SELF;
        if (basename($PHP_SELF) == 'index.php' && !\frontend\design\Info::is_totally_admin()) {
            $pl_currs = Info::platform_currencies();
            $currency = \Yii::$app->settings->get('currency');
            if (!Info::is_admin() && $currency !== false && (is_array($pl_currs) && !in_array($currency, $pl_currs) || (!$pl_currs and $currency != DEFAULT_CURRENCY))) {
                /// I don't know why session is destroyed instead of changing its data (currency, language) and clean up cart.....
                if ($frontends_session && $frontends_session::allowed()) {
                    ...
                    // be sure currency from payment is in platform currencies list
                }
            }
            $pl_langs = Info::platform_languages();
            $languages_id = (int) \Yii::$app->settings->get('languages_id');
            $_code = \common\helpers\Language::get_language_code($languages_id);
            if (!Info::is_admin() && $languages_id > 0 && (is_array($pl_langs) && !in_array($_code['code'], $pl_langs) || (!$pl_langs and !in_array($languages_id, \common\helpers\Language::get_language_id(strtolower(DEFAULT_LANGUAGE)))))) {
                /// I don't know why session is destroyed instead of changing its data (currency, language) and clean up cart.....
                if ($frontends_session && $frontends_session::allowed()) {
        ...
        if (\Yii::$app->id == 'app-pos') {
            \pos\components\Admin_Factory::init();
        } elseif (\frontend\design\Info::is_totally_admin()) {
            \backend\components\Admin_Factory::init();
