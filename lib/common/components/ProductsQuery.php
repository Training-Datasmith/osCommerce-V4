...
class ProductsQuery
{
    ...
    protected static $allowedFilterTypes = ['boxes', 'radio', 'pulldown'];
                            ...
                            if (is_array($values)) {
                                foreach ($values as $_ix => $value) {
                                    if (preg_match("/[\d]+\,/", $value)) {
                                        $vList = array_merge($vList, array_map('intval', explode(',', $value)));
                                    } else {
                                        $vList = $values;
                                    }
                                }
            ...
            if (!isset($ret[$v['properties_id']])) {
                $ret[$v['properties_id']] =
                    [
                      ...
                      'type' => (
                          ($v['range_select'] == 1 && $v['extra_values'] == 1) ? 'extra' :
    ...
    protected function getBrandFiltersArray($setting)
    {
        ...
        if (!empty($manufacturers_array)) {
            $retSorted[$pos] = [
              ...
              'not_selected' => empty(array_filter($manufacturers_array, function ($el) {
                  return $el['selected'];
              })),
