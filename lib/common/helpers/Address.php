...
use common\models\Address_Format;
use yii\helpers\Array_Helper;
class Address
{
    public static $allowed_fields = ['gender', 'firstname', 'lastname', 'street_address', 'suburb', 'postcode', 'city', 'state', 'country', 'reg_number', 'company', 'company_vat', 'customs_number', 'telephone', 'email_address', '(', ')', ','];
        ...
        if ($hidden) {
            ...
            if (is_array($d) && $d['countries_id']) {
                ...
                while ($countries = tep_db_fetch_array($countries_query)) {
                    $countries_array[] = ['id' => $countries['countries_name'], 'text' => $countries['countries_name']];
                }
            ...
            if (in_array(ACCOUNT_STATE, ['required', 'required_register', 'visible', 'visible_register'])) {
                ...
                if ($check['total'] > 0) {
                    ...
                    while ($zones = tep_db_fetch_array($zones_query)) {
                        $zones_array[] = ['id' => $zones['zone_name'], 'text' => $zones['zone_name']];
                    }
    ...
    public static function address_format($address_format_id, $address, $html, $boln, $eoln, $micro_data = false, $show_phone = false, $prefix = 'ACCOUNT_')
    {
        if ($address_format = static::get_format_by_id($address_format_id)) {
            ...
            $gender = static::get_gender_name(@$address['gender']);
            ...
            if ($country == '' && is_string(Array_Helper::get_value($address, 'country'))) {
                $country = Output::output_string_protected($address['country']);
            }
            ...
            $rows = self::prepare_format($fmt);
            if (is_array($rows)) {
                foreach ($rows as $key_r => $row) {
                    $replaces = [];
                    foreach ($row as $key => $field) {
                        if (isset($defined[$field]) && tep_not_null($defined[$field])) {
                            $value = '';
                            if ($field == 'company_vat' && defined($prefix . 'COMPANY_VAT') && !in_array(constant($prefix . 'COMPANY_VAT'), ['required', 'required_register', 'visible', 'visible_register']) && !$micro_data) {
                                continue;
                            }
                            if ($field == 'customs_number' && defined($prefix . 'CUSTOMS_NUMBER') && !in_array(constant($prefix . 'CUSTOMS_NUMBER'), ['required', 'required_register', 'visible', 'visible_register', 'required_company']) && !$micro_data) {
                                continue;
                            }
                            if ($field == 'company' && defined($prefix . 'COMPANY') && !in_array(constant($prefix . 'COMPANY'), ['required', 'required_register', 'visible', 'visible_register']) && !$micro_data) {
                                continue;
                            }
                            if (isset($row[$key - 1]) && in_array($row[$key - 1], ['('])) {
                                $value .= $row[$key - 1];
                            }
                            $value .= $defined[$field];
                            if (isset($row[$key + 1]) && in_array($row[$key + 1], [')', ','])) {
    ...
    public static function check_buisy_address_formats(array $ids)
    {
    ...
    public static function find_order_ab($order_id, $ab = 'delivery')
    {
        ...
        if (in_array($ab, ['customers', 'billing', 'delivery'])) {
        ...
        */
        ...
        $find_ab_sql = 'SELECT ab.*, ' . implode(', ', $columns) . ', IFNULL(z.zone_name, ab.entry_state) AS entry_state ' . 'FROM ' . TABLE_ORDERS . ' o ' . ' LEFT JOIN ' . TABLE_ADDRESS_BOOK . ' ab ON ab.customers_id=o.customers_id AND (' . implode('AND ', $ab_compare) . ') ' . ' LEFT JOIN ' . TABLE_ZONES . ' z on (ab.entry_zone_id = z.zone_id) ' . "WHERE o.orders_id='" . (int) $order_id . "'";
        $get_ab_r = tep_db_query($find_ab_sql);
        $ab_data = false;
        if (tep_db_num_rows($get_ab_r) > 0) {
            while ($_ab = tep_db_fetch_array($get_ab_r)) {
                if ($ab_data === false) {
                    $country_match = tep_db_fetch_array(tep_db_query('SELECT countries_id, countries_iso_code_2, countries_iso_code_3 ' . 'FROM ' . TABLE_COUNTRIES . ' ' . "WHERE countries_name='" . tep_db_input($_ab['order_country']) . "' " . 'LIMIT 1'));
                    $ab_data = ['address_book_id' => ''];
                    ...
                    $ab_data['country'] = ['id' => $ab_data['country_id'], 'title' => $country_info['countries_name'], 'iso_code_2' => $country_info['countries_iso_code_2'], 'iso_code_3' => $country_info['countries_iso_code_3']];
                    $ab_data['zone_id'] = 0;
                    if (!empty($_ab['order_state'])) {
                        $_check_zone_id_r = tep_db_query('SELECT zone_id ' . 'FROM ' . TABLE_ZONES . ' ' . "WHERE zone_name='" . tep_db_input($_ab['order_state']) . "' AND zone_country_id='" . $ab_data['country_id'] . "' " . 'LIMIT 1');
    ...
    public static function get_address_formats()
    {
        ...
        foreach (Address_Format::find()->order_by('address_format_id')->all() as $address_format_values) {
            $address_format_array[] = ['id' => $address_format_values['address_format_id'], 'text' => $address_format_values['address_format_title']];
        }
        return $address_format_array;
    }
    /*array address_format objects */
    public static function perpare_form_old_format($formats)
    {
        if (is_array($formats)) {
            foreach ($formats as $format) {
                ...
                $format->address_format = self::prepare_format($format->address_format);
            }
        }
        return $formats;
    }
    protected static function prepare_format($address_format)
    {
        $rows = [];
        ...
        } else if (strrpos($address_format, '$cr') !== false) {
            $rows = self::prepare_format(explode('$cr', $address_format));
        } else {
    ...
    {
        return \Yii::$app->cache->get_or_set('address_format_' . (int) $format_id, function () use ($format_id) {
            return Address_Format::find_one(['address_format_id' => $format_id]);
        }, 0, new \yii\caching\Tag_Dependency(['tags' => ['address_format', 'address_format_' . (int) $format_id]]));
        //return AddressFormat::findOne(['address_format_id' => $format_id]);
    }
    public static function get_formats()
    {
        $formats = Address_Format::find()->all();
        Address::perpare_form_old_format($formats);
        return $formats;
    }
    public static function save_address_format($format_id, array $format)
    {
        $a_format = Address_Format::find_one(['address_format_id' => $format_id]);
        if (!$a_format) {
            $a_format = new Address_Format();
        }
        $format = array_values($format);
        $a_format->address_format = json_encode($format);
        if ($a_format->save()) {
            return $a_format;
        } else {
            return false;
    ...
    public static function get_gender_name($code)
    {
        switch ($code) {
            case 'm':
                return MR;
    ...
    public static function get_genders_list()
    {
        return ['m' => self::get_gender_name('m'), 'f' => self::get_gender_name('f'), 's' => self::get_gender_name('s'), 'n' => self::get_gender_name('n')];
    }
    /**
     ...
     * @return type
     */
    public static function skip_entry_key($addresses = [])
    {
        if (is_array($addresses)) {
            foreach ($addresses as $key => $address) {
                ...
                $addresses[$key] = self::skip_entry($address);
            }
        }
        return $addresses;
    }
     ...
     * @return array
     */
    public static function skip_entry($address = [])
    {
        if (is_array($address) && !empty($address)) {
            $keys = array_keys($address);
            $keys = array_map(function ($el) {
                return str_replace('entry_', '', $el);
            }, $keys);
            $address = array_combine($keys, array_values($address));
        }
        return $address;
    }
    public static function add_countries_key(array $address)
    {
        if ($address) {
     ...
     * @return bool
     **/
    public static function cmp_addresses($addr1, $addr2)
    {
        ...
        } elseif ($addr1 instanceof \yii\base\Model && $addr2 instanceof \yii\base\Model) {
            return self::cmp_addresses($addr1->get_attributes(), $addr2->get_attributes());
        } elseif (is_string($addr1) && is_string($addr2)) {
     ...
     */
    public static function extract_country_id($address)
    {
        $ret = 0;
    ...
    public static function is_empty($address, $with_country = false)
    {
        return !not_empty($address, $with_country);
    }
    public static function not_empty($address, $with_country = false)
    {
        $ret = false;
        $check_keys = ['company', 'firstname', 'lastname', 'postcode', 'street_address', 'city', 'state'];
        if ($with_country) {
            $check_keys[] = 'country';
        }
        $tmp = self::skip_entry($address);
        if (is_array($tmp)) {
            foreach ($tmp as $k => $v) {
                if (in_array($k, $check_keys) && !empty($v)) {
                    $ret = true;
                    break;
                }
            }
