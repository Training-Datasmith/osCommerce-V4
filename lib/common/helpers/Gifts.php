...
class Gifts
{
    ...
    public static function allow_gift_wrap($products_id)
    {
        ...
        $check_gift_wrap_r = tep_db_query('SELECT COUNT(*) AS c ' . 'FROM ' . TABLE_GIFT_WRAP_PRODUCTS . ' ' . "WHERE products_id='" . (int) $products_id . "' " . " and groups_id='" . (int) $customer_groups_id . "' " . (USE_MARKET_PRICES != 'True' ? " and currencies_id in (0, '" . (int) $currency_id . "')" : " and currencies_id='" . (int) $currency_id . "' "));
    ...
    public static function get_gift_wrap_price($products_id)
    {
        ...
        $check_gift_wrap_price_r = tep_db_query('SELECT gift_wrap_price ' . 'FROM ' . TABLE_GIFT_WRAP_PRODUCTS . ' ' . "WHERE products_id='" . (int) $products_id . "' " . " and groups_id='" . (int) $customer_groups_id . "' " . (USE_MARKET_PRICES != 'True' ? " and currencies_id in (0, '" . (int) $currency_id . "') " : " and currencies_id='" . (int) $currency_id . "' ") . 'LIMIT 1');
    ...
    public static function virtual_gift_card_process($virtual_gift_card_id, $from_email = STORE_OWNER_EMAIL_ADDRESS, $from_name = STORE_OWNER)
    {
        ...
        $sql_data_array = ['coupon_code' => $virtual_gift_card_code, 'coupon_amount' => $virtual_gift_card['products_price'], 'coupon_currency' => $virtual_gift_card['currency_code'], 'coupon_type' => 'G', 'uses_per_coupon' => 1, 'uses_per_user' => 1, 'coupon_minimum_order' => 0, 'restrict_to_products' => '', 'restrict_to_categories' => '', 'coupon_start_date' => 'now()', 'coupon_active' => 'N', 'coupon_expire_date' => date('Y-m-d', mktime(0, 0, 0, date('m'), date('d'), date('Y') + 3)), 'date_created' => 'now()', 'date_modified' => 'now()'];
        $query = tep_db_perform(TABLE_COUPONS, $sql_data_array);
        $insert_id = tep_db_insert_id();
        $sql_data_array = ['coupon_name' => $currencies->display_gift_card_price($virtual_gift_card['products_price'], \common\helpers\Tax::get_tax_rate($virtual_gift_card['products_tax_class_id']), $virtual_gift_card['currency_code']) . ' - ' . $virtual_gift_card['products_name']];
    ...
    public static function activate($virtual_gift_card_id, \common\classes\extended\Order_Abstract $order)
    {
        if ($virtual_gift_card_id) {
            $virtual_gift_card = \common\models\Virtual_Gift_Card_Basket::find_one(['virtual_gift_card_basket_id' => $virtual_gift_card_id]);
            if ($virtual_gift_card && $virtual_gift_card->customers_id == $order->customer['customer_id'] && strlen($virtual_gift_card->virtual_gift_card_code)) {
                if (!$virtual_gift_card->activated && $virtual_gift_card->activate()) {
                    if (!intval($virtual_gift_card->send_card_date)) {
                        $product = Array_Helper::to_array($virtual_gift_card->product);
                        $virtual_gift_card = Array_Helper::to_array($virtual_gift_card);
                        $virtual_gift_card = array_merge($product, $virtual_gift_card);
                        $gift_card_info = new \common\models\Virtual_Gift_Card_Info();
                        $gift_card_info->set_attributes($virtual_gift_card);
                        $gift_card_info->save();
                        self::prepare_and_send($virtual_gift_card, $order->customer['email_address'], $order->customer['firstname'] . ' ' . $order->customer['lastname']);
                    }
                }
            }
        }
    }
    public static function prepare_and_send($virtual_gift_card, $from_email = STORE_OWNER_EMAIL_ADDRESS, $from_name = STORE_OWNER)
    {
        $currencies = \Yii::$container->get('currencies');
        // Instantiate a new mail object
        $contents = Yii::$app->run_action('catalog/gift', ['page_name' => 'gift_card']);
        $contents = str_replace(["\r\n", "\n", "\r"], '', $contents);
        $search = ["'##PRICE##'i", "'##PERSONAL_MESSAGE##'i", "'##SENDERS_NAME##'i", "'##CARD_CODE##'i"];
        $replace = [$currencies->display_gift_card_price($virtual_gift_card['products_price'], \common\helpers\Tax::get_tax_rate($virtual_gift_card['products_tax_class_id']), $virtual_gift_card['currency_code']), nl2br($virtual_gift_card['virtual_gift_card_message']), $virtual_gift_card['virtual_gift_card_senders_name'] ?: $from_name, $virtual_gift_card['virtual_gift_card_code']];
        ...
        // Send message
        \common\helpers\Mail::send($virtual_gift_card['virtual_gift_card_recipients_name'], $virtual_gift_card['virtual_gift_card_recipients_email'], $email_subject, $email_content, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, [], 'Reply-To: "' . $from_name . '" <' . $from_email . '>', '', ['add_br' => 'no']);
        return '';
    }
    /**
     ...
     */
    public static function get_give_aways_sql($products_id = 0, $only_active = false, $sorted = true, $only_buy_get = false)
    {
        ...
        if ($only_buy_get) {
            ...
            $total = self::get_cart()->show_total();
        }
        /*
               $products2c_join = '';
               if ( $platform_id ) {
               ...
               ;
        */
        $giveaway_query = \common\models\Give_Away_Products::find()->and_where([
            'and',
            ['groups_id' => (int) $customer_groups_id],
            ['<=', 'begin_date', new \yii\db\Expression('now()')],
            // date/datetime 0000/1970 or anything else date not null
            [
                'or',
        ...
        if ((int) $products_id > 0) {
            $giveaway_query->and_where(['products_id' => (int) $products_id]);
        }
        if ($only_active) {
            $giveaway_query->and_where(['<=', 'shopping_cart_price', number_format($total, 4, '.', '')]);
        }
        if ($only_buy_get) {
            $giveaway_query->and_where(['>', 'buy_qty', 0]);
        }
        // check other restriction - customer, platform etc
        $products = new \common\components\Products_Query(['get' => [], 'currentCategory' => false, 'outOfStock' => false, 'skipInTop' => false, 'skipInTopOnly' => false, 'customAndWhere' => ['p.products_id' => $giveaway_query->distinct()->select('products_id')->as_array()->column()], 'orderBy' => ['products_model' => SORT_ASC]]);
        $pids = $products->build_query()->all_ids();
        $giveaway_query->alias('gaw')->and_where(['products_id' => $pids])->select('gaw.products_id, shopping_cart_price as price, gaw.products_qty as qty, gaw.buy_qty as buy_qty, use_in_qty_discount, gap_id as gaw_id')->add_select(new \yii\db\Expression('if(gaw.shopping_cart_price <= ' . number_format($total, 4, '.', '') . ', 1, 0) as active'));
        if ($sorted) {
            $giveaway_query->add_order_by('shopping_cart_price');
            $giveaway_query->add_order_by(new \yii\db\Expression('shopping_cart_price<=' . number_format($total, 4, '.', '') . ' desc'));
            $giveaway_query->add_order_by('buy_qty');
        }
     ...
     * @return type
     */
    public static function get_give_aways_query($products_id = 0)
    {
        // backward compatibility
        $response = self::get_give_aways_sql($products_id);
     ...
     */
    public static function get_give_aways_array($products_id = 0)
    {
        $response = self::get_give_aways_sql($products_id);
        $products = $response['giveaway_query']->as_array()->all();
        $details = [];
        foreach (\frontend\design\Info::get_list_products_details(array_unique(\yii\helpers\Array_Helper::get_column($products, 'products_id')), ['listing_type' => 'gwa' . $response['cart_total'], 0 => ['show_attributes' => 0, 'show_image' => 0], 'itemElements' => ['attributes' => 1, 'image' => 1]]) as $detail) {
     ...
     * @return type
     */
    public static function get_give_aways($products_id = 0)
    {
        //returns array of GAW products for design module at shopping cart page.
        $cart = self::get_cart();
        $currencies = \Yii::$container->get('currencies');
        $products = [];
        $response = self::get_give_aways_array($products_id);
        $p = $response['products'];
        ...
        if (count($p) > 0) {
            foreach ($p as $d) {
                $price_b = '';
                if ($d['buy_qty'] > 0) {
                    $in_cart_qty = $cart->get_qty($d['products_id']);
                    $price_b = sprintf(TEXT_QTY_BEFORE, $d['buy_qty'], $d['qty']);
                    if ($d['buy_qty'] > $in_cart_qty) {
                        $collect = $d['buy_qty'] - $in_cart_qty;
    ...
    public static function get_max_quantity($prid, $gaw_id = false)
    {
        ...
        if ((int) $prid > 0) {
            $cart = self::get_cart();
            if ($gaw_id) {
                $response = \common\helpers\Gifts::get_give_aways_sql($prid, true, false);
                // product, only active , no default sort order
                $giveaway_query = $response['giveaway_query']->and_where(['gap_id' => (int) $gaw_id])->as_array()->all();
                $total = $response['cart_total'];
            } else {
                $response = self::get_give_aways_array($prid);
                $giveaway_query = $response['products'];
                $total = $response['cart_total'];
            }
            if (is_array($giveaway_query)) {
                $in_cart_qty = $cart->get_qty($prid);
                foreach ($giveaway_query as $d) {
                    if ($d['buy_qty'] > 0) {
                        if ($d['buy_qty'] <= $in_cart_qty) {
                            $ret = ['qty' => $d['qty'], 'gaw_id' => $d['gaw_id']];
                        } else {
                            break;
                        }
                    } else {
    ...
    public static function allowed_gaw($prid, $get_qty)
    {
        $cart = self::get_cart();
        $response = self::get_give_aways_array($prid);
        $giveaway_query = $response['products'];
        ...
        if (is_array($giveaway_query)) {
            ...
            foreach ($giveaway_query as $d) {
                if ($d['buy_qty'] > 0) {
                    if ($d['buy_qty'] <= $in_cart_qty && $d['qty'] == $get_qty) {
                        return $d['gaw_id'];
                    }
                } else if ($d['price'] <= $total && $d['qty'] == $get_qty) {
                    return $d['gaw_id'];
                }
            }
        }
    ...
    public static function in_qty_discount($gaw_id)
    {
        ...
        if (!isset($cache[$gaw_id])) {
            $response = self::get_give_aways_sql(0, true, false, true);
            $cache = $response['giveaway_query']->index_by('gap_id')->select('use_in_qty_discount')->as_array()->column();
     ...
     * @param int $products_id
     */
    public static function prepare_gwa(&$view, $products_id)
    {
        $gaws = \common\models\Give_Away_Products::find()->and_where(['products_id' => (int) $products_id])->as_array()->all();
        $i = 0;
        $view->gaw[0] = [];
        if (is_array($gaws)) {
            foreach ($gaws as $gaw) {
                ...
                if ($gaw['buy_qty'] == 0) {
                    ...
                    //mix? shouldn't work with marketing prices.
    ...
    public static function get_virtual_gift_card_model()
    {
