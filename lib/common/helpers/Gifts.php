...
class Gifts
{
    ...
    public static function virtual_gift_card_process($virtual_gift_card_id, $from_email = STORE_OWNER_EMAIL_ADDRESS, $from_name = STORE_OWNER)
    {
        ...
        $sql_data_array = ['coupon_code' => $virtual_gift_card_code,
    ...
    public static function getGiveAwaysArray($products_id = 0)
    {
        ...
        $details = [];
        foreach (\frontend\design\Info::getListProductsDetails(array_unique(\yii\helpers\ArrayHelper::getColumn($products, 'products_id')), ['listing_type' => 'gwa' . $response['cart_total'], 0 => ['show_attributes' => 0, 'show_image' => 0], 'itemElements' => ['attributes' => 1, 'image' => 1] ]) as $detail) {
