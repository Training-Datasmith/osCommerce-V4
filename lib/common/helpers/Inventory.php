...
class Inventory
{
    ...
    {
        if (static::disabledOnProduct($uprid)) {
            return true;
        }
    ...
    public static function getDetails($products_id, $current_uprid, $params = [])
    {
        global $languages_id;
        ...
        if ($product = tep_db_fetch_array($product_query)) {
            ...
            if ($products_attributes['total'] > 0) {
                ...
                while ($inventory = tep_db_fetch_array($inventory_query)) {
                    ...
                    $inventory['stock_indicator'] = \common\classes\StockIndication::product_info([
        ...
        if (strpos($inventoryId, '{') !== false) {
            ...
            for ($i = 1; $i < count($attributeArray); $i = ($i + 2)) {
                ...
                $povRecord = \common\models\ProductsOptionsValues::find()->asArray(true)
                    ...
                    ->column();
                $return[] = (
                    ((is_array($poRecord) and isset($poRecord[0])) ? ucfirst($poRecord[0]) : 'N/A')
