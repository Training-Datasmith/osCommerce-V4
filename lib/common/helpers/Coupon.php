...
class Coupon
{
     ...
     */
    public static function get_coupon_by_code($code, $as_array = true)
    {
        if (empty($code)) {
            return false;
        }
        $main_counter = \common\models\Coupons::find()->alias('c')->where(['c.coupon_code' => $code])->as_array($as_array)->one();
        if (!empty($main_counter)) {
            return $main_counter;
        }
        $customer_restrict_counter = \common\models\Coupons::find()->alias('c')->join('inner join', \common\models\Coupons_Customer_Codes_List::table_name() . ' ccl', 'c.coupon_id=ccl.coupon_id')->where(['ccl.coupon_code' => $code])->as_array($as_array)->one();
        if (!empty($customer_restrict_counter)) {
            return $customer_restrict_counter;
        }
        return false;
    }
    public static function is_coupon_code_exists($code)
    {
        $main_counter = \common\models\Coupons::find()->alias('c')->where(['c.coupon_code' => $code])->count();
        if ($main_counter > 0) {
            return true;
        }
        $customer_restrict_counter = \common\models\Coupons_Customer_Codes_List::find()->alias('ccl')->join('inner join', \common\models\Coupons::table_name() . ' c', 'c.coupon_id=ccl.coupon_id')->where(['ccl.coupon_code' => $code])->count();
        if ($customer_restrict_counter > 0) {
            return true;
        }
        return false;
    }
    ...
    public static function create_coupon_code($salt = 'secret', $length = 8, $prefix = '')
    {
        ...
        while ($good_result == 0) {
            ...
            if (!static::is_coupon_code_exists($id1)) {
                $good_result = 1;
            }
        }
        return $id1;
    ...
    public static function gv_account_update($customer_id, $gv_id)
    {
        ...
        if (tep_db_num_rows($customer_gv_query) > 0) {
            ...
            tep_db_perform(TABLE_CUSTOMERS_CREDIT_HISTORY, ['customers_id' => $customer_id, 'credit_prefix' => '+', 'credit_amount' => $coupon_gv['coupon_amount'], 'currency' => DEFAULT_CURRENCY, 'currency_value' => $currencies->currencies[DEFAULT_CURRENCY]['value'], 'customer_notified' => 0, 'comments' => 'Redeem ' . $customer_gv['coupon_code'], 'date_added' => 'now()', 'admin_id' => 0]);
        }
    }
    public static function credit_order_check_state($order_id)
    {
        ...
        $ordered_gv_r = tep_db_query('SELECT o.orders_id, o.customers_id, o.orders_status, op.products_model, op.orders_products_id, op.final_price, op.products_tax, op.products_quantity, op.gv_state ' . 'FROM ' . TABLE_ORDERS_PRODUCTS . ' op, ' . TABLE_ORDERS . ' o ' . "WHERE o.orders_id='" . (int) $order_id . "' AND op.orders_id=o.orders_id AND op.gv_state='pending' ");
    ...
    public static function credit_order_product_release($order_products)
    {
        ...
        if (is_numeric($order_products)) {
            $ordered_gv_r = tep_db_query('SELECT o.orders_id, o.customers_id, o.orders_status, op.products_model, op.orders_products_id, op.final_price, op.products_tax, op.products_quantity, op.gv_state ' . 'FROM ' . TABLE_ORDERS_PRODUCTS . ' op, ' . TABLE_ORDERS . ' o ' . "WHERE op.orders_products_id='" . (int) $order_products . "' AND op.orders_id=o.orders_id ");
            if (tep_db_num_rows($ordered_gv_r) > 0) {
        ...
        if (is_array($release_info) && !empty($release_info['gv_state']) && $release_info['gv_state'] != 'released' && $release_info['gv_state'] != 'none') {
            ...
            if (MODULE_ORDER_TOTAL_GV_CREDIT_TAX == 'true') {
                $gv_order_amount = $gv_order_amount * (100 + $release_info['products_tax']) / 100;
            }
            ...
            tep_db_perform(TABLE_CUSTOMERS_CREDIT_HISTORY, ['customers_id' => $release_info['customers_id'], 'credit_prefix' => '+', 'credit_amount' => $gv_order_amount, 'currency' => DEFAULT_CURRENCY, 'currency_value' => $currencies->currencies[DEFAULT_CURRENCY]['value'], 'customer_notified' => 0, 'comments' => 'Order ' . $release_info['products_model'] . ' order #' . $release_info['orders_id'], 'date_added' => 'now()', 'admin_id' => 0]);
    ...
    {
        $ordered_gv_r = tep_db_query('SELECT o.orders_id, o.customers_id, o.orders_status, op.products_model, op.orders_products_id, op.final_price, op.products_tax, op.products_quantity, op.gv_state ' . 'FROM ' . TABLE_ORDERS_PRODUCTS . ' op, ' . TABLE_ORDERS . ' o ' . "WHERE op.orders_id=o.orders_id AND op.gv_state!='none' " . " AND op.orders_products_id='" . (int) $orders_products_id . "' ");
        if (tep_db_num_rows($ordered_gv_r) > 0) {
            while ($ordered_gv = tep_db_fetch_array($ordered_gv_r)) {
                if ($ordered_gv['gv_state'] == 'released') {
                    continue;
                }
    ...
    public static function generate_customer_gvcc($coupon_id, $mail, $amount, $currency, $customer_id = 0, $basket_id = 0)
    {
        ...
        if (class_exists('\backend\models\Admin')) {
            ...
            $admin_fname = $admin->get_info('admin_firstname');
            $admin_lname = $admin->get_info('admin_lastname');
        }
     ...
     * @return array
     */
    public static function get_active_list($selected = '')
    {
        $coupons = Coupons::find()->where(['coupon_active' => 'Y'])->order_by('coupon_code')->all();
        $ret = [];
        $ret[] = ['name' => TEXT_ALL, 'value' => '', 'selected' => ''];
        if (is_array($coupons)) {
    ...
    {
        $coupons = \common\models\Coupons::find()->inner_join_with(['redeemTrack' => function (\yii\db\Active_Query $query) {
            $query->inner_join_with(['order'])->where(['!=', 'order_id', 0]);
        }])->as_array()->all();
        return $coupons;
    }
    public static function save_csv_customers_coupons($coupon_csv_loaded_coupon_file_name, $cid)
    {
        $path = \Yii::get_alias('@webroot');
        $path .= DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR;
        $tmp_name = $path . $coupon_csv_loaded_coupon_file_name;
        if (file_exists($tmp_name)) {
            ...
            if (($handle = fopen($tmp_name, 'r')) !== false) {
                ...
                while (($data = fgetcsv($handle, 300, ',', '"')) !== false) {
                    ...
                    if (Coupons::get_coupon_by_code($code, true)) {
                        //exists, and active
                        continue;
                    }
                    $data_to_save[] = [$cid, $code, $email, date('Y-m-d')];
                }
                if (count($data_to_save)) {
                    \Yii::$app->db->create_command()->batch_insert(\common\models\Coupons_Customer_Codes_List::table_name(), ['coupon_id', 'coupon_code', 'only_for_customer', 'date_added'], $data_to_save)->execute();
                }
                fclose($handle);
            }
            @unlink($tmp_name);
        }
        return !empty($data_to_save);
    }
    public static function get_customers_coupons_emails_list($cid)
    {
        if (!$cid) {
            return '';
        }
        $records = \common\models\Coupons_Customer_Codes_List::find()->and_where(['coupon_id' => (int) $cid])->as_array()->all();
        if (empty($records)) {
            return '';
        }
        $emails = [];
    ...
    public static function coupon_used_by($coupon_id, $email)
    {
        return \common\models\Coupon_Redeem_Track::find()->alias('crt')->inner_join(TABLE_ORDERS . ' o', 'crt.order_id = o.orders_id')->and_where(['coupon_id' => $coupon_id])->and_where(['o.customers_email_address' => $email])->count();
    }
}