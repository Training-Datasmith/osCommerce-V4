...
class Tax
{
    public const CACHE_TIME = 600;
    public static function get_default_tax_class_id_for_products()
    {
        $most_used_tax_class_id = \common\models\Products::find()->select('products_tax_class_id')->group_by('products_tax_class_id')->order_by(new \yii\db\Expression('COUNT(*) DESC'))->limit(1)->scalar();
        return (int) $most_used_tax_class_id;
    }
    public static function add_tax($price, $tax)
    {
        ...
        if (defined('PRICE_WITH_BACK_TAX') && PRICE_WITH_BACK_TAX == 'True') {
            return round(self::round_price_exc($price), 6);
        } elseif (DISPLAY_PRICE_WITH_TAX == 'true' && $tax > 0) {
            return round(self::round_price_exc($price) + self::round_tax(self::calculate_tax($price, $tax)), 6);
    ...
    public static function calculate_tax($price, $tax)
    {
        //$currency = \Yii::$app->settings->get('currency');
        //$currencies = \Yii::$container->get('currencies');
        return round((float) $price * $tax / 100, 6);
        //$currencies->currencies[$currency]['decimal_places']
    }
     ...
     * @return int
     */
    public static function get_tax_rate($class_id, $country_id = -1, $zone_id = -1, $extra_address = '', $check_group = true, $customer_groups_id = null)
    {
        if (is_null($customer_groups_id)) {
            $customer_groups_id = (int) \Yii::$app->storage->get('customer_groups_id');
        }
        $tax_rate = false;
        foreach (\common\helpers\Hooks::get_list('tax/get-tax-rate') as $filename) {
        ...
        $_pos = 1;
        if ($check_group && $ext = \common\helpers\Acl::check_extension_allowed('BusinessToBusiness', 'allowed')) {
        ...
        if ($vat_on_order = \common\helpers\Acl::check_extension_allowed('VatOnOrder', 'allowed')) {
            if ($vat_on_order::check_tax_rate($country_id != -1 ? true : false)) {
                if (defined('PRICE_WITH_BACK_TAX') && PRICE_WITH_BACK_TAX == 'True') {
                    $_pos = -1;
                } else {
                    return 0;
        ...
        if ($country_id == -1 && $zone_id == -1) {
            if (\Yii::$app->user->is_guest) {
                $country_id = \Yii::$app->storage->has('customer_country_id') ? \Yii::$app->storage->get('customer_country_id') : Platform_Config::get_value('STORE_COUNTRY');
                $zone_id = \Yii::$app->storage->has('customer_zone_id') ? \Yii::$app->storage->get('customer_zone_id') : Platform_Config::get_value('STORE_ZONE');
                //$country_id = STORE_COUNTRY;
                //$zone_id = STORE_ZONE;
            } else {
                $country_id = \Yii::$app->user->get_identity()->get('customer_country_id') ?? Platform_Config::get_value('STORE_COUNTRY');
                $zone_id = \Yii::$app->user->get_identity()->get('customer_zone_id') ?? Platform_Config::get_value('STORE_ZONE');
            }
        }
        if (\Yii::$app->storage->has('skipTaxRates')) {
            ...
            $q = \common\models\Tax_Rates::find()->alias('tr')->left_join(['za' => \common\models\Zones_To_Tax_Zones::table_name()], 'tr.tax_zone_id = za.geo_zone_id')->left_join(['tz' => \common\models\Tax_Zones::table_name()], 'tz.geo_zone_id = tr.tax_zone_id')->and_where(['tr.tax_class_id' => (int) $class_id])->and_where(['not in', 'tr.tax_rates_id', $skip])->and_where([
                'or',
                0,
                //['is', 'za.zone_country_id', null], field is not null def 0
                ['za.zone_country_id' => [0, (int) $country_id]],
            ])->and_where(['or', ['is', 'za.zone_id', null], ['za.zone_id' => [0, (int) $zone_id]]])->add_group_by('tr.tax_priority')->select(['tax_rate' => new \yii\db\Expression('sum(tax_rate)')]);
        ...
        if (tep_db_num_rows($tax_query)) {
            ...
            $tax_rates_array[$class_id][$country_id] = [$zone_id => $ret];
            //echo '<pre>';print_r($tax_rates_array);die;
            return $ret;
        } else {
            $tax_rates_array[$class_id][$country_id] = [$zone_id => 0];
            return 0;
        }
    }
    public static function get_tax_zones($country_id, $zone_id)
    {
     ...
     */
    public static function is_taxable_country($tax_country_id, $platform_id = null, $tax_class_id = null)
    {
        $ret = 0;
        $paltform_config = \Yii::$app->get('platform')->get_config($platform_id);
        $platform_address = $paltform_config->get_platform_address();
        $ret = (int) $platform_address['country_id'] == $tax_country_id ? 1 : 0;
        if ($ret == 0 && self::countries_in_same_tax_zone((int) $platform_address['country_id'], $tax_country_id, $tax_class_id)) {
            $ret = 1;
        } else {
            $c = Country::get_platform_countries($platform_id);
            if (is_array($c) && !empty($c)) {
                $c_ids = \yii\helpers\Array_Helper::get_column($c, 'id');
                $ret = in_array($tax_country_id, $c_ids) ? 2 : 0;
            }
        }
        return $ret;
    }
    public static function countries_in_same_tax_zone($cid1, $cid2, $tax_class_id = null)
    {
        $s_query_tax_class_cond = '1';
        if (!empty((int) $tax_class_id)) {
            $s_query_tax_class_cond = ['tr.tax_class_id' => (int) $tax_class_id];
        }
        return \common\models\Zones_To_Tax_Zones::find()->alias('z2tz')->and_where(['zone_country_id' => [0, (int) $cid1, (int) $cid2]])->add_select('geo_zone_id')->add_select(['zone_country_id' => new \yii\db\Expression('min(zone_country_id)')])->and_where(['exists', \common\models\Tax_Rates::find()->alias('tr')->and_where('tr.tax_zone_id=z2tz.geo_zone_id')->and_where($s_query_tax_class_cond)])->group_by('geo_zone_id')->having('zone_country_id=0 or count(distinct zone_country_id)=2')->exists();
    }
    public static function get_tax_values($platform_id, $tax_class_id, $tax_country_id, $tax_zone_id)
    {
        $tax_values = false;
        foreach (\common\helpers\Hooks::get_list('tax/get-tax-values') as $filename) {
            $tax_values = include $filename;
            if (is_array($tax_values) && isset($tax_values['tax'])) {
                return $tax_values;
            }
        }
        $tax = 0;
        ...
        if ($platform_address) {
            ...
            $check_tax_zones = \common\helpers\Tax::get_tax_zones($tax_country_id, $tax_zone_id);
            ...
            if (isset($check_platform_zone['geo_zone_id']) && is_array($check_tax_zones) && in_array($check_platform_zone['geo_zone_id'], $check_tax_zones)) {
                if (defined('TAX_BY_PLATFORM_ADDRESS') && TAX_BY_PLATFORM_ADDRESS == 'True') {
                    // probably not needed any more but can be defined in configure.php for compatibility
            ...
            } elseif (defined('ALLOW_SEVERAL_TAX_COUNTRIES') && ALLOW_SEVERAL_TAX_COUNTRIES == 'True' && (int) $tax_country_id > 0 && self::is_taxable_country($tax_country_id, $platform_id)) {
                // tax for other countries (conditional)
                $tax = \common\helpers\Tax::get_tax_rate($tax_class_id, $tax_country_id, $tax_zone_id);
                $tax_description = \common\helpers\Tax::get_tax_description($tax_class_id, $tax_country_id, $tax_zone_id);
            }
        } else {
        ...
        return ['tax_class_id' => $tax_class_id, 'tax' => $tax, 'tax_description' => $tax_description];
    }
    ...
    public static function get_tax_classes_variants($with_none = false)
    {
        $tax_classes = [];
        if ($with_none) {
            $tax_classes[] = ['id' => 0, 'text' => ''];
        }
        $classes_query = tep_db_query('select tax_class_id, tax_class_title from ' . TABLE_TAX_CLASS . ' order by tax_class_title');
        if (tep_db_num_rows($classes_query) > 0) {
            while ($classes = tep_db_fetch_array($classes_query)) {
                $tax_classes[] = ['id' => $classes['tax_class_id'], 'text' => $classes['tax_class_title']];
            }
        }
        return $tax_classes;
    }
    public static function tax_classes_pull_down($parameters, $selected = '')
    {
        ...
        foreach (self::get_tax_classes_variants() as $variant) {
    ...
    public static function is_tax_rates_in_backend()
    {
        return !defined('BACKEND_TAX_RATES_OR_CLASSES') || BACKEND_TAX_RATES_OR_CLASSES == 'RATES';
    }
    public static function normalize_tax_selected($tax_selected)
    {
        if (\common\helpers\System::is_backend() && !self::is_tax_rates_in_backend()) {
            $tax_array = array_pad(explode('_', $tax_selected), 2, 0);
            $tax_array[1] = 0;
            return implode('_', $tax_array);
        }
        return $tax_selected;
    }
    public static function get_complex_classes_list()
    {
        if (self::is_tax_rates_in_backend()) {
            $tax_class_array = [];
            ...
            while ($tax_class = tep_db_fetch_array($tax_class_query)) {
                ...
                $tax_class_array[$tax_class['tax_class_id'] . '_' . $tax_class['tax_zone_id']] = $tax_class['tax_class_title'];
            ...
            $classes = \common\models\Tax_Class::find()->select('tax_class_title, tax_class_id')->as_array();
            $res = [];
    ...
    public static function get_order_tax_rates($class_id = null, $country_id = -1, $zone_id = -1, $extra_address = '', $check_group = true, $group_id = null)
    {
        if (self::is_tax_rates_in_backend()) {
            $where = $class_id ? "tr.tax_class_id = {$class_id}" : 1;
            ...
            $classes = \common\models\Tax_Class::find()->select('tax_class_id')->filter_where(['tax_class_id' => $class_id])->as_array();
            $res = [];
            foreach ($classes->each() as $rec) {
                $res[$rec['tax_class_id'] . '_0'] = self::get_tax_rate($rec['tax_class_id'], $country_id, $zone_id, $extra_address, $check_group, $group_id);
            }
            return $res;
        }
    }
    public static function round_price_exc($price)
    {
        $ret = $price;
    ...
    public static function round_tax($price)
    {
        $ret = $price;
    ...
    public static function display_taxable()
    {
        if (\Yii::$app->storage->has('taxable')) {
            $ret = \Yii::$app->storage->get('taxable');
        } else {
            $def_country = Platform_Config::get_value('STORE_COUNTRY');
            if (\Yii::$app->user->is_guest) {
                $country_id = \Yii::$app->storage->has('customer_country_id') ? \Yii::$app->storage->get('customer_country_id') : $def_country;
            } else {
                $country_id = \Yii::$app->user->get_identity()->get('customer_country_id') ?? $def_country;
            }
            $t_country = self::is_taxable_country($country_id);
            $ret = $t_country != 0 && Platform_Config::get_value('DISPLAY_PRICE_WITH_TAX') == 'true';
        }
        return $ret;
    }
    public static function get_tax_address_option()
    {
    ...
    public static function get_tax_classes_with_rates($country_id = -1, $zone_id = -1, $extra_address = '', $check_group = true, $with_none = true)
    {
        $ret = [];
        if ($with_none) {
            $ret[0] = ['id' => 0, 'text' => defined('TEXT_NONE') ? TEXT_NONE : '', 'rate' => 0.0];
        }
        $tax_classes = \common\models\Tax_Class::find()->select(['id' => 'tax_class_id', 'text' => 'tax_class_title'])->order_by('tax_class_title')->as_array()->cache(self::CACHE_TIME)->all();
        foreach ($tax_classes as $tax_class) {
            $ret[$tax_class['id']] = $tax_class;
            $ret[$tax_class['id']]['rate'] = self::get_tax_rate($tax_class['id'], $country_id, $zone_id, $extra_address, $check_group);
        }
        return $ret;
    }
    public static function get_rate_details($tax_rate, $by_field = 'tax_description')
    {
        $q = \common\models\Tax_Rates::find();
        if ($by_field == 'id') {
            $q->and_where(['tax_rates_id' => $tax_rate]);
        } else {
            $q->and_where(['tax_description' => $tax_rate]);
        }
        $q->cache(5);
        $data = $q->as_array()->one();
        return $data;
    }
    public static function get_company_vat_id($tax_rate, $by_field = 'tax_description', $platform_id = 0)
    {
        $data = self::get_rate_details($tax_rate, $by_field);
        if (!empty($data['company_number'])) {
            $ret = $data['company_number'];
        } else {
            //from platform details
            if ($platform_id == 0) {
                $platform_id = intval(\common\classes\platform::current_id());
            }
            $default_address = \Yii::$app->get('platform')->get_config($platform_id)->get_platform_address();
            $ret = $default_address['entry_company_vat'] ?? '';
        }
        return $ret;
    }
    public static function get_company_name($tax_rate, $by_field = 'tax_description', $platform_id = 0)
    {
        $data = self::get_rate_details($tax_rate, $by_field);
        if (!empty($data['company_name'])) {
            ...
            $default_address = \Yii::$app->get('platform')->get_config($platform_id)->get_platform_address();
            $ret = $default_address['company'] ?? '';
        }
        return $ret;
    }
    public static function get_company_address($tax_rate, $by_field = 'tax_description', $platform_id = 0)
    {
        $data = self::get_rate_details($tax_rate, $by_field);
        if (!empty($data['company_address'])) {
            ...
            $data = \Yii::$app->get('platform')->get_config($platform_id)->get_platform_address();
     ...
     * @throws \Exception
     */
    public static function get_product_tax_class($product, $products_tax_class_id = null, $group_id = null)
    {
        if (is_null($products_tax_class_id)) {
            $product = \common\models\Products::find_by_var_check($product);
            $products_tax_class_id = $product->{$products_tax_class_id} ?? 0;
        }
        if (is_null($group_id)) {
            $group_id = (int) \Yii::$app->storage->get('customer_groups_id');
        }
        foreach (\common\helpers\Hooks::get_list('tax/get-product-tax-class-id') as $filename) {
            $overwritten_class_id = include $filename;
            if ($overwritten_class_id !== false) {
                return $overwritten_class_id;
            }
        }
        return (int) $products_tax_class_id;
    }
    public static function get_product_tax_rate($product, $products_tax_class_id = null, $group_id = null)
    {
        $tax_rate = 0;
        if (is_null($group_id)) {
            $group_id = (int) \Yii::$app->storage->get('customer_groups_id');
        }
        if (\common\helpers\Group::is_tax_applicable($group_id)) {
            $products_tax_class_id = self::get_product_tax_class($product, $products_tax_class_id, $group_id);
            $tax_rate = self::get_tax_rate($products_tax_class_id, -1, -1, '', true, $group_id);
        }
