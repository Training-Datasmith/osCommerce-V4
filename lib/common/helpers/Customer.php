...
class Customer
{
    public static function check_customer_groups($groups_id, $field)
    {
        ...
        if ($field == 'groups_discount' && $personal_discount = \common\helpers\Acl::check_extension_allowed('PersonalDiscount', 'allowed')) {
        ...
        if (!\Yii::$app->user->is_guest && !empty($multi_customer_id) && !isset($cached_u[(int) $multi_customer_id][$field])) {
            /** @var \common\extensions\CustomersMultiEmails\CustomersMultiEmails $CustomersMultiEmails */
            if ($customers_multi_emails = \common\helpers\Acl::check_extension('CustomersMultiEmails', 'allowed')) {
                if ($customers_multi_emails::allowed() && in_array($field, ['groups_is_show_price', 'groups_disable_checkout', 'groups_disable_cart'])) {
                    $d = \common\extensions\Customers_Multi_Emails\models\Customers_Multi_Emails::find_one($multi_customer_id);
                    if ($d) {
                        //only disable
                        if (!$d->show_price && $field == 'groups_is_show_price') {
                            $cached_u[(int) $multi_customer_id][$field] = false;
                        }
                        if (!$d->allow_checkout && $field == 'groups_disable_checkout') {
                            //inverse flag!!!!
                            $cached_u[(int) $multi_customer_id][$field] = true;
                        }
                        if (!$d->allow_cart && $field == 'groups_disable_cart') {
                            //inverse flag!!!!
                            $cached_u[(int) $multi_customer_id][$field] = true;
                        }
                    }
            ...
            if ($dealers_multi_customers = \common\helpers\Acl::check_extension('DealersMultiCustomers', 'allowed')) {
                $user = \common\extensions\Dealers_Multi_Customers\models\Users::find()->where(['user_id' => $multi_customer_id])->one();
                if ($user) {
                    $role = \common\extensions\Dealers_Multi_Customers\models\Roles::find()->where(['role_id' => $user->customers_role])->one();
                    if ($role) {
    ...
    public static function get_additional_discount($groups_id, $customers_id = 0)
    {
        ...
        if ($additionals->additional_discounts_ncs ?? null) {
            if (\Yii::$app->user->is_guest && $customers_id) {
                $customer = \common\models\Customers::find_one($customers_id);
            } else {
                $customer = \Yii::$app->user->get_identity();
            }
            if ($customer) {
                $ordered_amount = $customer->fetch_order_total_amount(true);
                $discounts = \yii\helpers\Array_Helper::index($additionals->additional_discounts_ncs, 'groups_discounts_amount');
                if (is_array($discounts)) {
                    krsort($discounts);
                    foreach ($discounts as $amount => $data) {
                        if ($ordered_amount > $amount && !$data['check_supersum']) {
    ...
    public static function get_additional_superdiscount($customers_id = 0, $cart_total = 0)
    {
        if (\Yii::$app->user->is_guest && $customers_id) {
            $customer = \common\models\Customers::find_one($customers_id);
        } else {
        ...
        if ($customer) {
            ...
            if ($additionals instanceof \common\models\Groups && $additionals->additional_discounts_cs) {
                $ordered_amount = $customer->fetch_order_total_amount(true);
                $discounts = \yii\helpers\Array_Helper::index($additionals->additional_discounts_cs, 'groups_discounts_amount');
                $current_discount = self::check_customer_groups($customer->groups_id, 'groups_discount');
                if (is_array($discounts)) {
                    krsort($discounts);
                    foreach ($discounts as $amount => $data) {
                        if ($ordered_amount > $amount && $data['check_supersum'] && $cart_total >= $additionals->superdiscount_summ) {
                            return max(0, $data['groups_discounts_value'] - (float) $current_discount);
                        }
                    }
                }
            }
    ...
    public static function count_customer_address_book_entries($id = '', $check_session = true)
    {
        if (is_numeric($id) == false) {
            if (!\Yii::$app->user->is_guest) {
                $id = \Yii::$app->user->get_id();
            } else {
                return 0;
            }
        }
        if ($check_session == true) {
            if (\Yii::$app->user->is_guest || $id != \Yii::$app->user->get_id()) {
                return 0;
            }
        }
    ...
    public static function get_customers_group($customer_id)
    {
        if (\common\helpers\Extensions::is_customer_groups_allowed()) {
            ...
            $check_data = tep_db_fetch_array($check);
            return $check_data['groups_id'];
        } else {
            return 0;
        }
    }
    ...
    public static function trunk_customers()
    {
        ...
        $var_tables = ['regular_offers', TABLE_CUSTOMERS_BASKET, TABLE_CUSTOMERS_BASKET_ATTRIBUTES];
        foreach ($var_tables as $table) {
            if (\Yii::$app->db->schema->get_table_schema($table)) {
                tep_db_query("TRUNCATE TABLE {$table}");
            }
        }
        foreach (\common\helpers\Hooks::get_list('customers/after-trunk') as $filename) {
            include $filename;
        }
    }
    ...
    public static function find_create_anonymous_customer()
    {
        ...
        if (tep_db_num_rows($check_customer_query) == 0) {
            $sql_data = ['customers_firstname' => 'removed', 'customers_lastname' => 'removed', 'customers_email_address' => 'removed', 'customers_status' => 0];
            tep_db_perform(TABLE_CUSTOMERS, $sql_data);
            $removed_id = tep_db_insert_id();
            tep_db_query('insert into ' . TABLE_CUSTOMERS_INFO . " (customers_info_id, customers_info_number_of_logons, customers_info_date_account_created) values ('" . (int) $removed_id . "', '0', now())");
        } else {
            $check_customer = tep_db_fetch_array($check_customer_query);
            $removed_id = $check_customer['customers_id'];
        }
        return $removed_id;
    }
    public static function delete_customer($customer_id = 0, $notify = true)
    {
        $removed_id = self::find_create_anonymous_customer();
        if ($removed_id == $customer_id) {
            return false;
        }
        ...
        if (!defined('ANONIMIZE_ORDERS_ON_CUSTOMER_DELETE') || ANONIMIZE_ORDERS_ON_CUSTOMER_DELETE == 'True') {
            self::anonimize_orders($customer_id, $removed_id);
        }
        tep_db_query('update ' . TABLE_COUPON_REDEEM_TRACK . " set customer_id = '" . (int) $removed_id . "', redeem_ip = '' where customer_id = '" . (int) $customer_id . "';");
        tep_db_query('update ' . TABLE_COUPON_GV_QUEUE . " set customer_id = '" . (int) $removed_id . "', ipaddr = '' where customer_id = '" . (int) $customer_id . "';");
        // remove customer
        ...
        if ($ext = \common\helpers\Acl::check_extension_allowed('CustomerProducts', 'allowed')) {
            $ext::delete_customer($customer_id);
        }
        /** @var \common\extensions\CustomerModules\CustomerModules $CustomerModules */
        if ($customer_modules = \common\helpers\Acl::check_extension_allowed('CustomerModules', 'allowed')) {
            $customer_modules::delete_customer($customer_id);
        }
        if (\common\helpers\Acl::check_extension_allowed('Messages', 'allowed')) {
        ...
        // todo: AdminHooks
        $var_tables = [TABLE_CUSTOMERS_BASKET, TABLE_CUSTOMERS_BASKET_ATTRIBUTES];
        foreach ($var_tables as $table) {
            if ($schema = \Yii::$app->db->schema->get_table_schema($table)) {
        ...
        foreach (\common\helpers\Hooks::get_list('customers/after-delete') as $filename) {
            include $filename;
        }
    }
    public static function has_open_orders($customer_id)
    {
        $ret = false;
        if (defined('GDPR_CUSTOMER_DELETE_OPEN_ORDER_STATUSES') && !empty(trim(GDPR_CUSTOMER_DELETE_OPEN_ORDER_STATUSES))) {
            ...
            if (is_array($tmp) && !empty($tmp)) {
                $ret = \common\models\Orders::find()->and_where(['customers_id' => $customer_id, 'orders_status' => $tmp])->exists();
            }
        }
        return $ret;
    }
    public static function anonimize_orders($customer_id, $removed_id)
    {
        $sql_data = [
        ...
        tep_db_perform(TABLE_ORDERS, $sql_data, 'update', "customers_id = '" . (int) $customer_id . "'" . $status_check_where);
        tep_db_perform('tmp_' . TABLE_ORDERS, $sql_data, 'update', "customers_id = '" . (int) $customer_id . "'" . $status_check_where);
        unset($sql_data['basket_id']);
        foreach (['quote_' . TABLE_ORDERS, 'sample_' . TABLE_ORDERS] as $table) {
            if (\Yii::$app->db->schema->get_table_schema($table)) {
                tep_db_perform($table, $sql_data, 'update', "customers_id = '" . (int) $customer_id . "'" . $status_check_where);
            }
        }
        foreach (\common\helpers\Hooks::get_list('customers/order-anonymize') as $filename) {
            include $filename;
        }
    }
    public static function has_address_book($customer_id, $addr_id)
    {
    ...
    public static function update_basket_id($customers_id, $old_basket_id, $new_basket_id)
    {
        if ($customers_id && $old_basket_id && $new_basket_id) {
            \common\models\Customers_Errors::update_all(['basket_id' => $new_basket_id], ['customers_id' => $customers_id, 'basket_id' => $old_basket_id]);
        }
    }
    /**
     ...
     */
    public static function get_default_addressbook($customers_id)
    {
        $ret = [];
        $data = \common\models\Customers::find()->alias('c')->and_where(['c.customers_id' => $customers_id])->join_with('addressBooks ab')->and_where('c.customers_default_address_id=ab.address_book_id')->as_array()->cache(300)->one();
     ...
     * @return null|float
     */
    public static function get_credit_amount($customer)
    {
        return \common\models\Customers::find_by_var($customer)->credit_amount ?? null;
    }
}