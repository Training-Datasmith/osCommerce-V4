...
class Language
{
    ...
    {
        return \Yii::$app->get_cache()->get_or_set('lang_' . $code, function () use ($code) {
    ...
    public static function system_language_code()
    {
        static $default_system_language = false;
        if ($default_system_language === false) {
            $_data = tep_db_fetch_array(tep_db_query('SELECT configuration_value FROM ' . TABLE_CONFIGURATION . " WHERE configuration_key='DEFAULT_LANGUAGE'"));
            $default_system_language = $_data['configuration_value'];
        }
        return $default_system_language;
    }
    public static function system_language_id()
    {
        static $default_system_language_id = false;
        if ($default_system_language_id === false) {
            $default_system_language_id = static::get_language_id(static::system_language_code());
        }
        return $default_system_language_id['languages_id'];
    }
    /**
     *
    ...
    public static function get_languages($all = false, $include_admin_hidden = false)
    {
        ...
        $q = \common\models\Languages::find()->select('languages_id, name, code, image_svg as image, image_svg, locale, shown_language, searchable_language, directory')->order_by(new \yii\db\Expression("IF(code='" . tep_db_input(strtolower(DEFAULT_LANGUAGE)) . "',0,1)"))->add_order_by('sort_order');
        if (!$all) {
            $q->and_where(['languages_status' => 1]);
        }
        $skip_admin_hidden = !$include_admin_hidden && \frontend\design\Info::is_totally_admin();
        if ($skip_admin_hidden && defined('FORCE_HIDE_ADMIN_LANGUAGE') && FORCE_HIDE_ADMIN_LANGUAGE == 'True') {
            $q->and_where(['hide_in_admin' => 0]);
            $admin = new Admin();
            $admin_info = $admin->get_additional_info();
            if (is_array($admin_info) && isset($admin_info['hidden_admin_language']) && is_array($admin_info['hidden_admin_language'])) {
                $q->and_where(['not in', 'languages_id', $admin_info['hidden_admin_language']]);
            }
        }
        $languages_array = [];
        $_new = [];
        foreach ($q->as_array()->all() as $languages) {
            //while ($languages = tep_db_fetch_array($languages_query)) {
            $_tmp = ['id' => $languages['languages_id'], 'name' => $languages['name'], 'code' => strtolower($languages['code']), 'image' => tep_image(DIR_WS_CATALOG . DIR_WS_ICONS . $languages['image'], $languages['name'], '24', '16', 'class="language-icon"'), 'image_svg' => tep_image(DIR_WS_CATALOG . DIR_WS_ICONS . $languages['image_svg'], $languages['name']), 'locale' => $languages['locale'], 'shown_language' => $languages['shown_language'], 'searchable_language' => $languages['searchable_language'], 'directory' => $languages['directory']];
     ...
     * @return array
     */
    public static function get_admin_hidden_languages($admin = false)
    {
        $ret = \common\models\Languages::find()->select('languages_id')->and_where(['hide_in_admin' => 1, 'languages_status' => 1])->as_array()->cache(1)->column();
        $admin = new Admin($admin);
        $admin_info = $admin->get_additional_info();
        if (!empty($admin_info['hidden_admin_language']) && is_array($admin_info['hidden_admin_language'])) {
            $ret = array_merge($ret, $admin_info['hidden_admin_language']);
        }
    ...
    public static function pull_languages()
    {
        ...
        foreach ($languages as $item) {
            $lang[] = ['id' => $item['code'], 'text' => $item['directory']];
        }
        return $lang;
    }
    public static function get_possible_language($char)
    {
        $ret = false;
        ...
        if (!$ret) {
            $ls = \common\models\Languages::find()->select('code')->and_where(['and', ['languages_status' => 1], ['<>', 'languages_id', self::get_default_language_id()]])->order_by('sort_order')->as_array()->all();
            foreach ($ls as $l) {
    ...
    {
        $ret = [];
        foreach (['en' => 'abcdefghijklmnopqrstuvwxyz', 'ru' => 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя', 'uk' => 'абвгґдеєжзиіїйклмнопрстуфхцчшщьюя'] as $l => $a) {
            if (empty($l) || empty($a)) {
                continue;
            }
    ...
    private static function copy_table(string $table, int $from_lang_id, int $to_lang_id, $options = null)
    {
        $cols = \Yii::$app->db->schema->get_table_schema($table)->get_column_names();
        // excludedColumns
        if (is_array($options) && isset($options['excludeColumns'])) {
            $excluded_columns = $options['excludeColumns'];
            if (is_string($excluded_columns)) {
                $excluded_columns = explode(',', $excluded_columns);
            }
            $cols = array_filter($cols, function ($value) use ($excluded_columns) {
                return !in_array($value, $excluded_columns);
            });
        }
        $to_columns = implode(',', $cols);
        $lang_column_name = null;
        foreach (['language_id', 'languages_id'] as $supposed_col_name) {
            $i = array_search($supposed_col_name, $cols);
            if (false !== $i) {
                \common\helpers\Assert::assert(is_null($lang_column_name), "There are two language columns in table {$table}: {$lang_column_name} and {$supposed_col_name}");
                $lang_column_name = $supposed_col_name;
                $cols[$i] = $to_lang_id;
            }
        }
        \common\helpers\Assert::is_not_null($lang_column_name, "Language column not found in table {$table}: " . var_export($cols, true));
        $from_columns = implode(',', $cols);
        if ($options['deleteBefore'] ?? false) {
            $deleted_rows = \Yii::$app->db->create_command("DELETE FROM {$table} WHERE {$lang_column_name} = :toLangId", [':toLangId' => $to_lang_id])->execute();
            \Yii::info("Copying language {$from_lang_id} to {$to_lang_id}. Table {$table} - deleted {$deleted_rows} rows<br>");
        }
        $affected_rows = \Yii::$app->db->create_command("INSERT IGNORE INTO {$table}({$to_columns}) SELECT {$from_columns} FROM {$table} WHERE {$lang_column_name} = :fromLangId", [':fromLangId' => $from_lang_id])->execute();
        \Yii::info("Copying language {$from_lang_id} to {$to_lang_id}. Table {$table} - {$affected_rows} rows<br>");
    }
    /**
     ...
     * @param int $toLangId
     */
    public static function copy_language(int $from_lang_id, int $to_lang_id)
    {
        $tables = [TABLE_CATEGORIES_DESCRIPTION, TABLE_PRODUCTS_DESCRIPTION, TABLE_PRODUCTS_IMAGES_DESCRIPTION, TABLE_PRODUCTS_OPTIONS, TABLE_PRODUCTS_OPTIONS_VALUES, TABLE_COUPONS_DESCRIPTION, TABLE_PROPERTIES_CATEGORIES_DESCRIPTION, TABLE_PROPERTIES_DESCRIPTION, TABLE_PROPERTIES_VALUES, TABLE_INFORMATION, TABLE_MANUFACTURERS_INFO, TABLE_ORDERS_STATUS, TABLE_ORDERS_STATUS_GROUPS, TABLE_PRODUCTS_STOCK_INDICATION_TEXT, TABLE_PRODUCTS_STOCK_DELIVERY_TERMS_TEXT, TABLE_PRODUCTS_XSELL_TYPE, TABLE_COUNTRIES, TABLE_BANNERS_LANGUAGES => ['excludeColumns' => 'blang_id'], TABLE_DESIGN_BOXES_SETTINGS => ['excludeColumns' => 'id'], TABLE_DESIGN_BOXES_SETTINGS_TMP => ['excludeColumns' => 'id'], TABLE_EMAIL_TEMPLATES_TEXTS, TABLE_MENU_TITLES => ['excludeColumns' => 'id'], TABLE_TRANSLATION => ['excludeColumns' => 'translated,checked']];
        foreach ($tables as $key => $value) {
            ...
            self::copy_table($table, $from_lang_id, $to_lang_id, $options);
        }
    }
    /*
    ...
    */
    public static function drop_language(int $l_id)
    {
        ...
        if ($l_id) {
            ...
            if ($lng['languages_id'] == $l_id) {
                throw new \Exception('Can\'t delete default language');
            }
            $code = \common\models\Languages::find()->where(['languages_id' => (int) $l_id])->as_array()->one();
            tep_db_query('delete from ' . TABLE_CATEGORIES_DESCRIPTION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PRODUCTS_DESCRIPTION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PRODUCTS_OPTIONS . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PRODUCTS_OPTIONS_VALUES . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_MANUFACTURERS_INFO . " where languages_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_ORDERS_STATUS . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_COUPONS_DESCRIPTION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_ORDERS_STATUS_GROUPS . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_COUNTRIES . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_LANGUAGES_FORMATS . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_BANNERS_LANGUAGES . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_DESIGN_BOXES_SETTINGS . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_DESIGN_BOXES_SETTINGS_TMP . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_EMAIL_TEMPLATES_TEXTS . " where language_id = '" . (int) $l_id . "' and affiliate_id = 0");
            tep_db_query('delete from ' . TABLE_MENU_TITLES . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PRODUCTS_IMAGES_DESCRIPTION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_TRANSLATION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PLATFORM_FORMATS . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PROPERTIES_CATEGORIES_DESCRIPTION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PROPERTIES_DESCRIPTION . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PROPERTIES_VALUES . " where language_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_INFORMATION . " where languages_id = '" . (int) $l_id . "'");
            tep_db_query('delete from ' . TABLE_PRODUCTS_XSELL_TYPE . " where language_id = '" . (int) $l_id . "'");
            if (!empty($code)) {
                ...
                tep_db_query('delete from ' . TABLE_LANGUAGES . " where languages_id = '" . (int) $l_id . "'");
            }
        }
    }
}