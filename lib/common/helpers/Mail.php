...
class Mail
{
    ...
    public static function sendPlain($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address, $email_params = [], $headers = '')
    {
        if (SEND_EMAILS != 'true') {
            return false;
        }
    ...
    public static function copyPlatformEmails($target_platform_id, $source_platform_id = 0)
    {
        if (empty($source_platform_id)) {
            $source_platform_id = \common\classes\platform::defaultId();
        }
