...
class Bundles
{
     ...
     * @return array|''
     */
    public static function get_details($params, $attributes_details = [], $skip_attributes = false)
    {
        ...
        if (is_array($attributes)) {
            ...
            $main_uprid = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($params['products_id'], $main_attributes));
        } else {
            $main_uprid = $params['products_id'];
        }
        $products_join = '';
        ...
        $sql_where = '';
        $sql_where .= ' AND p.products_status = 1 ';
        $bundle_sets_query = tep_db_query('select p.products_id, p.products_model, p.products_image, p.products_tax_class_id, sp.num_product, ' . 'parent_p.products_tax_class_id AS parent_products_tax_class_id, ' . 'IF(p.is_listing_product AND p.products_status, 1, 0) AS _status, ' . 'if(length(pd1.products_name), pd1.products_name, pd.products_name) as products_name ' . 'from ' . TABLE_PRODUCTS . " p {$products_join} " . '  left join ' . TABLE_PRODUCTS_DESCRIPTION . " pd1 on pd1.products_id = p.products_id and pd1.language_id = '" . (int) $languages_id . "' and pd1.platform_id = '" . intval(\Yii::$app->get('platform')->config()->get_platform_to_description()) . "' " . ' left join ' . TABLE_PRODUCTS_PRICES . " pgp on p.products_id = pgp.products_id and pgp.groups_id = '" . (int) $customer_groups_id . "' and pgp.currencies_id = '" . (int) (USE_MARKET_PRICES == 'True' ? (int) \Yii::$app->settings->get('currency_id') : 0) . "', " . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . '' . TABLE_SETS_PRODUCTS . ' sp ' . ' inner join ' . TABLE_PRODUCTS . ' parent_p on parent_p.products_id=sp.sets_id ' . "where sp.product_id = p.products_id and sp.product_id = pd.products_id and pd.language_id = '" . (int) $languages_id . "' " . " and sp.sets_id = '" . (int) $params['products_id'] . "' and p.products_status_bundle = '1' " . ' ' . (isset($_SESSION['affiliate_ref']) && $_SESSION['affiliate_ref'] > 0 ? ' and p2a.affiliate_id is not null ' : '') . " and pd.platform_id = '" . intval(\common\classes\platform::default_id()) . "' and if(pgp.products_group_price is null, 1, pgp.products_group_price != -1 ) " . $sql_where . ' ' . 'group by p.products_id ' . 'order by sp.sort_order, products_name');
        if (tep_db_num_rows($bundle_sets_query) > 0) {
            ...
            while ($bundle_sets = tep_db_fetch_array($bundle_sets_query)) {
                ...
                $uprid = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($products_id, $attributes));
                unset($params['products_id']);
                $price_instance = \common\models\Product\Price::get_instance($uprid);
                $products_price = $price_instance->get_inventory_price(['qty' => $params['qty']]);
                $special_price = $price_instance->get_inventory_special_price(['qty' => $params['qty']]);
                ...
                */
                ...
                if (!$skip_attributes) {
                    ...
                    while ($products_options_name = tep_db_fetch_array($products_options_name_query)) {
                        ...
                        while ($products_options = tep_db_fetch_array($products_options_query)) {
                            ...
                            reset($comb_arr);
                            $mask = str_replace('0000000', '%', Inventory_Helper::normalize_inventory_id(Inventory_Helper::get_uprid($products_id, $comb_arr), $vids, $virtual_vids));
                            $check_inventory = tep_db_fetch_array(tep_db_query('select inventory_id, max(products_quantity) as products_quantity, min(non_existent) as non_existent, products_id, stock_indication_id, stock_delivery_terms_id, stock_control, products_id ' . 'from ' . TABLE_INVENTORY . ' i ' . "where products_id like '" . tep_db_input($mask) . "' " . Inventory_Helper::get_sql_inventory_restrictions(['i', 'ip']) . ' ' . 'order by products_quantity desc ' . 'limit 1'));
                            if (!$check_inventory['inventory_id'] && strstr($mask, '{')) {
                                continue;
                            }
                            if ($check_inventory['non_existent']) {
                                continue;
                            }
                            if ($ext = \common\helpers\Acl::check_extension_allowed('UserGroupsRestrictions', 'isAllowed')) {
                                if ($ext::is_restricted($mask)) {
                                    continue;
                                }
                            }
                            ...
                            $check_inventory['products_quantity'] = \common\helpers\Product::get_available($check_inventory['products_id'], 0);
                        ...
                        if (count($products_options_array) > 0) {
                            ...
                            $attributes_array[] = ['title' => htmlspecialchars($products_options_name['products_options_name']), 'name' => 'id[' . $products_options_name['products_options_id'] . '-' . $products_id . ']', 'id' => $products_options_name['products_options_id'] . '-' . $products_id, 'options' => $products_options_array, 'selected' => $selected_attribute];
                        }
                    }
                    // {{
                }
                ...
                $current_uprid = Inventory_Helper::normalize_id(Inventory_Helper::get_uprid($products_id, $attributes));
                $bundle_sets['current_uprid'] = $current_uprid;
                $check_inventory = tep_db_fetch_array(tep_db_query('select inventory_id, products_quantity, stock_indication_id, stock_delivery_terms_id ' . 'from ' . TABLE_INVENTORY . ' ' . "where products_id like '" . tep_db_input(Inventory_Helper::normalize_inventory_id($current_uprid)) . "' " . 'limit 1'));
                \common\helpers\Php8::null_arr_props($check_inventory, ['inventory_id', 'products_quantity', 'stock_indication_id', 'stock_delivery_terms_id']);
                ...
                $price_instance = \common\models\Product\Bundle_Price::get_instance($uprid);
                $bundle_price = $price_instance->get_bundle_price(['qty' => $params['qty'] * $bundle_sets['num_product'], 'parent' => $main_uprid]);
                $bundle_special_price = $price_instance->get_bundle_special_price(['qty' => $params['qty'] * $bundle_sets['num_product'], 'parent' => $main_uprid]);
                ...
                $bundle_sets['image'] = \common\classes\Images::get_image_url($bundle_sets['products_id'], 'Small');
            ...
            $price_instance = \common\models\Product\Price::get_instance($main_uprid);
            $products_price = $price_instance->get_inventory_price(['qty' => $params['qty']]);
            $special_price = $price_instance->get_inventory_special_price(['qty' => $params['qty']]);
            $check_parent = \common\helpers\Product::get_product_columns((int) \common\helpers\Inventory::get_prid($main_uprid), ['use_sets_discount']);
            ...
            if (count($stock_indicators_ids) > 0) {
                ...
                if (count($tmp) == 1) {
                    ...
                    $stock_indicators_sorted = \common\classes\Stock_Indication::sort_stock_flags($tmp);
                    $bundle_stock_indicator = $stock_indicators_sorted[count($tmp) - 1];
                }
            } elseif (count($all_stock_indicators) > 0) {
                $stock_indicators_sorted = \common\classes\Stock_Indication::sort_stock_flags($all_stock_indicators);
            ...
            $return_data = ['product_valid' => min($all_filled_array) && (isset($attributes_details['product_valid']) ? $attributes_details['product_valid'] : true) ? '1' : '0', 'product_qty' => min($product_qty_array), 'bundle_products' => $bundle_products, 'stock_indicator' => $bundle_stock_indicator, 'product_price' => $currencies->display_price($products_price, Tax::get_tax_rate($bundle_tax_class_id), 1), 'special_price' => $currencies->display_price($special_price, Tax::get_tax_rate($bundle_tax_class_id), 1), 'full_bundle_price' => $currencies->format($full_bundle_price, false), 'actual_bundle_price' => $currencies->format($actual_bundle_price, false), 'full_bundle_price_clear' => $full_bundle_price, 'actual_bundle_price_clear' => $actual_bundle_price, 'actual_bundle_price_unit' => $actual_bundle_price_unit];
            if (defined('DISPLAY_BOTH_PRICES') && DISPLAY_BOTH_PRICES == 'True') {
                $return_data += ['product_price_ex' => $currencies->display_price($products_price, 0, 1, false, false), 'special_price_ex' => $currencies->display_price($special_price, 0, 1, false, false), 'full_bundle_price_ex' => $currencies->format($full_bundle_price_ex, false), 'actual_bundle_price_ex' => $currencies->format($actual_bundle_price_ex, false), 'full_bundle_price_clear_ex' => $full_bundle_price_ex, 'actual_bundle_price_clear_ex' => $actual_bundle_price_ex, 'actual_bundle_price_unit_ex' => $actual_bundle_price_unit_ex];
            }
    ...
    public static function parse_uprid($uprid)
    {
        ...
        if (preg_match_all('/(\d+)\|(.*?){sub}' . (int) $prid . '/', $uprid, $matches)) {
            foreach ($matches[2] as $_idx => $part_uprid) {
                $bundle_parts[] = $part_uprid;
            }
        }
        return $bundle_parts;
    }
