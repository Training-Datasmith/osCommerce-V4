...
class OscLink extends \common\classes\modules\ModuleExtensions
{
    ...
    public static function adminActionIndex()
    {
        ...
        Yii::$app->controller->view->ViewTable = [
            [
    ...
    public static function getConfigurationArray($key = '')
    {
        if (is_null(self::$config)) {
            ...
            foreach ($return as &$itemArray) {
                ...
                unset($title);
                if (in_array($itemArray['cmc_key'], ['api_status_map', 'api_tax_map'])) {
                    $itemArray['cmc_value'] = json_decode($itemArray['cmc_value'], true);
                    $itemArray['cmc_value'] = (is_array($itemArray['cmc_value']) ? $itemArray['cmc_value'] : []);
                }
            }
            unset($itemArray);
    ...
    public static function getPlatformValueArray($key = '')
    {
        ...
        if (!is_array(self::$platformArray)) {
            ...
            if ($platformId > 0) {
                $platformSelectArray = (
                    \common\models\Platforms::find()->alias('p')
        ...
        return (($key == '') ? self::$platformArray : (
            isset(self::$platformArray[$key]) ? self::$platformArray[$key] : null
        )
        );
    }
