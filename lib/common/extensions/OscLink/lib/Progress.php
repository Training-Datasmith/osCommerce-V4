...
class Progress
{
    ...
    private static function get_text_def($value)
    {
        return "{$value}%";
    }
    private static function is_important_msg($params)
    {
        return !empty($params['reset']) || !empty($params['message']) || self::$progress_last_update == 0 || microtime(true) - self::$progress_last_update > 1;
    }
    public static function Update(array $params)
    {
        if (self::is_important_msg($params)) {
            ...
            if (!array_key_exists('progress', $params) && is_array(self::$last_not_important_params) && array_key_exists('progress', self::$last_not_important_params)) {
                ...
                $params['text'] = self::$last_not_important_params['text'] ?? self::get_text_def($params['progress']);
            }
            ...
            flush();
        } else if (array_key_exists('progress', $params)) {
    ...
    public static function Percent(int $value, $txt = null)
    {
        ...
        $txt = $txt ?? self::get_text_def($value);
        self::Update(['progress' => $value, 'text' => $txt]);
    }
    ...
    public static function show_log_file($format = 'Detailed log: %s <a href="extensions?module=OscLink&action=adminActionShowLog&log=%s" target="_blank">view</a>')
    {
        $fn = \Osc_Link\Logger::get()->get_filename();
        \Osc_Link\Progress::Log(sprintf($format, $fn, basename($fn, '.log')));
    }
}