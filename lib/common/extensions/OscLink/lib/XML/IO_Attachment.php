...
class Io_Attachment extends Complex
{
    ...
    public function to_import_model()
    {
        ...
        if (!empty($source_file)) {
            if (empty($this->value)) {
                $this->value = Io_Core::get()->normalize_local_file_name(basename($source_file));
            }
            if (isset($this->options) && !empty($this->options['importVia']) && $this->options['importVia'] === 'File') {
                ...
                $this->value = Io_Core::get()->normalize_local_file_name($this->value);
                $origin = $physical_file = Io_Core::get()->get_local_location($this->location . '/' . $this->value);
                Io_Core::get()->download($source_file, $physical_file);
                if ($origin != $physical_file) {
                    \Osc_Link\Logger::print("Warning while saving images: originFN={$origin} but result={$physical_file}");
                }
