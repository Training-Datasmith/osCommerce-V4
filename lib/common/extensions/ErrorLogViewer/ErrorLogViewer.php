...
class ErrorLogViewer extends \common\classes\modules\ModuleExtensions
{
    ...
    public function Zipping()
    {
        ...
        try {
            ...
            foreach (self::sourceList() as $source) {
                if (!file_exists(Yii::getAlias('@'.$source).$this->path) && !is_dir(Yii::getAlias('@'.$source).$this->path)) {
                    continue;
                }
        ...
        if (file_exists($this->zipPath) && is_dir($this->zipPath)) {
            try {
                foreach (scandir($this->zipPath) as $file) {
                    if (!is_file($this->zipPath . DIRECTORY_SEPARATOR . $file)) {
                        continue;
                    }
                    if ((time() - filemtime($this->zipPath . DIRECTORY_SEPARATOR . $file)) > 86400) {
                        unlink($this->zipPath . DIRECTORY_SEPARATOR . $file);
                    }
                }
                return true;
            } catch (\Exception $ex) {
