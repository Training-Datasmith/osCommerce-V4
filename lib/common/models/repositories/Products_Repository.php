...
class ProductsRepository
{
    ...
    public function getSearchDetails($ids)
    {
        $products = Products::find()->alias('p')->select('p.products_id, p.manufacturers_id')
            ->addSelect(array_map(function ($el) {
                return 'p.' . $el;
                                ...
                                case 'categories':
                                    $getValue = implode(
                                        ...
                                        \yii\helpers\ArrayHelper::getColumn(
                                            array_filter($ret[$key]['searchCategories'], function ($cat) use ($languageId) {
                                                return  $cat['language_id'] == $languageId;
                                            }),
                                            'categories_name'
                                        )
                                    );
