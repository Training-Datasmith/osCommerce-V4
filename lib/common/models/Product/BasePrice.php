...
class BasePrice
{
    ...
    public function getInventoryPrice($params)
    {
        ...
        if (\common\helpers\Extensions::isAllowed('Inventory') && strpos($this->uprid, '{') !== false && !InventoryHelper::disabledOnProduct($_uprid)) {
            ...
            $check_inventory = tep_db_fetch_array(tep_db_query('select inventory_id, price_prefix, inventory_price, inventory_full_price from ' . TABLE_INVENTORY . " i where products_id = '" . tep_db_input($_uprid) . "' and non_existent = '0' " . InventoryHelper::get_sql_inventory_restrictions(['i', 'ip']) . ' limit 1'));
            if ($check_inventory) {
                if (!$this->calculate_full_price) {
                    ...
                    if ($check_inventory['price_prefix'] == '-') {
