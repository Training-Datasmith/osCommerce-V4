...
class Properties2ProductsQuery extends ActiveQuery
{
    public function withPropertiesDescription(int $languages_id = null, string $alias = 'prd')
    {
        ...
        return $this->joinWith(["propertiesDescriptions $alias" => function ($query) use ($alias, $languages_id) {
            $query->andOnCondition(["$alias.language_id" => $languages_id]);
        }], false);
    }
    ...
    public function withPropertiesValue(int $languages_id = null, string $alias = 'pv')
    {
        ...
        return $this->joinWith(["values $alias" => function ($query) use ($alias, $languages_id) {
            $query->andOnCondition(["$alias.language_id" => $languages_id]);
        }], false);
    }
