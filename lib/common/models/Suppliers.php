...
class Suppliers extends ActiveRecord
{
    ...
    public function beforeDelete()
    {
        ...
        if ($this->supplierProducts) {
            foreach ($this->supplierProducts as $sProduct) {
                $sProduct->delete();
            }
        }
        if ($this->supplierProductsOptions) {
            foreach ($this->supplierProductsOptions as $sO) {
                $sO->delete();
            }
        }

        if ($this->supplierProductsOptionsValues) {
            foreach ($this->supplierProductsOptionsValues as $sOV) {
                $sOV->delete();
            }
        }
    ...
    public function saveSupplier($params)
    {
        ...
        if (!$this->currencies_id) {
            $this->currencies_id = \common\helpers\Currencies::getCurrencyId(DEFAULT_CURRENCY);
        }
    ...
    public function saveCurrencies($curList = [])
    {
        if (is_array($curList)) {
