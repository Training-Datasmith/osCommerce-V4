...
class CartFactory
{
    ...
    public static function work()
    {
        ...
        if ($cart->count_contents() > 0 && ($cart->currency != $currency || $cart->language_id != $languages_id) && method_exists($cart, 'update_basket_info')) {
            $cart->update_basket_info();
        }
                    ...
                    if (isset($_GET['product_id']) && is_numeric($_GET['product_id'])) {
                        if (isset($_POST['giveaway_switch'])) {
                            foreach ($_POST['giveaway_switch'] as $gaw_id => $data) {
                                ...
                                if ($_POST['giveaways'][$gaw_id]['products_id'] != $_GET['product_id']) {
                                    continue;
                                }
            ...
            $cart->add_cart($_GET['product_id'], $cart->get_quantity($_GET['product_id'], 1) + $_POST['qty'], '', 1, ($_POST['gaw_id']>0?$_POST['gaw_id']:true));
                ...
                case 'update_product':
                    if (isset($_POST['products_id']) && is_array($_POST['products_id'])) {
                        ...
                        for ($i = 0, $n = sizeof($_POST['products_id']); $i < $n; $i++) {
                            if ($_POST['ga'][$i]) {
                                continue;
                            } // GA are not processed
                            $gift_wrap = (isset($_POST['gift_wrap']) && is_array($_POST['gift_wrap'])) ? $_POST['gift_wrap'] : [];
                            if (in_array($_POST['products_id'][$i], (isset($_POST['cart_delete']) && is_array($_POST['cart_delete']) ? $_POST['cart_delete'] : []))) {
                                $cart->remove($_POST['products_id'][$i]);
                            } else {

                                $posted_uprid = $_POST['products_id'][$i];
                                // {{ allow update only in cart products - prevent lost props part
                                if (!$cart->in_cart($posted_uprid)) {
                                    continue;
                                }
                ...
                case 'recovery_restore':
                    ...
                    if ($email_address) {
                        ...
                        if ($customer->loginCustomer($email_address, $token)) {
                            if (isset($_GET['utmgclid'])) {
                                \common\helpers\System::setcookie('__utmz', 'utmgclid=' . $_GET['utmgclid'], time() + 3600);
                            }
                            /** @var \common\extensions\CouponsAndVauchers\CouponsAndVauchers $ext */
                            if ($ext = \common\helpers\Extensions::isAllowed('CouponsAndVauchers')) {
                ...
                case 'notify':         if (!Yii::$app->user->isGuest) {
                    if (isset($_GET['products_id'])) {
                        ...
                        tep_redirect(tep_href_link(basename($PHP_SELF), \common\helpers\Output::get_all_get_params(['action', 'notify'])));
                    }
                    if (!is_array($notify)) {
                        $notify = [$notify];
                    }
                ...
                case 'notify_remove':  if (!Yii::$app->user->isGuest && isset($_GET['products_id'])) {
                    ...
                    tep_redirect(tep_href_link(basename($PHP_SELF), \common\helpers\Output::get_all_get_params(['action'])));
                } else {
                    $navigation->set_snapshot();
                    tep_redirect(tep_href_link(FILENAME_LOGIN, '', 'SSL'));
                }
