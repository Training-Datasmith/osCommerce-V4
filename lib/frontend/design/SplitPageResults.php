...
class splitPageResults
{
    ...
    /* class constructor */
    public function __construct($query, $max_rows, $count_key = '*', $page_holder = 'page', $skipCount = -1)
    {
    ...
    public function display_links($max_page_links, $parameters = '', $action = '')
    {
    ...
    public function display_count_mobile($text_output)
    {
        $to_num = ($this->number_of_rows_per_page * $this->current_page_number);
        if ($to_num > $this->number_of_rows) {
            $to_num = $this->number_of_rows;
