...
class ListingFunctionality extends Widget
{
    ...
    public function run()
    {
        ...
        if ($listing_split->number_of_rows > 0) {
            ...
            $sorting_link = tep_href_link($this->params['this_filename'], \common\helpers\Output::get_all_get_params(['sort']));
            ...
            if (!$searchResults) {
                $searchResults = SEARCH_RESULTS_1;
            }
            ...
            if (Info::widgetSettings('Listing', 'listing_type') != 'no') {
                $grid_link = tep_href_link($this->params['this_filename'], \common\helpers\Output::get_all_get_params(['gl']) . 'gl=grid');
            }
            if (Info::widgetSettings('Listing', 'listing_type_rows') != 'no') {
                $list_link = tep_href_link($this->params['this_filename'], \common\helpers\Output::get_all_get_params(['gl']) . 'gl=list');
            }
            if (Info::widgetSettings('Listing', 'listing_type_b2b')) {
                $b2b_link = tep_href_link($this->params['this_filename'], \common\helpers\Output::get_all_get_params(['gl']) . 'gl=b2b');
            }
            Info::sortingId();
            return IncludeTpl::widget([
              ...
              'params' => [
                ...
                'hidden_fields' => \common\helpers\Output::get_all_get_params(['sort','max_items'], true),
                'grid_link' => $grid_link,
                'list_link' => $list_link,
                'b2b_link' => $b2b_link,
                'gl' => $_SESSION['gl'],
