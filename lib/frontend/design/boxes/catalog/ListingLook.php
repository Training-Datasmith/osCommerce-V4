...
class ListingLook extends Widget
{
    ...
    public function run()
    {
        ...
        if ($listing_split->number_of_rows > 0) {
            ...
            if (Info::widgetSettings('Listing', 'listing_type') != 'no' && $this->params['listing_type'] != 'no') {
                $grid_link = tep_href_link($this->params['this_filename'], \common\helpers\Output::get_all_get_params(['gl']) . 'gl=grid');
            }
            if (Info::widgetSettings('Listing', 'listing_type_rows') != 'no' && $this->params['listing_type_rows'] != 'no') {
