...
class Filters extends Widget
{
    ...
    public static function products_filters_cmp($a, $b)
    {
    ...
    public function run()
    {
        ...
        while ($manufacturers = tep_db_fetch_array($manufacturers_query)) {
            $manufacturers_array[$manufacturers['manufacturers_id']] = [
                'id' => $manufacturers['manufacturers_id'],
                'text' => $manufacturers['manufacturers_name'],
                'count' => 0, // next step // (int)$manufacturers['products_count'],
                'selected' => in_array($manufacturers['manufacturers_id'], is_array($_GET[$name]) ? $_GET[$name] : [$_GET[$name]]),
            ];
        }
        ...
        if (count($manufacturers_array) > 0 && $products_count > 0) {
            usort($manufacturers_array, ['self', 'products_filters_cmp']);
            $filters_array[] = [
                'title' => TEXT_BRAND,
                'name' => $name,
                'type' => 'boxes',
        ...
        if (count($filters_array) > 0) {
            return IncludeTpl::widget([
                ...
                'params' => [
                  ...
                  'filters_hiddens' => \common\helpers\Output::get_all_get_params(array_merge(['cPath'], $exclude_params), true),
