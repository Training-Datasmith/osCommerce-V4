...
class Info
{
     ...
     */
    public static function getListProductsDetails($products_ids, $settings = [])
    {
                ...
                foreach ($products as $products_arr) {
                    ...
                    if (!($settings[0]['show_stock'] ?? false) || !($settings[0]['show_price'] ?? false) || !($settings[0]['show_qty_input'] ?? false)) {
                        ...
                        if ($products_arr['products_pctemplates_id'] > 0 || $products_arr['is_bundle']) {
                            ...
                            $products_arr['stock_indicator'] = \common\classes\StockIndication::product_info([
     ...
     * @return array products array (all data according settings)
     */
    public static function getProducts($products_query, $settings = [])
    {
        global $cart;
    ...
    public static function widgetSettings($widget_name = false, $setting_name = false, $block_name = false, $include_blocks = false, $theme_name = '')
    {
        ...
        if ($widget_name) {
            ...
            foreach ($widgets as $key => $box) {
                ...
                while ($item = tep_db_fetch_array($query)) {
                    $settings[$box][] = [$item[$item['language_id']]['setting_name'] => $item['setting_value']];
                }
                if ($setting_name) {
        ...
        if ($platform_id) {
            ...
            foreach (Yii::$app->get('platform')->getConfig($platform_id)->getOpenHours() as $item) {
                ...
                foreach ($days as $dayItem) {
                    if ($dayItem) {
                        $item['days_arr'][] = $allDays[(int)$dayItem];
                    }
                }
                ...
                $day = str_replace(',', ', ', $day);
                $item['days_short'] = $day;
                if (str_replace('0,', '', $item['days']) == '1,2,3,4,5') {
                    $item['days_short'] = TEXT_DAY_MO . ' - ' . TEXT_DAY_FR;
                }
                $times['open'][] = $item;
            }
        }
    ...
    public static function blockWidthMultiplier($id)
    {
        ...
        $arr[] = ['multiplier' => $multiplier, 'padding' => $padding];
    ...
    public static function sortingId()
    {
        ...
        if (isset($get['sort'])) {
            ...
            if (true) {
                ...
                $key = array_shift($arr);
                switch ($key) {
                    case 'sort_pos_0': $sorting_id = 0;
                        break;
                    case 'sort_pos_1': $sorting_id = 'ma';
                        break;
                    case 'sort_pos_2': $sorting_id = 'md';
                        break;
                    case 'sort_pos_3': $sorting_id = 'na';
                        break;
                    case 'sort_pos_4': $sorting_id = 'nd';
                        break;
                    case 'sort_pos_5': $sorting_id = 'ba';
                        break;
                    case 'sort_pos_6': $sorting_id = 'bd';
                        break;
                    case 'sort_pos_7': $sorting_id = 'pa';
                        break;
                    case 'sort_pos_8': $sorting_id = 'pd';
                        break;
                    case 'sort_pos_9': $sorting_id = 'qa';
                        break;
                    case 'sort_pos_10': $sorting_id = 'qd';
                        break;
                    case 'sort_pos_11': $sorting_id = 'wa';
                        break;
                    case 'sort_pos_12': $sorting_id = 'wd';
                        break;
                    case 'sort_pos_13': $sorting_id = 'da';
                        break;
                    case 'sort_pos_14': $sorting_id = 'dd';
                        break;
                    case 'sort_pos_15': $sorting_id = 'ya';
                        break;
                    case 'sort_pos_16': $sorting_id = 'yd';
                        break;
                    default: $sorting_id = 0;
                        break;
                }
            }
        }
    ...
    public static function createJs($ajax = false)
    {
        ...
        foreach (Block::$widgetsList as $widget) {
            if (isset(self::$includeJsFiles['boxes/' . $widget]) && self::$includeJsFiles['boxes/' . $widget]) {
                continue;
            }
            if (isset(self::$includeJsFiles['boxes/' . str_replace('\\', '/', $widget)]) && self::$includeJsFiles['boxes/' . str_replace('\\', '/', $widget)]) {
                continue;
            }
