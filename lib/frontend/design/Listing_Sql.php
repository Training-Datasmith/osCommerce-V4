...
class ListingSql
{
    ...
    public static function get_listing_sql_array($filename = '')
    {
        ...
        $listing_where .= \common\helpers\Product::get_sql_product_restrictions(['p', 'pd', 's', 'sp', 'pp']) . ' ';
            ...
            default: case 'catalog': case 'catalog/index':
                if ($current_category_id > 0) {
                    if (\frontend\design\Info::themeSetting('show_products_from_subcategories')) {
                        $categories_array = [$current_category_id];
        ...
        return ['from' => $listing_from, 'left_join' => $listing_left_join, 'where' => $listing_where, 'order' => $order];
    }
    ...
    public static function get_filters_sql_array($exclude = '')
    {
            ...
            if ($pfrom > 0 || $pto > 0) {
                ...
                if (($ext = \common\helpers\Extensions::isAllowed('ProductPriceIndex')) && !empty($extModel = $ext::getModel('ProductPriceIndex'))) {
                    ...
                    if (USE_MARKET_PRICES == 'True' || \common\helpers\Extensions::isCustomerGroupsAllowed()) {
                        ...
                        $query = tep_db_query('select p.products_id, p.products_tax_class_id, p.products_price from ' . TABLE_PRODUCTS . " p {$products_join} where " . \common\helpers\Product::getState() . \common\helpers\Product::get_sql_product_restrictions(['p', 'pd', 's', 'sp', 'pp']) . ' group by p.products_id');
                    }
                        ...
                        if (tep_not_null($values)) {
                            ...
                            if (is_array($values)) {
                                if ($values[0] > 0) {
                                    ...
                                    $filters_where .= ' and p2p' . $prop_id . ".values_id = '0' and p2p" . $prop_id . ".values_flag in ('" . implode("','", array_map(function ($v) {
                                        return (int)($v == 'Y');
                                    }, $values)) . "') ";
                                }
                            } else {
        ...
        return ['left_join' => $filters_left_join, 'where' => $filters_where, 'relevance_order' => $relevance_order];
    }
}
