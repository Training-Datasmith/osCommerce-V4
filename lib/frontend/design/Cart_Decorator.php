...
class CartDecorator
{
    ...
    private function decorateProducts()
    {
            ...
            if (isset($this->products[$i]['attributes']) && is_array($this->products[$i]['attributes'])) {
                ...
                if (isset($this->products[$i]['virtual_gift_card']) && $this->products[$i]['virtual_gift_card'] && $this->products[$i]['attributes'][0] > 0) {
                    ...
                    if (tep_not_null($virtual_gift_card['virtual_gift_card_recipients_name'])) {
                        $this->products[$i]['attr'][0]['products_options_values_name'] .= TEXT_GIFT_CARD_RECIPIENTS_NAME . ' ' . $virtual_gift_card['virtual_gift_card_recipients_name'] . "\n";
                    }
                    if (tep_not_null($virtual_gift_card['virtual_gift_card_recipients_email'])) {
                        $this->products[$i]['attr'][0]['products_options_values_name'] .= TEXT_GIFT_CARD_RECIPIENTS_EMAIL . ' ' . $virtual_gift_card['virtual_gift_card_recipients_email'] . "\n";
                    }
                    if (tep_not_null($virtual_gift_card['virtual_gift_card_message'])) {
                        $this->products[$i]['attr'][0]['products_options_values_name'] .= TEXT_GIFT_CARD_MESSAGE . ' ' . $virtual_gift_card['virtual_gift_card_message'] . "\n";
                    }
                    if (tep_not_null($virtual_gift_card['virtual_gift_card_senders_name'])) {
                        $this->products[$i]['attr'][0]['products_options_values_name'] .= TEXT_GIFT_CARD_SENDERS_NAME . ' ' . $virtual_gift_card['virtual_gift_card_senders_name'] . "\n";
                    }
                    if (tep_not_null($virtual_gift_card['send_card_date']) && $virtual_gift_card['send_card_date'] != '0000-00-00 00:00:00') {
                        $this->products[$i]['attr'][0]['products_options_values_name'] .= '<br>'.TEXT_GIFT_CARD_SEND . ': ' . \common\helpers\Date::date_short($virtual_gift_card['send_card_date']) . "\n";
                    }
                } else {
        ...
        for ($i = 0, $n = sizeof($this->products); $i < $n; $i++) {
            ...
            if (!isset($this->products[$i]['bundles_info']) || !is_array($this->products[$i]['bundles_info'])) {
                continue;
            }
            $this->products[$i]['is_bundle'] = false;
            foreach ($this->products[$i]['bundles_info'] as $bpid => $bundle_info) {
                $this->products[$i]['bundles_info'][$bpid]['attr'] = [];
                ...
                if (isset($this->products[$i]['attr']) && is_array($this->products[$i]['attr']) && count($this->products[$i]['attr']) > 0) {
                    foreach ($this->products[$i]['attr'] as $__option_id => $__option_value_data) {
                        if (strpos($__option_id.'-', '-'.$bpid.'-') === false) {
                            continue;
                        }
