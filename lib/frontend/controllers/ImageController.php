...
class ImageController extends Sceleton
{
    ...
    public function actionCached()
    {
        ...
        if (!$isWatermarkedImage && defined('SEO_IMAGE_URL_PARTS_NAME') && SEO_IMAGE_URL_PARTS_NAME == 'False') {
            ...
            die;
        } elseif (preg_match('#(?P<seoRef>.*?)/(products\/\d+/)?(?P<nameInsideProductDir>(?P<imageId>[\d]+)/(?<imageSize>\d+x\d+)/(?P<lang_code>[a-zA-Z]{2}/)?(?P<image_name>.+))$#', $requested_image, $imagePattern)) {
