...
class Sceleton extends Controller
{
    ...
    public function __construct($id, $module = null)
    {
        ...
        $themes_path = Info::getThemesPath([$theme]);
    ...
    protected function setMeta($id, $params)
    {
        ...
        if (tep_db_num_rows($get_def_q) > 0) {
            while ($get_def = tep_db_fetch_array($get_def_q)) {
                if (defined(trim($get_def['meta_tags_key']))) {
                    continue;
                }
                ...
                if (!is_array($the_data) || empty($the_data['head_title_tag'])) {
                    if ($with_category) {
                        ...
                        if (strstr($the_title, '##BREADCRUMB##')) {
                            ...
                            if (count($breadcrumb_array) == 0 && Seo::getMetaDefaultBreadcrumb($full_action)) {
                                $breadcrumb_array[] = Seo::getMetaDefaultBreadcrumb($full_action);
                            }
        ...
        $the_title = strip_tags(str_replace(["\n","\r","\r\n","\n\r"], ' ', str_replace('<', ' <', $the_title)));
        ...
        $the_key_words = strip_tags(str_replace(["\n","\r","\r\n","\n\r"], ' ', str_replace('<', ' <', $the_key_words)));
        ...
        $the_desc = strip_tags(str_replace(["\n","\r","\r\n","\n\r"], ' ', str_replace('<', ' <', $the_desc)));
