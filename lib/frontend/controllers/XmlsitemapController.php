...
//      ),
        ...
        */
        ...
        if ($ext = \common\helpers\Acl::checkExtensionAllowed('DeliveryLocation', 'allowed')) {
            $this->registered_map['delivery-location'] = [
    ...
    public function actionIndex()
    {
        ...
        foreach ($this->registered_map as $registered_feed) {
            if (isset($registered_feed['index_feed']) && $registered_feed['index_feed']) {
                continue;
            }
            if (in_array($registered_feed['feedname'], ['products', 'images', 'delivery-location', 'categories', 'manufacturers'])) {
                ...
                if (method_exists($this, $registered_feed['processedBy'])) {
                    $feed_count = call_user_func([$this,$registered_feed['processedBy']], 'count');
                }
    ...
    protected function do_create_delivery_location($pageNumber = 0)
    {
        ...
        if ($ext = \common\helpers\Acl::checkExtensionAllowed('DeliveryLocation', 'allowed')) {
            $per_page = 100000;
            if ((int)$this->config['GSM_DELIVERY_LOCATION_PAGE_COUNT'] > 0) {
                $per_page = (int)$this->config['GSM_DELIVERY_LOCATION_PAGE_COUNT'];
            }
    ...
    {
        if (empty($imageFile) || !is_file(Yii::getAlias('@webroot') . '/images/' . $imageFile)) {
            return false;
        }
    ...
    protected function do_create_categories($pageNumber = 0)
    {
        ...
        foreach ($lng->paltform_languages as $allowed_lang_code) {
            if (!isset($lng->catalog_languages[$allowed_lang_code])) {
                continue;
            }
        ...
        while ($categories = tep_db_fetch_array($categories_r)) {
            ...
            if (count($this->urlParams) == 0) {
                ...
                foreach ($this->urlParams as $urlParam) {
                    if (!empty($urlParam['language']) && strpos(",{$categories['no_follow_allowed_languages']},", ",{$platform_enabled_languages[$urlParam['language']]},") === false) {
                        continue;
                    }
    ...
    protected function do_create_products($pageNumber = 0)
    {
        ...
        $shop_products_sql = (
            ...
            'AND p.products_status!=0  ' . \common\helpers\Product::get_sql_product_restrictions(['p', 'pd', 's', 'sp', 'pp']) . ' '.
        ...
        $on_page = 100000;
        if ((int)$this->config['GSM_PROD_PAGE_COUNT'] > 0) {
            $on_page = (int)$this->config['GSM_PROD_PAGE_COUNT'];
        }
                ...
                */
                ...
                if (count($this->urlParams) == 0) {
                    ...
                    foreach (Images::getImageList($shop_products['products_id']) as $_image) {
                        if (!isset($_image['image']) || !isset($_image['image']['Large'])) {
                            continue;
                        }
                        $uri = $_image['image']['Large']['url'];
                        if (strpos($uri, DIR_WS_THEME_IMAGES) !== false) {
                            continue;
                        }
                        $images_set[$uri] = [
                    ...
                    foreach ($this->urlParams as $urlParam) {
                        if (!empty($urlParam['language']) && strpos(",{$shop_products['no_follow_allowed_languages']},", ",{$platform_enabled_languages[$urlParam['language']]},") === false) {
                            continue;
                        }
    ...
    protected function do_create_manufacturers($pageNumber = 0)
    {
      ...
      'WHERE m.manufacturers_id=p.manufacturers_id AND p.products_status!=0  ' . \common\helpers\Product::get_sql_product_restrictions(['p', 'pd', 's', 'sp', 'pp']) . ''.
      (
          (USE_MARKET_PRICES == 'True' || \common\helpers\Extensions::isCustomerGroupsAllowed()) ?
        'AND if(pp.products_group_price is null, 1, pp.products_group_price != -1) ' :
        ''
    ...
    protected function do_create_pages($pageNumber = 0)
    {
        ...
        while ($row = tep_db_fetch_array($sql_query)) {
            ...
            if (count($this->urlParams) == 0) {
                ...
                foreach ($this->urlParams as $urlParam) {
                    if (!empty($urlParam['language']) && strpos(",{$row['no_follow_allowed_languages']},", ",{$platform_enabled_languages[$urlParam['language']]},") === false) {
                        continue;
                    }
    ...
    protected function do_create_images($pageNumber = 0)
    {
      ...
      (
        ...
        do {
            ...
            $flush_count = 100;
            while ($shop_products = tep_db_fetch_array($shop_products_r)) {
                if (isset($used_ids[$shop_products['products_id']])) {
                    continue;
                }
    ...
    protected static function _gsm_urlset_start($urlset_attributes = '')
    {
    ...
    protected static function _gsm_urlset($url)
    {
        echo utf8_encode(
            "\t<url>\n".
      "\t\t<loc>{$url['loc']}</loc>\n".
    ...
    public static function _gsm_image_urlset($url)
    {
        $media = '';
    ...
    protected static function _gsm_urlset_stop()
    {
        echo '</urlset>';
    }

    protected static function __xml_site_map_encode_image_path($image_path, $type = 'none')
    {
        if ($type !== 'self') {
            return $image_path;
        }
    ...
    protected static function _get_max_ratio_param_in_categories($ratio_param = 'viewed', $categories_ids = false)
    {
        $languages_id = \Yii::$app->settings->get('languages_id');
    ...
    protected static function _get_categories_map_tree()
    {
        $languages_id = \Yii::$app->settings->get('languages_id');
        ...
        if ($categoriesMap === false) {
            ...
            if (isset($categoriesMap[0])) {
                $categoriesMap[0]['status'] = 1;
            }
        }
