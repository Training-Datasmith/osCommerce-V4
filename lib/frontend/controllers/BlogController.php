...
class BlogController extends Sceleton
{
    public function actionIndex()
    {
        ...
        if (isset($routeParams['currency'])) {
            unset($routeParams['currency']);
        }
        ...
        $alternateLanguages = array_filter(array_keys($this->getView()->linkTags), function ($key) {
            return strpos($key, 'alternate_lang_') === 0;
        });
        ...
        // {{ use WP meta tags
        $registeredMetaKeys = array_filter(array_keys($this->getView()->metaTags), function ($key) {
