...
class CatalogController extends Sceleton
{
    ...
    public function actionProduct()
    {
        ...
        */
        ...
        switch ($product['jsonld_product_type']) {
            case 1: $jsonldProductType = 'Product';
                break;
            case 2: $jsonldProductType = 'Service';
                break;
        ...
        return $this->render('product.tpl', [
          'action' => tep_href_link('catalog/product', \common\helpers\Output::get_all_get_params(['action']) . 'action=add_product'),
    ...
    public function actionProductAttributes($boxId = 0)
    {
        ...
        $attributes = tep_db_prepare_input(Yii::$app->request->get('id', []));
        ...
        if ($type == 'listing' || $type == 'productListing') {
            $listid = tep_db_prepare_input(Yii::$app->request->get('listid', []));
    ...
    public function actionAdvancedSearch()
    {
        ...
        $controls = [
          ...
          'categories' => tep_draw_pull_down_menu('categories_id', \common\helpers\Categories::get_categories([['id' => '', 'text' => TEXT_ALL_CATEGORIES]])),
        ...
        if (count($site_manufacturers) > 0) {
            $site_manufacturers = array_merge([['id' => '', 'text' => TEXT_ALL_MANUFACTURERS]], $site_manufacturers);
        ...
        if (PRODUCTS_PROPERTIES == 'True') {
            ...
            $properties_yes_no_array = [['id' => '', 'text' => OPTION_NONE], ['id' => 'true', 'text' => OPTION_TRUE], ['id' => 'false', 'text' => OPTION_FALSE]];
    ...
    public function actionGiftCard()
    {
        ...
        if (isset($_GET['action']) && ($_GET['action'] == 'add_gift_card')) {
            ...
            if ($error == false) {
                ...
                $sql_data_array = ['customers_id' => Yii::$app->user->getId(),
    ...
    public function actionGetPrice()
    {
        ...
        if ($ext = \common\helpers\Acl::checkExtensionAllowed('PackUnits', 'allowed')) {
            $listid = tep_db_prepare_input(Yii::$app->request->post('listid', []));
    ...
    public function actionFreeSamples()
    {
        ...
        $sorting[] = ['id' => 'da', 'title' => TEXT_BY_DATE . ' &darr;'];
        $sorting[] = ['id' => 'dd', 'title' => TEXT_BY_DATE . ' &uarr;'];
        ...
        $params = [
          'listing_split' => SplitPageResults::make(ListingSql::query(['filename' => 'catalog/free-samples', 'only_samples' => true]), (isset($_SESSION['max_items']) ? $_SESSION['max_items'] : $search_results), 'p.products_id')->withSeoRelLink(),
