...
use yii\base\Widget;
class Send_Form extends Widget
{
    public $id;
    public $params;
    public $settings;
    ...
    public function run()
    {
        ...
        while ($email_templates = tep_db_fetch_array($groups_query)) {
            $templates_list[] = $email_templates['email_templates_key'];
        }
        return $this->render('send-form.tpl', ['id' => $this->id, 'params' => $this->params, 'settings' => $this->settings, 'languages' => $lang, 'languages_id' => $languages_id, 'visibility' => $this->visibility, 'templatesList' => $templates_list, 'text' => htmlspecialchars($text), 'success' => $success]);
    }
}