...
class ProductsGroupsController extends Sceleton
{
    ...
    {
        $this->selectedMenu = ['catalog', 'products-groups'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('products-groups/index'), 'title' => HEADING_TITLE];
        $this->view->headingTitle = HEADING_TITLE;
        $this->topButtons[] = '<a href="#" class="btn btn-primary" onclick="return productsGroupEdit(0)">' . IMAGE_NEW_PRODUCTS_GROUP . '</a>';

        $this->view->productsGroupTable = [
            [
        ...
        return $this->render('index', ['messages' => $messages, 'eID' => $eID]);
    }
    ...
    public function actionSave()
    {
        ...
        */
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            ...
            if ($products_groups_id == 0) {
                $insert_sql_data = ['products_groups_id' => $insert_id,
                ...
                if (!$check['products_groups_exists']) {
                    $insert_sql_data = ['products_groups_id' => $products_groups_id,
                                             'language_id' => $language_id,
                                             'date_added' => 'now()'];
                    $sql_data_array = array_merge($sql_data_array, $insert_sql_data);
    ...
    public function actionConfirmdelete()
    {
        ...
        if ($products_groups_id > 0) {
            ...
            echo tep_draw_form('products_groups', FILENAME_PRODUCTS_GROUPS, \common\helpers\Output::get_all_get_params(['eID', 'action']) . 'dID=' . $eInfo->products_groups_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return productsGroupDelete();"');
    ...
    public function actionProducts()
    {
        ...
        $this->navigation[]       = ['link' => Yii::$app->urlManager->createUrl('products-groups/products'), 'title' => sprintf(HEADING_TITLE_EDIT_PRODUCTS, $eInfo->products_groups_name)];
