...
class PropertiesController extends Sceleton
{
    ...
    {
        $this->navigation[]       = [ 'link' => Yii::$app->urlManager->createUrl('properties/index'), 'title' => TEXT_PROPERTIES_TITLE ];
        $this->view->headingTitle = TEXT_PROPERTIES_TITLE;

        $this->selectedMenu = [ 'catalog', 'properties' ];
        ...
        foreach ($this->getPossiblePropertiesFlags() as $propConfig) {
            if (!isset($propConfig['show_on_listing']) || !$propConfig['show_on_listing']) {
                continue;
            }
            $this->view->PropertyTable[] =  [
        ...
        return $this->render('index', ['messages' => $messages, 'pID' => $pID, 'parID' => $parID]);
    }
    ...
    public function actionList()
    {
        ...
        if ($filter['parID'] > 0) {
            ...
            if ($parent = tep_db_fetch_array($parent_query)) {
                ...
                foreach ($listingFlags as $listingFlag) {
                    $row[] = '';
                }
                $row[] = '';
                $responseList[] = $row;
    ...
    public function actionEdit()
    {
        ...
        $this->navigation[]       = [ 'link' => Yii::$app->urlManager->createUrl('properties/edit'), 'title' => TEXT_PROPERTIES_TITLE ];
        $this->view->headingTitle = TEXT_PROPERTIES_TITLE;
    ...
    public function actionCategory()
    {
        ...
        $this->navigation[]       = [ 'link' => Yii::$app->urlManager->createUrl('properties/category'), 'title' => TEXT_PROPERTIES_TITLE ];
        $this->view->headingTitle = TEXT_PROPERTIES_TITLE;
    ...
    public function actionSave()
    {
        ...
        $sql_data_array = ['properties_type' => $properties_type,
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            ...
            $sql_data_array = ['properties_name'        => $properties_name[$languages[$i]['id']],
            ...
            if ($check['properties_description_exists']) {
                ...
                $insert_sql_data = ['properties_id' => $properties_id,
        ...
        if (in_array($properties_type, ['text', 'number', 'interval', 'file'])) {
            foreach ($values as $val_id => $val) {
                ...
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    ...
                    $sql_data_array = ['values_text'          => $values[$val_id][$languages[$i]['id']],
                    ...
                    if ($check['properties_values_exists']) {
                        ...
                        $insert_sql_data = ['values_id' => $values_id,
                                                 'properties_id' => $properties_id,
                                                 'language_id' => $languages[$i]['id']];
                        $sql_data_array = array_merge($sql_data_array, $insert_sql_data);
    ...
    public function actionConfirmdelete()
    {
        ...
        if ($properties_id > 0) {
            ...
            echo tep_draw_form('properties', FILENAME_PROPERTIES, \common\helpers\Output::get_all_get_params(['pID', 'action']) . 'dID=' . $pInfo->properties_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return propertyDelete();"');
