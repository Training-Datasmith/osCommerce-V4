...
use Yii;
class Products_Groups_Controller extends Sceleton
{
    ...
    {
        $this->selected_menu = ['catalog', 'products-groups'];
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('products-groups/index'), 'title' => HEADING_TITLE];
        $this->view->heading_title = HEADING_TITLE;
        $this->top_buttons[] = '<a href="#" class="btn btn-primary" onclick="return productsGroupEdit(0)">' . IMAGE_NEW_PRODUCTS_GROUP . '</a>';
        $this->view->products_group_table = [['title' => TABLE_HEADING_PRODUCTS_GROUPS_NAME, 'not_important' => 0], ['title' => TABLE_HEADING_PRODUCTS_COUNT, 'not_important' => 0]];
        $messages = [];
        ...
        $e_id = Yii::$app->request->get('eID', 0);
        return $this->render('index', ['messages' => $messages, 'eID' => $e_id]);
    }
    public function action_list()
    {
        ...
        if (isset($_GET['order'][0]['column']) && $_GET['order'][0]['dir']) {
            switch ($_GET['order'][0]['column']) {
                case 0:
                    $order_by = 'pg.products_groups_name ' . tep_db_input(tep_db_prepare_input($_GET['order'][0]['dir']));
                    break;
                case 1:
                    $order_by = 'products_count ' . tep_db_input(tep_db_prepare_input($_GET['order'][0]['dir']));
                    break;
                default:
                    $order_by = 'pg.products_groups_name';
                    break;
            }
        } else {
            $order_by = 'pg.products_groups_name';
        }
        $products_groups_query_raw = 'select pg.products_groups_id, pg.products_groups_name, count(p.products_id) as products_count from ' . TABLE_PRODUCTS_GROUPS . ' pg left join ' . TABLE_PRODUCTS . " p on pg.products_groups_id = p.products_groups_id where pg.language_id = '" . (int) $languages_id . "' " . $search . ' group by pg.products_groups_id order by ' . $order_by;
        $products_groups_split = new \Split_Page_Results($current_page_number, $length, $products_groups_query_raw, $products_groups_query_numrows, 'pg.products_groups_id');
        ...
        while ($products_groups = tep_db_fetch_array($products_groups_query)) {
            ...
            $response_list[] = [
                /* (tep_not_null($image) && $image != TEXT_IMAGE_NONEXISTENT ? '<span class="prodImgC">' . $image . '</span>' : '<span class="cubic"></span>') . */
                '<table class="wrapper"><tr><td><span class="prodNameC">' . $products_groups['products_groups_name'] . '</span></td></tr></table>' . tep_draw_hidden_field('id', $products_groups['products_groups_id'], 'class="cell_identify"') . '<input class="cell_type" type="hidden" value="products_group">',
                $products_groups['products_count'],
            ];
        }
        $response = ['draw' => $draw, 'recordsTotal' => $products_groups_query_numrows, 'recordsFiltered' => $products_groups_query_numrows, 'data' => $response_list];
    ...
    public function action_statusactions()
    {
        ...
        if ($products_groups_id) {
            ...
            $e_info = new \Object_Info($products_groups, false);
            $heading = [];
            $contents = [];
            if (is_object($e_info)) {
                echo '<div class="or_box_head">' . $e_info->products_groups_name . '</div>';
                $products_group_inputs_string = '';
                $languages = \common\helpers\Language::get_languages();
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $products_group_inputs_string .= '<div class="col_desc">' . $languages[$i]['image'] . '&nbsp;' . \common\helpers\Product::products_groups_name($e_info->products_groups_id, $languages[$i]['id']) . '</div>';
                }
                echo $products_group_inputs_string;
                echo '<div class="btn-toolbar btn-toolbar-order">';
                echo '<a class="btn btn-primary btn-process-order btn-edit" href="' . Yii::$app->url_manager->create_url(['products-groups/products', 'products_groups_id' => $products_groups_id]) . '">' . FIELDSET_ASSIGNED_PRODUCTS . '</a>';
    ...
    public function action_edit()
    {
        ...
        $products_groups = tep_db_fetch_array(tep_db_query('select products_groups_id, products_groups_name from ' . TABLE_PRODUCTS_GROUPS . " where products_groups_id = '" . (int) $products_groups_id . "' and language_id = '" . (int) $languages_id . "'"));
        $e_info = new \Object_Info($products_groups, false);
        echo tep_draw_form('products_group', FILENAME_PRODUCTS_GROUPS . '/save', 'products_groups_id=' . ($e_info->products_groups_id ?? null), 'post', 'onsubmit="return productsGroupSave(' . ($e_info->products_groups_id ?? 0) . ');"');
        if ($products_groups_id) {
        ...
        $languages = \common\helpers\Language::get_languages();
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            $products_group_inputs_string .= '<div class="langInput">' . $languages[$i]['image'] . tep_draw_input_field('products_groups_name[' . $languages[$i]['id'] . ']', \common\helpers\Product::products_groups_name($e_info->products_groups_id ?? null, $languages[$i]['id'])) . '</div>';
        }
        ...
        */
        echo '<div class="btn-toolbar btn-toolbar-order">';
        if ($products_groups_id) {
            echo '<input type="submit" value="' . IMAGE_UPDATE . '" class="btn btn-no-margin"><input type="button" value="' . IMAGE_CANCEL . '" class="btn btn-cancel" onclick="resetStatement(' . (int) ($e_info->products_groups_id ?? null) . ')">';
        } else {
            echo '<input type="submit" value="' . IMAGE_NEW . '" class="btn btn-no-margin"><input type="button" value="' . IMAGE_CANCEL . '" class="btn btn-cancel" onclick="resetStatement(' . (int) ($e_info->products_groups_id ?? null) . ')">';
        }
        echo '</div>';
        echo '</form>';
    }
    public function action_save()
    {
        ...
        $products_groups_name = tep_db_prepare_input(Yii::$app->request->post('products_groups_name', []));
        ...
        */
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            ...
            $sql_data_array = ['products_groups_name' => $products_groups_name[$language_id]];
            if ($products_groups_id == 0) {
                $insert_sql_data = ['products_groups_id' => $insert_id, 'language_id' => $language_id, 'date_added' => 'now()'];
                $sql_data_array = array_merge($sql_data_array, $insert_sql_data);
                tep_db_perform(TABLE_PRODUCTS_GROUPS, $sql_data_array);
                $action = 'Added';
            } else {
                $update_sql_data = ['date_last_modified' => 'now()'];
                ...
                if (!$check['products_groups_exists']) {
                    $insert_sql_data = ['products_groups_id' => $products_groups_id, 'language_id' => $language_id, 'date_added' => 'now()'];
                    $sql_data_array = array_merge($sql_data_array, $insert_sql_data);
                    tep_db_perform(TABLE_PRODUCTS_GROUPS, $sql_data_array);
                } else {
        ...
        echo json_encode(['message' => 'Products Group ' . $action, 'messageType' => 'alert-success']);
    }
    public function action_confirmdelete()
    {
        ...
        if ($products_groups_id > 0) {
            ...
            $e_info = new \Object_Info($products_groups, false);
            echo tep_draw_form('products_groups', FILENAME_PRODUCTS_GROUPS, \common\helpers\Output::get_all_get_params(['eID', 'action']) . 'dID=' . $e_info->products_groups_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return productsGroupDelete();"');
            echo '<div class="or_box_head">' . $e_info->products_groups_name . '</div>';
    ...
    public function action_products()
    {
        ...
        $products_groups = tep_db_fetch_array(tep_db_query('select products_groups_id, products_groups_name from ' . TABLE_PRODUCTS_GROUPS . " where products_groups_id = '" . (int) $products_groups_id . "' and language_id = '" . (int) $languages_id . "'"));
        $e_info = new \Object_Info($products_groups, false);
        $this->top_buttons[] = '<span class="btn btn-confirm" onclick="$(\'#products_groups_products\').trigger(\'submit\')">' . IMAGE_SAVE . '</span>';
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('products-groups/products'), 'title' => sprintf(HEADING_TITLE_EDIT_PRODUCTS, $e_info->products_groups_name)];
        $this->view->heading_title = sprintf(HEADING_TITLE_EDIT_PRODUCTS, $e_info->products_groups_name);
        ...
        $group_products = [];
        $query = tep_db_query('select p.products_id, p.products_model, p.products_status, pd.products_name from ' . TABLE_PRODUCTS . ' p, ' . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = '" . (int) $languages_id . "' and pd.platform_id = '" . intval(\common\classes\platform::default_id()) . "' and p.products_groups_id = '" . (int) $e_info->products_groups_id . "' order by p.products_groups_sort, pd.products_name");
        while ($data = tep_db_fetch_array($query)) {
            $group_products[] = ['products_id' => $data['products_id'], 'id' => $data['products_id'], 'products_name' => $data['products_name'], 'name' => $data['products_name'], 'products_model' => $data['products_model'], 'image' => \common\classes\Images::get_image($data['products_id'], 'Small'), 'price' => $currencies->format(\common\helpers\Product::get_products_price($data['products_id'])), 'status_class' => $data['products_status'] == 0 ? 'dis_prod' : ''];
        }
        $this->view->group_products = $group_products;
        return $this->render('products.tpl', ['eInfo' => $e_info]);
    }
    public function action_products_update()
    {
        $products_groups_id = Yii::$app->request->post('products_groups_id');
        $products_group_products_id = Yii::$app->request->post('products_group_products_id', []);
        $all_products_array = [];
        ...
        if (Yii::$app->request->is_ajax) {
            ...
            return $this->redirect(Yii::$app->url_manager->create_url(['products-groups/index', 'eID' => $products_groups_id]));
        }
    }
    ...
    public function action_new_product()
    {
        ...
        if (count($data) > 0) {
            $group_product = ['products_id' => $data['products_id'], 'products_name' => $data['products_name'], 'products_model' => $data['products_model'], 'image' => \common\classes\Images::get_image($data['products_id'], 'Small'), 'price' => $currencies->format(\common\helpers\Product::get_products_price($data['products_id'])), 'status_class' => $data['products_status'] == 0 ? 'dis_prod' : ''];
            return $this->render('new-product.tpl', ['group_product' => $group_product]);
        }
    }
}