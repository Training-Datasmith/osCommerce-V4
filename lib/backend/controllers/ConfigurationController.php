...
class ConfigurationController extends Sceleton
{
    ...
    public function actionIndex()
    {
        ...
        if ($sms && $groupid == $sms::getConfigurationGroupID()) {
            $this->selectedMenu = ['settings', 'sms_messages', 'configuration?groupid='.$groupid];
        } else {
            $this->selectedMenu = ['settings', 'configuration', "configuration/index?groupid=$groupid"];
        }
        ...
        $this->navigation[]       = [ 'link' => Yii::$app->urlManager->createUrl('configuration/index'), 'title' => BOX_HEADING_CONFIGURATION . ' :: ' . $this->group_title ];
    ...
    public function actionSaveparam()
    {
        ...
        global $access_levels_id;
        if ($access_levels_id != 1
            and in_array($configuration_key, ['ADMIN_TWO_STEP_AUTH_ENABLED', 'ADMIN_TWO_STEP_AUTH_SERVICE', 'ADMIN_MULTI_SESSION_ENABLED'])
        ) {
