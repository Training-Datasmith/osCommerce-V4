...
class StockDeliveryTermsController extends Sceleton
{
    ...
    public function actionIndex()
    {
        ...
        $this->selectedMenu = ['settings', 'product_stock_indication', FILENAME_STOCK_INDICATION_DELIVERY_TERMS];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl(FILENAME_STOCK_INDICATION_DELIVERY_TERMS.'/index'), 'title' => HEADING_TITLE_DELIVERY_TERMS];
    ...
    public function actionEdit()
    {
        ...
        $predefined_codes = [
            ...
            ['id' => 'transit','text' => TEXT_INVENTORY_TRANSIT],
            ['id' => 'pre-order','text' => TEXT_PRE_ORDER],
        ];
        echo '<div class="check_linear"><label><span>' . TEXT_SHOW_STOCK_CODE . '</span> ' . tep_draw_pull_down_menu('stock_code', array_merge($predefined_codes, (in_array($oInfo->stock_code, ['out-stock', 'in-stock', 'transit', 'pre-order']) ? [] : [['id' => $oInfo->stock_code, 'text' => $oInfo->stock_code]])), $oInfo->stock_code) . '</label></div>';
        echo '<div class="check_linear"><label><span>' . TEXT_SHOW_STOCK_CODE_NEW . '</span> ' . tep_draw_input_field('stock_code_new', '') . '</label></div>';

        echo '<div class="check_linear"><label><span>' . TEXT_SHOW_TEXT_STOCK_CODE . '</span> ' . tep_draw_pull_down_menu('text_stock_code', array_merge($predefined_codes, (in_array($oInfo->text_stock_code, ['out-stock', 'in-stock', 'transit', 'pre-order']) ? [] : [['id' => $oInfo->text_stock_code, 'text' => $oInfo->text_stock_code]])), $oInfo->text_stock_code) . '</label></div>';
    ...
    public function actionSave()
    {
        ...
        if ($stock_delivery_terms_id == 0) {
            ...
            tep_db_perform(TABLE_PRODUCTS_STOCK_DELIVERY_TERMS, [
