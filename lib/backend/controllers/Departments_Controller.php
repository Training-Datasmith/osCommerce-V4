 ...
 */
class Departments_Controller extends Sceleton
{
    public $acl = ['BOX_HEADING_DEPARTMENTS', 'BOX_HEADING_DEPARTMENTS'];
    public function action_index()
    {
        $this->selected_menu = ['departments', 'departments'];
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('departments/index'), 'title' => BOX_HEADING_DEPARTMENTS];
        $this->top_buttons[] = '<a href="' . Yii::$app->url_manager->create_url('departments/new') . '" class="create_item"><i class="icon-file-text"></i>' . TEXT_CREATE_NEW_DEPARTMENT . '</a>';
        $this->view->heading_title = BOX_HEADING_DEPARTMENTS;
        $this->view->departments_table = [['title' => DEPARTMENT_DATE_ADDED, 'not_important' => 0], ['title' => TABLE_HEADING_STORE_NAME, 'not_important' => 0], ['title' => TABLE_HEADING_HTTP_SERVER, 'not_important' => 0], ['title' => TABLE_HEADING_FULLNAME, 'not_important' => 0], ['title' => TABLE_HEADING_EMAIL, 'not_important' => 0], ['title' => TABLE_HEADING_STATUS, 'not_important' => 0], ['title' => DEPARTMENT_KEEP_ALIVE, 'not_important' => 0]];
        $message_stack = \Yii::$container->get('message_stack');
        if (isset($message_stack->size) && $message_stack->size > 0) {
        ...
        $d_id = Yii::$app->request->get('dID', 0);
        return $this->render('index', ['dID' => $d_id]);
    }
    public function action_list()
    {
        ...
        if (isset($_GET['search']['value']) && tep_not_null($_GET['search']['value'])) {
            ...
            $search .= " and ( departments_store_name like '%" . tep_db_input($keywords) . "%' " . " or departments_http_server like '%" . tep_db_input($keywords) . "%' " . " or departments_firstname like '%" . tep_db_input($keywords) . "%' " . " or departments_lastname like '%" . tep_db_input($keywords) . "%' " . " or departments_email_address like '%" . tep_db_input($keywords) . "%' )";
        }
        ...
        if (isset($_GET['order'][0]['column']) && $_GET['order'][0]['dir']) {
            switch ($_GET['order'][0]['column']) {
                case 0:
                    $order_by = 'departments_created ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                case 1:
                    $order_by = 'departments_store_name ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                case 2:
                    $order_by = 'departments_http_server ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                case 3:
                    $order_by = 'departments_firstname ' . tep_db_prepare_input($_GET['order'][0]['dir']) . ', departments_lastname ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                case 4:
                    $order_by = 'departments_email_address ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                case 5:
                    $order_by = 'departments_status ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                default:
                    $order_by = 'departments_http_server';
                    break;
            }
        } else {
            $order_by = 'locked asc, departments_status desc, departments_sort_order, departments_id';
        }
        $departments_query_raw = 'select * from ' . TABLE_DEPARTMENTS . ' where 1 ' . $search . ' order by ' . $order_by;
        $departments_split = new \Split_Page_Results($current_page_number, $length, $departments_query_raw, $departments_query_numrows);
        ...
        $response = ['draw' => $draw, 'recordsTotal' => $departments_query_numrows, 'recordsFiltered' => $departments_query_numrows, 'data' => $response_list];
    ...
    public function action_view()
    {
        ...
        $d_info = new \Object_Info($departments, false);
        if ($d_info->departments_id > 0) {
            $multiplatform = '';
            if (count(\common\classes\department::get_catalog_assign_list()) > 1) {
                $multiplatform .= '<a href="' . Yii::$app->url_manager->create_url(['departments/edit-catalog', 'id' => $departments_id]) . '" class="btn btn-edit btn-process-order js-open-tree-popup">' . BUTTON_ASSIGN_CATEGORIES_PRODUCTS . '</a>';
            }
            echo '<div class="or_box_head">' . $d_info->departments_store_name . '</div>';
            echo '<div class="btn-toolbar btn-toolbar-order">';
            if ($departments['locked'] == 0) {
                echo '<a href="' . Yii::$app->url_manager->create_url(['departments/edit', 'dID' => $departments_id]) . '"><button class="btn btn-edit btn-primary btn-process-order ">' . IMAGE_EDIT . '</button></a>';
                echo $multiplatform;
            }
            ...
            if (\common\helpers\Acl::check_extension_allowed('SoapServer', 'allowed')) {
                if (\common\helpers\Acl::rule(['BOX_HEADING_FRONENDS', 'BOX_SOAP_SERVER_SETTINGS'])) {
                    echo '<a href="' . \yii\helpers\Url::to_route(['departments/soap-server-configure', 'id' => $d_info->departments_id]) . '"><button class="btn btn-edit btn-primary btn-process-order ">Configure department SOAP server</button></a>';
                }
            }
            if (\common\helpers\Acl::check_extension_allowed('RestServer', 'allowed')) {
                if (\common\helpers\Acl::rule(['BOX_HEADING_FRONENDS', 'BOX_REST_SERVER_SETTINGS'])) {
                    echo '<a href="' . \yii\helpers\Url::to_route(['departments/rest-server-configure', 'id' => $d_info->departments_id]) . '"><button class="btn btn-edit btn-primary btn-process-order ">Configure department rest server</button></a>';
                }
            }
            if ($d_info->departments_status) {
                $check_admin = tep_db_fetch_array(tep_db_query('SELECT admin_email_address FROM ' . TABLE_ADMIN . " WHERE admin_id='" . (int) $login_id . "'"));
                tep_db_close();
                try {
                    if (tep_db_connect($d_info->departments_db_server_host, $d_info->departments_db_server_username, $d_info->departments_db_server_password, $d_info->departments_db_database)) {
                        ...
                        if ($admin) {
                            echo '<div align="center"><a href="http://' . $d_info->departments_http_server . $d_info->departments_http_catalog . DIR_WS_HTTP_ADMIN_CATALOG . FILENAME_LOGIN . '?uid=' . urlencode($admin['admin_id']) . '&tr=' . urlencode($admin['admin_password']) . '" onclick="return confirm(\'' . sprintf(TEXT_YOU_ARE_GOING_TO_LOGIN_AS_DEPARTMENT, $d_info->departments_store_name) . '\')"><input type="button" value="' . IMAGE_LOGIN . '" class="btn btn-primary"></a></div>';
                        }
                        tep_db_close();
                    }
            ...
            echo '<br>' . TEXT_DATE_DEPARTMENTS_CREATED . ' ' . \common\helpers\Date::date_short($d_info->departments_created);
            if ($d_info->departments_modified) {
                echo '<br>' . TEXT_DATE_DEPARTMENTS_LAST_MODIFIED . ' ' . \common\helpers\Date::date_short($d_info->departments_modified);
            }
            if (\common\helpers\Acl::check_extension_allowed('NetSuite')) {
                echo \common\extensions\Net_Suite\helpers\Net_Suite_Helper::department_link($d_info);
            }
            echo '</div>';
        }
    }
    public function action_edit()
    {
        ...
        $d_info = new \Object_Info($departments);
        /*
        ...
        */
        $d_info->packages_array = ['' => TEXT_NONE];
        if (defined('TABLE_PACKAGES') && tep_db_table_exists('TABLE_PACKAGES')) {
            ...
            while ($packages = tep_db_fetch_array($packages_query)) {
                $d_info->packages_array[$packages['packages_id']] = $packages['packages_title'];
            }
        }
        $d_info->api = ['client_statuses' => [], 'local_statuses' => [0 => ''], 'local_statuses_wo_create' => [0 => ''], 'client_create_status_map' => [], 'order_status_mapped' => []];
        $share_departments_list = Array_Helper::map(\common\classes\department::get_list(false), 'id', 'text');
        $share_departments_list_selected = [];
        $_share_map = json_decode(Api::get_department_server_key_value((int) $departments_id, 'catalog/createShare'), true);
        if (!is_array($_share_map)) {
            $_share_map = ['all' => true, 'selected' => []];
            if ((int) $departments_id) {
                $_share_map['selected'][] = (int) $departments_id;
            }
        }
        $d_info->api['createShareAll'] = $_share_map['all'] ? 'all' : 'selected';
        $d_info->api['shareDepartmentsList'] = ['items' => $share_departments_list, 'selected' => $share_departments_list_selected, 'options' => ['class' => 'assign-multi-checkbox', 'multiple' => 'multiple', 'data-role' => 'multiselect', 'options' => []]];
        if ((int) $departments_id) {
            $d_info->api['shareDepartmentsList']['selected'][] = (int) $departments_id;
            $d_info->api['shareDepartmentsList']['options']['options'][(int) $departments_id]['disabled'] = 'disabled';
        }
        $current_group = '';
        foreach (\common\helpers\Order::get_statuses_grouped(true) as $order_status_variant) {
            if (strpos($order_status_variant['id'], 'group_') === 0) {
                $current_group = $order_status_variant['text'];
                $d_info->api['local_statuses'][$current_group]['new_in_' . $order_status_variant['id']] = TEXT_API_SYNCHRONIZATION_ORDER_STATUS_CREATE_CLIENT_STATUS;
                continue;
            }
            $d_info->api['local_statuses'][$current_group][str_replace('status_', '', $order_status_variant['id'])] = str_replace('&nbsp;', '', $order_status_variant['text']);
            $d_info->api['local_statuses_wo_create'][$current_group][str_replace('status_', '', $order_status_variant['id'])] = str_replace('&nbsp;', '', $order_status_variant['text']);
        }
        if ((int) $departments_id) {
            $d_info->api['department-service-url'] = tep_catalog_href_link('api-department/service', '', 'SSL');
            $_formula = json_decode($d_info->api_outgoing_price_formula, true);
            if (is_array($_formula) && !empty($_formula['text'])) {
                $d_info->api_outgoing_price_formula_text = $_formula['text'];
            }
            $client_status_map = json_decode(Api::get_department_server_key_value((int) $departments_id, 'clientOrder/StatusesMapping'), true);
            $d_info->api['client_create_status_map'] = json_decode(Api::get_department_server_key_value((int) $departments_id, 'clientOrder/StatusesNeedCreate'), true);
            $_key_value = Api::get_department_server_key_value($departments_id, 'clientOrder/Statuses');
            if (!empty($_key_value)) {
                $extract_value = json_decode(base64_decode($_key_value), true);
                if (is_array($extract_value)) {
                    ...
                    foreach ($extract_value as $status) {
                        if ($status['group_id'] != $last_group_id) {
                            ...
                            $d_info->api['client_statuses'][] = ['id' => 'group_' . $status['group_id'], '_id' => $status['group_id'], 'group_id' => $status['group_id'], 'text' => $group_name];
                        }
                        ...
                        $d_info->api['client_statuses'][] = ['id' => 'status_' . $status['id'], '_id' => $status['id'], 'group_id' => $status['group_id'], 'text' => $status_name];
                        if (is_array($client_status_map)) {
                            $d_info->api['order_status_mapped'][$status['id']] = isset($client_status_map[$status['id']]) ? $client_status_map[$status['id']] : 0;
                        } else {
                            $d_info->api['order_status_mapped'][$status['id']] = isset($status['external_status_id']) ? $status['external_status_id'] : 0;
                        }
                    }
                }
            }
            $d_info->api_product_flags = Api::product_flags();
            $d_info->api_products_update_custom_flags = false;
            $d_info->api_products_update_custom = [];
            $d_info->api_products_update_custom = json_decode(Api::get_department_server_key_value((int) $departments_id, 'product/UpdateDataFlag'), true);
            foreach ($d_info->api_product_flags as $check_flag) {
                if (isset($check_flag['server']) && isset($d_info->api_products_update_custom[$check_flag['server']]) && $d_info->api_products_update_custom[$check_flag['server']]) {
                    $d_info->api_products_update_custom_flags = true;
                    break;
                }
                if (isset($check_flag['server_own']) && isset($d_info->api_products_update_custom[$check_flag['server_own']]) && $d_info->api_products_update_custom[$check_flag['server_own']]) {
                    $d_info->api_products_update_custom_flags = true;
                    break;
                }
            }
        ...
        $this->selected_menu = ['departments', 'departments'];
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('departments/index'), 'title' => HEADING_TITLE];
        $this->top_buttons[] = '';
        $this->view->heading_title = $HEADING_TITLE;
        $this->view->show_sap = class_exists('\common\classes\SapClient');
        return $this->render('edit', ['dInfo' => $d_info]);
    }
    public function action_update()
    {
        ...
        if (!($departments_id > 0)) {
            if (Yii::$app->request->is_ajax) {
                ...
                $message_stack->add_session('Wrong department ID!', 'error');
                return $this->redirect(Yii::$app->url_manager->create_url('departments/index'));
            }
        }
        ...
        $sql_data_array = array_merge($sql_data_array, ['departments_modified' => 'now()']);
        ...
        if ((int) $departments_id && isset($api['client_status_map']) && is_array($api['client_status_map'])) {
            ...
            Api::set_department_server_key_value((int) $departments_id, 'clientOrder/StatusesMapping', json_encode($client_status_map));
        }
        if ((int) $departments_id && isset($api['create_on_client']) && is_array($api['create_on_client'])) {
            ...
            Api::set_department_server_key_value((int) $departments_id, 'clientOrder/StatusesNeedCreate', json_encode($client_create_status_map));
        }
        if ((int) $departments_id) {
            ...
            if (Yii::$app->request->post('api_products_update_custom_flags', false)) {
                ...
                if (!is_array($api_products_update_custom)) {
                    $api_products_update_custom = [];
                }
                foreach (Api::product_flags() as $flag_info) {
                    if (!empty($flag_info['server'])) {
                        $_key = $flag_info['server'];
                        $api_products_update_custom[$_key] = isset($api_products_update_custom[$_key]);
                    }
                    if (!empty($flag_info['server_own'])) {
                        $_key = $flag_info['server_own'];
                        $api_products_update_custom[$_key] = isset($api_products_update_custom[$_key]);
                    }
                }
            }
            Api::set_department_server_key_value((int) $departments_id, 'product/UpdateDataFlag', json_encode($api_products_update_custom));
            $api_create_share = ['all' => $api['createShareAll'] == 'all', 'selected' => isset($api['createShare']) && is_array($api['createShare']) ? array_map('intval', $api['createShare']) : []];
            if (!in_array((int) $departments_id, $api_create_share['selected'])) {
                $api_create_share['selected'][] = (int) $departments_id;
            }
            Api::set_department_server_key_value((int) $departments_id, 'catalog/createShare', json_encode($api_create_share));
        }
        // }} API
            ...
            try {
                if (!tep_db_connect($departments_db_server_host, $departments_db_server_username, $departments_db_server_password, $departments_db_database)) {
                    ...
                    if (Yii::$app->request->is_ajax) {
                        ...
                        $message_stack->add_session(TEXT_UNABLE_CONNECT_DEPARTMENTS_DB, 'error');
                        return $this->redirect(Yii::$app->url_manager->create_url(['departments/edit', 'dID' => $departments_id]));
                    }
                } else {
        ...
        $message_stack->add_session('The department has been successfully updated!', 'success');
        if (Yii::$app->request->is_ajax) {
            echo 'OK';
            exit;
        } else {
    ...
    public function action_new()
    {
        ...
        $this->top_buttons[] = '';
        $this->view->heading_title = HEADING_TITLE;
        $d_info = new \Object_Info(['project_codes' => []]);
        /*
         $dInfo->templates_array = array();
         $template_query = tep_db_query("select template_id, template_name from " . TABLE_TEMPLATE . " order by template_name");
         while ($template = tep_db_fetch_array($template_query)) {
        ...
        */
        ...
        return $this->render('new', ['dInfo' => $d_info]);
    }
    public function action_create()
    {
        ...
        */
        ...
        $sql_data_array = ['departments_store_name' => $departments_store_name, 'departments_http_server' => $departments_http_server, 'departments_https_server' => $departments_https_server, 'departments_enable_ssl' => $departments_enable_ssl, 'departments_http_catalog' => $departments_http_catalog, 'departments_https_catalog' => $departments_https_catalog, 'departments_already_registered_domain' => $department_install, 'departments_firstname' => $departments_firstname, 'departments_lastname' => $departments_lastname, 'departments_email_address' => $departments_email_address, 'departments_telephone' => $departments_telephone, 'departments_street_address' => $departments_street_address, 'departments_suburb' => $departments_suburb, 'departments_postcode' => $departments_postcode, 'departments_city' => $departments_city, 'departments_state' => $departments_state, 'departments_zone_id' => $departments_zone_id, 'departments_country_id' => $departments_country_id, 'departments_address' => $departments_address, 'departments_ftp_type' => $departments_ftp_type, 'departments_ftp_host' => $departments_ftp_host, 'departments_ftp_port' => $departments_ftp_port, 'departments_ftp_path' => $departments_ftp_path, 'departments_ftp_username' => $departments_ftp_username, 'departments_ftp_password' => $departments_ftp_password, 'departments_ftp_pasv' => $departments_ftp_pasv, 'departments_db_server_host' => $departments_db_server_host, 'departments_db_server_username' => $departments_db_server_username, 'departments_db_server_password' => $departments_db_server_password, 'departments_db_database' => $departments_db_database, 'departments_db_use_pconnect' => $departments_db_use_pconnect, 'departments_db_store_sessions' => $departments_db_store_sessions, 'api_key' => $api_key, 'departments_status' => $departments_status, 'departments_created' => 'now()', 'alias' => implode(';', $domain_alias)];
        tep_db_perform(TABLE_DEPARTMENTS, $sql_data_array);
        $departments_id = tep_db_insert_id();
        if (true) {
            tep_db_perform(TABLE_DEPARTMENTS_CATEGORIES, ['departments_id' => $departments_id, 'categories_id' => 0]);
        } else {
            ...
            while ($platforms_categories = tep_db_fetch_array($platforms_categories_query)) {
                tep_db_perform(TABLE_DEPARTMENTS_CATEGORIES, ['departments_id' => $departments_id, 'categories_id' => $platforms_categories['categories_id']]);
            }
            ...
            while ($platforms_products = tep_db_fetch_array($platforms_products_query)) {
                tep_db_perform(TABLE_DEPARTMENTS_PRODUCTS, ['departments_id' => $departments_id, 'products_id' => $platforms_products['products_id']]);
            }
        }
        $conf = '';
        $conf .= "action=create\n";
        ...
        $data = ['username' => $user, 'password' => $password, 'db_name' => $departments_db_database, 'db_user' => $departments_db_server_username, 'db_password' => $departments_db_server_password, 'departments_http_server' => $departments_http_server, 'departments_enable_ssl' => $departments_enable_ssl == 1 ? 2 : 0, 'platform_owner' => $platform_owner, 'platform_name' => $platform_name, 'departments_telephone' => $departments_telephone, 'departments_store_name' => $departments_store_name, 'departments_firstname' => $departments_firstname, 'departments_lastname' => $departments_lastname, 'departments_email_address' => $departments_email_address, 'departments_address' => $departments_address, 'departments_street_address' => $departments_street_address, 'departments_suburb' => $departments_suburb, 'platform_landline' => $platform_landline, 'platform_companyname' => $platform_companyname, 'platform_companyvat' => $platform_companyvat, 'platform_companyno' => $platform_companyno, 'departments_postcode' => $departments_postcode, 'departments_city' => $departments_city, 'departments_state' => $departments_state, 'departments_zone_id' => $departments_zone_id, 'departments_country_id' => $departments_country_id, 'departments_api_key' => $api_key, 'callback_url' => tep_catalog_href_link('', '', 'SSL'), 'admin_username' => $admin_username, 'admin_password' => $user_password, 'alias' => $domain_alias];
        $xml_data = new \Simple_Xml_Element('<?xml version="1.0" encoding="UTF-8"?><response/>');
        $this->array_to_xml($data, $xml_data);
        $xml_data->save_xml(CPANEL_PARENT_PATH . $departments_http_server_short . '_create.xml');
        $message_stack = \Yii::$container->get('message_stack');
        $message_stack->add_session('A request for registration of the new department was sent!', 'success');
        if (Yii::$app->request->is_ajax) {
            echo $departments_id;
            exit;
        } else {
            return $this->redirect(Yii::$app->url_manager->create_url(['departments/new', 'dID' => $departments_id]));
        }
    }
    ...
    public function action_switch_status()
    {
        ...
        if ($status == 'true') {
            ...
            $conf .= 'domain=' . $d_info->departments_http_server . "\n";
            $conf .= 'username=' . $d_info->departments_ftp_username . "\n";
            fwrite(fopen(CPANEL_PARENT_PATH . $d_info->departments_ftp_username . '_unsuspend.settings', 'w'), $conf);
            @unlink(CPANEL_PARENT_PATH . $d_info->departments_ftp_username . '_suspend.settings');
        } else {
            $conf = '';
            $conf .= "action=suspend\n";
            $conf .= 'domain=' . $d_info->departments_http_server . "\n";
            $conf .= 'username=' . $d_info->departments_ftp_username . "\n";
            fwrite(fopen(CPANEL_PARENT_PATH . $d_info->departments_ftp_username . '_suspend.settings', 'w'), $conf);
            @unlink(CPANEL_PARENT_PATH . $d_info->departments_ftp_username . '_unsuspend.settings');
        }
    ...
    public function action_keep_alive()
    {
        $departments_id = Yii::$app->request->post('dID');
        $status = Yii::$app->request->post('status');
    ...
    public function action_confirmitemdelete()
    {
        ...
        if ($departments_id > 0) {
            ...
            $d_info = new \Object_Info($departments, false);
            echo tep_draw_form('departments', 'departments', \common\helpers\Output::get_all_get_params(['dID', 'action']) . 'dID=' . $c_info->departments_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return deleteItem();"');
            echo '<div class="or_box_head">' . TEXT_INFO_HEADING_DELETE_DEPARTMENT . '</div>';
            echo TEXT_DELETE_INTRO . '<br><br><b>' . $d_info->departments_store_name . '</b>';
            echo '<div class="btn-toolbar btn-toolbar-order">';
    ...
    public function action_itemdelete()
    {
        ...
        $conf .= "action=delete\n";
        $conf .= 'username=' . $d_info->departments_ftp_username . "\n";
        fwrite(fopen(CPANEL_PARENT_PATH . $d_info->departments_ftp_username . '_delete.settings', 'w'), $conf);
        ...
        $message_stack = \Yii::$container->get('message_stack');
        $message_stack->add_session('The department has been successfully deleted!', 'success');
        if (Yii::$app->request->is_ajax) {
            echo 'OK';
            exit;
        } else {
    ...
    public function action_sort_order()
    {
        ...
        $ref_array = isset($_POST['item']) && is_array($_POST['item']) ? array_map('intval', $_POST['item']) : [];
        if ($moved_id && in_array($moved_id, $ref_array)) {
            // {{ normalize
            $order_counter = 0;
            $order_list_r = tep_db_query('SELECT departments_id, departments_sort_order ' . 'FROM ' . TABLE_DEPARTMENTS . ' ' . 'WHERE 1 ' . 'ORDER BY departments_sort_order, departments_store_name');
            ...
            // }} normalize
            $get_current_order_r = tep_db_query('SELECT departments_id, departments_sort_order ' . 'FROM ' . TABLE_DEPARTMENTS . ' ' . "WHERE departments_id IN('" . implode("','", $ref_array) . "') " . 'ORDER BY departments_sort_order');
            $ref_ids = [];
            $ref_so = [];
    ...
    public function action_edit_catalog()
    {
        ...
        return $this->render('edit-catalog.tpl', ['selected_data' => $selected_data, 'tree_data' => $tree_init_data, 'tree_server_url' => Yii::$app->url_manager->create_url(['departments/load-tree', 'department_id' => $department_id]), 'tree_server_save_url' => Yii::$app->url_manager->create_url(['departments/update-catalog-selection', 'department_id' => $department_id])]);
    }
    ...
    public function action_update_catalog_selection()
    {
        ...
        if (is_array($selected_data)) {
            ...
            foreach ($selected_data as $selection) {
                ...
                if (isset($assigned[$selection])) {
                    ...
                    tep_db_perform(TABLE_DEPARTMENTS_CATEGORIES, ['departments_id' => $department_id, 'categories_id' => $cat_id]);
                    unset($assigned[$selection]);
                }
            }
            foreach ($selected_products as $pid) {
                if (isset($assigned_products[$pid])) {
                    ...
                    tep_db_perform(TABLE_DEPARTMENTS_PRODUCTS, ['departments_id' => $department_id, 'products_id' => $pid]);
                }
            }
        }
        foreach ($assigned as $clean_key) {
            if (substr($clean_key, 0, 1) == 'c') {
                ...
                tep_db_query('DELETE FROM ' . TABLE_DEPARTMENTS_CATEGORIES . ' ' . "WHERE categories_id!=0 AND departments_id ='" . $department_id . "' AND categories_id = '" . $cat_id . "' ");
                unset($assigned[$clean_key]);
            }
        }
        if (count($assigned_products) > 1000) {
            foreach ($assigned_products as $assigned_product_id) {
                tep_db_query('DELETE FROM ' . TABLE_DEPARTMENTS_PRODUCTS . ' ' . "WHERE departments_id ='" . $department_id . "' AND products_id = '" . $assigned_product_id . "' ");
            }
        } elseif (count($assigned_products) > 0) {
            tep_db_query('DELETE FROM ' . TABLE_DEPARTMENTS_PRODUCTS . ' ' . "WHERE departments_id ='" . $department_id . "' AND products_id IN ('" . implode("','", $assigned_products) . "') ");
        }
        Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        Yii::$app->response->data = ['status' => 'ok'];
    }
    public function action_create_locally_api_client_status()
    {
        $this->layout = false;
        $departments_id = intval(Yii::$app->request->get('dID', 0));
        $client_status_id = intval(Yii::$app->request->post('clientStatusId', 0));
        $local_status_group_id = intval(Yii::$app->request->post('localStatusGroupId', 0));
        $result = ['status' => 'ok'];
        if ($departments_id && $client_status_id && $local_status_group_id) {
            $_key_value = Api::get_department_server_key_value($departments_id, 'clientOrder/Statuses');
            if (!empty($_key_value)) {
                $extract_value = json_decode(base64_decode($_key_value), true);
                if (is_array($extract_value)) {
                    foreach ($extract_value as $client_status) {
                        if ($client_status['id'] != $client_status_id) {
                            continue;
                        }
                        $default_name = current($client_status['names']);
                        if (isset($client_status['names'][DEFAULT_LANGUAGE])) {
                            $default_name = $client_status['names'][DEFAULT_LANGUAGE];
                            $check_exists_r = tep_db_query('SELECT orders_status_id ' . 'FROM ' . TABLE_ORDERS_STATUS . ' ' . "WHERE orders_status_name='" . tep_db_input($default_name) . "' " . " AND language_id='" . \common\classes\language::get_id(DEFAULT_LANGUAGE) . "' " . " AND orders_status_groups_id='{$local_status_group_id}' " . 'LIMIT 1');
                            ...
                            $check_exists_r = tep_db_query('SELECT orders_status_id ' . 'FROM ' . TABLE_ORDERS_STATUS . ' ' . "WHERE orders_status_name='" . tep_db_input($default_name) . "' " . " AND orders_status_groups_id='{$local_status_group_id}' " . 'LIMIT 1');
                            if (tep_db_num_rows($check_exists_r) > 0) {
                                $check_exists = tep_db_fetch_array($check_exists_r);
                                $result['status_id'] = $check_exists['orders_status_id'];
                                break;
                        ...
                        $get_current_status_id = tep_db_fetch_array(tep_db_query('SELECT MAX(orders_status_id) AS current_max_id FROM ' . TABLE_ORDERS_STATUS . ' '));
                        $new_status_id = intval($get_current_status_id['current_max_id']) + 1;
                        tep_db_query('INSERT INTO ' . TABLE_ORDERS_STATUS . ' (orders_status_id, orders_status_groups_id, language_id, orders_status_name) ' . " SELECT {$new_status_id}, {$local_status_group_id}, languages_id, '" . tep_db_input($default_name) . "' FROM " . TABLE_LANGUAGES . ' ');
                        $def_lang_id = \common\classes\language::get_id(DEFAULT_LANGUAGE);
                        foreach ($client_status['names'] as $lang_code => $lang_name) {
                            $lang_id = \common\classes\language::get_id($lang_code);
                            if ($def_lang_id == $lang_id) {
                                continue;
                            }
                            tep_db_query('UPDATE ' . TABLE_ORDERS_STATUS . ' ' . "SET orders_status_name='" . tep_db_input($default_name) . "' " . "WHERE orders_status_id='{$new_status_id}' AND language_id='" . $lang_id . "'");
                        }
            ...
            if (isset($result['status_id']) && $result['status_id']) {
                ...
                foreach (\common\helpers\Order::get_statuses_grouped(true) as $order_status_variant) {
                    ...
                    $local_statuses[$current_group][str_replace('status_', '', $order_status_variant['id'])] = str_replace('&nbsp;', '', $order_status_variant['text']);
                }
                $result['dropDownList'] = Html::drop_down_list('newLocalStatuses', '', $local_statuses, ['class' => 'form-control js-status-map']);
            }
        }
        Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        Yii::$app->response->data = $result;
    }
    public function action_departmentsdelete()
    {
        $this->layout = false;
