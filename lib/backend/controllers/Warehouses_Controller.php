...
use Yii;
class Warehouses_Controller extends Sceleton
{
    ...
    {
        $this->selected_menu = ['catalog', 'warehouses'];
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('warehouses/index'), 'title' => HEADING_TITLE];
        $this->top_buttons[] = '<a href="' . Yii::$app->url_manager->create_url('warehouses/edit') . '" class="btn btn-primary addprbtn"><i class="icon-tag"></i>' . TEXT_CREATE_NEW_WAREHOUSE . '</a>';
        $this->top_buttons[] = '<a href="' . Yii::$app->url_manager->create_url('warehouses/location-blocks') . '" class="btn btn-primary">' . TEXT_MANAGE_LOCATION_BLOCKS . '</a>';
        $this->view->heading_title = HEADING_TITLE;
        $this->view->groups_table = [['title' => TABLE_HEADING_WAREHOUSE_NAME, 'not_important' => 1], ['title' => TABLE_HEADING_STATUS, 'not_important' => 1]];
        ...
        foreach ($_platforms as $_idx => $_platform) {
            $_platforms[$_idx]['link'] = Yii::$app->url_manager->create_url(['warehouses/index', 'platform_id' => $_platform['id']]);
        }
        return $this->render('index', ['platforms' => $_platforms, 'isMultiPlatforms' => \common\classes\platform::is_multi(false), 'selected_platform_id' => $this->selected_platform_id]);
    }
    public function action_list()
    {
        ...
        if (isset($_GET['order'][0]['column']) && $_GET['order'][0]['dir']) {
            switch ($_GET['order'][0]['column']) {
                case 0:
                    $order_by = 'w.warehouse_name ' . tep_db_input(tep_db_prepare_input($_GET['order'][0]['dir']));
                    break;
                case 1:
                    $order_by = 'ifnull(w2p.sort_order, w.sort_order) ' . tep_db_input(tep_db_prepare_input($_GET['order'][0]['dir'])) . ', w.warehouse_id ';
                    break;
                default:
                    $order_by = 'ifnull(w2p.sort_order, w.sort_order), w.warehouse_name';
                    break;
            }
        } else {
            $order_by = 'ifnull(w2p.sort_order, w.sort_order), w.warehouse_name';
        ...
        $_split = new \Split_Page_Results($current_page_number, $length, $warehouses_query_raw, $query_numrows, 'w.warehouse_id');
        ...
        while ($warehouses = tep_db_fetch_array($warehouses_query)) {
            ...
            $response_list[] = ['<div class="handle_cat_list"><span class="handle"><i class="icon-hand-paper-o"></i></span><div class="cat_name cat_name_attr cat_no_folder">' . $warehouses['warehouse_name'] . '<input class="cell_identify" type="hidden" value="' . $warehouses['warehouse_id'] . '">' . '<input class="cell_type" type="hidden" value="top">' . '</div></div>', $status];
            $query_show++;
        }
        $response = ['draw' => $draw, 'recordsTotal' => $query_numrows, 'recordsFiltered' => $query_show, 'data' => $response_list];
    ...
    public function action_switch_status()
    {
        ...
        if ($this->selected_platform_id == \common\classes\platform::default_id()) {
    ...
    public function action_item_preedit()
    {
        ...
        $m_info = new \Object_Info($warehouses);
        echo '<div class="or_box_head">' . $m_info->warehouse_name . '</div>';
        echo '<div class="row_or"><div>' . TEXT_DATE_ADDED . '</div><div>' . \common\helpers\Date::date_short($m_info->date_added) . '</div></div>';
        if (tep_not_null($m_info->last_modified)) {
            echo '<div class="row_or"><div>' . TEXT_LAST_MODIFIED . '</div><div>' . \common\helpers\Date::date_short($m_info->last_modified) . '</div></div>';
        }
        echo '<div class="btn-toolbar btn-toolbar-order">
            <a href="' . Yii::$app->url_manager->create_url(['warehouses/edit', 'id' => $item_id]) . '" class="btn btn-edit btn-primary btn-process-order ">' . IMAGE_EDIT . '</a>
            ...
            <a href="' . Yii::$app->url_manager->create_url(['warehouses/locations', 'id' => $item_id]) . '" class="btn btn-edit btn-primary btn-process-order ">' . TEXT_WAREHOUSE_LOCATIONS . '</a>';
        if (\common\helpers\Acl::check_extension_allowed('ProductsRelocate')) {
            echo '<a href="' . Yii::$app->url_manager->create_url(['products-relocate/create', 'warehouse_id' => $item_id]) . '" class="btn btn-primary btn-process-order ">' . BOX_PRODUCTS_RELOCATE . '</a>';
        }
        echo '</div>';
    }
    public function action_edit()
    {
        ...
        if ($item_id > 0) {
            $warehouses_query = tep_db_query('select * from ' . TABLE_WAREHOUSES . " where warehouse_id = '" . (int) $item_id . "'");
            $warehouses = tep_db_fetch_array($warehouses_query);
            $p_info = new \Object_Info($warehouses);
        } else {
        ...
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('warehouses/'), 'title' => $text_new_or_edit . ' ' . ($p_info->warehouses_name ?? null)];
        $this->selected_menu = ['catalog', 'warehouses'];
        if (Yii::$app->request->is_post) {
            $this->layout = false;
        }
        ...
        $checkbox_default_warehouse_attr = [];
        if ($p_info->is_default ?? null) {
        ...
        return $this->render('edit.tpl', ['pInfo' => $p_info, 'addresses' => $addresses, 'open_hours' => $open_hours, 'count_open_hours' => count($open_hours), 'days' => $days, 'checkbox_default_warehouse_attr' => $checkbox_default_warehouse_attr, 'have_one_or_more_warehouse' => $have_one_or_more_warehouse, 'platforms' => $_platforms, 'isMultiPlatforms' => \common\classes\platform::is_multi(false)]);
    }
    public function action_submit()
    {
        ...
        $status = (int) Yii::$app->request->post('status', $platform_status[\common\classes\platform::default_id()] ?? null);
        ...
        if ($error === false) {
            $sql_data_array = ['warehouse_owner' => $warehouse_owner, 'warehouse_name' => $warehouse_name, 'warehouse_email_address' => $warehouse_email_address, 'warehouse_telephone' => $warehouse_telephone, 'warehouse_landline' => $warehouse_landline, 'is_store' => $is_store, 'status' => (int) $status, 'shipping_additional_charge' => (float) $shipping_additional_charge];
            if ($is_default !== false) {
                $sql_data_array['is_default'] = $is_default;
            }
            $message = 'Item updated';
            $warehouse_updated = true;
            $warehouse_model = \common\models\Warehouses::find_one($item_id);
            if (!$warehouse_model) {
                $message = 'Item inserted';
                $warehouse_updated = false;
                $warehouse_model = new \common\models\Warehouses(['date_added' => new \yii\db\Expression('NOW()')]);
                $warehouse_model->load_default_values();
            }
            $sql_data_array['last_modified'] = new \yii\db\Expression('NOW()');
            $warehouse_model->set_attributes($sql_data_array, false);
            $status_changed = false;
            if (!$warehouse_model->is_new_record && $warehouse_model->is_attribute_changed('status')) {
                $status_changed = true;
            }
            $warehouse_model->save(false);
            $item_id = $warehouse_model->warehouse_id;
            if ($status_changed) {
                \common\helpers\Warehouses::warehouse_status_change();
            }
            if ($is_default) {
                tep_db_query('UPDATE ' . TABLE_WAREHOUSES . ' SET is_default=0 ' . "WHERE warehouse_id!='" . (int) $item_id . "'");
            }
            $google = new \common\components\Google_Tools();
            ...
            foreach ($address_book_ids as $address_book_key => $address_book_id) {
                ...
                $sql_data_array = ['entry_street_address' => $entry_street_address[$address_book_key], 'entry_postcode' => $entry_postcode[$address_book_key], 'entry_city' => $entry_city[$address_book_key], 'entry_country_id' => $entry_country_id[$address_book_key], 'entry_company_reg_number' => $entry_company_reg_number[$address_book_key], 'is_default' => 1];
                ...
                $location = $google->get_geocoding_location($address);
                ...
                if ((int) $address_book_id > 0) {
                    ...
                    tep_db_perform(TABLE_WAREHOUSES_ADDRESS_BOOK, array_merge($sql_data_array, ['warehouse_id' => $item_id]));
            ...
            foreach ($warehouses_open_hours_ids as $warehouses_open_hours_key => $warehouses_open_hours_id) {
                ...
                $sql_data_array = ['open_days' => implode(',', $open_days), 'open_time_from' => $open_time_from[$warehouses_open_hours_key], 'open_time_to' => $open_time_to[$warehouses_open_hours_key]];
                if ((int) $warehouses_open_hours_id > 0) {
                    ...
                    tep_db_perform(TABLE_WAREHOUSES_OPEN_HOURS, array_merge($sql_data_array, ['warehouse_id' => $item_id]));
        ...
        if (\common\classes\platform::is_multi(false)) {
            $_platforms = \common\classes\platform::get_list(false);
            foreach ($_platforms as $_idx => $_platform) {
            ...
            tep_db_query('UPDATE ' . TABLE_WAREHOUSES_TO_PLATFORMS . " SET status = '" . (int) $status . "' WHERE warehouse_id = '" . (int) $item_id . "' AND platform_id = '" . (int) \common\classes\platform::default_id() . "'");
        }
        if ($es = \common\helpers\Extensions::is_allowed('EventSystem')) {
        ...
        <?php 
        echo '<script>location.replace("' . Yii::$app->url_manager->create_url(['warehouses/edit', 'id' => $item_id]) . '");</script>';
        die;
    ...
    public function action_confirmitemdelete()
    {
        ...
        $warehouses = tep_db_fetch_array($warehouses_query);
        $p_info = new \Object_Info($warehouses);
        echo tep_draw_form('item_delete', 'warehouses', \common\helpers\Output::get_all_get_params(['action']) . 'action=update', 'post', 'id="item_delete" onSubmit="return deleteItem();"');
        ...
        echo '<div class="col_desc">' . $p_info->warehouse_name . '</div>';
        ?>
        <p class="btn-toolbar">
        <?php 
    ...
    public function action_itemdelete()
    {
        ...
        $check_is_default = tep_db_fetch_array(tep_db_query('SELECT COUNT(*) AS c FROM ' . TABLE_WAREHOUSES . " WHERE is_default = 1 AND warehouse_id = '" . (int) $item_id . "'"));
        if ($check_is_default['c']) {
        } else {
            \common\models\Warehouses::delete_all(['warehouse_id' => (int) $item_id]);
            \common\models\Warehouses_Address_Book::delete_all(['warehouse_id' => (int) $item_id]);
            \common\models\Warehouses_Open_Hours::delete_all(['warehouse_id' => (int) $item_id]);
            \common\models\Warehouses_Products::delete_all(['warehouse_id' => (int) $item_id]);
            \common\models\Warehouses_Platforms::delete_all(['warehouse_id' => (int) $item_id]);
        }
        ?>
        <div class="popup-box-wrap pop-mess">
            <div class="around-pop-up"></div>
    ...
    public function action_sort_order()
    {
        ...
        if ($moved_id && in_array($moved_id, $ref_array)) {
            // {{ normalize
            $order_counter = 0;
            $order_list_r = tep_db_query('SELECT w.warehouse_id, ifnull(w2p.status, w.status) as status, ifnull(w2p.sort_order, w.sort_order) as sort_order ' . 'FROM ' . TABLE_WAREHOUSES . ' w ' . 'LEFT JOIN ' . TABLE_WAREHOUSES_TO_PLATFORMS . " w2p ON w.warehouse_id = w2p.warehouse_id AND w2p.platform_id = '" . (int) $this->selected_platform_id . "'" . 'WHERE 1 ' . 'ORDER BY ifnull(w2p.sort_order, w.sort_order), w.warehouse_name');
            ...
            // }} normalize
            $get_current_order_r = tep_db_query('SELECT w.warehouse_id, ifnull(w2p.sort_order, w.sort_order) as sort_order ' . 'FROM ' . TABLE_WAREHOUSES . ' w ' . 'LEFT JOIN ' . TABLE_WAREHOUSES_TO_PLATFORMS . " w2p ON w.warehouse_id = w2p.warehouse_id AND w2p.platform_id = '" . (int) $this->selected_platform_id . "'" . "WHERE w.warehouse_id IN ('" . implode("','", $ref_array) . "') " . 'ORDER BY ifnull(w2p.sort_order, w.sort_order)');
            $ref_ids = [];
            $ref_so = [];
    ...
    private function location_tree($locations, $parrent_id, $blocks_list)
    {
        $tree = [];
        if (is_array($locations)) {
            foreach ($locations as $value) {
                if ($value['parrent_id'] == $parrent_id) {
                    if (isset($blocks_list[$value['block_id']])) {
                        $value['block_name'] = $blocks_list[$value['block_id']];
                    } else {
                        $value['block_name'] = '';
                    }
                    if ($value['is_final'] == 0) {
                        $value['children'] = $this->location_tree($locations, $value['location_id'], $blocks_list);
                    }
                    $tree[] = $value;
                }
            }
    ...
    public function action_locations()
    {
        $item_id = (int) Yii::$app->request->get('id');
        \common\helpers\Translation::init('admin/warehouses');
        $warehouse = \common\models\Warehouses::find_one($item_id);
        if (!is_object($warehouse)) {
            return $this->redirect(Yii::$app->url_manager->create_url('warehouses/'));
        }
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('warehouses/locations'), 'title' => IMAGE_EDIT . ' ' . TEXT_WAREHOUSE_LOCATIONS . ': ' . $warehouse->warehouse_name];
        $this->view->heading_title = IMAGE_EDIT . ' ' . TEXT_WAREHOUSE_LOCATIONS . ': ' . $warehouse->warehouse_name;
        $blocks = \common\models\Location_Blocks::find()->as_array()->all();
        $blocks_list = [];
        foreach ($blocks as $value) {
            $blocks_list[$value['block_id']] = $value['block_name'];
        }
        $locations = \common\models\Locations::find()->where(['warehouse_id' => $item_id])->order_by('sort_order')->as_array()->all();
        $locations_tree = $this->location_tree($locations, 0, $blocks_list);
        return $this->render('locations', ['id' => $item_id, 'blocks' => $blocks, 'locationsTree' => $locations_tree, 'warehouse' => $warehouse]);
    }
    private $used_location_ids = [];
    private function update_location($data, $parrent_id, $warehouse_id, $sort_order = 0)
    {
        if (!is_array($data)) {
            return false;
        }
        ...
        $obj = false;
        if ($id > 0) {
            $obj = \common\models\Locations::find_one($id);
        }
        if (!is_object($obj)) {
        ...
        if (is_array($children) && count($children) > 0) {
            ...
            foreach ($children as $sort => $child) {
                $this->update_location($child, $obj->location_id, $warehouse_id, $sort);
            }
        } else {
            $obj->is_final = 1;
            $obj->save();
        }
        $this->used_location_ids[$obj->location_id] = $obj->location_id;
    }
    public function action_save_location()
    {
        ...
        if (is_array($locations[0])) {
            $this->used_location_ids = [];
            foreach ($locations[0] as $sort => $location) {
                $this->update_location($location, 0, $item_id, $sort);
            }
            \common\models\Locations::delete_all(['AND', ['warehouse_id' => $item_id], ['NOT IN', 'location_id', $this->used_location_ids]]);
        }
        echo json_encode(['status' => 'ok']);
    }
    public function action_location_blocks()
    {
        \common\helpers\Translation::init('admin/warehouses');
        $this->view->filters = new \stdClass();
        $this->view->filters->row = (int) Yii::$app->request->get('row', 0);
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('warehouses/location-blocks'), 'title' => TEXT_MANAGE_LOCATION_BLOCKS];
        $this->view->heading_title = TEXT_MANAGE_LOCATION_BLOCKS;
        $this->top_buttons[] = '<a href="#" class="create_item" onclick="return editItem(0)">' . TEXT_INFO_HEADING_NEW_LOCATION_BLOCK . '</a>';
        $this->view->groups_table = [['title' => TABLE_TEXT_NAME, 'not_important' => 1]];
        return $this->render('location-blocks');
    }
    public function action_location_blocks_list()
    {
        $this->layout = false;
        ...
        $query_raw = \common\models\Location_Blocks::find();
        if (isset($_GET['search']['value']) && tep_not_null($_GET['search']['value'])) {
            $keywords = tep_db_input(tep_db_prepare_input($_GET['search']['value']));
        ...
        $blocks = $query_raw->as_array()->all();
        $response_list = [];
        if (is_array($blocks)) {
            foreach ($blocks as $block) {
                $response_list[] = [$block['block_name'] . '<input class="cell_identify" type="hidden" value="' . $block['block_id'] . '">' . '<input class="cell_type" type="hidden" value="top">'];
            }
        }
        $response = ['draw' => $draw, 'recordsTotal' => $query_numrows, 'recordsFiltered' => $query_numrows, 'data' => $response_list];
        echo json_encode($response);
    }
    public function action_location_blocks_preview()
    {
        \common\helpers\Translation::init('admin/warehouses');
        $this->layout = false;
        $item_id = (int) Yii::$app->request->post('item_id');
        $block = \common\models\Location_Blocks::find_one($item_id);
        if (is_object($block)) {
            $loations_qty = \common\models\Locations::find()->where(['block_id' => $item_id])->count();
            ...
            echo '<div class="btn-toolbar btn-toolbar-order">
                ...
                ' . ($loations_qty > 0 ? '' : '<button onclick="return deleteItemConfirm(' . $item_id . ')" class="btn btn-delete btn-no-margin btn-process-order ">' . IMAGE_DELETE . '</button>') . '
            </div>';
        }
    }
    public function action_location_blocks_confirm_delete()
    {
        \common\helpers\Translation::init('admin/warehouses');
        $this->layout = false;
        $item_id = (int) Yii::$app->request->post('item_id');
    ...
    public function action_location_blocks_delete()
    {
        $item_id = (int) Yii::$app->request->post('item_id');
        \common\models\Location_Blocks::delete_all(['block_id' => $item_id]);
    }
    public function action_location_blocks_edit()
    {
        \common\helpers\Translation::init('admin/warehouses');
        $this->layout = false;
    ...
    public function action_location_blocks_save()
    {
        $item_id = (int) Yii::$app->request->get('item_id');
        if ($item_id > 0) {
            $block = \common\models\Location_Blocks::find_one($item_id);
        } else {
            $block = new \common\models\Location_Blocks();
        }
