...
use Yii;
class Reviews_Controller extends Sceleton
{
    ...
    {
        $this->selected_menu = ['catalog', 'reviews'];
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('reviews/index'), 'title' => BOX_CATALOG_REVIEWS];
        $this->view->heading_title = BOX_CATALOG_REVIEWS;
        $this->view->reviews_table = [['title' => '<input type="checkbox" class="uniform">', 'not_important' => 2], ['title' => TABLE_HEADING_PRODUCTS, 'not_important' => 1], ['title' => TEXT_REVIEW, 'not_important' => 1], ['title' => TABLE_HEADING_RATING, 'not_important' => 1], ['title' => TEXT_INFO_DATE_ADDED1, 'not_important' => 0], ['title' => TEXT_CUSTOMERS, 'not_important' => 1], ['title' => TEXT_STATUS, 'not_important' => 1]];
        $this->view->filters = new \stdClass();
        $status = [['name' => TEXT_ALL, 'value' => '', 'selected' => ''], ['name' => TEXT_NEW, 'value' => 'new', 'selected' => ''], ['name' => TEXT_APPROVED, 'value' => 'approved', 'selected' => ''], ['name' => TEXT_DECLINED, 'value' => 'declined', 'selected' => '']];
        ...
        $c_id = (int) Yii::$app->request->get('cID', 0);
        if ($c_id == 0) {
            $c_id = '';
        }
        $this->view->filters->c_id = $c_id;
    ...
    public function action_list()
    {
        ...
        $query_numrows = 0;
        $search_condition = " where r.reviews_id=rd.reviews_id /*and rd.languages_id = '{$languages_id}'*/ and r.products_id = p.products_id  and p.language_id = '{$languages_id}' and p.platform_id='" . intval(\common\classes\platform::default_id()) . "'";
        //TODO search
        ...
        if (isset($_GET['order'][0]['column']) && $_GET['order'][0]['dir']) {
            switch ($_GET['order'][0]['column']) {
                case 0:
                    $order_by = 'r.date_added ' . tep_db_prepare_input($_GET['order'][0]['dir']);
                    break;
                default:
        ...
        if (tep_not_null($output['from'])) {
            ...
            $filter .= " and to_days(r.date_added) >= to_days('" . \common\helpers\Date::prepare_input_date($from) . "')";
        }
        if (tep_not_null($output['to'])) {
            $to = tep_db_prepare_input($output['to']);
            $filter .= " and to_days(r.date_added) <= to_days('" . \common\helpers\Date::prepare_input_date($to) . "')";
        }
        ...
        $_split = new \Split_Page_Results($current_page_number, $length, $reviews_query_raw, $query_numrows, 'r.reviews_id');
        ...
        while ($reviews = tep_db_fetch_array($reviews_query)) {
            ...
            if ((int) $r_info->status > 0) {
                $status .= '<input type="checkbox" name="check_status" class="check_on_off" value="' . $r_info->reviews_id . '" checked /></div>';
            } else {
                $status .= '<input type="checkbox" name="check_status" class="check_on_off" value="' . $r_info->reviews_id . '" /></div>';
            }
            $short_desc = $r_info->reviews_text;
            if (strlen($short_desc) > 60) {
                $short_desc = substr($short_desc, 0, 66) . '...';
            }
            $short_desc = mb_convert_encoding($short_desc, 'UTF-8', 'UTF-8');
            $response_list[] = ['<input type="checkbox" class="uniform">' . '<input class="cell_identify" type="hidden" value="' . $r_info->reviews_id . '">', '<div class="ls-name-rev ord-name click_double" data-click-double="' . \Yii::$app->url_manager->create_url(['reviews/edit', 'reviews_id' => $r_info->reviews_id]) . '"><a href="' . Yii::$app->url_manager->create_url(['categories/productedit', 'pID' => $r_info->products_id]) . '" target="_blank">' . $r_info->products_name . '</a>' . '<input class="cell_identify" type="hidden" value="' . $r_info->reviews_id . '"></div>', '<div class="ls-review-rev click_double" data-click-double="' . \Yii::$app->url_manager->create_url(['reviews/edit', 'reviews_id' => $r_info->reviews_id]) . '">' . $short_desc . '<div class="ord-total-info"><div class="ord-box-img"></div><div>' . $r_info->reviews_text . '</div></div></div>', '<div class="click_double" data-click-double="' . \Yii::$app->url_manager->create_url(['reviews/edit', 'reviews_id' => $r_info->reviews_id]) . '"><img src="' . Yii::$app->view->theme->base_url . '/img/reviews/stars_' . $r_info->reviews_rating . '.png" /></div>', '<div class="ls-date-rev click_double" data-click-double="' . \Yii::$app->url_manager->create_url(['reviews/edit', 'reviews_id' => $r_info->reviews_id]) . '"><span>' . \common\helpers\Date::datetime_short($r_info->date_added) . '</span></div>', '<div class="ls-name-cus-rev click_double" data-click-double="' . \Yii::$app->url_manager->create_url(['reviews/edit', 'reviews_id' => $r_info->reviews_id]) . '"><a href="' . \Yii::$app->url_manager->create_url(['customers/customeredit', 'customers_id' => $r_info->customers_id]) . '">' . $r_info->customers_name . '</a></div>', $status];
        }
    ...
    public function action_itempreedit()
    {
        ...
        $r_info_array = array_merge($reviews, $products, $products_name, $reviews_text, $reviews_average);
        ?>
<div class="row_or_img"><?php 
        echo \common\classes\Images::get_image($reviews['products_id'], 'Small');
        ?></div>
        <div class="or_box_head or_box_head_no_margin"><?php 
        echo $r_info_array['products_name'];
        ?></div>
        <div class="row_or"><?php 
        echo '<div>' . TEXT_INFO_DATE_ADDED . '</div><div>' . \common\helpers\Date::date_short($reviews['date_added']);
        ?></div></div>
        
        <div class="row_or"><?php 
        echo '<div>' . TEXT_INFO_REVIEW_AUTHOR . '</div><div>' . $reviews['customers_name'];
        ?></div></div>
        <div class="row_or"><?php 
        echo '<div>' . TEXT_INFO_REVIEW_RATING . '</div><div><img src="' . Yii::$app->view->theme->base_url . '/img/reviews/stars_' . $r_info_array['reviews_rating'] . '.png" /></div></div>';
        ?>
        <div class="row_or"><?php 
        echo '<div>' . TEXT_INFO_REVIEW_READ . '</div><div>' . $r_info_array['reviews_read'];
        ?></div></div>
        <div class="row_or"><?php 
        echo '<div>' . TEXT_INFO_REVIEW_SIZE . '</div><div>' . $r_info_array['reviews_text_size'] . ' bytes';
        ?></div></div>
        <div class="row_or"><?php 
        echo '<div>' . TEXT_INFO_PRODUCTS_AVERAGE_RATING . '</div><div>' . number_format($r_info_array['average_rating'], 2) . '%';
        ?></div></div>
        <div class="btn-toolbar btn-toolbar-order">
            <?php 
        echo '<a href="' . \Yii::$app->url_manager->create_url(['reviews/edit', 'reviews_id' => $item_id]) . '" class="btn btn-edit btn-no-margin">' . IMAGE_EDIT . '</a>';
        ?>
            <button onclick="return deleteItemConfirm( <?php 
        echo $item_id;
        ?>)" class="btn btn-delete btn-no-margin"><?php 
        echo IMAGE_DELETE;
        ?></button>
            <button onclick="confirmItemCopy(<?php 
        echo $item_id;
        ?>)" class="btn btn-copy btn-no-margin"><?php 
        echo IMAGE_COPY;
        ?></button>
        </div>
    <?php 
    ...
    public function action_edit()
    {
        ...
        $image = '<span class="prodImgC">' . \common\classes\Images::get_image($reviews['products_id'], 'Thumbnail') . '</span>';
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('reviews/'), 'title' => T_EDITING_REVIEW . ' ' . $r_info->products_name];
        $this->selected_menu = ['catalog', 'reviews'];
        if (Yii::$app->request->is_post) {
            $this->layout = false;
        }
        return $this->render('edit.tpl', ['rInfo' => $r_info, 'status' => $status, 'image' => $image]);
    }
    public function action_submit()
    {
        ...
        if ($error === false) {
            if ($item_id > 0) {
                ...
                $this->after_status_change($reviews_id, $status);
    ...
    public function after_status_change($reviews_id, $status)
    {
        foreach (\common\helpers\Hooks::get_list('reviews/after-status-change') as $filename) {
            include $filename;
        }
    }
    public function action_confirmitemcopy()
    {
        \common\helpers\Translation::init('admin/reviews');
        $this->layout = false;
        $item_id = (int) Yii::$app->request->post('item_id');
        ...
        echo tep_draw_form('copy_item_form', 'reviews/itemcopy', \common\helpers\Output::get_all_get_params(['action']), 'post', 'id="copy_item_form" onSubmit="return itemCopy();"');
...
<?php 
    ...
    public function action_itemcopy()
    {
        $this->layout = false;
        \common\helpers\Translation::init('admin/reviews');
    ...
    public function action_confirmitemdelete()
    {
        ...
        $r_info = new \Object_Info($reviews);
        ...
        echo '<div class="col_desc">' . $r_info->products_name . '</div>';
        ?>
        <div class="btn-toolbar btn-toolbar-order">
            <?php 
    ...
    public function action_switch_status()
    {
        ...
        $this->after_status_change($id, $status == 'true');
    }
}