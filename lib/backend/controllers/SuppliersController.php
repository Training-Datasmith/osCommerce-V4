...
class SuppliersController extends Sceleton
{
    ...
    {
        $this->selectedMenu = ['catalog', 'suppliers'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('suppliers/index'), 'title' => HEADING_TITLE];
        ...
        return $this->render('index', ['messages' => $messages, 'suppliers_id' => $sID]);
    }
    ...
    public function actionEdit()
    {
        ...
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('suppliers/'), 'title' => ($supplier->isNewRecord ? TEXT_HEADING_NEW_SUPPLIER : TEXT_HEADING_EDIT_SUPPLIER) . ' ' . $supplier->suppliers_name];
    ...
    public function actionConfirmdelete()
    {
        ...
        if ($suppliers_id > 0) {
            ...
            echo tep_draw_form('suppliers', 'suppliers/', \common\helpers\Output::get_all_get_params(['sID', 'action']) . 'dID=' . $sInfo->suppliers_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return supplierDelete();"');
