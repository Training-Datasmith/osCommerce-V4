 ...
 */
class Countries_Controller extends Sceleton
{
    public $acl = ['TEXT_SETTINGS', 'BOX_HEADING_LOCATION', 'BOX_TAXES_COUNTRIES'];
    public function action_index()
    {
        global $language;
        $this->selected_menu = ['settings', 'locations', 'countries'];
        $this->navigation[] = ['link' => Yii::$app->url_manager->create_url('countries/index'), 'title' => HEADING_TITLE];
        $this->view->heading_title = HEADING_TITLE;
        $this->top_buttons[] = '<a href="#" class="btn btn-primary" onclick="return countryEdit(0)">' . IMAGE_INSERT . '</a>';
        $this->view->countries_table = [['title' => '<input type="checkbox" class="uniform">', 'not_important' => 2], ['title' => TABLE_HEADING_COUNTRY_NAME, 'not_important' => 0], ['title' => TABLE_HEADING_COUNTRY_CODES, 'not_important' => 0], ['title' => TABLE_HEADING_ACTIVE, 'not_important' => 0]];
        $this->view->filters = new \stdClass();
        $this->view->filters->row = (int) Yii::$app->request->get('row', 0);
        $params = ['ns_country' => '', 'ns_countries' => []];
        if (\common\helpers\Acl::check_extension_allowed('NetSuite') && \common\extensions\Net_Suite\helpers\Net_Suite_Helper::any_configured()) {
            $params['ns_countries'] = \common\extensions\Net_Suite\helpers\Net_Suite_Helper::get_countries_list();
        }
    ...
    public function action_list()
    {
        ...
        if (isset($_GET['order'][0]['column']) && $_GET['order'][0]['dir']) {
            switch ($_GET['order'][0]['column']) {
                case 0:
                    $order_by = 'countries_name ' . tep_db_input(tep_db_prepare_input($_GET['order'][0]['dir']));
                    break;
                case 1:
                    $order_by = 'countries_iso_code_2 ' . tep_db_input(tep_db_prepare_input($_GET['order'][0]['dir']));
                    break;
                default:
                    $order_by = 'sort_order, countries_name';
                    break;
            }
        } else {
        ...
        $response_list = [];
        $countries_query_raw = 'select countries_id, countries_name, countries_iso_code_2, countries_iso_code_3, address_format_id, status, sort_order, lat, lng, zoom from ' . TABLE_COUNTRIES . " where language_id = '" . (int) $languages_id . "' " . $search . 'order by ' . $order_by;
        $countries_split = new \Split_Page_Results($current_page_number, $length, $countries_query_raw, $countries_query_numrows);
        $countries_query = tep_db_query($countries_query_raw);
        while ($countries = tep_db_fetch_array($countries_query)) {
            $response_list[] = ['<input type="checkbox" class="uniform">' . tep_draw_hidden_field('id', $countries['countries_id'], 'class="cell_identify"'), $countries['countries_name'], $countries['countries_iso_code_2'] . '&nbsp;-&nbsp;' . $countries['countries_iso_code_3'], '<input type="checkbox" value="' . $countries['countries_id'] . '" name="categories_status" class="check_on_off"' . ($countries['status'] == 1 ? ' checked="checked"' : '') . '>'];
        }
        $response = ['draw' => $draw, 'recordsTotal' => $countries_query_numrows, 'recordsFiltered' => $countries_query_numrows, 'data' => $response_list];
        echo json_encode($response);
    }
    public function action_countriesactions()
    {
        $languages_id = \Yii::$app->settings->get('languages_id');
        ...
        if ($countries_id) {
            ...
            $c_info = new \Object_Info($country, false);
            $code_types = [0 => 'numeric', 1 => 'alphanumeric', 2 => 'alphabetical'];
            echo '<div class="or_box_head">' . $c_info->countries_name . '</div>';
            echo '<div class="row_or_wrapp">';
            echo '<div class="row_or"><div>' . TEXT_INFO_COUNTRY_NAME . '</div><div>' . $c_info->countries_name . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_COUNTRY_CODE_2 . '</div><div>' . $c_info->countries_iso_code_2 . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_COUNTRY_CODE_3 . '</div><div>' . $c_info->countries_iso_code_3 . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_STATUS . '</div><div>' . ($c_info->status == 1 ? 'Yes' : 'No') . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_SORT_ORDER . '</div><div>' . $c_info->sort_order . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_ADDRESS_FORMAT . '</div><div>' . $c_info->address_format_id . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_VAT_CODE_PREFIX . '</div><div>' . $c_info->vat_code_prefix . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_VAT_CODE_TYPE . '</div><div>' . $code_types[$c_info->vat_code_type] . '</div></div>';
            echo '<div class="row_or"><div>' . TEXT_INFO_VAT_CODE_CHARS . '</div><div>' . $c_info->vat_code_chars . '</div></div>';
            \common\helpers\Translation::init('admin/sms');
            echo '<div class="row_or"><div>' . TEXT_COUNTRY_PHONE_PREFIX . '</div><div>' . $c_info->dialling_prefix . '</div></div>';
            echo '</div>';
            ...
            $ns_block = '';
            if (\common\helpers\Acl::check_extension_allowed('NetSuite') && \common\extensions\Net_Suite\helpers\Net_Suite_Helper::any_configured()) {
                $r = tep_db_query('select key_value, ld.directory_id, ld.directory  ' . " from ep_directories ld left join ep_holbi_soap_kv_storage lp on ld.directory_id=lp.ep_directory_id and key_name='" . tep_db_input($c_info->countries_iso_code_2) . "'" . " where ld.directory_config like '%NetSuiteLink%'  and ld.directory_type='datasource' " . ' ');
                while ($d = tep_db_fetch_array($r)) {
                    $ns_block .= '<div class="ep-sync ep-sync-ns"> <div class="ns-info">' . $d['directory'] . ' ' . (!empty($d['key_value']) ? $d['key_value'] : '') . '</div><div class="ns-buttons"><button class="btn btn-sync btn-no-margin" onclick="linkNS(\'' . $d['key_value'] . '\',\'' . $c_info->countries_iso_code_2 . '\',' . (int) $d['directory_id'] . ')">' . TEXT_UPDATE_EXTERNAL_ID . '</button>' . '</div></div>';
                }
    ...
    public function action_edit()
    {
        ...
        $country = tep_db_fetch_array(tep_db_query('select * from ' . TABLE_COUNTRIES . " where language_id = '" . (int) $languages_id . "' and countries_id ='" . (int) $countries_id . "'"));
        $c_info = new \Object_Info($country, false);
        $c_info->countries_id = $c_info->countries_id ?? null;
        $languages = \common\helpers\Language::get_languages();
        $text = '';
        for ($i = 0; $i < sizeof($languages); $i++) {
            if ($i == 0) {
                $text .= '<div class="col_desc">' . TEXT_INFO_COUNTRY_NAME . '</div>';
            }
            $text .= '<div class="langInput">' . $languages[$i]['image'] . tep_draw_input_field('countries_name[' . $languages[$i]['id'] . ']', \common\helpers\Country::get_country_name($c_info->countries_id, $languages[$i]['id'])) . '</div>';
        }
        echo tep_draw_form('countries', FILENAME_COUNTRIES, 'page=' . \Yii::$app->request->get('page') . '&cID=' . $c_info->countries_id . '&action=save');
        if ($countries_id) {
        ...
        echo '<div class="main_value">' . tep_draw_input_field('countries_iso_code_2', $c_info->countries_iso_code_2 ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_COUNTRY_CODE_3 . '</div>';
        echo '<div class="main_value">' . tep_draw_input_field('countries_iso_code_3', $c_info->countries_iso_code_3 ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_ADDRESS_FORMAT . '</div>';
        echo '<div class="main_value">' . tep_draw_pull_down_menu('address_format_id', \common\helpers\Address::get_address_formats(), $c_info->address_format_id ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_SORT_ORDER . '</div>';
        echo '<div class="main_value">' . tep_draw_input_field('sort_order', $c_info->sort_order ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        ...
        echo '<div class="main_value">' . tep_draw_input_field('vat_code_prefix', $c_info->vat_code_prefix ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_VAT_CODE_TYPE . '</div>';
        echo '<div class="main_value">' . tep_draw_pull_down_menu('vat_code_type', [['id' => 0, 'text' => 'numeric'], ['id' => 1, 'text' => 'alphanumeric'], ['id' => 2, 'text' => 'alphabetical']], $c_info->vat_code_type ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        ...
        echo '<div class="main_value">' . tep_draw_input_field('vat_code_chars', $c_info->vat_code_chars ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_LATITUDE . '</div>';
        echo '<div class="main_value">' . tep_draw_input_field('lat', $c_info->lat ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_LANGITUTE . '</div>';
        echo '<div class="main_value">' . tep_draw_input_field('lng', $c_info->lng ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        echo '<div class="main_title">' . TEXT_INFO_ZOOM . '</div>';
        echo '<div class="main_value">' . tep_draw_input_field('zoom', $c_info->zoom ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        \common\helpers\Translation::init('admin/sms');
        echo '<div class="main_title">' . TEXT_COUNTRY_PHONE_PREFIX . '</div>';
        echo '<div class="main_value">' . tep_draw_input_field('dialling_prefix', $c_info->dialling_prefix ?? null) . '</div>';
        echo '</div>';
        echo '<div class="main_row">';
        ...
        echo '<div class="main_value">' . tep_draw_input_field('currency_code', $c_info->currency_code ?? null) . '</div>';
        echo '</div>';
        if (\common\helpers\Extensions::is_allowed('UploadCustomerId')) {
            echo '<div class="check_linear">';
            echo tep_draw_checkbox_field('is_customer_id_required', 1, $c_info->is_customer_id_required ?? null) . '<span>' . TEXT_CUSTOMER_ID_REQUIRED . '</span>';
            echo '</div>';
        }
        echo '<div class="check_linear">';
        echo tep_draw_checkbox_field('status', 1, $c_info->status ?? null) . '<span>' . TEXT_INFO_STATUS . '</span>';
        echo '</div>';
        echo '<div class="btn-toolbar btn-toolbar-order">';
        echo '<input type="button" value="' . IMAGE_UPDATE . '" class="btn btn-no-margin" onclick="countrySave(' . ($c_info->countries_id ? $c_info->countries_id : 0) . ')"><input type="button" value="' . IMAGE_CANCEL . '" class="btn btn-cancel" onclick="resetStatement(' . $countries_id . ')">';
        echo '</div>';
        echo '</form>';
    }
    public function action_save()
    {
        ...
        if ($countries_id == 0) {
            ...
            if ($ext_class = \common\helpers\Extensions::is_allowed('UploadCustomerId')) {
                $ext_class::country_save($id);
            }
    ...
    public function action_approve_selected()
    {
        $this->layout = false;
        $selected_ids = Yii::$app->request->post('selected_ids');
        foreach ($selected_ids as $id) {
    ...
    public function action_decline_selected()
    {
        $this->layout = false;
        $selected_ids = Yii::$app->request->post('selected_ids');
        foreach ($selected_ids as $id) {
    ...
    public function action_address_city()
    {
        ...
        if (\common\helpers\Acl::rule(['BOX_HEADING_CATALOG', 'BOX_CATALOG_EASYPOPULATE'])) {
            ...
            if ($d_id > 0 && !empty($n_id) && !empty($n_id) && !empty($n_id)) {
                if (\common\helpers\Acl::check_extension_allowed('NetSuite')) {
                    \common\extensions\Net_Suite\helpers\Net_Suite_Helper::set_key_value($d_id, $l_id, $n_id);
                }
                $ret = ['status' => 'OK'];
            } else {
