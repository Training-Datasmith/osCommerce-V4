...
class Coupon_adminController extends Sceleton
{
    ...
    public function actionIndex()
    {
        ...
        $this->selectedMenu = ['marketing', 'gv_admin', 'coupon_admin'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('coupon_admin/index'), 'title' => HEADING_TITLE];
    ...
    public function actionVoucherSubmit()
    {
        ...
        do {
            ...
            $sql_data_array = ['coupon_code' => $coupon_code,
            ...
            for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                $language_id = $languages[$i]['id'];
                $sql_data_marray[$i] = ['coupon_name' => tep_db_prepare_input(\Yii::$app->request->post('coupon_name')[$language_id] ?? null),
    ...
    public function actionVoucherreport()
    {
        ...
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('coupon_admin/index'), 'title' => 'Reedem report'];
        ...
        if (empty($coupon_id)) {
            array_splice($this->view->catalogTable, 4, null, [['title' => 'Coupon code', 'not_important' => 0]]);
        }
        $this->view->filters = new \stdClass();
...
//                'dto' => ['list' => ['\common\helpers\Date', 'prepareInputDate']]
        ...
        return $this->render('voucherreport', [
              'coupon_id' => $coupon_id,
        ]);
    }
...
//                   'dto' => ['list' => ['\common\helpers\Date', 'prepareInputDate']]
        ...
        while ($cc_list = tep_db_fetch_array($cc_query)) {
            ...
            if (empty($coupon_id)) {
                array_splice($responseRow, 4, null, [$cc_list['coupon_code']]);
            }
    ...
    public function actionCouponemail()
    {
        ...
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('coupon_admin/couponemail'), 'title' => $this->view->headingTitle];
        $msg = '';
        ...
        return $this->render('couponemail', [
              'customers_variants' => $customers,
              'customers_selected' => Yii::$app->request->get('customers', ''),
              'email_from' => EMAIL_FROM,
              'email_subject' => $email_subject,
              'email_text' => $email_text,
              'send_coupon_action' => tep_href_link('coupon_admin/couponemail', \common\helpers\Output::get_all_get_params(['action'])),
        ]);
    }
