...
class DepartmentsController extends Sceleton
{
    ...
    {
        $this->selectedMenu = ['departments', 'departments'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('departments/index'), 'title' => BOX_HEADING_DEPARTMENTS];
        ...
        return $this->render('index', ['dID' => $dID]);
    }
    ...
    public function actionEdit()
    {
         ...
         */
        $dInfo->packages_array = ['' => TEXT_NONE];
        ...
        $this->selectedMenu = ['departments', 'departments'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('departments/index'), 'title' => HEADING_TITLE];
        $this->topButtons[] = '';
        $this->view->headingTitle = $HEADING_TITLE;
        $this->view->showSAP = class_exists('\common\classes\SapClient');

        return $this->render('edit', ['dInfo' => $dInfo]);
    }
    ...
    public function actionUpdate()
    {
        ...
        $sql_data_array = ['departments_store_name' => $departments_store_name,
...
//            'departments_commission_percentage' => $departments_commission_percentage,
    ...
    public function actionNew()
    {
        ...
        return $this->render('new', ['dInfo' => $dInfo]);
    }
    ...
    public function actionConfirmitemdelete()
    {
        ...
        if ($departments_id > 0) {
            ...
            echo tep_draw_form('departments', 'departments', \common\helpers\Output::get_all_get_params(['dID', 'action']) . 'dID=' . $cInfo->departments_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return deleteItem();"');
            ...
            if (!empty($_keyValue)) {
                ...
                if (is_array($extract_value)) {
                    foreach ($extract_value as $clientStatus) {
                        if ($clientStatus['id'] != $clientStatusId) {
                            continue;
                        }
