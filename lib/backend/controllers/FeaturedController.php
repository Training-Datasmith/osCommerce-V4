...
class FeaturedController extends Sceleton
{
    ...
    {
        $this->selectedMenu = ['marketing', 'featured'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('featured/index'), 'title' => HEADING_TITLE];
        $this->view->headingTitle = HEADING_TITLE;
        $this->topButtons[] = '<a href="' . \Yii::$app->urlManager->createUrl(['featured/featurededit']) . '" class="btn btn-primary">' . IMAGE_INSERT . '</a>';

        $this->view->featuredTable = [
          [
            'title' => ' ',
            'not_important' => 2,
          ],
    ...
    public function actionItempreedit($item_id = null)
    {
        $this->layout = false;
    ...
    public function actionFeaturededit()
    {
        ...
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('featured/index'), 'title' => BOX_CATALOG_FEATURED];
