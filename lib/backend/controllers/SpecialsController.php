...
class SpecialsController extends Sceleton
{
    ...
    {
        $this->selectedMenu = ['marketing', 'specials'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('specials/index'), 'title' => HEADING_TITLE];
        $this->topButtons[] = '<a href="' . Yii::$app->urlManager->createUrl(['specials/specialedit']) . '" class="btn btn-primary" >' . IMAGE_INSERT . '</a>';
        $this->view->headingTitle = HEADING_TITLE;
        $this->view->specialsTable = [
          [
    ...
    public function actionItemedit()
    {
        ...
        if ($item_id === 0) {
            ...
            if (USE_MARKET_PRICES == 'True') {
                ...
                foreach ($currencies->currencies as $key => $value) {
                    ...
                    $specials_products_price_html = tep_draw_input_field(
                        ...
                        \common\helpers\Product::get_specials_price($sInfo->specials_id, $currencies->currencies[$key]['id']),
                        'size="20"'
                    );
            ...
            if (USE_MARKET_PRICES == 'True') {
                ...
                $fields[] = ['name' => 'specials_price', 'title' => TEXT_SPECIALS_SPECIAL_PRICE, 'value' => \common\helpers\Product::get_specials_price($sInfo->specials_id)];

                $fields[] = ['name' => 'products_price', 'type' => 'hidden', 'value' => (isset($sInfo->products_price) ? $sInfo->products_price : '')];
            }
            ...
            $fields[] = ['name' => 'expires_date', 'title' => TEXT_SPECIALS_EXPIRES_DATE, 'class' => 'datepicker', 'value' => \common\helpers\Date::date_short($sInfo->expires_date)];
    ...
    public function actionSpecialedit()
    {
        ...
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('specials/index'), 'title' => BOX_CATALOG_SPECIALS];
