...
class SpecialsTypesController extends Sceleton
{
    ...
    {
        $this->selectedMenu = ['settings', 'specials-types'];
        $this->navigation[] = ['link' => Yii::$app->urlManager->createUrl('specials-types/index'), 'title' => TEXT_SPECIALS_TAGS];
    ...
    public function actionSave()
    {
        ...
        foreach ($languages as $language) {
            ...
            try {
                ...
                $r = $type->save(false);
                if (!$r) {
                    $res = ['message' => 'not validated' . $e->getMessage(), 'messageType' => 'alert-warning'];
                }
            } catch (\Exception $e) {
                $res = ['message' => $e->getMessage(), 'messageType' => 'alert-warning'];
                if ($new) {
