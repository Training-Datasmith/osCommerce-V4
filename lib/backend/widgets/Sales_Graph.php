...
use yii\base\Widget;
class Sales_Graph extends Widget
{
    public function run()
    {
        ...
        $orders_data_array = ['blue' => [], 'green' => [], 'red' => [], 'blue2' => [], 'green2' => [], 'red2' => []];
        $date_from = date('Y-m-d H:i:s', mktime(0, 0, 0, date('m') + 1, 1, date('Y') - 1));
        $orders_query = tep_db_query('select year(o.date_purchased) as date_year, month(o.date_purchased) as date_month, ' . '  count(*) as total_orders, avg(ost.value*o.currency_value) as avg_order_amount, ' . '  sum(ot.value*o.currency_value) as total_amount ' . 'from ' . TABLE_ORDERS . ' o ' . ' inner join ' . TABLE_ORDERS . " oin ON oin.orders_id=o.orders_id AND oin.date_purchased >= '" . tep_db_input($date_from) . "' " . ' left join ' . TABLE_ORDERS_TOTAL . " ot on (o.orders_id = ot.orders_id and ot.class = 'ot_total') " . ' left join ' . TABLE_ORDERS_TOTAL . " ost on (o.orders_id = ost.orders_id and ost.class = 'ot_subtotal') " . "where o.orders_status not in ('" . implode("','", $exclude_order_statuses_array) . "') " . (count($filter_by_platform) > 0 ? " and o.platform_id in ('" . implode("','", $filter_by_platform) . "') " : '') . 'group by year(o.date_purchased), month(o.date_purchased) ' . 'order by year(o.date_purchased), month(o.date_purchased)');
        ...
        //$startY = 2019;
        for ($i = 0; $i < 12; $i++) {
            $monthes[] = ($start_m + $i) % 12 + 1;
            if ($i < 11) {
                $orders_data_array['blue'][] = '[' . mktime(14, 0, 0, $start_m + $i - 11, 1, $start_y) . '000,' . 0 . ']';
                $orders_data_array['green'][] = '[' . mktime(14, 0, 0, $start_m + $i - 11, 1, $start_y) . '000,' . 0 . ']';
                $orders_data_array['red'][] = '[' . mktime(14, 0, 0, $start_m + $i - 11, 1, $start_y) . '000,' . 0 . ']';
            }
        }
        ...
        while ($orders = tep_db_fetch_array($orders_query)) {
            ...
            if ($orders_count == $orders_counts) {
                // Last month
                if ((int) $orders['date_month'] != (int) $start_m) {
                    // No orders in current month yet
                    $orders_data_array['blue2'] = $orders_data_array['green2'] = $orders_data_array['red2'] = [];
                    //$orders_data_array['blue2'][$monthes[$orders['date_month']]+1] =
                    $orders_data_array['blue2'][$monthes[$orders['date_month']]] = '[' . mktime(14, 0, 0, $start_m, 1, $start_y) . '000,' . $orders['total_orders'] . ']';
                    ...
                    $orders_data_array['green2'][$monthes[$orders['date_month']]] = '[' . mktime(14, 0, 0, $start_m, 1, $start_y) . '000,' . $orders['avg_order_amount'] . ']';
                    ...
                    $orders_data_array['red2'][$monthes[$orders['date_month']]] = '[' . mktime(14, 0, 0, $start_m, 1, $start_y) . '000,' . $orders['total_amount'] . ']';
                    ...
                    // Estimate to full month interval
                    $first_months_day = mktime(0, 0, 0, $start_m, 1, $start_y);
                    $percent = (mktime(0, 0, 0, $start_m + 1, 1, $start_y) - $first_months_day) / (time() - $first_months_day);
        ...
        \Yii::$app->view->register_js('var data_blue  = [ ' . implode(' , ', $orders_data_array['blue']) . ' ];' . 'var data_blue2  = [ ' . implode(' , ', $orders_data_array['blue2']) . ' ];' . 'var data_green = [ ' . implode(' , ', $orders_data_array['green']) . ' ];' . 'var data_green2 = [ ' . implode(' , ', $orders_data_array['green2']) . ' ];' . 'var data_red   = [ ' . implode(' , ', $orders_data_array['red']) . ' ];' . 'var data_red2   = [ ' . implode(' , ', $orders_data_array['red2']) . ' ];', \yii\web\View::POS_BEGIN);
