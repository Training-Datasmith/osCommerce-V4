    ...
    public function run()
    {
        ...
        if (defined('SHOW_GOOGLE_MAPS')) {
            ...
            if (isset($params['enabled_map']['configuration_title'])) {
                $_t = \common\helpers\Translation::get_translation_value('SHOW_GOOGLE_MAPS_TITLE', 'configuration');
                $params['enabled_map']['configuration_title'] = $_t ? $_t : $params['enabled_map']['configuration_title'];
            }
            if (SHOW_GOOGLE_MAPS == 'true') {
                $params['mapskey'] = (new \common\components\Google_Tools())->get_map_provider()->get_maps_key();
                $orig_place = [0, 0, 2];
