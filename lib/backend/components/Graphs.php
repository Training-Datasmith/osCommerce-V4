...
class Graphs
{
    ...
    public static $tmp = [];
    // array for bad days (with 0 total(s))
    ...
    public static function get_currency(): array
    {
        ...
        foreach ($currencies->currencies as $code => $currency) {
            $currencies_array[] = ['id' => $code, 'text' => $currency['title'] . ' [' . $code . ']'];
        }
        return $currencies_array;
    }
    public static function prepare_query_string(): ?string
    {
        $patterns = ['/filter_name=[^&]*&/', '/([?&])x=[^&]*&/', '/([?&])y=[^&]*&/', '/apply_filter=[^&]*&/', '/remove_filter=[^&]*&/', '/&$/', '/month=[^&]*&/', '/year=[^&]*&/', '/&+/', '/^&/', '/' . tep_session_name() . '=[^&]*&/'];
        $replace = ['', '\1', '\1', '', '', '', '', '', '&', '', ''];
        return preg_replace($patterns, $replace, $_SERVER['QUERY_STRING'] . '&');
    }
    public static function insert_filter($str): void
    {
    ...
    public static function delete_filter($str): bool
    {
    ...
    public static function prepare_filter($only_delete = false)
    {
        $str = self::prepare_query_string();
        if (self::delete_filter($str)) {
            return true;
        }
        if (!$only_delete) {
            self::insert_filter($str);
            ...
            if ($d = tep_db_fetch_array($r)) {
                ...
                return Yii::$app->url_manager->create_url($params);
            }
        }
        return false;
    }
    ...
    public static function get_years(): array
    {
        ...
        while ($years = tep_db_fetch_array($years_query)) {
            $years_array[] = ['id' => $years['order_year'], 'text' => $years['order_year']];
        }
        return $years_array;
    }
    /**
    ...
    public static function get_months(): array
    {
        $months_array = [];
        for ($i = 1; $i < 13; $i++) {
            $months_array[] = ['id' => $i, 'text' => defined('_' . strtoupper(date('F', mktime(0, 0, 0, $i, 1)))) ? constant('_' . strtoupper(date('F', mktime(0, 0, 0, $i, 1)))) : date('F', mktime(0, 0, 0, $i, 1))];
        }
        return $months_array;
    }
    /**
     * @return string[]
     */
    public static function get_colors($colors_graph): array
    {
        $colors_html = [];
     ...
     * @return mixed[]
     */
    public static function get_head_colors($colors_graph): array
    {
        $list = [];
    ...
    {
        return [['id' => 'daily', 'text' => STATISTICS_TYPE_DAILY], ['id' => 'monthly', 'text' => STATISTICS_TYPE_MONTHLY], ['id' => 'yearly', 'text' => STATISTICS_TYPE_YEARLY]];
    }
    /**
     * @return mixed[]
     */
    public static function get_filters(): array
    {
        ...
        while ($d = tep_db_fetch_array($filters_query)) {
            ...
            array_unshift($params, 'sales_statistics/index');
            $filters[] = ['id' => Yii::$app->url_manager->create_url($params), 'text' => $d['sales_filter_name']];
        }
        return $filters;
    }
    public static function calculate_yearly_report(array &$stats)
    {
        ...
        return self::convert_to_chart($stats);
    }
     ...
     * @return mixed[][]
     */
    public static function convert_to_chart($stats): array
    {
        $new_stats = [];
        foreach ($stats as $calendar_item) {
            foreach ($calendar_item as $key => $progress_line) {
                if (!is_array($new_stats[$key])) {
                    $new_stats[$key] = [];
                }
                array_push($new_stats[$key], round($progress_line, 2));
            }
        }
        return $new_stats;
    }
