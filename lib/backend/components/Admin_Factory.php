...
class Admin_Factory
{
    public static function init(): void
    {
        ...
        if (!tep_session_is_registered('language') || isset($_GET['language'])) {
            ...
            \Yii::$app->settings->set('locale', $lng->language['locale']);
            \Yii::$app->settings->set('languages_id', $lng->language['id']);
            //preparing to switch
        }
        $lng->set_locale();
        $lng->load_vars();
        ...
        if (is_object($navigation) && method_exists($navigation, 'add_current_page')) {
            $route = false;
            $tmp = Yii::$app->url_manager->parse_request(Yii::$app->request);
            if (is_array($tmp) && !empty($tmp[0])) {
                $route = $tmp[0];
            }
            if (!Yii::$app->request->is_ajax && $route && !in_array($route, ['index/load-languages-js'])) {
        ...
        if (defined('XML_DUMP_ENABLE') && XML_DUMP_ENABLE == 'True') {
            ...
            if (is_dir(DIR_FS_CATALOG_XML)) {
                if (!is_writeable(DIR_FS_CATALOG_XML)) {
                    $message_stack->add(ERROR_CATALOG_XML_DIRECTORY_NOT_WRITEABLE);
                    $can_backup_xml = false;
                }
            } else {
                $message_stack->add(ERROR_CATALOG_XML_DIRECTORY_DOES_NOT_EXIST);
                $can_backup_xml = false;
            }
        ...
        if (!defined('DEFAULT_CURRENCY')) {
            $message_stack->add(ERROR_NO_DEFAULT_CURRENCY_DEFINED);
        }
        // check if a default language is set
        if (!defined('DEFAULT_LANGUAGE')) {
            $message_stack->add(ERROR_NO_DEFAULT_LANGUAGE_DEFINED);
        }
        if (function_exists('ini_get') && (bool) ini_get('file_uploads') == false) {
            $message_stack->add(WARNING_FILE_UPLOADS_DISABLED, 'header', 'warning');
        }
