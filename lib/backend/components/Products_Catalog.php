...
class Products_Catalog extends Widget
{
    public $post = [];
    public $settings = ['popup_product' => true, 'extra_header' => true, 'add_sku' => true, 'search_suggest' => true, 'only_active' => false];
    public function make_html($post = [])
    {
        ...
        if (!$this->settings['search_suggest']) {
            ...
            $params['searchsuggest'] = \common\models\Products::find()->inner_join_with('platform')->where(['products_status' => 1])->count() > 50;
        }
        if (!$params['searchsuggest']) {
            $category_tree_array = $this->build_tree(0);
        }
        $params['rates'] = \common\helpers\Tax::get_order_tax_rates();
        $params['settings'] = $this->settings;
        $params['category_tree_array'] = $category_tree_array ?? null;
        $params['queryParams'] = array_merge(['categories/seacrh-product'], Yii::$app->request->get_query_params());
        $params['tree_server_url'] = array_merge(['categories/load-tree'], Yii::$app->request->get_query_params());
        return $this->render('choose_product_box', $params);
    }
    public function make($post = [])
    {
        $html = $this->make_html($post);
    ...
    public function search($search_text)
    {
        ...
        $products_query = \common\models\Products::find()->distinct()->alias('p')->select(['p.*', 'pd.*'])->add_select(['products_name_res' => new \yii\db\Expression(\backend\models\Product_Name_Decorator::instance()->listing_query_expression('pd1', 'pd'))])->join_with('manufacturer m')->w_description('pd', \common\helpers\Language::get_default_language_id())->w_description('pd1');
        if (!empty($this->setting['only_active'])) {
            $products_query->and_where(['p.products_status' => 1]);
        }
        ...
        if (!($this->post['suggest'] ?? null)) {
            ...
            foreach ($products_query->limit(20)->as_array()->all() as $product) {
                ...
                $tree[] = [
                    ...
                    'image' => '',
                    'price' => $currencies->display_price($ins->get_product_price(['qty' => 1]), 0, 1),
                ];
            }
        }
    ...
    public function tree()
    {
        ...
        if ($do == 'missing_lazy') {
            ...
            $response_data['tree_data'] = $this->build_tree($category_id);
    ...
    public function build_tree($category_id = 0)
    {
        ...
        if ($category_id > 0) {
            $_check = tep_db_fetch_array(tep_db_query('SELECT COUNT(*) AS c FROM ' . TABLE_PLATFORMS_CATEGORIES . " WHERE categories_id='" . (int) $category_id . "' "));
            $category_selected_state = $_check['c'] > 0;
        }
        $languages_id = \Yii::$app->settings->get('languages_id');
        $get_categories_r = tep_db_query("SELECT CONCAT('c',c.categories_id) as `key`, cd.categories_name as title, c.categories_image " . 'FROM ' . TABLE_CATEGORIES_DESCRIPTION . ' cd, ' . TABLE_CATEGORIES . ' c ' . "WHERE cd.categories_id=c.categories_id and cd.language_id='" . $languages_id . "' AND cd.affiliate_id=0 and c.parent_id='" . (int) $category_id . "' " . 'order by c.sort_order, cd.categories_name');
        while ($_categories = tep_db_fetch_array($get_categories_r)) {
            ...
            if (is_file(Images::get_fs_catalog_images_path() . $_categories['categories_image'])) {
                $_categories['image'] = Html::img(Images::get_ws_catalog_images_path() . $_categories['categories_image']);
            }
            $_categories['selected'] = $category_selected_state && !!Array_Helper::get_value($_categories, 'selected');
            $tree_init_data[] = $_categories;
        }
        $get_products_r = tep_db_query("SELECT concat('p',p.products_id,'_',p2c.categories_id) AS `key`, pd.products_name as title, pd.products_description, pd.products_description_short, p.products_id, p.products_model " . 'from ' . TABLE_PRODUCTS_DESCRIPTION . ' pd , ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c, ' . TABLE_PRODUCTS . ' p ' . "WHERE pd.products_id=p.products_id and pd.language_id='" . $languages_id . "' and pd.platform_id='" . \common\classes\platform::default_id() . "' and p2c.products_id=p.products_id and p2c.categories_id='" . (int) $category_id . "' " . 'order by p.sort_order, title');
        ...
        if (tep_db_num_rows($get_products_r) > 0) {
            while ($_product = tep_db_fetch_array($get_products_r)) {
                if (!isset($_product['title']) || !$_product['title']) {
                    $products_description = Products_Description::find()->select('products_name')->where(['products_id' => $_product['products_id']])->and_where("products_name <> ''")->as_array()->one();
                    if (isset($products_description['products_name'])) {
                        $_product['title'] = $products_description['products_name'];
                    }
                }
                ...
                //$category_selected_state && !!$_product['selected'];
                ...
                if (Array_Helper::get_value($this->settings, 'add_sku') && !empty($_product['products_model'])) {
