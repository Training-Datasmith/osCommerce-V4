...
class Navigation extends Widget
{
     ...
     * @return mixed[]
     */
    public function buildTree($parent_id, $queryResponse, $rule = []): array
    {
        $tree = [];
        if ($parent_id == 0) {
            ...
            if ($response['parent_id'] == $parent_id) {
                ...
                if (\common\helpers\Acl::rule($ruleTmp)) {// enabled
                    ...
                    if (!empty($response['acl_check'])) {
                        [$moduleName, $actionName] = explode(',', (string) $response['acl_check']);
                        if (false === \common\helpers\Acl::checkExtensionAllowed($moduleName, $actionName)) {
                            $response['disabled'] = true;
                            $response['dis_module'] = true;
                        } elseif (false === \common\helpers\Acl::checkExtensionAllowed($moduleName, $actionName)) {
                            $response['dis_module'] = true;
                        }
                    }

                    if (!empty($response['config_check'])) {
                        [$configKey, $configValue] = explode(',', (string) $response['config_check']);
