...
class SuppliersRules
{
    ...
    protected function fillSupplierData($discountCollection, $rulesCollection, $skipRoFill = false)
    {
        ...
        foreach ($discountCollection as $supplierDiscount) {
            if (!is_array($supplier_data[$supplierDiscount->suppliers_id])) {
                $supplier_data[$supplierDiscount->suppliers_id] = [];
            }
        ...
        foreach ($rulesCollection as $supplierPriceRule) {
            if (!is_array($supplier_data[$supplierPriceRule->suppliers_id] ?? null)) {
                $supplier_data[$supplierPriceRule->suppliers_id] = [];
            }
        ...
        if (count($supplier_data) > 0) {
            ...
            foreach ($supplierInfos as $supplierInfo) {
                ...
                if (!isset($supplier_data[$supplierInfo->suppliers_id]['discount_table'])) {
                    $supplier_data[$supplierInfo->suppliers_id]['discount_table'] = [];
                }
                if (!isset($supplier_data[$supplierInfo->suppliers_id]['rule_condition'])) {
                    $supplier_data[$supplierInfo->suppliers_id]['rule_condition'] = '';
                }
                if (!isset($supplier_data[$supplierInfo->suppliers_id]['price_rule'])) {
                    $supplier_data[$supplierInfo->suppliers_id]['price_rule'] = [];
                }
        ...
        if (!$skipRoFill) {
            ...
            foreach ($supplierInfos as $supplierInfo) {
                if (isset($supplier_data[$supplierInfo->suppliers_id])) {
                    continue;
                }
