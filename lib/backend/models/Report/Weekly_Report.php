...
use Yii;
class Weekly_Report extends Basic_Report implements Report_Interface
{
    ...
    private $name = 'weekly';
    protected $sql_params = ['group' => ['day', 'month', 'year'], 'select_period' => 'date'];
    protected $range = ['week' => TEXT_WEEK_COMMON, 'custom' => TEXT_CUSTOM];
    ...
    public function __construct($data)
    {
        switch ($data['range']) {
            case 'week':
                if (isset($data['week'])) {
                    $this->week = $data['week'];
                    $range = $this->parse_date($data['week'], true);
                    if ($range) {
                        ...
                        if (isset($data['week_cmp']) and $data['week'] != $data['week_cmp']) {
                            $week_cmp = $this->parse_date($data['week_cmp'], true);
            ...
            case 'custom':
                if (isset($data['start_custom']) && !empty($data['start_custom'])) {
                    $range = $this->parse_date($data['start_custom']);
                    $this->start_day = $range->start['day'];
                    $this->start_month = $range->start['month'];
                    $this->start_year = $range->start['year'];
                }
                if (isset($data['end_custom']) && !empty($data['end_custom'])) {
                    $range = $this->parse_date($data['end_custom']);
    ...
    public function get_options($range)
    {
        switch ($range) {
            case 'week':
                return Yii::$app->controller->render_ajax('week', ['week' => $this->week, 'week_cmp' => trim($this->week_cmp)]);
                break;
            case 'custom':
                return Yii::$app->controller->render_ajax('custom_day_month_year', ['start_custom' => $this->start_custom, 'end_custom' => $this->end_custom]);
                break;
        }
    }
    public function parse_date($date, $is_range = false)
    {
        $_start = $_end = [];
        if ($is_range) {
            $ex = explode('-', $date);
        ...
        //TODO checkdate
        if (count($_start)) {
            $range->start = ['day' => $_start[0], 'month' => $_start[1], 'year' => $_start[2]];
        }
        if (count($_end)) {
            $range->end = ['day' => $_end[0], 'month' => $_end[1], 'year' => $_end[2]];
        }
        return $range;
    }
    ...
    {
        if ($value == 'period') {
            return parent::convert_column_title(TEXT_DATE);
        }
