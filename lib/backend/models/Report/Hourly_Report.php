...
use Yii;
class Hourly_Report extends Basic_Report implements Report_Interface
{
    ...
    protected $sql_params = ['group' => ['hour', 'dayofmonth', 'month', 'year'], 'select_period' => ''];
    protected $range = ['day' => TEXT_DAY, 'month' => TEXT_MONTH_COMMON, 'year' => TITLE_YEAR, 'custom' => TEXT_CUSTOM];
        ...
        switch ($data['range']) {
            case 'day':
                if (isset($data['day']) && !empty($data['day'])) {
                    $day = $this->parse_date($data['day'], true);
                    ...
                    if (isset($data['day_cmp']) and $data['day'] != $data['day_cmp']) {
                        $day_cmp = $this->parse_date($data['day_cmp'], true);
            ...
            case 'custom':
                if (isset($data['start_custom']) && !empty($data['start_custom'])) {
                    $this->predefine_start_custom_day_month_year($data['start_custom']);
                }
                if (isset($data['end_custom']) && !empty($data['end_custom'])) {
                    $this->predefine_end_custom_day_month_year($data['end_custom']);
                }
                break;
        }
        if (isset($data['range'])) {
    ...
    public function get_options($range)
    {
        switch ($range) {
            case 'day':
                return Yii::$app->controller->render_ajax('day_month_year', ['day' => $this->day, 'day_cmp' => trim($this->day_cmp)]);
                break;
            case 'month':
                return Yii::$app->controller->render_ajax('month_year', ['month_year' => $this->month_year, 'holder' => TEXT_MONTH_COMMON . '/' . strtolower(TITLE_YEAR)]);
                break;
            case 'year':
                return Yii::$app->controller->render_ajax('year', ['year' => $this->start_year, 'years' => $this->get_years_list()]);
                break;
            case 'custom':
                return Yii::$app->controller->render_ajax('custom_day_month_year', ['start_custom' => $this->start_custom, 'end_custom' => $this->end_custom, 'holder' => TEXT_DAY_COMMON . '/' . TEXT_MONTH_COMMON . '/' . strtolower(TITLE_YEAR)]);
                break;
        }
    }
    public function fill_the_time(&$data, $info)
    {
        static $last_day;
    ...
    public function fill_time_gaps($data, $pattern)
    {
        $_hours = [];
        for ($i = 0; $i < 24; $i++) {
    ...
    public function calculate_avg($data)
    {
        if (is_array($data)) {
            $avg = ['orders' => [], 'total' => []];
            ...
            foreach ($data as $day => $hours) {
                foreach ($hours as $hour => $value) {
                    if ($this->orders_avg) {
                        $this->insert_at($data[$day][$hour], 'orders', 'orders_avg', round($avg['orders'][$hour] / count($data), 0));
                    }
                    if ($this->total_avg) {
                        $this->insert_at($data[$day][$hour], 'ot_total', 'total_avg', round($avg['total'][$hour] / count($data), 2));
                    }
                }
            }
        }
    ...
    public function load_purchases($for_map = false)
    {
        ...
        $new_data = $this->prepare_days_range();
        if (is_array($data)) {
            $filled = false;
            foreach ($data as $k => $v) {
                if (!$filled) {
                    ...
                    $new_data = $this->fill_time_gaps($new_data, $template);
                    $filled = true;
                }
                $this_day = date('d-m-Y', strtotime($v['period']));
                if (!empty($v['period'])) {
                    $this->fill_the_time($new_data, $v);
                }
            }
            if (($this->orders_avg || $this->total_avg) && $this->interval > 0) {
                $new_data = $this->calculate_avg($new_data);
            }
            //echo'<pre>';print_r($this);die;
            $_temp = [];
