...
use Yii;
class Quarterly_Report extends Basic_Report implements Report_Interface
{
    public const DELIMETER = '/';
    public const SHOW_ROWS = 25;
    ...
    private $name = 'yearly';
    protected $sql_params = ['group' => ['month', 'year'], 'select_period' => 'date'];
    private $quarters = ['1' => 'Q1', '2' => 'Q2', '3' => 'Q3', '4' => 'Q4'];
    protected $range = ['year' => TITLE_YEAR, 'all' => TEXT_ALL_PERIOD, 'custom' => TEXT_CUSTOM];
    ...
    public function get_options($range)
    {
        switch ($range) {
            case 'all':
                ...
                break;
            case 'year':
                return Yii::$app->controller->render_ajax('year', ['year' => $this->end_year, 'year_cmp' => trim($this->end_year_cmp), 'years' => $this->get_years_list()]);
                break;
            case 'custom':
                return Yii::$app->controller->render_ajax('quaterly_options', ['start_custom_quarter' => $this->start_custom_quarter, 'end_custom_quarter' => $this->end_custom_quarter, 'start_custom_year' => $this->start_custom_year, 'end_custom_year' => $this->end_custom_year, 'quarter' => !tep_not_null($this->start_custom_quarter) ? $this->start_quarter : '', 'year' => !tep_not_null($this->end_custom_year) ? $this->end_year : date('Y'), 'years' => $this->get_years_list(), 'quarters' => $this->quarters]);
                break;
        }
    }
    public function update_quarter_period(&$data)
    {
        if (is_array($data)) {
            $q = [];
    ...
    public function calculate_avg($data)
    {
        if (is_array($data)) {
            ...
            foreach ($data as $day => $value) {
                if ($this->orders_avg) {
                    $this->insert_at($data[$day], 'orders', 'orders_avg', round($avg['orders'] / count($data), 0));
                }
                if ($this->total_avg) {
                    $this->insert_at($data[$day], 'ot_total', 'total_avg', round($avg['total'] / count($data), 2));
                }
            }
        }
        return $data;
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $k => $v) {
                if (!$filled) {
                    ...
                    $new_data = $this->prepare_month_range($template, 'M', $this->class_range);
                    $this->update_quarter_period($new_data);
                    $filled = true;
                }
                if (!empty($v['period'])) {
    ...
    {
        if ($value == 'period') {
            return parent::convert_column_title(TEXT_MONTH_COMMON);
        }
