...
class ViewPriceData
{
    ...
    public function populateView($view)
    {
        ...
        $gift_wrap = tep_db_query('select gw_id as gift_wrap_id, groups_id, currencies_id, gift_wrap_price, round(gift_wrap_price +round(gift_wrap_price *' . (float)$_tax . ', 6), ' . (int)$_roundTo. ') as gift_wrap_price_gross from ' . TABLE_GIFT_WRAP_PRODUCTS . " where products_id ='" . (int) $pInfo->products_id . "'");
        ...
        if (!$view->useMarketPrices) {
            $qty_discounts = [];
            $tmp = \common\helpers\Product::parseQtyDiscountArray($pInfo->products_price_discount);
            if (count($tmp) > 0) {
                foreach ($tmp as $key => $value) {
                    $qty_discounts[$key]['price'] = $value;
                    $qty_discounts[$key]['price_gross'] = round($value + round($value * (float)$_tax, 6), $_roundTo);
                }
            }
            ...
            $qty_discounts = [];
            $tmp = \common\helpers\Product::parseQtyDiscountArray($pInfo->products_price_discount_pack_unit);
            if (count($tmp) > 0) {
                foreach ($tmp as $key => $value) {
                    $qty_discounts[$key]['price'] = $value;
                    $qty_discounts[$key]['price_gross'] = round($value + round($value * (float)$_tax, 6), $_roundTo);
            ...
            $qty_discounts = [];
            $tmp = \common\helpers\Product::parseQtyDiscountArray($pInfo->products_price_discount_packaging);
            if (count($tmp) > 0) {
                foreach ($tmp as $key => $value) {
                    $qty_discounts[$key]['price'] = $value;
                    $qty_discounts[$key]['price_gross'] = round($value + round($value * (float)$_tax, 6), $_roundTo);
        ...
        if ($view->useMarketPrices || \common\helpers\Extensions::isCustomerGroupsAllowed()) {
            $products_price_query = tep_db_query('select pp.groups_id, pp.currencies_id, pp.products_sets_discount, pp.supplier_price_manual, pp.products_group_price, round(pp.products_group_price +round(pp.products_group_price *' . (float)$_tax . ', 6), ' . (int)$_roundTo. ') as products_group_price_gross, pp.bonus_points_price,	pp.bonus_points_cost, pp.products_group_discount_price, pp.products_group_price_pack_unit, round(pp.products_group_price_pack_unit +round(pp.products_group_price_pack_unit *' . (float)$_tax . ', 6), ' . (int)$_roundTo. ') as products_group_price_gross_pack_unit, pp.products_group_discount_price_pack_unit, pp.products_group_price_packaging, round(pp.products_group_price_packaging +round(pp.products_group_price_packaging *' . (float)$_tax . ', 6), ' . (int)$_roundTo. ') as products_group_price_gross_packaging, pp.products_group_discount_price_packaging, sp.specials_new_products_price as products_group_special_price, round(sp.specials_new_products_price +round(sp.specials_new_products_price *' . (float)$_tax . ', 6), ' . (int)$_roundTo. ') as products_group_special_price_gross, s.expires_date, s.start_date, ifnull(s.status, 0) as sales_status, ifnull(s.specials_disabled, -1) as specials_disabled, ifnull(s.specials_enabled, -1) as specials_enabled, (ifnull(s.start_date, now())>now()) as specials_scheduled, max_per_order, total_qty, pp.shipping_surcharge_price, round(pp.shipping_surcharge_price +round(pp.shipping_surcharge_price *' . (float)$_tax . ', 6), ' . (int)$_roundTo. ') as shipping_surcharge_price_gross from ' . TABLE_PRODUCTS_PRICES . ' pp left join ' . TABLE_SPECIALS . " s on pp.products_id=s.products_id and s.specials_id='" . (isset($pInfo->specials_id) ? (int)$pInfo->specials_id : 0) . "' left join " . TABLE_SPECIALS_PRICES . " sp on pp.groups_id=sp.groups_id and pp.currencies_id=sp.currencies_id and s.specials_id=sp.specials_id where pp.products_id = '" . (int)$pInfo->products_id . "' order by pp.currencies_id, pp.groups_id");
            $_tmp_keys = [
