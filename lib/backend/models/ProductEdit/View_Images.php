...
class View_Images
{
    ...
    public function populate_view($view)
    {
        ...
        $image_path = DIR_WS_CATALOG_IMAGES . 'products' . '/' . $p_info->products_id . '/';
        //$image_path = Yii::getAlias('@web');
        $image_path_upload = \Yii::get_alias('@webroot') . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR;
        $images = [];
        // {{
        $_ext_images_array = [];
        ...
        $image_product_id = $p_info->products_id;
        if (empty($p_info->products_id) && $p_info->parent_products_id) {
            $image_clone_mode = true;
            $image_product_id = $p_info->parent_products_id;
        }
        ...
        while ($images_data = tep_db_fetch_array($images_query)) {
            ...
            for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                ...
                $image_size = null;
                $image_src = DIR_FS_CATALOG . DIR_WS_IMAGES . 'products' . '/' . $images_data['products_id'] . '/' . ($images_description['products_images_id'] ?? null) . '/' . ($images_description['hash_file_name'] ?? null);
                if (file_exists($image_src)) {
                    ...
                    if ($image_clone_mode) {
                        if (@copy($image_src, $image_path_upload . $images_description['orig_file_name'])) {
                            $uploaded_ws_path = \Yii::get_alias('@web/uploads/') . $images_description['orig_file_name'];
                ...
                $description[$languages[$i]['id']] = ['key' => $i + 1, 'id' => $languages[$i]['id'], 'code' => $languages[$i]['code'], 'name' => $languages[$i]['name'], 'logo' => isset($languages[$i]['logo']) ? $languages[$i]['logo'] : '', 'image_title' => $images_description['image_title'] ?? null, 'image_alt' => $images_description['image_alt'] ?? null, 'orig_file_name' => $images_description['orig_file_name'] ?? null, 'use_origin_image_name' => $images_description['use_origin_image_name'] ?? null, 'hash_file_name' => $images_description['hash_file_name'] ?? null, 'file_name' => $images_description['file_name'] ?? null, 'alt_file_name' => $images_description['alt_file_name'] ?? null, 'no_watermark' => $images_description['no_watermark'] ?? null, 'external_image_original' => $images_description['external_image_original'] ?? null, 'image_name' => empty($images_description['hash_file_name']) ? $uploaded_ws_path : $image_path . $images_description['products_images_id'] . '/' . $images_description['hash_file_name'], 'preload' => $preload_image, 'imageSize' => $image_size, 'use_external_images' => !!($images_description['use_external_images'] ?? null), 'external_images' => $_ext_images_array, 'link_video_id' => $images_description['link_video_id'] ?? null];
            }
            $inventory = [];
            if (is_array($view->selected_inventory ?? null)) {
                foreach ($view->selected_inventory as $key => $value) {
            ...
            $get_image_url_data_r = tep_db_query('SELECT * FROM ' . TABLE_PRODUCTS_IMAGES_EXTERNAL_URL . ' ' . "WHERE products_images_id='" . (int) $images_data['products_images_id'] . "'");
            ...
            $preload_image = '';
            $uploaded_ws_path = '';
            $image_src = DIR_FS_CATALOG . DIR_WS_IMAGES . 'products' . '/' . $images_data['products_id'] . '/' . $images_data['products_images_id'] . '/' . $images_data['hash_file_name'];
            if (file_exists($image_src)) {
                try {
                    $image_size = getimagesize($image_src);
                } catch (\Exception $ex) {
                ...
                if ($image_clone_mode) {
                    if (@copy($image_src, $image_path_upload . $images_data['orig_file_name'])) {
                        $uploaded_ws_path = \Yii::get_alias('@web/uploads/') . $images_data['orig_file_name'];
            ...
            if ($ext = \common\helpers\Extensions::is_allowed('ProductImagesByPlatform')) {
                $ext::image_view($images, $images_data);
            }
        }
        $view->images = $images;
        $view->images_qty = count($images);
    }
}