    ...
    public function populateView($view)
    {
        ...
        while ($data = tep_db_fetch_array($query)) {
            ...
            if (!empty($pInfo->options_templates_id)) {
                ...
                $fullpriceTmp = $price0;
            } elseif (empty($pInfo->products_id)) {
                $price0 = 0;
                $fullpriceTmp = 0;
            } elseif (!$without_inventory && \common\helpers\Extensions::isAllowed('Inventory') && ($options_count == 1) && !$is_virtual_option) {
            ...
            if (strpos($data['price_prefix'], '%') !== false && $pInfo->products_price_full != 1) {
                ...
                $gross_price_formatted = $currencies->display_price(($pInfo->products_price_full == 1 ? $fullpriceTmp : $price0), (float)$_tax, 1, false);
            }
            $products_file_url = '';
            ...
            if (!isset($selectedAttributes[$data['products_options_id']])) {
                ...
                $products_options_values[] = [
                    ...
                    'prices' => (!empty($pInfo->options_templates_id) ?
                        \common\helpers\Attributes::get_template_attributes_prices($data['options_templates_attributes_id'], (float)$_tax) :
                        \common\helpers\Attributes::get_attributes_prices($data['products_attributes_id'], (float)$_tax)),
