...
use yii;
class Save_Attributes_And_Inventory
{
    protected $product;
    ...
    public function save()
    {
        ...
        if (is_array($products_attributes_id)) {
            ...
            foreach ($attributes_array as $val) {
                ...
                $sql_data_array = [
                    'price_prefix' => self::get_from_post_arrays(['post' => 'inventorypriceprefix_' . $old_products_id . '-' . $option_id . '-' . $value_id, 'dbdef' => '+'], (int) $_def_curr_id),
                    'options_values_price' => self::get_from_post_arrays(['post' => 'products_group_price_' . $old_products_id . '-' . $option_id . '-' . $value_id, 'dbdef' => '0'], (int) $_def_curr_id),
                ...
                if ($ext = \common\helpers\Extensions::is_allowed('TypicalOperatingTemp')) {
                    $_ext_data = $ext::save_product_attribute($option_id, $value_id);
                ...
                */
                ...
                ///group prices
                if ($without_inventory || \common\helpers\Extensions::is_allowed('Inventory') || \common\helpers\Attributes::is_virtual_option($option_id)) {
                    ...
                    if (USE_MARKET_PRICES == 'True' || \common\helpers\Extensions::is_customer_groups_allowed()) {
                        foreach ($currencies_ids as $post_currencies_id => $currencies_id) {
                            foreach ($groups as $groups_id => $non) {
                                $sql_data_array = ['attributes_group_price' => tep_db_prepare_input(self::get_from_post_arrays(['db' => 'attributes_group_price', 'dbdef' => $groups_id == 0 ? 0 : -2, 'post' => 'products_group_price_' . $old_products_id . '-' . $option_id . '-' . $value_id, 'f' => ['self', 'defGroupPrice']], $post_currencies_id, $groups_id)), 'groups_id' => tep_db_prepare_input($groups_id), 'products_attributes_id' => tep_db_prepare_input($products_attributes_id), 'currencies_id' => USE_MARKET_PRICES == 'True' ? tep_db_prepare_input($post_currencies_id) : 0];
            ...
            if (($inventory_ext = \common\helpers\Extensions::is_allowed('Inventory')) && !$without_inventory) {
                ...
                foreach ($inventory_options as $tmp) {
                    ...
                    if (Yii::$app->request->post('inventoryexistent_' . $post_uprid) === null && Yii::$app->request->post('inventorystock_delivery_terms_' . $post_uprid) === null) {
                        ...
                        $check_inventory = \common\models\Inventory::find_one(['products_id' => $db_uprid]);
                        if (is_object($check_inventory)) {
                            $all_inventory_ids_array[] = $check_inventory->inventory_id;
                            $all_inventory_uprids_array[] = $db_uprid;
                            continue;
                        }
                        if (!method_exists($inventory_ext, 'optionGenerateInventoryPartly') || $inventory_ext::option_generate_inventory_partly()) {
                            continue;
                        }
                    }
                    $arr = [];
                    ...
                    $sql_data_array = ['products_name' => tep_db_prepare_input($label), 'non_existent' => tep_db_prepare_input($non_existent)];
                    ...
                    $price = self::get_from_post_arrays(['post' => 'products_group_price_' . $post_uprid, 'dbdef' => '0'], (int) $_def_curr_id);
                    if ($products_price_full) {
                        ...
                        if ($ext = \common\helpers\Acl::check_extension_allowed('AttributesQuantity', 'allowed')) {
                            ...
                            $sql_data_array['inventory_discount_full_price'] = self::get_from_post_arrays(['db' => 'inventory_discount_full_price', 'dbdef' => '', 'post' => 'discount_price' . $post_uprid, 'flag' => 'qty_discount_status' . $post_uprid, 'f' => ['self', 'formatDiscountString']], $_def_curr_id);
                        }
                    } else {
                        $sql_data_array['price_prefix'] = tep_db_prepare_input(self::get_from_post_arrays(['post' => 'inventorypriceprefix_' . $post_uprid, 'dbdef' => '+'], (int) $_def_curr_id, 0));
                        $sql_data_array['inventory_price'] = tep_db_prepare_input($price);
                        if ($ext = \common\helpers\Acl::check_extension_allowed('AttributesQuantity', 'allowed')) {
                            //2do nice to have check parent q-ty discount threshholds
                            $sql_data_array['inventory_discount_price'] = self::get_from_post_arrays(['db' => 'inventory_discount_price', 'dbdef' => '', 'post' => 'discount_price' . $post_uprid, 'flag' => 'qty_discount_status' . $post_uprid, 'f' => ['self', 'formatDiscountString']], $_def_curr_id);
                        }
                    }
                    $sql_data_array['supplier_price_manual'] = self::get_from_post_arrays(['dbdef' => 'null', 'post' => 'supplier_auto_price_' . $post_uprid], (int) $_def_curr_id, 0);
                    ...
                    if (USE_MARKET_PRICES == 'True' || \common\helpers\Extensions::is_customer_groups_allowed()) {
                        foreach ($currencies_ids as $post_currencies_id => $currencies_id) {
                            ...
                            foreach ($groups as $groups_id => $non) {
                                $sql_data_array = ['inventory_id' => tep_db_prepare_input($inventory_id), 'products_id' => tep_db_prepare_input($db_uprid), 'prid' => tep_db_prepare_input($products_id), 'groups_id' => tep_db_prepare_input($groups_id), 'currencies_id' => tep_db_prepare_input($currencies_id)];
                                $price = self::get_from_post_arrays(['post' => 'products_group_price_' . $post_uprid, 'dbdef' => (int) $groups_id == 0 ? 0 : '-2', 'f' => ['self', 'defGroupPrice']], (int) $post_currencies_id, $groups_id);
                                if ($products_price_full) {
                                    //inventory_full_price, inventory_discount_full_price,
                                    $sql_data_array['inventory_full_price'] = tep_db_prepare_input($price);
                                    if ($ext = \common\helpers\Acl::check_extension_allowed('AttributesQuantity', 'allowed')) {
                                        //2do nice to have check parent q-ty discount thresholds
                                        $sql_data_array['inventory_discount_full_price'] = self::get_from_post_arrays(['dbdef' => '', 'post' => 'discount_price' . $post_uprid, 'flag' => 'qty_discount_status' . $post_uprid, 'f' => ['self', 'formatDiscountString']], $post_currencies_id, $groups_id);
                                    }
                                } else {
                                    //inventory_group_price, inventory_group_discount_price,
                                    $sql_data_array['inventory_group_price'] = tep_db_prepare_input($price);
                                    $sql_data_array['price_prefix'] = tep_db_prepare_input(self::get_from_post_arrays(['post' => 'inventorypriceprefix_' . $post_uprid, 'dbdef' => '+'], (int) $post_currencies_id, $groups_id));
                                    if ($ext = \common\helpers\Acl::check_extension_allowed('AttributesQuantity', 'allowed')) {
                                        //2do nice to have check parent q-ty discount threshholds
                                        $sql_data_array['inventory_group_discount_price'] = self::get_from_post_arrays(['dbdef' => '', 'post' => 'discount_price' . $post_uprid, 'flag' => 'qty_discount_status' . $post_uprid, 'f' => ['self', 'formatDiscountString']], $post_currencies_id, $groups_id);
                                    }
                                }
                                $sql_data_array['supplier_price_manual'] = self::get_from_post_arrays(['db' => 'supplier_price_manual', 'dbdef' => 'null', 'post' => 'supplier_auto_price_' . $post_uprid], (int) $post_currencies_id, $groups_id);
                                if ($groups_id == 0) {
                                    // posted auto, make manual
                                    $sql_data_array['supplier_price_manual'] = $sql_data_array['supplier_price_manual'] == '1' ? 0 : 1;
                                    // reset matched with current config
                    ...
                    if ($ext = \common\helpers\Acl::check_extension_allowed('UserGroupsRestrictions', 'allowed')) {
                        $ext::save_inventory($db_uprid, $post_uprid);
                    }
                    if ($ext = \common\helpers\Acl::check_extension_allowed('ProductStockFlagsByPlatform', 'allowed')) {
                        $ext::save_inventory($db_uprid, $post_uprid);
                    }
                    if ($ext = \common\helpers\Acl::check_extension_allowed('AttributesDetails', 'allowed')) {
                        $ext::save_product($inventory_id, $post_uprid);
                    }
                    ...
                    if ($ext_scl = \common\helpers\Acl::check_extension_allowed('StockControl', 'allowed')) {
                        $ext_scl::save_attributes_and_inventory_save($db_uprid);
                    }
                }
                \common\models\Inventory::delete_all(['AND', ['prid' => (int) $products_id], ['NOT IN', 'inventory_id', $all_inventory_ids_array]]);
                \common\models\Inventory_Prices::delete_all(['AND', ['prid' => (int) $products_id], ['NOT IN', 'inventory_id', $all_inventory_ids_array]]);
                if ($groups_inventory = \common\helpers\Acl::check_extension_table_exist('UserGroupsRestrictions', 'GroupsInventory', 'allowed')) {
                    $groups_inventory::delete_all(['AND', ['prid' => (int) $products_id], ['NOT IN', 'products_id', $all_inventory_uprids_array]]);
                }
                $all_inventory_uprids_array[] = trim((int) $products_id);
                \common\models\Warehouses_Products::delete_all(['AND', ['prid' => (int) $products_id], ['NOT IN', 'products_id', $all_inventory_uprids_array]]);
                if ($platform_products_stock_flags = \common\helpers\Acl::check_extension_table_exist('ProductStockFlagsByPlatform', 'PlatformProductsStockFlags', 'allowed')) {
                    $platform_products_stock_flags::delete_all(['AND', ['prid' => (int) $products_id], ['NOT IN', 'products_id', $all_inventory_uprids_array]]);
                }
            } elseif (\common\helpers\Extensions::is_allowed('Inventory') && $without_inventory) {
                \common\models\Inventory::delete_all(['prid' => (int) $products_id]);
                \common\models\Inventory_Prices::delete_all(['prid' => (int) $products_id]);
                \common\models\Warehouses_Products::delete_all(['AND', ['prid' => (int) $products_id], ['!=', 'products_id', trim((int) $products_id)]]);
            }
            if (\common\helpers\Extensions::is_allowed('Inventory')) {
                $inventory_quantity = tep_db_fetch_array(tep_db_query('SELECT SUM(products_quantity) AS left_quantity ' . 'FROM ' . TABLE_INVENTORY . ' ' . "WHERE prid = '" . (int) $products_id . "' AND IFNULL(non_existent,0)=0 " . ' AND products_quantity>0'));
        ...
        if (!empty($switch_off_stock_ids)) {
            tep_db_query('update ' . TABLE_PRODUCTS . ' ' . 'set products_status = 0 ' . "where products_id = '" . (int) $products_id . "' " . ' AND products_quantity<=0 ' . " AND stock_indication_id IN ('" . implode("','", $switch_off_stock_ids) . "')");
        }
        $settings = \common\helpers\Product::get_settings($products_id);
        if ($settings) {
            ...
            $settings->save_settings($products_id, ['show_attributes_quantity' => $saq]);
        }
