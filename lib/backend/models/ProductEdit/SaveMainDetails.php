...
class Save_Main_Details
{
    ...
    public function prepare_save()
    {
        ...
        } elseif (!\common\helpers\Extensions::is_allowed('Inventory')) {
        ...
        if ($ext = \common\helpers\Extensions::is_allowed('AutomaticallyStatus')) {
            $_sql_data = $ext::on_product_save();
        ...
        $brand_name = tep_db_prepare_input(Yii::$app->request->post('brand'));
        if (empty($brand_name)) {
            $sql_data_array['manufacturers_id'] = 0;
        } else {
            $brands_query = tep_db_query('select manufacturers_id from ' . TABLE_MANUFACTURERS . " where manufacturers_name = '" . tep_db_input($brand_name) . "'");
        ...
        $sql_data_array['product_unit_label'] = \common\helpers\Product::get_unit_label_list(true, Yii::$app->request->post('product_unit_label', ''), false);
        $products_date_available = Yii::$app->request->post('products_date_available');
        if (!empty($products_date_available)) {
            $sql_data_array['products_date_available'] = \common\helpers\Date::prepare_input_date($products_date_available);
        } else {
        ...
        if (!empty($products_new_until)) {
            $sql_data_array['products_new_until'] = \common\helpers\Date::prepare_input_date($products_new_until);
        } else {
        ...
        $this->product->set_attributes(['stock_control' => 0], false);
