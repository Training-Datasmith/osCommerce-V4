...
class Catalog_Properties extends Properties implements Import_Interface, Export_Interface
{
    protected $fields = [];
    protected $data = [];
    /**
    ...
    {
        $this->fields = [];
        $this->fields[] = ['name' => 'key_field', 'value' => 'EXISTING_REF', 'is_key' => true, 'calculated' => true, 'get' => 'get_key_field'];
        ...
        foreach (\common\helpers\Language::get_languages() as $_lang) {
            ...
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'properties_name', '_is_name_column' => true, 'name' => 'properties_name_' . $_lang['code'], 'value' => 'Properties Name ' . $_lang['code']];
            $data_descriptor = '@|property_values|' . $_lang['id'];
            $this->fields[] = ['data_descriptor' => $data_descriptor, '_lang_id' => $_lang['id'], '_is_value_column' => true, 'name' => 'value_' . $_lang['code'], 'value' => 'Values ' . $_lang['code'], 'set' => 'set_values', 'get' => 'get_values'];
        }
        $this->fields[] = ['name' => 'properties_type', 'value' => 'Type'];
        $this->fields[] = ['name' => 'sort_order', 'value' => 'Sort Order'];
        $this->fields[] = ['name' => 'multi_choice', 'value' => 'Is multi choice?', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        $this->fields[] = ['name' => 'multi_line', 'value' => 'Is multi line?', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        $this->fields[] = ['name' => 'decimals', 'value' => 'Format (decimals places)'];
        $this->fields[] = ['name' => 'display_product', 'value' => 'Display mode: Product info', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        $this->fields[] = ['name' => 'display_listing', 'value' => 'Display mode: Listing', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        $this->fields[] = ['name' => 'display_filter', 'value' => 'Display mode: Filter', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        $this->fields[] = ['name' => 'display_search', 'value' => 'Display mode: Search', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        $this->fields[] = ['name' => 'display_compare', 'value' => 'Display mode: Compare', 'get' => 'get_flag_yes_no', 'set' => 'set_flag_yes_no'];
        foreach (\common\helpers\Language::get_languages() as $_lang) {
            $data_descriptor = '%|' . TABLE_PROPERTIES_DESCRIPTION . '|' . $_lang['id'];
            /*$this->fields[] = array(
            ...
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'properties_name_alt', 'name' => 'properties_name_alt_' . $_lang['code'] . '_0', 'value' => 'Properties Alternative Name ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'properties_description', 'name' => 'properties_description_' . $_lang['code'] . '_0', 'value' => 'Properties Description ' . $_lang['code']];
            /*$this->fields[] = array(
                'data_descriptor' => $data_descriptor,
            ...
            $this->fields[] = ['data_descriptor' => $data_descriptor, '_lang_id' => $_lang['id'], 'column_db' => 'properties_units_id', 'name' => 'properties_units_id_' . $_lang['code'] . '_0', 'value' => 'Units ' . $_lang['code'], 'get' => 'get_units', 'set' => 'set_units'];
        }
    }
    public function get_key_field($field_data, $properties_id, $value_id = '')
    {
    ...
    private function fetch_properties_data($_prop_id)
    {
        $main_source = $this->main_source;
        $export_columns = $this->export_columns;
        $data_sources = $this->data_sources;
        $main_sql = "SELECT {$main_source['select']} properties_id " . 'FROM ' . TABLE_PROPERTIES . ' ' . "WHERE properties_id='{$_prop_id}' " . '';
        ...
        if ($this->data = tep_db_fetch_array($query)) {
            ...
            foreach ($export_columns as $db_key => $export) {
                if (isset($export['get']) && method_exists($this, $export['get'])) {
                    $this->data[$db_key] = call_user_func_array([$this, $export['get']], [$export, $this->data['properties_id']]);
                }
            }
        }
        return $this->data;
    ...
    public function export_row()
    {
        ...
        if ($this->last_properties_id != $_prop_id) {
            $this->data = $this->fetch_properties_data($_prop_id);
        ...
        if (count($value_sources) == 0 || !isset($this->data['_values']) || count($this->data['_values']) == 0) {
            ...
            foreach ($value_sources as $value_column_name) {
                ...
                if (!isset($values_data[$_column_lang_id])) {
                    continue;
                }
    ...
    public function export(Formatter\Formatter_Interface $output, $selected_fields, $filter)
    {
        ...
        foreach (\common\helpers\Properties::get_properties_tree(isset($filter['properties_id']) && $filter['properties_id'] > 0 ? $filter['properties_id'] : 0, '', '', false) as $_prop_variant) {
            ...
            while ($this->data = tep_db_fetch_array($query)) {
                ...
                foreach ($export_columns as $db_key => $export) {
                    if (isset($export['get']) && method_exists($this, $export['get'])) {
                        $write_data[$db_key] = call_user_func_array([$this, $export['get']], [$export, $this->data['properties_id']]);
                    }
                    $write_data[$db_key] = isset($this->data[$db_key]) ? $this->data[$db_key] : '';
                }
                // {{
                if (empty($this->data['key_field'])) {
                    continue;
                }
                // }}
    ...
    public function import_row($data, Messages $message)
    {
        ...
        if ($have_property_name) {
            ...
            foreach ($main_source['columns'] as $file_column => $db_column) {
                ...
                if (isset($export_columns[$db_column]['set']) && method_exists($this, $export_columns[$db_column]['set'])) {
                    call_user_func_array([$this, $export_columns[$db_column]['set']], [$export_columns[$db_column], $this->data['properties_id']]);
                }
                $update_data_array[$db_column] = $this->data[$file_column];
            }
            if (isset($update_data_array['properties_type'])) {
                $update_data_array['properties_type'] = strtolower(trim($update_data_array['properties_type']));
            }
            if (isset($update_data_array['properties_type']) && !in_array($update_data_array['properties_type'], ['text', 'number', 'interval', 'flag', 'file', 'category'])) {
            ...
            if (!isset($this->clean_up[intval($this->data['properties_id'])])) {
                $this->clean_up[intval($this->data['properties_id'])] = [];
            ...
            $this->last_property_data = tep_db_fetch_array(tep_db_query('SELECT * FROM ' . TABLE_PROPERTIES . " WHERE properties_id='" . $this->last_properties_id . "'"));
            foreach ($data_sources as $source_key => $source_data) {
                if ($source_data['table']) {
                    $new_data = [];
                    foreach ($source_data['columns'] as $file_column => $db_column) {
                        ...
                        if (isset($export_columns[$file_column]['set']) && method_exists($this, $export_columns[$file_column]['set'])) {
                            call_user_func_array([$this, $export_columns[$file_column]['set']], [$export_columns[$file_column], $this->data['properties_id']]);
                        }
                    ...
                    if ($source_data['table'] == TABLE_PROPERTIES_DESCRIPTION) {
                        ...
                        $insert_pk = ['properties_id' => $this->last_properties_id, 'language_id' => $source_data['params'][0]];
        ...
        if ($this->last_properties_id && $have_property_value) {
            ...
            if ($properties_type == 'flag' || $properties_type == 'category') {
                ...
                if ($values_id) {
                    $check_values_id = tep_db_fetch_array(tep_db_query('SELECT COUNT(*) AS c FROM ' . TABLE_PROPERTIES_VALUES . " WHERE values_id='" . (int) $values_id . "' AND properties_id='" . $this->last_properties_id . "' "));
                    if ($check_values_id['c'] == 0) {
                        $values_id = 0;
                    }
                ...
                foreach ($value_sources as $value_field_name) {
                    ...
                    if ($properties_type == 'interval') {
                        ...
                        $value_data = ['values_id' => $values_id, 'properties_id' => $this->last_properties_id, 'language_id' => $export_columns[$value_field_name]['_lang_id'], 'values_text' => trim($value), 'values_number' => (float) number_format(floatval(trim($from)), $prop_decimals, '.', ''), 'values_number_upto' => (float) number_format(floatval(trim($upto)), $prop_decimals, '.', '')];
                        /*tep_db_query(
                        ...
                        $value_data = ['values_id' => $values_id, 'properties_id' => $this->last_properties_id, 'language_id' => $export_columns[$value_field_name]['_lang_id'], 'values_text' => trim($value), 'values_number' => (float) number_format(floatval(trim($value)), $prop_decimals, '.', ''), 'values_number_upto' => 0];
                        /*tep_db_query(
                        ...
                        $value_data = ['values_id' => $values_id, 'properties_id' => $this->last_properties_id, 'language_id' => $export_columns[$value_field_name]['_lang_id'], 'values_text' => trim($value), 'values_number' => 0, 'values_number_upto' => 0];
                        /*tep_db_query(
    ...
    public function post_process(Messages $message)
    {
        ...
        $message->info('Done');
        $this->e_ptools->done('properties_settings_import');
    }
    public function import(Formatter\Formatter_Interface $input, EP\Messages $message)
    {
        while ($data = $input->read_array()) {
            $this->import_row($data, $message);
        }
    }
    protected function property_values()
    {
        if (isset($this->data['_values'])) {
            return;
        }
        $this->data['_values'] = [];
        $get_values_r = tep_db_query('SELECT * ' . 'FROM ' . TABLE_PROPERTIES_VALUES . ' ' . "WHERE properties_id='" . intval($this->data['properties_id']) . "' " . 'ORDER BY language_id ');
        if (tep_db_num_rows($get_values_r) > 0) {
            while ($get_value = tep_db_fetch_array($get_values_r)) {
                ...
                if (!isset($this->data['_values'][$id])) {
                    $this->data['_values'][$id] = [];
                }
                $this->data['_values'][$id][$_lang_id] = ['values_text' => $get_value['values_text'], 'values_number' => $get_value['values_number'], 'values_number_upto' => $get_value['values_number_upto'], 'values_alt' => $get_value['values_alt']];
            }
        }
        ...
        if (!empty($value)) {
            if (!isset($prefetch[$key])) {
                ...
                if (tep_db_num_rows($get_id_r) > 0) {
                    ...
                    tep_db_perform(TABLE_PROPERTIES_UNITS, ['properties_units_title' => $value]);
