...
class Order extends ProviderAbstract implements ExportInterface, ImportInterface
{
    ...
    protected function initFields()
    {
        ...
        foreach ($this->getOrderTotalModules() as $totalCode => $totalTitle) {
            $this->fields[] = ['name' => 'totals.'.$totalCode.'.title', 'value' => 'Total: '.$totalTitle .' (Title)'];
            $this->fields[] = ['name' => 'totals.'.$totalCode.'.value_exc_vat', 'value' => 'Total: '.$totalTitle.' (value exl vat)'];
            $this->fields[] = ['name' => 'totals.'.$totalCode.'.value_inc_tax', 'value' => 'Total: '.$totalTitle.' (value inc vat)'];
        }
        // }} totals
...
<div class="widget box box-no-shadow">
    ...
    protected function assignCustomer(\common\models\Orders $order)
    {
        ...
        if (!$customer->customers_default_address_id) {
            ...
            if (!$orderCustomer['customers_country_id']) {
                $orderCustomer['customers_country_id'] = $PlatformAddress['country_id'];
            }
    ...
    protected function persistCollectedOrder(Messages $message)
    {
        ...
        if ($orderModel->isNewRecord) {
            ...
            if (!isset($this->collectOrder['customer']) || count(array_intersect_key($this->collectOrder['customer'], array_flip($address_columns))) == 0) {
                if (!isset($this->collectOrder['customer'])) {
                    $this->collectOrder['customer'] = [];
                }
        ...
        foreach (['customer','billing','delivery'] as $copyKey) {
            if (!isset($this->collectOrder[$copyKey])) {
                continue;
            }
        ...
        if (empty($orderModel->currency)) {
            ...
            if (empty($orderModel->currency)) {
                $orderModel->currency = \Yii::$app->get('platform')->getConfig(\common\classes\platform::defaultId())->getDefaultCurrency();
            }
        }
        ...
        if (!empty($this->collectOrder['info']['currency_value'])) {
            $orderModel->currency_value = $this->collectOrder['info']['currency_value'];
        }
        ...
        if (isset($this->collectOrder['info']) && is_array($this->collectOrder['info'])) {
            foreach ($this->collectOrder['info'] as $_key => $_val) {
                ...
                if ($orderModel->canSetProperty($_key)) {
                    $orderModel->setAttribute($_key, $_val);
                }
            }
        }
        ...
        foreach ($this->collectOrder['products'] as $_iidx => $importedProduct) {
            ...
            if ($importedProduct['matched'] ?? null) {
                continue;
            }
        ...
        if (!is_array($this->collectOrder['totals'])) {
            $this->collectOrder['totals'] = [];
        }
        $importTotals = $this->collectOrder['totals'];
        $totalPaidAmount = 0;
        foreach ($importTotals as $__idx => $_totalInfo) {
            if (strlen($_totalInfo['value_exc_vat'] ?? null) > 0) {
                $importTotals[$__idx]['value_exc_vat'] = number_format($_totalInfo['value_exc_vat'], 6, '.', '');
            }
            if (strlen($_totalInfo['value_inc_tax'] ?? null) > 0) {
                $importTotals[$__idx]['value_inc_tax'] = number_format($_totalInfo['value_inc_tax'], 6, '.', '');
            }
        ...
        if (isset($this->collectOrder['transaction']) && is_array($this->collectOrder['transaction'])) {
            if (!empty($this->collectOrder['transaction']['transaction_id'])) {
                if (OrdersTransactions::find()
                    ...
                    if ($transactionDate < 1971) {
                        $transactionDate = $orderModel->date_purchased;
                    }
