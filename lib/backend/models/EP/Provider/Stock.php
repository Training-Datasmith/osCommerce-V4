...
class Stock extends Provider_Abstract implements Import_Interface, Export_Interface
{
    protected $fields = [];
    protected $data = [];
    protected $e_ptools;
    ...
    {
        $this->fields[] = ['name' => 'products_id', 'value' => 'STOCK_KEY', 'prefix' => 'i', 'is_key' => true];
        $this->fields[] = ['name' => 'products_quantity', 'value' => 'Quantity', 'calculated' => true];
        $this->fields[] = ['name' => 'products_name', 'value' => 'Name', 'calculated' => true];
        $this->fields[] = ['name' => 'products_model', 'value' => 'Model', 'calculated' => true];
        /*
        ...
        */
        $this->fields[] = ['name' => 'p_products_model', 'value' => 'Main Model', 'calculated' => true];
        /*
    ...
    public function prepare_export($use_columns, $filter)
    {
        ...
        $main_sql = "SELECT {$main_source['select']} IFNULL(i.products_id, p.products_id) AS products_id, " . ' IFNULL(i.products_quantity, p.products_quantity) AS products_quantity, ' . ' IFNULL(i.products_name, pd.products_name) AS products_name, ' . ' IFNULL(i.products_model, p.products_model) AS products_model, ' . ' IFNULL(i.products_ean, p.products_ean) AS products_ean, ' . ' IFNULL(i.products_upc, p.products_upc) AS products_upc, ' . ' IFNULL(i.products_asin, p.products_asin) AS products_asin, ' . ' IFNULL(i.products_isbn, p.products_isbn) AS products_isbn, ' . ' p.products_model AS p_products_model, p.products_ean AS p_products_ean, ' . ' p.products_upc AS p_products_upc, p.products_asin AS p_products_asin, p.products_isbn AS p_products_isbn, ' . ' i.inventory_price AS products_price_def/*, i.products_price_discount AS i.products_price_discount_def*/ ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . ' LEFT JOIN ' . TABLE_INVENTORY . ' i ON p.products_id=i.prid ' . ' LEFT JOIN ' . TABLE_PRODUCTS_DESCRIPTION . " pd ON pd.products_id=p.products_id AND pd.language_id='" . $this->languages_id . "' AND pd.platform_id='" . intval(\common\classes\platform::default_id()) . "' " . "WHERE 1 {$filter_sql} " . '  AND p.parent_products_id=0 ' . "ORDER BY IFNULL(i.prid,p.products_id), IFNULL(i.products_id,'') " . '/*LIMIT 3*/';
    ...
    public function import_row($data, Messages $message)
    {
        ...
        $get_main_data_r = tep_db_query('SELECT ' . ' p.products_id, p.products_quantity AS p_qty, ' . ' i.inventory_id, i.products_quantity AS i_qty, i.products_id AS uprid ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . ' LEFT JOIN ' . TABLE_INVENTORY . ' i ON i.prid=p.products_id ' . "WHERE IFNULL(i.products_id, p.products_id)='" . tep_db_input($file_primary_value) . "' ");
        ...
        if ($found_rows > 1) {
            ...
            $lookup_product_conditions = [];
            if (isset($this->data['p_products_model']) && !empty($this->data['p_products_model'])) {
                $lookup_product_conditions[] = "p.products_model='" . tep_db_input($this->data['p_products_model']) . "'";
            }
            if (isset($this->data['p_products_ean']) && !empty($this->data['p_products_ean'])) {
                $lookup_product_conditions[] = "p.products_ean='" . tep_db_input($this->data['p_products_ean']) . "'";
            }
            if (isset($this->data['p_products_upc']) && !empty($this->data['p_products_upc'])) {
                $lookup_product_conditions[] = "p.products_upc='" . tep_db_input($this->data['p_products_upc']) . "'";
            }
            if (isset($this->data['p_products_asin']) && !empty($this->data['p_products_asin'])) {
                $lookup_product_conditions[] = "p.products_asin='" . tep_db_input($this->data['p_products_asin']) . "'";
            }
            if (isset($this->data['p_products_isbn']) && !empty($this->data['p_products_isbn'])) {
                $lookup_product_conditions[] = "p.products_isbn='" . tep_db_input($this->data['p_products_isbn']) . "'";
            }
            $file_assigned_options = [];
            foreach (preg_grep('/^option_\d+$/', array_keys($this->data)) as $option_key) {
                if (empty($this->data[$option_key])) {
                    continue;
                }
                $option_id = (int) substr($option_key, 7);
                $value_id = $this->e_ptools->get_option_value_by_name($option_id, $this->data[$option_key]);
                $file_assigned_options[$option_id] = $value_id;
            }
            ksort($file_assigned_options);
            if (count($lookup_product_conditions) > 0) {
                $get_product_r = tep_db_query('SELECT p.products_id ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . 'WHERE 1 AND (' . implode(' OR ', $lookup_product_conditions) . ') ');
                if (tep_db_num_rows($get_product_r) == 1) {
                    ...
                    $get_main_data_r = tep_db_query('SELECT * FROM ' . TABLE_INVENTORY . " WHERE products_id='" . tep_db_input($uprid) . "'");
                    $found_rows = tep_db_num_rows($get_main_data_r);
                    if ($found_rows == 1) {
        ...
        if ($db_main_data['inventory_id']) {
            ...
            if ($quantity >= 0) {
                $inventory_id = trim($db_main_data['uprid']);
                \common\models\Warehouses_Products::delete_all(['prid' => (int) $inventory_id, 'products_id' => $inventory_id]);
                $location_id = 0;
                foreach (\common\helpers\Product::get_warehouse_id_priority_array($inventory_id, $quantity, false) as $warehouse_id) {
                    foreach (\common\helpers\Product::get_supplier_id_priority_array($inventory_id) as $supplier_id) {
                        $quantity = \common\helpers\Warehouses::update_products_quantity($inventory_id, $warehouse_id, $quantity, $quantity > 0 ? '+' : '-', $supplier_id, $location_id, ['comments' => 'EP Stock feed update', 'admin_id' => $login_id]);
                        if ($quantity >= 0) {
                            \common\helpers\Product::do_cache($inventory_id);
                            $this->stock_updated++;
                        }
                        break 2;
                    }
            ...
            //if ($products_quantity_update > 0) {
            if ($quantity >= 0) {
                $product_id = trim((int) $db_main_data['products_id']);
    ...
    public function get_products_option_value($field_data, $products_id)
    {
        if (strpos($field_data['name'], 'option_') !== 0) {
            return '';
        }
        $id = intval(substr($field_data['name'], 7));
        if (strpos($products_id, '{' . $id . '}') !== false && preg_match('/\{' . $id . '\}(\d+)/', $products_id, $match_value_id)) {
            return $this->e_ptools->get_option_value_name($match_value_id[1], $this->languages_id);
        }
    }
}