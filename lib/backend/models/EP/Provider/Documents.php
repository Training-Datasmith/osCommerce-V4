...
class Documents extends Provider_Abstract implements Import_Interface, Export_Interface
{
    ...
    public function prepare_export($use_columns, $filter)
    {
        ...
        $filter_sql = '';
        if (is_array($filter)) {
            $this->with_files = isset($filter['with_images']) && $filter['with_images'];
        ...
        $main_sql = "SELECT {$main_source['select']} p.products_id " . 'FROM ' . TABLE_PRODUCTS . ' p ' . '  INNER JOIN ' . TABLE_PRODUCTS_DOCUMENTS . ' pd ON pd.products_id=p.products_id ' . "WHERE 1 {$filter_sql} " . 'GROUP BY p.products_id ' . '/*LIMIT 3*/';
    ...
    public function export_row()
    {
        ...
        if (!$this->data) {
            ...
            $this->rows_data = [];
            $product = \common\api\models\AR\Products::find_one($this->product_data['products_id']);
            $product_documents = $product->export_array(['documents' => ['*' => ['*']]]);
            foreach ($product_documents['documents'] as $product_document) {
                $document_row = Array_Transform::convert_multi_dimensional_to_flat(['document' => $product_document]);
        ...
        if ($this->with_files) {
            $files_add = [];
            $document_filename = rtrim(DIR_FS_CATALOG, '/') . '/' . 'documents/' . $this->data['document.filename'];
            if (!$this->data['document.is_link'] && !empty($this->data['document.filename']) && is_file($document_filename)) {
                $files_add[] = ['filename' => $document_filename, 'localname' => 'documents/' . $this->data['document.filename']];
            }
    ...
    public function import_row($data, Messages $message)
    {
        ...
        if (!isset($this->rows_data[$file_primary_value])) {
            $this->process_collected_data();
            $this->rows_data = [];
            $this->rows_data[$file_primary_value] = ['model' => false, 'documents' => []];
            $matched_products = \common\api\models\AR\Products::find()->where([$file_primary_column => $file_primary_value])->all();
            if (count($matched_products) != 1) {
                ...
                $this->rows_data[$file_primary_value]['model'] = $matched_products[0];
            }
        }
        if (!is_object($this->rows_data[$file_primary_value]['model'])) {
            return false;
        }
        $doc_data = Array_Transform::convert_flat_to_multi_dimensional($this->data);
        ...
        if (isset($doc_data['document']['filename']) && !empty($doc_data['document']['filename'])) {
            if (!preg_match('/^[a-z]{3,4}:\/\//i', $doc_data['document']['filename'])) {
                if (is_file($this->directory_obj->files_root() . 'documents/' . $doc_data['document']['filename'])) {
                    $doc_data['document']['document_url'] = $this->directory_obj->files_root() . 'documents/' . $doc_data['document']['filename'];
                } elseif (is_file($this->directory_obj->files_root() . $doc_data['document']['filename'])) {
                    $doc_data['document']['document_url'] = $this->directory_obj->files_root() . $doc_data['document']['filename'];
                }
            }
        }
    ...
    protected function process_collected_data()
    {
        if (is_array($this->rows_data) && count($this->rows_data) > 0) {
            foreach ($this->rows_data as $model => $data) {
                ...
                $data['model']->import_array(['documents' => $data['documents']]);
                $data['model']->save();
                $this->entry_counter++;
            }
        }
