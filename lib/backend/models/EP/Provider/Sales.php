...
class Sales extends Provider_Abstract implements Import_Interface, Export_Interface
{
    ...
    public function init()
    {
        ...
        if (is_array($groups) && !empty($groups)) {
            ...
            $groups = array_filter($groups, function ($el) {
                return $el['groups_discount'] > 0 && $el['apply_groups_discount_to_specials'];
            });
            if (is_array($groups) && !empty($groups)) {
                $ret = sprintf($ret, "\n" . constant('TEXT_GROUP_WITH_SALE_DISCOUNT') . implode(', ', \yii\helpers\Array_Helper::map($groups, 'groups_name', 'groups_discount')));
            } else {
    ...
    public function get_export_description()
    {
    ...
    protected function init_fields()
    {
        ...
        $check_uniq_key = \Yii::$app->get_db()->create_command("select count(*) as c from products group by products_model having count(*)>1 or products_model=''")->query_scalar();
        if ($check_uniq_key) {
            $this->fields[] = ['name' => 'products_id', 'value' => 'Products Id', 'is_key' => true, 'is_key_part' => true, 'calculated' => true];
        ...
        $this->fields[] = ['name' => 'status', 'value' => 'Status', 'type' => 'int'];
        $this->fields[] = ['name' => 'specials_enabled', 'value' => 'Manually Enabled', 'type' => 'int'];
        $this->fields[] = ['name' => 'specials_disabled', 'value' => 'Manually Disabled', 'type' => 'int'];
        $this->fields[] = ['name' => 'start_date', 'value' => 'Start Date', 'get' => 'get_formatted_datetime', 'set' => 'set_formatted_datetime'];
        $this->fields[] = ['name' => 'expires_date', 'value' => 'Expires Date', 'get' => 'get_formatted_datetime', 'set' => 'set_formatted_datetime'];
        $this->fields[] = ['name' => 'total_qty', 'value' => TEXT_MAX_QTY_TO_SELL, 'type' => 'int'];
        $this->fields[] = ['name' => 'max_per_order', 'value' => TEXT_MAX_QTY_TO_SELL_IN_ORDER, 'type' => 'int'];
        $this->fields[] = ['name' => 'promote_type', 'value' => TEXT_SALE_PROMOTE_EP, 'get' => 'get_promote_type', 'set' => 'set_promote_type'];
        if (defined('USE_MARKET_PRICES') && USE_MARKET_PRICES == 'True') {
            foreach ($currencies->currencies as $key => $value) {
                $data_descriptor = '%|' . TABLE_SPECIALS_PRICES . '|' . $value['id'] . '|0';
                $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'specials_new_products_price', 'name' => 'products_price_' . $value['id'] . '_0', 'value' => 'Sale Price ' . $key . ' ' . TEXT_MAIN, 'get' => 'get_products_price', 'set' => 'set_products_price', 'type' => 'numeric'];
                foreach (\common\helpers\Group::get_customer_groups() as $groups_data) {
                    ...
                    $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'products_group_price', 'name' => 'products_price_' . $value['id'] . '_' . $groups_data['groups_id'], 'value' => 'Sale Price ' . $key . ' ' . $groups_data['groups_name'], 'get' => 'get_products_price', 'set' => 'set_products_price', 'type' => 'numeric'];
                }
            }
        } else {
            $this->fields[] = ['column_db' => 'specials_new_products_price', 'name' => 'products_price_0', 'value' => 'Sale Price' . ' ' . TEXT_MAIN, 'get' => 'get_products_price', 'set' => 'set_products_price', 'type' => 'numeric', 'get' => 'get_products_price_main', 'set' => 'set_products_price_main'];
            foreach (\common\helpers\Group::get_customer_groups() as $groups_data) {
                $data_descriptor = '%|' . TABLE_SPECIALS_PRICES . '|0|' . $groups_data['groups_id'];
                $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'specials_new_products_price', 'name' => 'products_price_' . $groups_data['groups_id'], 'value' => 'Sale Price ' . $groups_data['groups_name'], 'get' => 'get_products_price', 'set' => 'set_products_price', 'type' => 'numeric'];
            }
        }
        $this->fields[] = ['name' => "'_remove'", 'value' => 'Delete (enter 1 to delete)'];
...
<div class="widget box box-no-shadow">
    ...
    public function prepare_export($use_columns, $filter)
    {
        ...
        $main_sql = "SELECT {$main_source['select']} p.products_id, s.specials_id, p.products_tax_class_id AS _tax_id " . 'FROM ' . TABLE_PRODUCTS . ' p join ' . TABLE_SPECIALS . ' s on p.products_id=s.products_id ' . "WHERE 1 {$filter_sql} " . 'order by p.products_id, s.status desc, s.start_date ' . '/*LIMIT 3*/';
    ...
    public function import_row($data, Messages $message)
    {
        ...
        $get_main_data_r = tep_db_query('SELECT p.* ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . "WHERE {$product_where} ");
        ...
        if ($found_rows > 1) {
            ...
            if (count($update_data_array) > 0) {
                ...
                $w_sql = "where products_id='" . (int) $products_id . "'";
                foreach ($update_data_array as $key => $value) {
                    if (in_array($key, ['specials_new_products_price', "'_remove'", '_remove', 'status', 'specials_enabled', 'specials_disabled', 'promote_type', 'total_qty', 'max_per_order'])) {
                        continue;
                    }
                    if (strtolower($value) == 'null') {
                        $w_sql .= " and {$key} is null";
                    } else {
                        $w_sql .= " and {$key} = '" . tep_db_input($value) . "'";
                    }
                }
                //echo "select specials_id, specials_new_products_price, status from " . TABLE_SPECIALS . " " . $wSql;
                $special_q = tep_db_query('select specials_id, specials_new_products_price, status from ' . TABLE_SPECIALS . ' ' . $w_sql . ' order by specials_id desc');
                if (tep_db_num_rows($special_q) > 1) {
                    $message->info('<b>"' . $file_primary_value . '"</b> - several (' . tep_db_num_rows($special_q) . ') sales ' . $w_sql . ' only last one is updated');
                }
                ...
                if (!empty($update_data_array["'_remove'"])) {
                    if (!empty($special['specials_id'])) {
                        ...
                        //not found
                        $message->info('"' . $file_primary_value . '" - not found special: ' . $w_sql);
                        return false;
                    }
                } else {
                    ...
                    if ($update_data_array['start_date'] != 'null' && $update_data_array['expires_date'] != 'null' && $update_data_array['start_date'] > $update_data_array['expires_date'] || empty($special['status']) && $update_data_array['expires_date'] != 'null' && $update_data_array['expires_date'] <= date(\common\helpers\Date::DATABASE_DATETIME_FORMAT)) {
                        $message->info('"' . $file_primary_value . '" - skipped - incorrect start or expire date/time');
                        return false;
                    }
                    $update_data_array['products_id'] = (int) $products_id;
                    if (!empty($special['specials_id'])) {
                        $add_sql = " and specials_id<>'" . (int) $special['specials_id'] . "'";
                    } else {
                        $add_sql = '';
                    }
                    if (defined('SALE_STRICT_DATE') && SALE_STRICT_DATE == 'True') {
                        ...
                        if ($update_data_array['start_date'] != 'null') {
                            //expire other before me
                            $r = tep_db_query('select specials_id, start_date from ' . TABLE_SPECIALS . " where products_id='" . (int) $products_id . "' {$add_sql} " . " and (start_date is null or start_date = '0000-00-00 00:00:00' or start_date<='" . tep_db_input($update_data_array['start_date']) . "') " . " and (expires_date is null or expires_date = '0000-00-00 00:00:00' or expires_date>='" . tep_db_input($update_data_array['start_date']) . "') ");
                            while ($e = tep_db_fetch_array($r)) {
                                if ($e['start_date'] == $update_data_array['start_date']) {
                                    // 2check
                                    //if ==  then new expire date could be < start
                                    $s_u = ", start_date = DATE_SUB('" . tep_db_input($update_data_array['start_date']) . "', INTERVAL 1 SECOND) ";
                                } else {
                                    $s_u = '';
                                }
                                tep_db_query('update ' . TABLE_SPECIALS . " set specials_last_modified=now(), expires_date = DATE_SUB('" . tep_db_input($update_data_array['start_date']) . "', INTERVAL 1 SECOND) {$s_u} where specials_id='" . (int) $e['specials_id'] . "'");
                        ...
                        if ($update_data_array['expires_date'] != 'null') {
                            ...
                            $r = tep_db_query('select specials_id from ' . TABLE_SPECIALS . " where products_id='" . (int) $products_id . "' {$add_sql} " . " and (start_date is null or start_date = '0000-00-00 00:00:00' or start_date<='" . tep_db_input($update_data_array['expires_date']) . "') " . " and (expires_date is null or expires_date = '0000-00-00 00:00:00' or expires_date>='" . tep_db_input($update_data_array['expires_date']) . "') ");
                            while ($e = tep_db_fetch_array($r)) {
                        ...
                        $r = tep_db_query('select specials_id, start_date from ' . TABLE_SPECIALS . " where products_id='" . (int) $products_id . "' {$add_sql} " . " and (status=1 or start_date is null or start_date = '0000-00-00 00:00:00' " . ($update_data_array['start_date'] != 'null' ? " or start_date>='" . tep_db_input($update_data_array['start_date']) . "'" : ' or start_date>now() ') . '     ) ' . " and (status=1 or expires_date is null or expires_date = '0000-00-00 00:00:00' " . ($update_data_array['expires_date'] != 'null' ? " or expires_date<='" . tep_db_input($update_data_array['expires_date']) . "'" : ' or expires_date>now() ') . '     ) ');
                        while ($e = tep_db_fetch_array($r)) {
                    ...
                    if (empty($special['specials_id'])) {
                        ...
                        tep_db_perform(TABLE_SPECIALS_PRICES, ['specials_new_products_price' => $update_data_array['specials_new_products_price'], 'specials_id' => (int) $specials_id, 'groups_id' => 0, 'currencies_id' => 0]);
                        ...
                        if (abs($update_data_array['specials_new_products_price'] - $special['specials_new_products_price']) > 1.0E-5) {
                            tep_db_perform(TABLE_SPECIALS_PRICES, ['specials_new_products_price' => $update_data_array['specials_new_products_price']], 'update', "specials_id='" . (int) $specials_id . "' and groups_id=0 and currencies_id=0");
                            //kostyl no marketing price
                        }
            ...
            foreach ($data_sources as $source_key => $source_data) {
                if ($source_data['table']) {
                    ...
                    if ($source_data['table'] == TABLE_SPECIALS_PRICES) {
                        ...
                        $insert_pk = ['specials_id' => $specials_id, 'currencies_id' => $source_data['params'][0], 'groups_id' => $source_data['params'][1]];
        ...
        if ($is_updated) {
            ...
            if ($ext = \common\helpers\Extensions::is_allowed('ProductPriceIndex')) {
     ...
     */
    public function get_formatted_datetime($field_data, $products_id)
    {
     ...
     * @return string
     */
    public function set_formatted_datetime($field_data, $products_id)
    {
        if ($this->data[$field_data['name']] === '') {
            ...
            $tmp = \common\helpers\Date::prepare_input_date($this->data[$field_data['name']], true);
            // check for '0000'
    ...
    protected function get_tax_rate()
    {
        ...
        if (isset($this->data['_tax_rate'])) {
            ...
            if ($tax_class_id) {
                ...
                $country_id = (int) $default_address['country_id'];
                $zone_id = (int) $default_address['zone_id'];
                $tax_rate = \common\helpers\Tax::get_tax_rate((int) $tax_class_id, $country_id, $zone_id, '', false);
            }
    ...
    public function set_products_price($field_data, $products_id)
    {
        ...
        if ($this->make_net_prices_on_import && floatval($this->data[$field_data['name']]) > 0) {
            if ($tax_rate = $this->get_tax_rate()) {
    ...
    public function get_products_price_main($field_data, $products_id)
    {
    ...
    public function set_products_price_main($field_data, $products_id)
    {
        if ($this->make_net_prices_on_import && floatval($this->data[$field_data['name']]) > 0) {
            if ($tax_rate = $this->get_tax_rate()) {
                $this->data[$field_data['name']] = $this->data[$field_data['name']] * (100 / (100 + $tax_rate));
    ...
    public function get_promote_type($field_data, $products_id)
    {
        if (!isset($this->data[$field_data['name']]) || $this->data[$field_data['name']] === '') {
    ...
    public function set_promote_type($field_data, $products_id)
    {
        if ($this->data[$field_data['name']] === '') {
