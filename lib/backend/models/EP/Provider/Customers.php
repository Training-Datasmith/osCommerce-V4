...
class Customers extends ProviderAbstract implements ImportInterface, ExportInterface
{
    ...
    protected function initFields()
    {
        ...
        foreach ($attr as $key) {
            $columnDescribe = [ 'name' => 'customer.'.$key, 'value' => ucwords(preg_replace('/[ \._]/', ' ', $key)), ];
        ...
        $pattern = [];
        foreach ($attr as $key) {
            $columnDescribe = [ 'name' => 'address.'.$key, 'value' => ucwords(preg_replace('/[ \._]/', ' ', str_replace('entry_', 'address_', $key))), ];
            if (isset($columnCover[$key])) {
                if ($columnCover[$key] == false) {
                    continue;
                }
