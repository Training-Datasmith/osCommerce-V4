...
class Attributes extends Provider_Abstract implements Import_Interface, Export_Interface
{
    protected $fields = [];
    protected $data = [];
    protected $e_ptools;
    protected $_processed_pids = [];
    protected $_not_updated_option_values = [];
    ...
    protected function init_fields()
    {
        ...
        $this->fields = [];
        $this->fields[] = ['name' => 'products_model', 'calculated' => true, 'value' => 'Products Model', 'is_key' => true, 'get' => 'get_product_model'];
        $this->fields[] = ['name' => '_price_info', 'calculated' => true, 'value' => 'Price (info!)', 'get' => 'get_price_info'];
        foreach (\common\helpers\Language::get_languages() as $_lang) {
            $data_descriptor = '%|' . TABLE_PRODUCTS_OPTIONS . '|' . $_lang['id'];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'products_options_name', 'required' => true, 'pre_lookup' => 'options_id', 'name' => 'products_options_name_' . $_lang['code'], 'value' => 'Option Name ' . $_lang['code']];
            $data_descriptor = '%|' . TABLE_PRODUCTS_OPTIONS_VALUES . '|' . $_lang['id'];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'products_options_values_name', 'required' => true, 'pre_lookup' => 'options_values_id', 'name' => 'products_options_values_name_' . $_lang['code'], 'value' => 'Option Value ' . $_lang['code']];
        }
        $this->fields[] = ['name' => 'price_prefix', 'value' => 'Price Prefix'];
        if (defined('USE_MARKET_PRICES') && USE_MARKET_PRICES == 'True') {
            foreach ($currencies->currencies as $key => $value) {
                $data_descriptor = '%|' . TABLE_PRODUCTS_ATTRIBUTES_PRICES . '|' . $value['id'] . '|0';
                $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'attributes_group_price', 'name' => 'attributes_group_price_' . $value['id'] . '_0', 'value' => 'Option Price ' . $key, 'get' => 'get_products_attributes_price', 'set' => 'set_products_attributes_price', 'type' => 'numeric', 'isMain' => DEFAULT_CURRENCY == $key];
                /*
                $this->fields[] = array(
                ...
                */
                foreach (\common\helpers\Group::get_customer_groups() as $groups_data) {
                    ...
                    $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'attributes_group_price', 'name' => 'attributes_group_price_' . $value['id'] . '_' . $groups_data['groups_id'], 'value' => 'Option Price ' . $key . ' ' . $groups_data['groups_name'], 'get' => 'get_products_attributes_price', 'set' => 'set_products_attributes_price', 'type' => 'numeric'];
                    /*
                    $this->fields[] = array(
                      'data_descriptor' => $data_descriptor,
                      'column_db' => 'attributes_group_discount_price',
            ...
            $this->fields[] = ['data_descriptor' => '%|' . TABLE_PRODUCTS_ATTRIBUTES_PRICES . '|0|0', 'column_db' => 'attributes_group_price', 'name' => 'attributes_group_price_0_0', 'value' => 'Option Price', 'get' => 'get_products_attributes_price', 'set' => 'set_products_attributes_price', 'type' => 'numeric', 'isMain' => true];
            /*
            $this->fields[] = array(
              'data_descriptor' => $data_descriptor,
            ...
            */
            foreach (\common\helpers\Group::get_customer_groups() as $groups_data) {
                ...
                $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'attributes_group_price', 'name' => 'attributes_group_price_0_' . $groups_data['groups_id'], 'value' => 'Option Price ' . $groups_data['groups_name'], 'get' => 'get_products_attributes_price', 'set' => 'set_products_attributes_price', 'type' => 'numeric'];
                /*
                $this->fields[] = array(
                  'data_descriptor' => $data_descriptor,
                  'column_db' => 'attributes_group_discount_price',
        ...
        $this->fields[] = ['name' => 'products_attributes_weight_prefix', 'value' => 'Attributes Weight Prefix'];
        $this->fields[] = ['name' => 'products_attributes_weight', 'value' => 'Attributes Weight'];
        $this->fields[] = ['name' => 'default_option_value', 'value' => 'Select default'];
        $this->fields[] = ['name' => 'products_options_sort_order', 'value' => 'Product options sort order'];
        if (\common\helpers\Extensions::is_allowed('TypicalOperatingTemp')) {
            $this->fields[] = ['name' => 'tot_prefix', 'value' => 'TOT Prefix'];
            $this->fields[] = ['name' => 'tot_value', 'value' => 'TOT Value'];
        }
        $this->fields[] = ['name' => 'products_attributes_filename', 'value' => 'Product Attribute\'s File'];
        $this->fields[] = ['name' => 'products_attributes_maxdays', 'value' => 'Max days'];
        $this->fields[] = ['name' => 'products_attributes_maxcount', 'value' => 'Max count'];
        /*
...
<div class="widget box box-no-shadow">
    ...
    <div class="widget-content">
        <div class="row form-group">
            ...
            <div class="col-md-6">' . Html::drop_down_list('import_config[insert_new]', $insert_new, ['insert' => 'Yes, insert new and update', 'replace' => 'No, replace existing'], ['class' => 'form-control']) . '</div>
        </div>
    </div>
</div>
        ';
    ...
    public function prepare_export($use_columns, $filter)
    {
        ...
        $main_sql = "SELECT {$main_source['select']} p.products_model AS main_products_model, " . ' p.without_inventory AS without_inventory, ' . ' p.products_price_full AS products_price_full, ' . ' options_id, options_values_id, products_attributes_id ' . 'FROM ' . TABLE_PRODUCTS_ATTRIBUTES . ' pa, ' . TABLE_PRODUCTS . ' p ' . "WHERE pa.products_id=p.products_id {$filter_sql} " . '  AND p.parent_products_id=0 ' . 'ORDER BY pa.products_id, pa.options_id, pa.options_values_id';
    ...
    public function export_row()
    {
        ...
        foreach ($export_columns as $db_key => $export) {
            if (isset($export['get']) && method_exists($this, $export['get'])) {
                $this->data[$db_key] = call_user_func_array([$this, $export['get']], [$export, $this->data['products_attributes_id']]);
            }
        }
        return $this->data;
    }
    public function import_row($data, Messages $message)
    {
        ...
        if (!isset($this->last_product_lookup[$file_primary_value])) {
            $get_main_data_r = tep_db_query('SELECT products_id, parent_products_id FROM ' . TABLE_PRODUCTS . " WHERE products_model='" . tep_db_input($file_primary_value) . "'");
            ...
            //}}
            $this->last_product_lookup = [$file_primary_value => ['found_rows' => $found_rows, 'data' => $found_data]];
            if ($found_rows > 1) {
        ...
        // {{
        if ($replace_mode && !isset($this->_not_updated_option_values[(int) $this->data['products_id']])) {
            $this->_not_updated_option_values[(int) $this->data['products_id']] = Array_Helper::map(Products_Attributes::find()->select(['products_attributes_id'])->where(['products_id' => $this->data['products_id']])->as_array()->all(), 'products_attributes_id', 'products_attributes_id');
        }
        // }}
        $options_lookup = [];
        ...
        $this->data['options_values_id'] = $this->e_ptools->get_option_value_by_name($this->data['options_id'], $options_values_lookup);
        ...
        $get_main_data_r = tep_db_query("SELECT {$main_source['select']} products_attributes_id " . 'FROM ' . TABLE_PRODUCTS_ATTRIBUTES . ' ' . "WHERE products_id='{$this->data['products_id']}' AND options_id='{$this->data['options_id']}' AND options_values_id='{$this->data['options_values_id']}' ");
        if (tep_db_num_rows($get_main_data_r) > 0) {
            ...
            foreach ($main_source['columns'] as $file_column => $db_column) {
                ...
                if (isset($export_columns[$file_column]['set']) && method_exists($this, $export_columns[$file_column]['set'])) {
                    call_user_func_array([$this, $export_columns[$file_column]['set']], [$export_columns[$file_column], $this->data['products_attributes_id']]);
                }
        ...
        foreach ($data_sources as $source_key => $source_data) {
            if ($source_data['table']) {
                ...
                if ($source_data['table'] == TABLE_PRODUCTS_ATTRIBUTES_PRICES) {
                    ...
                    $insert_pk = ['products_attributes_id' => $products_attributes_id, 'currencies_id' => $source_data['params'][0], 'groups_id' => $source_data['params'][1]];
                ...
                foreach ($source_data['columns'] as $file_column => $db_column) {
                    if (isset($export_columns[$db_column]['set']) && method_exists($this, $export_columns[$db_column]['set'])) {
                        call_user_func_array([$this, $export_columns[$db_column]['set']], [$export_columns[$db_column], $this->data['products_attributes_id']]);
                    }
                }
            }
        }
    ...
    public function post_process(Messages $message)
    {
        ...
        $get_double_defaults_r = tep_db_query('SELECT products_id, options_id, GROUP_CONCAT(products_attributes_id) AS paids ' . 'FROM ' . TABLE_PRODUCTS_ATTRIBUTES . ' ' . 'WHERE default_option_value=1 ' . 'GROUP BY products_id, options_id HAVING COUNT(*)>1');
        if (tep_db_num_rows($get_double_defaults_r) > 0) {
            while ($get_double_default = tep_db_fetch_array($get_double_defaults_r)) {
                ...
                tep_db_query('UPDATE ' . TABLE_PRODUCTS_ATTRIBUTES . ' ' . 'SET default_option_value=0 ' . "WHERE products_id='" . $get_double_default['products_id'] . "' AND options_id='" . $get_double_default['options_id'] . "' " . " AND products_attributes_id!='" . $leave_default . "' ");
            }
        }
        if (count($this->_processed_pids) > 0) {
            if (count($this->_not_updated_option_values) > 0) {
                $removed_count = 0;
                foreach ($this->_not_updated_option_values as $products_id => $not_updated_attributes_ids) {
                    if (count($not_updated_attributes_ids) > 0) {
                        Products_Attributes::delete_all(['AND', ['products_id' => $products_id], ['IN', 'products_attributes_id', $not_updated_attributes_ids]]);
                        $removed_count += count($not_updated_attributes_ids);
                    }
                }
                $message->info('Removed ' . (int) $removed_count . ' option values');
            }
            $message->info('Check inventory');
            foreach ($this->_processed_pids as $_pid) {
                $updated[(int) $_pid] = [];
            }
            $get_updated_products_r = tep_db_query('SELECT products_id, options_id, options_values_id ' . 'FROM ' . TABLE_PRODUCTS_ATTRIBUTES . ' ' . "WHERE products_id IN('" . implode("','", $this->_processed_pids) . "') " . 'ORDER BY products_id, options_id, options_values_id');
            $updated = [];
            while ($_updated_product = tep_db_fetch_array($get_updated_products_r)) {
                if (!isset($updated[(int) $_updated_product['products_id']][(int) $_updated_product['options_id']])) {
                    $updated[(int) $_updated_product['products_id']][(int) $_updated_product['options_id']] = [];
                }
        ...
        $message->info('Done');
        $this->e_ptools->done('attributes_import');
    }
    protected function set_products_attributes_price($field_data, $products_attributes_id)
    {
        if (isset($field_data['isMain']) && $field_data['isMain']) {
            $product_attributes = \common\models\Products_Attributes::find_one(['products_attributes_id' => $products_attributes_id]);
            if ($product_attributes) {
                $product_attributes->set_attributes(['options_values_price' => $this->data[$field_data['name']]], false);
                $product_attributes->save(false);
            }
        }
    }
    protected function get_price_info($field_data, $products_id)
    {
        ...
        if (\common\helpers\Extensions::is_allowed('Inventory')) {
            ...
            if ($this->data['without_inventory'] || EP\Tools::get_instance()->is_option_virtual($this->data['options_id'])) {
    ...
    {
        $inventory_disabled = \common\models\Products::find()->where(['products_id' => $products_id])->and_where(['without_inventory' => 1])->count() > 0;
        $uprids = [];
        if (!$inventory_disabled && count($attributes) > 0) {
            $arr1 = [];
            foreach ($attributes as $oid => $val_arr) {
                if (EP\Tools::get_instance()->is_option_virtual($oid)) {
                    continue;
                }
                $arr1[] = self::__add_str($val_arr, '{' . $oid . '}');
            }
            $uprids = self::__get_all_uprid($products_id, $arr1, 0, count($arr1) - 1, []);
        } else {
            //$uprids[] = (string)$products_id; // only with attributes products in inventory
        }
        tep_db_query('DELETE FROM ' . TABLE_INVENTORY . ' ' . "WHERE prid='" . $products_id . "' " . (count($uprids) > 0 ? "AND products_id NOT IN ('" . implode("','", array_map('tep_db_input', $uprids)) . "')" : ''));
        $inventory_name_arr = [];
        ...
        foreach ($uprids as $uprid) {
            ...
            foreach ($matches[2] as $val_id) {
                $uprid_name[] = $this->e_ptools->get_option_value_name($val_id, \common\helpers\Language::get_default_language_id());
            }
            $inventory_name = implode(' ', $uprid_name);
            $get_inventory_r = tep_db_query('SELECT inventory_id, products_name, products_model ' . 'FROM ' . TABLE_INVENTORY . ' ' . "WHERE prid='" . $products_id . "' AND products_id='" . tep_db_input($uprid) . "' ");
            if (tep_db_num_rows($get_inventory_r) > 0) {
                ...
                tep_db_perform(TABLE_INVENTORY, ['products_id' => $uprid, 'prid' => $products_id, 'products_name' => $inventory_name, 'products_model' => '']);
                $new_records++;
            }
        }
        if (count($uprids) > 0) {
            tep_db_query('UPDATE ' . TABLE_PRODUCTS . ' ' . 'SET products_quantity = (SELECT SUM(IF(i.products_quantity>0 AND IFNULL(i.non_existent,0)=0,i.products_quantity,0)) FROM ' . TABLE_INVENTORY . " i WHERE i.prid='" . (int) $products_id . "' GROUP BY i.prid) " . "WHERE products_id = '" . $products_id . "'");
        }
        return $new_records;
    }
