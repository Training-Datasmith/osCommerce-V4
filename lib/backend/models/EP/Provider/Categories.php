...
class Categories extends Provider_Abstract implements Import_Interface, Export_Interface
{
    protected $data = [];
    protected $e_ptools;
    ...
    protected function init_fields()
    {
        ...
        $this->fields[] = ['name' => 'categories_image', 'value' => 'Categories Image'];
        $this->fields[] = ['name' => 'categories_image_2', 'value' => 'Categories Image (Hero image)'];
        $this->fields[] = ['name' => 'categories_image_3', 'value' => 'Categories Image (Homepage image)'];
        $this->fields[] = ['name' => 'sort_order', 'value' => 'Categories Sort Order'];
        $this->fields[] = ['name' => 'categories_status', 'value' => 'Categories Status'];
        $this->fields[] = ['name' => 'show_on_home', 'value' => 'Show on homepage'];
        $this->fields[] = ['name' => 'categories_old_seo_page_name', 'value' => 'Old SEO page name (URL)'];
        foreach (\common\helpers\Language::get_languages() as $_lang) {
            $data_descriptor = '%|' . TABLE_CATEGORIES_DESCRIPTION . '|' . $_lang['id'] . '|0';
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_name', 'name' => 'categories_name_' . $_lang['code'] . '_0', 'value' => 'Categories Name ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_seo_page_name', 'name' => 'categories_seo_page_name_' . $_lang['code'] . '_0', 'value' => 'Categories SEO page name (URL) ' . $_lang['code']];
            /*      $this->fields[] = array(
            ...
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_description', 'name' => 'categories_description_' . $_lang['code'] . '_0', 'value' => 'Categories Description ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_head_title_tag', 'name' => 'categories_head_title_tag_' . $_lang['code'] . '_0', 'value' => 'Meta Title Tag ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_head_keywords_tag', 'name' => 'categories_head_keywords_tag_' . $_lang['code'] . '_0', 'value' => 'Meta Keywords Tag ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_head_desc_tag', 'name' => 'categories_head_desc_tag_' . $_lang['code'] . '_0', 'value' => 'Meta Description Tag ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_h1_tag', 'name' => 'categories_h1_tag_' . $_lang['code'] . '_0', 'value' => 'Categories H1 tag ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_h2_tag', 'name' => 'categories_h2_tag_' . $_lang['code'] . '_0', 'value' => 'Categories H2 tags ' . $_lang['code']];
            $this->fields[] = ['data_descriptor' => $data_descriptor, 'column_db' => 'categories_h3_tag', 'name' => 'categories_h3_tag_' . $_lang['code'] . '_0', 'value' => 'Categories H3 tags ' . $_lang['code']];
        }
    ...
    {
        return $this->e_ptools->tep_get_categories_full_path((int) $this->data['categories_id']);
    }
    public function prepare_export($use_columns, $filter)
    {
        ...
        if (is_array($filter)) {
            ...
            if (isset($filter['products_id']) && is_array($filter['products_id']) && count($filter['products_id']) > 0) {
                $product_categories_ids = \common\models\Products2Categories::find()->where(['IN', 'products_id', array_map('intval', $filter['products_id'])])->select(['categories_id'])->distinct()->as_array()->column();
        ...
        $main_sql = "SELECT {$main_source['select']} categories_id " . 'FROM ' . TABLE_CATEGORIES . ' ' . "WHERE 1 {$filter_sql} " . 'ORDER BY categories_left';
    ...
    public function export_row()
    {
        ...
        if ($this->with_images) {
            $files_add = [];
            foreach (['categories_image', 'categories_image_2', 'categories_image_3'] as $image_column) {
    ...
    {
        $image_target_path = \common\classes\Images::get_fs_catalog_images_path();
        $image_path_import = \common\classes\Images::get_fs_catalog_images_path() . 'import/';
        ...
        $category_id = $this->e_ptools->tep_get_categories_by_name($file_primary_value, $message);
        if (empty($category_id)) {
            $message->info('Invalid KEY value');
            return false;
        }
        $get_main_data_r = tep_db_query('SELECT * FROM ' . TABLE_CATEGORIES . " WHERE categories_id='" . (int) $category_id . "'");
        $found_rows = tep_db_num_rows($get_main_data_r);
        if ($found_rows == 0) {
            // dummy
            $message->info('Category not found. Skipped');
            // N/A message
            return false;
        } else {
            ...
            foreach ($main_source['columns'] as $file_column => $db_column) {
                ...
                if (isset($export_columns[$file_column]['set']) && method_exists($this, $export_columns[$file_column]['set'])) {
                    call_user_func_array([$this, $export_columns[$file_column]['set']], [$export_columns[$file_column], $categories_id, $message]);
                }
                    ...
                    if (!is_file($target_filename)) {
                        if (is_file($image_path_import . $this->data[$file_column])) {
                            if (!is_dir(dirname($target_filename))) {
                                try {
                                    File_Helper::create_directory(dirname($target_filename), 0777);
                                } catch (\Exception $ex) {
                                }
                            }
        ...
        foreach ($data_sources as $source_key => $source_data) {
            if ($source_data['table']) {
                ...
                if ($source_data['table'] == TABLE_CATEGORIES_DESCRIPTION) {
                    ...
                    $insert_pk = ['categories_id' => $categories_id, 'language_id' => $source_data['params'][0], 'affiliate_id' => $source_data['params'][1]];
                    $data_sql .= "AND {$update_pk}";
                    if (empty($new_data['categories_seo_page_name'])) {
                        $new_data['categories_seo_page_name'] = \common\helpers\Seo::make_slug($new_data['categories_name']);
                    }
                    /*
                ...
                if (tep_db_num_rows($data_sql_r) > 0) {
                    ...
                    if ($source_data['table'] == TABLE_CATEGORIES_DESCRIPTION) {
                        if ($ext = \common\helpers\Acl::check_extension_allowed('SeoRedirectsNamed', 'allowed')) {
                            $ext::track_category_links($categories_id, $insert_pk['language_id'], null, $new_data, $_old_data);
                        }
                    }
                } else {
                ...
                foreach ($source_data['columns'] as $file_column => $db_column) {
                    if (isset($export_columns[$db_column]['set']) && method_exists($this, $export_columns[$db_column]['set'])) {
                        call_user_func_array([$this, $export_columns[$db_column]['set']], [$export_columns[$db_column], $this->data['categories_id'], $message]);
                    }
                }
            }
        }
        if ($is_updated) {
            ...
            tep_db_perform(TABLE_CATEGORIES, ['last_modified' => 'now()'], 'update', "categories_id='" . (int) $categories_id . "'");
        }
        $this->entry_counter++;
        return true;
    }
    public function post_process(Messages $message)
    {
        ...
        $message->info('Done');
        $this->e_ptools->done('categories_import');
    }
    ...
    public function set_platform($field_data, $categories_id)
    {
        if (empty($categories_id) || !array_key_exists($field_data['name'], $this->data)) {
            return;
        }
    ...
    {
        if (!isset($this->data['assigned_customer_groups']) && \Yii::$app->db->schema->get_table_schema('groups_categories')) {
    ...
    public function set_products_group_assign($field_data, $categories_id)
    {
        if (empty($categories_id) || !array_key_exists($field_data['name'], $this->data)) {
            return;
        }
        ...
        if (!\Yii::$app->db->schema->get_table_schema('groups_categories')) {
            return;
        }
