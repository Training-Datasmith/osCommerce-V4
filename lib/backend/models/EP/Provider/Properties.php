...
class Properties extends Provider_Abstract implements Import_Interface, Export_Interface
{
    protected $fields = [];
    protected $data = [];
    protected $e_ptools;
    protected $prop_data_id = [];
    protected $prop_data_path = [];
    protected $prop_values = [];
    ...
    protected function init_fields()
    {
        ...
        $this->fields[] = ['name' => 'properties_type', 'calculated' => true, 'value' => 'Properties Type'];
        $this->fields[] = ['name' => 'properties_id', 'calculated' => true, 'value' => 'Categories Name;Property Name'];
        $this->fields[] = ['name' => 'value', 'calculated' => true, 'value' => 'Value'];
        $this->fields[] = ['name' => 'extra_value', 'calculated' => true, 'value' => 'Extra Value'];
    }
    public function export(Formatter\Formatter_Interface $output, $selected_fields, $filter)
    {
        ...
        foreach ($this->fields as $_field) {
            ...
            if (isset($_field['data_descriptor'])) {
                if (!isset($data_sources[$_field['data_descriptor']])) {
                    ...
                    $data_sources[$_field['data_descriptor']] = ['select' => '', 'table' => $data_descriptor[0] == '%' ? $data_descriptor[1] : false, 'init_function' => $data_descriptor[0] == '@' ? $data_descriptor[1] : false, 'params' => array_slice($data_descriptor, 2)];
                }
        ...
        $main_sql = 'SELECT p.products_model, p.products_id, pp.properties_type, p2p.properties_id, p2p.values_id, p2p.values_flag, p2p.extra_value ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . ' INNER JOIN ' . TABLE_PROPERTIES_TO_PRODUCTS . ' p2p ON p2p.products_id=p.products_id ' . ' INNER JOIN ' . TABLE_PROPERTIES . ' pp ON pp.properties_id=p2p.properties_id ' . ' INNER JOIN ' . TABLE_PROPERTIES_DESCRIPTION . " ppd ON ppd.properties_id=p2p.properties_id AND ppd.language_id='" . $this->_language_id . "' " . "WHERE 1 {$filter_sql} " . 'ORDER BY p.products_id, pp.sort_order, ppd.properties_name ';
        $output->write_array(array_map(function ($data) {
            return $data['value'];
        }, $export_columns));
        $write_data_empty = [];
        foreach (array_keys($export_columns) as $column) {
            $write_data_empty[$column] = '';
        }
        ...
        while ($this->data = tep_db_fetch_array($query)) {
            ...
            foreach ($export_columns as $db_key => $export) {
                if (isset($export['get']) && method_exists($this, $export['get'])) {
                    $write_data[$db_key] = call_user_func_array([$this, $export['get']], [$export, $this->data['products_id']]);
                }
    ...
    public function import_row($data, Messages $message)
    {
        ...
        if ($check_required_columns) {
            $error = false;
            foreach (['products_model', 'properties_id', 'value'] as $required_file_column) {
        ...
        if (!isset($this->last_product_lookup[$file_primary_value])) {
            $get_main_data_r = tep_db_query('SELECT products_id FROM ' . TABLE_PRODUCTS . " WHERE products_model='" . tep_db_input($file_primary_value) . "'");
            $found_rows = tep_db_num_rows($get_main_data_r);
            $this->last_product_lookup = [$file_primary_value => ['found_rows' => $found_rows, 'data' => $found_rows > 0 ? tep_db_fetch_array($get_main_data_r) : false]];
            if ($found_rows > 1) {
        ...
        tep_db_query('REPLACE INTO ' . TABLE_PROPERTIES_TO_PRODUCTS . ' (products_id, properties_id, values_id, values_flag, extra_value) ' . "VALUES ('{$this->data['products_id']}', '{$this->data['properties_id']}', '{$this->data['values_id']}', {$this->data['values_flag']}, {$this->data['extra_value']})");
        return true;
    }
    ...
    {
        $categories_query = tep_db_query('select p.properties_id, p.properties_type, p.decimals, p.parent_id, pd.properties_name, p.decimals ' . 'from ' . TABLE_PROPERTIES . ' p, ' . TABLE_PROPERTIES_DESCRIPTION . ' pd ' . "where p.parent_id = '" . (int) $parent_id . "' and p.properties_id = pd.properties_id and pd.language_id = '" . (int) $this->_language_id . "' " . "order by (p.properties_type = 'category'), p.sort_order, pd.properties_name");
        while ($categories = tep_db_fetch_array($categories_query)) {
            $temp_prop_path = $prop_path;
            $temp_prop_path .= (empty($temp_prop_path) ? '' : ';') . $categories['properties_name'];
            $prop_data = ['properties_id' => $categories['properties_id'], 'properties_name' => $categories['properties_name'], 'properties_type' => $categories['properties_type'], 'decimals' => $categories['decimals'], 'parent_id' => $parent_id, 'properties_path' => $temp_prop_path];
    ...
    protected function get_property_value($values_id)
    {
        if (!isset($this->prop_values[(int) $values_id])) {
            $prop_value = [];
            $get_value_r = tep_db_query('SELECT properties_id, values_text, values_number, values_number_upto ' . 'FROM ' . TABLE_PROPERTIES_VALUES . ' ' . "WHERE values_id='" . (int) $values_id . "' AND language_id='" . $this->_language_id . "' ");
        ...
        if (!isset($this->prop_data_path[$path])) {
            ...
            foreach ($path_arr as $idx => $path_chunk) {
                ...
                if (isset($this->prop_data_path[$walk_path_check])) {
                    ...
                    $possible_names = [$path_chunk];
                    $possible_names[] = rtrim($path_chunk);
                    $possible_names[] = trim($path_chunk);
                    $possible_names = array_unique($possible_names);
                    $lookup_r = tep_db_query('SELECT p.properties_id, p.properties_type, p.decimals, p.parent_id, pd.properties_name, p.decimals ' . 'FROM ' . TABLE_PROPERTIES . ' p, ' . TABLE_PROPERTIES_DESCRIPTION . ' pd ' . "WHERE p.parent_id = '" . (int) $walk_property_id . "' AND p.properties_id = pd.properties_id AND pd.language_id = '" . (int) $this->_language_id . "' " . " AND pd.properties_name IN('" . implode("','", array_map('tep_db_input', $possible_names)) . "') " . "ORDER BY IF(pd.properties_name='" . tep_db_input($path_chunk) . "',0,1) " . 'LIMIT 1');
                    if (tep_db_num_rows($lookup_r) > 0) {
                        $categories = tep_db_fetch_array($lookup_r);
                        $prop_data = ['properties_id' => $categories['properties_id'], 'properties_name' => $categories['properties_name'], 'properties_type' => $categories['properties_type'], 'decimals' => $categories['decimals'], 'parent_id' => $categories['parent_id'], 'properties_path' => $walk_path_check];
                        ...
                        if (isset($this->data['properties_type']) && in_array($this->data['properties_type'], ['text', 'number', 'interval', 'flag', 'file'])) {
                            $new_properties_type = $this->data['properties_type'];
                        }
                        if (count($path_arr) > 1 && $idx != count($path_arr) - 1) {
                            $new_properties_type = 'category';
                        }
                        tep_db_perform(TABLE_PROPERTIES, ['parent_id' => $parent_id, 'properties_type' => $new_properties_type, 'date_added' => 'now()']);
                        $walk_property_id = tep_db_insert_id();
                        $property_seo_name = Seo::make_property_slug(['properties_id' => (int) $walk_property_id, 'properties_name' => trim($path_chunk)]);
                        tep_db_query('INSERT INTO ' . TABLE_PROPERTIES_DESCRIPTION . ' (properties_id, language_id, properties_name, properties_seo_page_name) ' . "SELECT '{$walk_property_id}', languages_id, '" . tep_db_input(trim($path_chunk)) . "', '" . tep_db_input($property_seo_name) . "' FROM " . TABLE_LANGUAGES . ' ');
                        $prop_data = ['properties_id' => $walk_property_id, 'properties_name' => trim($path_chunk), 'properties_type' => $new_properties_type, 'decimals' => 2, 'parent_id' => $parent_id, 'properties_path' => $walk_path_check];
                        $this->prop_data_path[$walk_path_check] = $prop_data;
    ...
    protected function get_property_value_id(EP\Messages $message, $properties_id, $value)
    {
        ...
        if ($type == 'interval') {
            ...
            $result_values_id = tep_db_query('SELECT values_id ' . 'FROM ' . TABLE_PROPERTIES_VALUES . ' ' . "WHERE properties_id = '" . (int) $properties_id . "' AND language_id = '" . (int) $this->_language_id . "' " . ' AND ABS(ROUND(values_number, ' . (int) $prop_decimals . ') - ' . (float) number_format(trim($from), $prop_decimals, '.', '') . ') < ' . (float) pow(10, -($prop_decimals + 1)) . ' ' . ' AND ABS(ROUND(values_number_upto, ' . (int) $prop_decimals . ') - ' . (float) number_format(trim($upto), $prop_decimals, '.', '') . ') < ' . (float) pow(10, -($prop_decimals + 1)) . ' ');
        } elseif ($type == 'number') {
            $result_values_id = tep_db_query('SELECT values_id ' . 'FROM ' . TABLE_PROPERTIES_VALUES . ' ' . "WHERE properties_id = '" . (int) $properties_id . "' AND language_id = '" . (int) $this->_language_id . "' " . ' AND ABS(ROUND(values_number, ' . (int) $prop_decimals . ') - ' . (float) number_format(trim($value), $prop_decimals, '.', '') . ') < ' . (float) pow(10, -($prop_decimals + 1)) . ' ');
        } else {
            $result_values_id = tep_db_query('SELECT values_id ' . 'FROM ' . TABLE_PROPERTIES_VALUES . ' ' . "WHERE properties_id = '" . (int) $properties_id . "' AND language_id = '" . (int) $this->_language_id . "' " . " AND values_text = '" . tep_db_input(trim($value)) . "'");
        }
        if (tep_db_num_rows($result_values_id) > 0) {
            ...
            if ($type == 'interval') {
                list($from, $upto) = explode('-', trim($value));
                tep_db_query('INSERT INTO ' . TABLE_PROPERTIES_VALUES . ' (values_id, properties_id, language_id, values_text, values_number, values_number_upto, values_seo_page_name) ' . " SELECT '" . (int) $values_id . "', '" . (int) $properties_id . "', languages_id, '" . tep_db_input(trim($value)) . "', " . "  '" . (float) number_format(trim($from), $prop_decimals, '.', '') . "', '" . (float) number_format(trim($upto), $prop_decimals, '.', '') . "', '" . tep_db_input(Seo::make_slug(trim($value))) . "' " . ' FROM ' . TABLE_LANGUAGES);
            } elseif ($type == 'number') {
                tep_db_query('INSERT INTO ' . TABLE_PROPERTIES_VALUES . ' (values_id, properties_id, language_id, values_text, values_number, values_seo_page_name) ' . " SELECT '" . (int) $values_id . "', '" . (int) $properties_id . "', languages_id, '" . tep_db_input(trim($value)) . "', " . "  '" . (float) number_format(trim($value), $prop_decimals, '.', '') . "', '" . tep_db_input(Seo::make_slug(trim($value))) . "' " . ' FROM ' . TABLE_LANGUAGES);
            } else {
                tep_db_query('INSERT INTO ' . TABLE_PROPERTIES_VALUES . ' (values_id, properties_id, language_id, values_text, values_seo_page_name) ' . " SELECT '" . (int) $values_id . "', '" . (int) $properties_id . "', languages_id, '" . tep_db_input(trim($value)) . "', '" . tep_db_input(Seo::make_slug(trim($value))) . "' " . ' FROM ' . TABLE_LANGUAGES);
            }
