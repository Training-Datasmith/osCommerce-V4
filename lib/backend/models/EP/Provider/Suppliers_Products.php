...
use common\models\Suppliers_Products as sModel;
class Suppliers_Products extends Provider_Abstract implements Import_Interface, Export_Interface
{
    protected $fields = [];
    protected $data = [];
    protected $e_ptools;
    ...
    {
        $this->fields = [];
        $this->fields[] = ['name' => 'suppliers_id', 'value' => 'Suppliers name', 'is_key_part' => true, 'set' => 'set_supplier', 'get' => 'get_supplier'];
        $this->fields[] = ['prefix' => 'p', 'name' => 'products_model', 'value' => 'Products Model', 'is_key_part' => true, 'set' => 'set_product'];
        $this->fields[] = ['prefix' => 'i', 'name' => 'products_name', 'value' => 'Products Name'];
        //'get' => 'get_products_name'
        $this->fields[] = ['name' => 'suppliers_model', 'value' => 'Suppliers Model'];
        //'prefix'=>'sp',
        $this->fields[] = ['name' => 'suppliers_quantity', 'value' => 'Q-ty', 'type' => 'int'];
        $this->fields[] = ['name' => 'status', 'value' => 'Status (put active first)', 'type' => 'int'];
        $this->fields[] = ['name' => 'suppliers_price', 'value' => 'Cost', 'type' => 'numeric'];
        $this->fields[] = ['name' => 'suppliers_price_discount', 'value' => 'Qty Discount', 'type' => 'price_table'];
        $this->fields[] = ['name' => 'currencies_id', 'value' => 'Currency code', 'set' => 'set_currency', 'get' => 'get_currency'];
        $this->fields[] = ['name' => 'supplier_discount', 'value' => 'Discount', 'type' => 'numeric'];
        $this->fields[] = ['name' => 'suppliers_surcharge_amount', 'value' => 'Surchase amount', 'type' => 'numeric'];
        $this->fields[] = ['name' => 'suppliers_margin_percentage', 'value' => 'Marging percentage', 'type' => 'numeric'];
        $this->fields[] = ['name' => 'suppliers_product_name', 'value' => 'Suppliers product name'];
        $this->fields[] = ['prefix' => 'sp', 'name' => 'source', 'value' => 'Source'];
        $this->fields[] = ['name' => 'suppliers_upc', 'value' => 'Suppliers UPC'];
        $this->fields[] = ['name' => 'suppliers_asin', 'value' => 'Suppliers ASIN'];
        $this->fields[] = ['name' => 'suppliers_isbn', 'value' => 'Suppliers ISBN'];
        $this->fields[] = ['name' => 'suppliers_ean', 'value' => 'Suppliers EAN'];
        $this->fields[] = ['name' => 'price_with_tax', 'value' => 'Price with TAX?'];
        $this->fields[] = ['name' => 'tax_rate', 'value' => 'Tax rate'];
    }
    public function import_options()
    {
        $options = [];
        $options[] = ['title' => TEXT_PROCESS_AS, 'description' => TEXT_PROCESS_AS_DESCRIPTION, 'name' => 'import_config[insert_new]', 'value' => isset($this->import_config['insert_new']) ? $this->import_config['insert_new'] : 'insert', 'values' => ['insert' => TEXT_INSERT_UPDATE, 'delete' => IMAGE_DELETE]];
        return \Yii::$app->view->render('@app/views/easypopulate/import-options', ['options' => $options]);
    }
    public function prepare_export($use_columns, $filter)
    {
        ...
        $main_sql = "SELECT {$main_source['select']} " . " ifnull(i.products_model, p.products_model) as products_model, concat(pd.products_name, ' ', ifnull(i.products_name, '')) as products_name " . 'FROM ' . TABLE_SUPPLIERS_PRODUCTS . ' sp ' . ' join ' . TABLE_PRODUCTS . ' p on p.products_id=sp.products_id left join ' . TABLE_INVENTORY . ' i on sp.products_id=i.prid and sp.uprid=i.products_id ' . ' join ' . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id=pd.products_id and pd.language_id ='" . (int) $this->languages_id . "' and pd.platform_id='" . \common\classes\platform::default_id() . "'" . 'WHERE 1 ' . " {$filter_sql} " . '  AND p.parent_products_id=0 ' . 'ORDER BY sp.suppliers_id, sp.products_id, sp.uprid ';
    ...
    public function import_row($data, Messages $message)
    {
        ...
        $tmp = [];
        foreach ($file_primary_columns as $pk => $db_col) {
            // error data empty
            if (empty($data[$pk])) {
        ...
        foreach ($main_source['columns'] as $file_column => $db_column) {
            ...
            if (isset($export_columns[$db_column]['set']) && method_exists($this, $export_columns[$db_column]['set'])) {
                call_user_func_array([$this, $export_columns[$db_column]['set']], [$export_columns[$db_column], 0, $message]);
            }
        ...
        try {
            ...
            if ($records->count() > 1) {
                ...
                if (!empty($update_data_array['suppliers_model'])) {
                    $records = S_Model::find()->where(array_merge($pk, ['suppliers_model' => $update_data_array['suppliers_model']]));
            ...
            if (!$record_ok) {
                ...
                if ($insert_delete == 'delete') {
                    if (is_null($record)) {
                        ...
                        return false;
                    } else if ($qty = $record->delete_supplier_product()) {
                    ...
                    if (is_null($record)) {
                        //$record  = new sModel;
                        $record = new S_Model(['scenario' => 'insert']);
                    ...
                    if ($record->save_supplier_product($update_data_array, true)) {
                        ...
                        $message->info('"' . $file_primary_value_text . '" ' . (count($record->get_errors()) > 0 ? print_r($record->get_errors(), 1) : '') . ' Skipped');
                        return false;
                    }
                }
            }
        } catch (Exception $e) {
            $message->info('"' . $file_primary_value_text . '"' . print_r($e->get_message(), 1) . ' Skipped');
            return false;
        }
        if ($this->data['products_id'] ?? null) {
            tep_db_perform(TABLE_PRODUCTS, ['products_last_modified' => 'now()'], 'update', "products_id='" . (int) $this->data['products_id'] . "'");
        }
        return true;
    }
    ...
    public function get_supplier($field_data, $products_id)
    {
        ...
        $tmp = \common\helpers\Suppliers::get_supplier_name($file_value);
    ...
    public function set_product($field_data, $products_id, $message)
    {
        ...
        if (is_null($p) && is_null($inv) || !is_null($p) && $p->count() > 1 || !is_null($inv) && $inv->count() > 1 || !is_null($inv) && $inv->count() == 1 && !is_null($p) && $p->count() == 1) {
    ...
    public function set_supplier($field_data, $products_id, $message)
    {
        static $fetched_map = [];
        $file_value = trim($this->data[$field_data['name']]);
        $ret = false;
        if (!isset($fetched_map[$file_value])) {
            $tmp = \common\helpers\Suppliers::get_supplier_id_by_name($file_value);
