...
class Order_Export extends Provider_Abstract implements Export_Interface, Import_Interface
{
    protected $fields = [];
    ...
    public function import_row($data, Messages $message)
    {
        ...
        if (is_array($data['OrderLine'])) {
            ...
            foreach ($data['OrderLine'] as $product) {
                ...
                if ($row !== false) {
                    $sql_data_array_add = ['products_id' => $row['products_id'], 'products_model' => $row['products_model'], 'products_name' => $row['products_name'], 'uprid' => \common\helpers\Inventory::normalize_id($row['products_id']), 'is_virtual' => $row['is_virtual']];
                } else {
                    $sql_data_array_add = ['products_id' => 0, 'products_model' => $product['Item'], 'products_name' => $product['Item'], 'uprid' => 0, 'is_virtual' => 0];
                }
                $sql_data_array = array_merge($sql_data_array_add, $sql_data_array);
                tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array);
            }
            $currencies = new \common\classes\Currencies();
            $sql_data_array = ['orders_id' => $order_id, 'title' => 'Total:', 'text' => $currencies->format($total, true, 'EUR', 1), 'value' => $total, 'class' => 'ot_total', 'sort_order' => 120, 'text_exc_tax' => $currencies->format($total, true, 'EUR', 1), 'text_inc_tax' => $currencies->format($total, true, 'EUR', 1), 'tax_class_id' => 0, 'value_exc_vat' => $total, 'value_inc_tax' => $total, 'is_removed' => 0, 'currency' => 'EUR', 'currency_value' => 1];
            tep_db_perform(TABLE_ORDERS_TOTAL, $sql_data_array);
            $sql_data_array = ['orders_id' => $order_id, 'orders_status_id' => 100006, 'date_added' => 'now()', 'customer_notified' => '0', 'comments' => 'Imported Order'];
    ...
    public function post_process(Messages $message)
    {
        ...
        $message->info('Done.');
        $this->e_ptools->done('orders_import');
    }
}