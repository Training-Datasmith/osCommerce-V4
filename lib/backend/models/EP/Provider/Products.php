...
<div class="widget box box-no-shadow">
    ...
    public function importRow($data, Messages $message)
    {
        ...
        if ($found_rows > 1) {
            ...
            foreach (['length_', 'width_', 'height_', 'inner_length_', 'inner_width_', 'inner_height_', 'outer_length_', 'outer_width_', 'outer_height_', ] as $metricKey) {
            ...
            foreach (['weight_', 'inner_weight_', 'outer_weight_', ] as $metricKey) {
                if (array_key_exists($metricKey.'cm', $create_data_array)) {
        ...
        if ($_is_product_added) {
            ...
            if ($_check['c'] == 0) {
                tep_db_perform(TABLE_PRODUCTS_TO_CATEGORIES, [
    ...
    public function set_products_group_assign($field_data, $products_id)
    {
        ...
        if (!\Yii::$app->db->schema->getTableSchema('groups_products')) {
            return;
        }
