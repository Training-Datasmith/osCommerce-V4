...
class Products_To_Categories extends Provider_Abstract implements Import_Interface, Export_Interface
{
    protected $fields = [];
    protected $data = [];
    protected $e_ptools;
    ...
    protected $import_config_default = ['assign_mode' => 'replace', 'create_category' => 'yes'];
    ...
    protected function init_fields()
    {
        ...
        $this->fields[] = ['name' => 'sort_order', 'calculated' => true, 'value' => 'Product Sort Order'];
        $this->fields[] = ['data_descriptor' => '@|linked_categories|0', 'name' => '_categories_', 'value' => 'Assigned Categories', 'get' => 'get_category', 'set' => 'set_category'];
    }
...
<div class="widget box box-no-shadow">
    ...
    <div class="widget-content">
        <div class="row form-group">
            ...
            <div class="col-md-6">' . Html::drop_down_list('import_config[assign_mode]', $assign_mode, ['replace' => 'Replace from file', 'assign' => 'Append new, keep existing'], ['class' => 'form-control']) . '</div>
        </div>
        <div class="row form-group">
            <div class="col-md-6"><label>Create categories?</label></div>
            <div class="col-md-6">' . Html::drop_down_list('import_config[create_category]', $create_category, ['yes' => 'Yes', 'no' => 'No, skip'], ['class' => 'form-control']) . '</div>
        </div>
    </div>
</div>
        ';
    ...
    public function prepare_export($use_columns, $filter)
    {
        ...
        $main_sql = "SELECT {$main_source['select']} p2c.categories_id, p2c.sort_order, p.products_id, p.products_model, p.products_ean, p.products_isbn, p.products_asin, pd.products_name " . 'FROM ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_PRODUCTS . ' p ' . ' INNER JOIN ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c ON p2c.products_id=p.products_id ' . "WHERE p.products_id=pd.products_id AND pd.language_id='" . intval($this->languages_id) . "' AND pd.platform_id='" . intval(\common\classes\platform::default_id()) . "' " . " {$filter_sql} " . 'ORDER BY p.products_id ';
    ...
    public function export(Formatter\Formatter_Interface $output, $selected_fields, $filter)
    {
        ...
        $main_sql = "SELECT {$main_source['select']} p2c.categories_id, p.products_id, p.products_model, p.products_ean, p.products_isbn, p.products_asin, pd.products_name " . 'FROM ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_PRODUCTS . ' p ' . ' INNER JOIN ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c ON p2c.products_id=p.products_id ' . "WHERE p.products_id=pd.products_id AND pd.language_id='" . intval($this->languages_id) . "' AND pd.platform_id='" . intval(\common\classes\platform::default_id()) . "' " . " {$filter_sql} " . 'ORDER BY p.products_id ';
        //echo $main_sql;
    ...
    public function import_row($data, Messages $message)
    {
        ...
        if (is_array($_result) && !isset($this->_products_lookup[$_result['primary_value']])) {
            ...
            if ($found_rows > 1) {
                ...
                if (!isset($this->_not_processed_product_categories[$products_id])) {
                    $this->_not_processed_product_categories[$products_id] = [];
        ...
        $allow_create_category = 'no' != (isset($this->import_config['create_category']) ? $this->import_config['create_category'] : $this->import_config_default['create_category']);
        $this->data['products_id'] = $products_id;
        $import_category_path = $this->data['_categories_'];
        if (trim($import_category_path) == '') {
            $category_id = 0;
        } else {
            $category_id = $this->e_ptools->tep_get_categories_by_name($import_category_path, $message, $allow_create_category);
            if (!$allow_create_category && !is_numeric($category_id) && !empty($import_category_path)) {
        ...
        if (is_numeric($category_id)) {
            ...
            if (isset($this->data['sort_order'])) {
                tep_db_query('INSERT INTO ' . TABLE_PRODUCTS_TO_CATEGORIES . ' ' . '  (products_id, categories_id, sort_order) ' . ' VALUES' . "  ('" . (int) $products_id . "', '" . (int) $category_id . "','" . intval($this->data['sort_order']) . "') " . " ON DUPLICATE KEY UPDATE sort_order='" . intval($this->data['sort_order']) . "' ");
            } else {
    ...
    public function post_process(Messages $message)
    {
        ...
        if ($assign_mode == 'replace') {
            foreach ($this->_not_processed_product_categories as $_clean_pid => $_clean_categories) {
                ...
                $check_ref = tep_db_fetch_array(tep_db_query('SELECT COUNT(*) AS assigned_count FROM ' . TABLE_PRODUCTS_TO_CATEGORIES . " WHERE products_id='{$_clean_pid}' "));
                if ($check_ref['assigned_count'] == 0) {
                    tep_db_perform(TABLE_PRODUCTS_TO_CATEGORIES, ['products_id' => $_clean_pid, 'categories_id' => 0]);
                }
            }
        }
        $message->info('Processed ' . $this->entry_counter . ' products');
        $message->info('Done');
        $this->e_ptools->done('products_to_categories_import');
    }
    public function import(Formatter\Formatter_Interface $input, EP\Messages $message)
    {
        $this->entry_counter = 0;
        $this->_products_lookup = [];
        $this->_not_processed_product_categories = [];
        while ($data = $input->read_array()) {
            if ($this->import_row($data, $message)) {
                $this->entry_counter++;
            }
        }
    ...
    {
        $this->data['_categories_'] = $this->e_ptools->tep_get_categories_full_path((int) $this->data['categories_id']);
