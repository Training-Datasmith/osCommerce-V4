...
class Directory extends BaseObject
{
    ...
    public function synchronizeDirectories($cronAsk)
    {
        ...
        foreach (glob($directoryRoot.'*', GLOB_ONLYDIR) as $checkDir) {
            ...
            if (preg_match('/^(images)$/', $checkDirectoryName)) {
                continue;
            }

            if (!isset($db_link_check[$checkDirectoryName])) {
                tep_db_perform(TABLE_EP_DIRECTORIES, [
                    'removable' => 1,
    ...
    public static function getAll($renewCache = false)
    {
        static $directoriesList = false;
        if ($renewCache) {
            $directoriesList = false;
        }
    ...
    public static function getAllRoots()
    {
        ...
        foreach (self::getAll() as $directory) {
            ...
            if ($directory->directory_id == 5 && count(DataSources::getAvailableList()) == 0) {
                continue;
            }
            $roots[] = $directory;
        }
    ...
    public function getJobConfigTemplate($jobId)
    {
        ...
        foreach ($this->directory_config as $idx => $jobConfig) {
            if (!($jobObj->job_provider == $jobConfig['job_provider'] && $jobObj->file_name == $jobConfig['filename_pattern'])) {
                continue;
            }
    ...
    {
        if (!is_array($newConfig) || !$this->directory_id) {
            return;
        }
