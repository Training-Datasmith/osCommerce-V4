...
class Tools
{
    ...
    public function tep_get_products_groups_by_name($name, $products_groups_id = 0, $languages_id = 0, $messages = false)
    {
        ...
        if (! isset($this->products_groups_name_to_id[$name])) {
            ...
            if (tep_db_num_rows($r) > 0) {
                ...
                tep_db_perform(TABLE_PRODUCTS_GROUPS, [
    ...
    public function tep_get_categories_by_name($categories_path, $messages = false, $allowCreateCategory = true)
    {
        $languages_id = $this->languages_id;
        ...
        if (!isset($processed[$brand_name])) {
          ...
          'LIMIT 1'
            ...
            if (tep_db_num_rows($get_brand_r) > 0) {
                ...
                tep_db_perform(TABLE_MANUFACTURERS, [
    ...
    public function get_option_by_name($option_names)
    {
        ...
        foreach ($option_names as $_lang => $_lookup_name) {
            if ($option_id) {
                //update
                tep_db_perform(TABLE_PRODUCTS_OPTIONS, [
    ...
    public function get_option_value_by_name($option_id, $value_names)
    {
        ...
        foreach ($value_names as $_lang => $_lookup_name) {
            if ($option_value_id) {
                //update
                tep_db_perform(TABLE_PRODUCTS_OPTIONS_VALUES, [
            ...
            if (tep_db_num_rows($get_id_r) > 0) {
                ...
                tep_db_perform(TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS, [
    ...
    public function getWarehouseLocationName($warehouseId, $warehouseLocationId)
    {
        ...
        if (!isset($cache[$warehouseLocationId])) {
            ...
            foreach ($locations as $location) {
                if ($warehouseLocationId != $location['location_id']) {
                    continue;
                }
