...
function tep_db_query($query, $link = 'db_link')
{
    ...
    if (defined('STORE_DB_TRANSACTIONS') && STORE_DB_TRANSACTIONS == 'true' && defined('STORE_PAGE_PARSE_TIME_LOG')) {
        ...
        Yii::begin_profile($query, 'tep::db_query');
    }
    ...
    if (defined('STORE_DB_TRANSACTIONS') && STORE_DB_TRANSACTIONS == 'true' && defined('STORE_PAGE_PARSE_TIME_LOG')) {
        ...
        Yii::end_profile($query, 'tep::db_query');
    }
    $trans_count++;
    return $result;
}
function tep_db_perform($table, $data, $action = 'insert', $parameters = '', $link = 'db_link')
{
    ...
    if ($table == TABLE_ORDERS && $action == 'insert' && $ext = \common\helpers\Acl::check_extension_allowed('InvoiceNumberFormat', 'allowed')) {
        if (empty($data['order_number'])) {
            $platform_id = $data['platform_id'] ?? (int) PLATFORM_ID;
            $data['order_number'] = $ext::get_order_number($platform_id);
            if (!$data['order_number']) {
                unset($data['order_number']);
            }
        }
    }
    if ($table == TABLE_ORDERS && $ext = \common\helpers\Acl::check_extension_allowed('InvoiceNumberFormat', 'allowed')) {
        if (empty($data['invoice_number']) && $action == 'insert' && !empty($data['orders_status'])) {
            $platform_id = $data['platform_id'] ?? (int) PLATFORM_ID;
            $data['invoice_number'] = $ext::get_invoice_number($data['orders_status'], $platform_id);
...
*/
function yii_setup_model(\yii\db\Active_Record $model, $sql_data_array)
{
    try {
        $schema_columns = $model->get_table_schema()->columns;
    ...
    foreach ($sql_data_array as $column => $value) {
        ...
        if (isset($schema_columns[$column])) {
            $model->set_attribute($column, $value);
            $table_column = $schema_columns[$column];
            /**
             * @var $tableColumn \yii\db\ColumnSchema
             */
            if (!$table_column->allow_null && $table_column->db_typecast($value) === null) {
                if (is_null($table_column->default_value)) {
                    $model->set_attribute($column, !is_null($table_column->php_typecast('')) ? $table_column->php_typecast('') : $table_column->php_typecast(0));
                } else {
                    $model->set_attribute($column, $table_column->default_value);
                }
            }
        }
    }
