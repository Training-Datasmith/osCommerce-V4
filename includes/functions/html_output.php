...
function tep_href_link($page = '', $parameters = '', $connection = '', $add_session_id = true, $search_engine_safe = true)
{
    ...
    if (class_exists('Yii') && is_object(Yii::$app)) {
        ...
        if (in_array($page, ['catalog/gift-card', 'catalog/product']) && !empty($url_params_array['platform_id']) && PLATFORM_ID != $url_params_array['platform_id']) {
            // save current params
            $host_info = Yii::$app->url_manager->get_host_info();
            $base_url = Yii::$app->url_manager->get_base_url();
            $pc = new \common\classes\platform_config($url_params_array['platform_id']);
            $parsed = parse_url($pc->get_catalog_base_url());
            Yii::$app->url_manager->set_host_info($parsed['scheme'] . '://' . $parsed['host'] . (!empty($parsed['port']) && !in_array($parsed['port'], ['80', '443']) ? ':' . $parsed['port'] : ''));
            Yii::$app->url_manager->set_base_url(rtrim($parsed['path']));
            if ($parsed['scheme'] != 'https') {
                $url_params_array[tep_session_name()] = tep_session_id();
            }
            $cross_url = Yii::$app->url_manager->create_absolute_url($url_params_array, $parsed['scheme'], true);
            // restore params
            Yii::$app->url_manager->set_host_info($host_info);
            Yii::$app->url_manager->set_base_url($base_url);
            return $cross_url;
        }
        return Yii::$app->url_manager->create_absolute_url($url_params_array, $connection == 'SSL' && ENABLE_SSL == true ? 'https' : 'http');
    }
    // }}
    ...
    if (SEARCH_ENGINE_FRIENDLY_URLS == 'true' && SEARCH_ENGINE_UNHIDE == 'True' && $search_engine_safe == true) {
        ...
        if (strstr($page, FILENAME_DEFAULT) && strstr($parameters, 'cPath=')) {
            ...
            $c_path_param = substr($parameters, strpos($parameters, 'cPath=') + 6);
            if (strpos($c_path_param, '&') !== false) {
                $c_path_param = substr($c_path_param, 0, strpos($c_path_param, '&'));
                $parameters = str_replace('cPath=' . $c_path_param . '&', '', $parameters);
            } else {
                $parameters = str_replace('cPath=' . $c_path_param, '', $parameters);
            }
            $c_path_array = \common\helpers\Categories::parse_category_path($c_path_param);
            $categories_id_param = $c_path_array[sizeof($c_path_array) - 1];
            ...
            } else if ($c_path_param) {
                $parameters = 'cPath=' . $c_path_param . '&' . $parameters;
            }
        ...
        } elseif (strstr($page, FILENAME_PRODUCT_INFO) && strstr($parameters, 'products_id=') && !strstr($parameters, '{') && !strstr($parameters, '}')) {
            ...
            } else if ($products_id_param) {
                $parameters = 'products_id=' . $products_id_param . '&' . $parameters;
            }
            $parameters = preg_replace('/&+$/', '', $parameters);
            if (tep_not_null($parameters)) {
                $link .= $url . '?' . \common\helpers\Output::output_string($parameters);
            ...
            } else if ($info_id_param) {
                $parameters = 'info_id=' . $info_id_param . '&' . $parameters;
            }
            $parameters = preg_replace('/&+$/', '', $parameters);
            if (tep_not_null($parameters)) {
                $link .= $url . '?' . \common\helpers\Output::output_string($parameters);
            ...
            } else if ($manufacturers_id_param) {
                $parameters = 'manufacturers_id=' . $manufacturers_id_param . '&' . $parameters;
            }
            $parameters = preg_replace('/&+$/', '', $parameters);
            if (tep_not_null($parameters)) {
                $link .= $url . '?' . \common\helpers\Output::output_string($parameters);
        ...
        } else if (tep_not_null($parameters)) {
            $separator = '&';
        } else {
            $separator = '?';
        }
...
function tep_image($image_src, $alt = '', $width = '', $height = '', $parameters = '', $flag = true)
{
    ...
    if ($flag) {
        ...
        if (!($size[0] <= $width && $size[1] <= $height)) {
            $newsize = \common\helpers\Image::get_new_size($image_filename, $width, $height);
            $width = (int) $newsize[0];
...
function tep_draw_selection_field($name, $type, $value = '', $checked = false, $parameters = '')
{
    ...
    if (tep_not_null($value)) {
        $selection .= ' value="' . \common\helpers\Output::output_string($value) . '"';
    }
...
function tep_draw_pull_down_menu($name, $values, $default = '', $parameters = '', $required = false)
{
    ...
    return str_replace(['&amp;nbsp;', '&amp;pound;'], ['&nbsp;', '&pound;'], $field);
}
////
