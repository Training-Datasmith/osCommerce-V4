...
if (!defined('AdditionalPlatforms_EXTENSION_STATUS')) {
    $arr = tep_db_fetch_array(tep_db_query("select configuration_value from platforms_configuration where configuration_key = 'AdditionalPlatforms_EXTENSION_STATUS'"));
    define('AdditionalPlatforms_EXTENSION_STATUS', is_array($arr) ? $arr['configuration_value'] ?? null : null);
}
if (file_exists('lib/common/extensions/AdditionalPlatforms/AdditionalPlatforms.php') && Additional_Platforms_extension_status == 'True') {
    if (!class_exists('\common\extensions\AdditionalPlatforms\AdditionalPlatforms')) {
        ...
        $platform = \common\extensions\Additional_Platforms\Additional_Platforms::configure();
    }
} else {
    ...
    $platform = tep_db_fetch_array(tep_db_query('select p.*, ' . ' IF(pu.url IS NULL,0,1) AS _platform_cdn_server, ' . ' IF(LENGTH(p.platform_url_secure)>0,p.platform_url_secure,p.platform_url) AS _platform_url_secure ' . 'from platforms p ' . " left join platforms_url pu ON pu.platform_id=p.platform_id AND pu.url!={$_search_platform_url} AND pu.url = '" . tep_db_input($REQUEST_PLATFORM_URL . '/') . "' AND pu.status=1 " . "where p.platform_id='1' " . 'LIMIT 1'));
}
// CONSOLE APPLICATION DEFAULT PLATFORM LOAD
// TO ENABLE - PASS '-dP' KEY
if (!isset($platform['platform_id']) and isset($_SERVER['argv']) and is_array($_SERVER['argv']) and isset($_SERVER['argv'][0]) and strtolower($_SERVER['argv'][0]) == 'yii.php' and in_array('-dP', $_SERVER['argv'])) {
    $platform = tep_db_fetch_array(tep_db_query('select p.*, ' . ' IF(pu.url IS NULL,0,1) AS _platform_cdn_server, ' . ' IF(LENGTH(p.platform_url_secure)>0,p.platform_url_secure,p.platform_url) AS _platform_url_secure ' . 'from platforms p ' . ' left join platforms_url pu ON pu.platform_id=p.platform_id AND pu.url!=IF(LENGTH(p.platform_url_secure)>0,p.platform_url_secure,p.platform_url) AND pu.status=1 ' . 'where p.is_default = 1 ' . 'LIMIT 1'));
}
...
if (isset($platform['platform_id']) && $platform['platform_id'] > 0) {
    if ($platform['is_marketplace'] == 1) {
        $default_platform = tep_db_fetch_array(tep_db_query('select *, IF(LENGTH(platform_url_secure)>0,platform_url_secure,platform_url) AS _platform_url_secure from platforms ' . 'where platform_id=' . $platform['default_platform_id'] . ' ' . 'LIMIT 1 '));
    ...
    if ($platform['is_virtual'] == 1) {
        $default_platform = tep_db_fetch_array(tep_db_query('select *, IF(LENGTH(platform_url_secure)>0,platform_url_secure,platform_url) AS _platform_url_secure from platforms ' . 'where is_default=1 ' . 'LIMIT 1 '));
        $platform['platform_url'] = $default_platform['platform_url'];
        $platform['_platform_url_secure'] = $default_platform['_platform_url_secure'];
        $platform['ssl_enabled'] = $default_platform['ssl_enabled'];
        $platform['need_login'] = $default_platform['need_login'];
    ...
    if (isset($_GET['theme_name'])) {
        ...
        defined('HTTP_SERVER') or define('HTTP_SERVER', $secure_protocol . '://' . $parsed['host'] . $port);
    ...
    if ($platform['is_default_contact'] == 1 && $platform['default_platform_id'] > 0) {
        $platform_default = tep_db_fetch_array(tep_db_query('select * from platforms ' . 'where platform_id=' . (int) $platform['default_platform_id'] . ' ' . 'LIMIT 1 '));
        define('EMAIL_FROM', $platform_default['platform_email_from']);
        define('STORE_OWNER_EMAIL_ADDRESS', $platform_default['platform_email_address']);
        define('SEND_EXTRA_ORDER_EMAILS_TO', $platform_default['platform_email_extra']);
    } elseif ($platform['is_default_contact'] == 1) {
        $platform_default = tep_db_fetch_array(tep_db_query('select * from platforms ' . 'where is_default=1 ' . 'LIMIT 1 '));
        define('EMAIL_FROM', $platform_default['platform_email_from']);
        define('STORE_OWNER_EMAIL_ADDRESS', $platform_default['platform_email_address']);
        define('SEND_EXTRA_ORDER_EMAILS_TO', $platform_default['platform_email_extra']);
    } else {
    ...
    } elseif ($platform['is_default_address'] == 1) {
